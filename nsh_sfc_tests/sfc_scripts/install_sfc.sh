...
if [ ${vpp_need_install} -eq 1 ]; then
    ...
    cd ${VPP_CODE_DIR}
    git clone -b ${VPP_VERSION} https://gerrit.fd.io/r/vpp
...
if [ ${nsh_need_install} -eq 1 ]; then
    ...
    cd ${NSH_SFC_CODE_DIR}
    git clone -b ${NSH_SFC_VERSION} https://gerrit.fd.io/r/nsh_sfc
...
#so the vpp can take over them
sudo ifconfig ${if_name1} down >/dev/null 2>&1
sudo ip addr flush dev ${if_name1} >/dev/null 2>&1

sudo ifconfig ${if_name2} down >/dev/null 2>&1
sudo ip addr flush dev ${if_name2} >/dev/null 2>&1

#restart the vpp, let vpp take over the NIC
