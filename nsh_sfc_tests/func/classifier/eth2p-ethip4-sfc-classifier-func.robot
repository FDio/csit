...
| | [Tags] | 3_NODE_SINGLE_LINK_TOPO | HW_ENV
| | ${frame_size}= | Set Variable | ${72}
| | Node "${src_node}" interface "${src_port}" send "${frame_size}" Bytes packet to node "${dst_node}" interface "${dst_port}" for "Classifier" test
