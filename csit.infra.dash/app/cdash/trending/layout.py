...
class Layout:
    ...
    def callbacks(self, app):
        ...
        def _update_plot_mod_telemetry(
            ...
            telemetry = TelemetryData(tests=store_sel)
            telemetry_json = no_update
            ...
            if trigger.type == "telemetry-btn":
                if trigger.idx == "open":
                    telemetry.from_dataframe(self._data)
                    telemetry_json = telemetry.to_json()
                    metrics = telemetry.unique_metrics
                    search_in = str()
                    search_out = get_list_group_items(metrics, "tele-cl", False)
                    is_open = (True, False)
                elif trigger.idx == "select":
                    telemetry.from_json(tele_data)
                    metrics = [a for a, b in zip(metrics, cl_metrics) if b]
                    labels = telemetry.get_selected_labels(metrics)
                    list_metrics = telemetry.str_metrics
                    is_open = (False, True) if metrics else (False, False)
                elif trigger.idx == "back":
                    telemetry.from_json(tele_data)
                    metrics = telemetry.unique_metrics
                    search_in = str()
                    search_out = get_list_group_items(metrics, "tele-cl", False)
                ...
                telemetry.from_metrics(metrics)
                search_out = get_list_group_items(
                    telemetry.search_unique_metrics(search_in),
                    type="tele-cl",
                    colorize=False
                )
                metrics = no_update
