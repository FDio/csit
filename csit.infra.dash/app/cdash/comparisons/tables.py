...
def select_comparison_data(
    ...
    def _compact_data(data_in: pd.DataFrame, ttype: str, drv: str) -> pd.DataFrame:
        """
        """
        ...
        for itm in tests:
            ...
            # d_data["name"].append(f"{nic}-{test}-{ttype}")
            # d_data["test"].append(test.replace(f"{drv}-", ""))
    ...
    for itm in selected:
        ...
        if not tmp_df.empty:
            tmp_df = _compact_data(tmp_df, itm["ttype"].lower(), itm["driver"])
...
def comparison_table(
    ...
    for _, row in r_data.iterrows():
        ...
        cmp_data[f"{c_name} mean"].append(c_row["mean"])
        cmp_data[f"{c_name} stdev"].append(c_row["stdev"])
