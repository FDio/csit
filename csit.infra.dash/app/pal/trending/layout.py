...
from ..utils.utils import get_color, show_tooltip, label, sync_checklists, list_tests, \
...
class Layout:
    ...
    def callbacks(self, app):
        ...
        def _generate_plotting_area(tests: list, normalize: bool) -> list:
            ...
            for idx, test in enumerate(tests):
                accordion_items.append(
                    dbc.AccordionItem(
                        title=test["id"],
                        children=[lat, ],
                        style={"accordion-button-color": get_color(idx)}
                    )
                )
...
#             return row_fig_tput, row_fig_lat, row_btn_dwnld`
        ...
        def _update_ctrl_panel(cp_data: dict, store_sel: list,
            ...
            if trigger_id in ("btn-ctrl-add", "url", "cl-ctrl-normalize"):
                if store_sel:
                    plotting_area = _generate_plotting_area(store_sel, bool(norm))
                else:
