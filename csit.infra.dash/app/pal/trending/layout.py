        ...
        def _generate_plotting_area(tests: list, normalize: bool,
            ...
            children_select = [
                ...
                dbc.Col(dbc.Select(
                    options=[{"label": i, "value": i} for i in range(5)],
                    #id=f"dd-4-{idx+1}",
                    placeholder="Select a Telemetry Bundle...",
                    disabled=False,
                    size="sm"
                )),
                dbc.Col(dbc.Select(
                    options=[{"label": i, "value": i} for i in range(5)],
                    #id=f"dd-5-{idx+1}",
                    placeholder="Select a Per Bundle Counters...",
                    disabled=False,
                    size="sm"
                ))
            ]
                ...
                collapse_items.append(dbc.Row(
                    [
                        dbc.Collapse(children=[
                            dbc.Row(
                                children=children_select if bool(idx==0) else children_dd,
                                class_name="g-0 p-0"
                            ),
