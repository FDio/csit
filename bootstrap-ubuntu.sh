...
for index in "${!VIRL_SERVERS[@]}"; do
    ...
    then
        ...
        VIRL_SERVERS=("${VIRL_SERVERS[@]:0:$index}" "${VIRL_SERVERS[@]:$[$index+1]}")
    fi
done
...
for index in "${!TEST_GROUPS[@]}"; do
    least_load_server_idx=$(echo "${VIRL_SERVER_LOAD[*]}" | tr ' ' '\n' | awk 'NR==1{min=$0}NR>1 && $1<min{min=$1;idx=NR}END{print idx}')
    least_load_server=${VIRL_SERVERS[$least_load_server_idx]}
    VIRL_SERVER[${index}]=$least_load_server
    # Adjusting load as we are not going run simulation immediately
    ${VIRL_SERVER_LOAD[$least_load_server]}=${VIRL_SERVER_LOAD[$least_load_server]}+1
done
...
for index in "${!VIRL_SERVER[@]}"; do
    ...
    VIRL_SID[${index}]=$(ssh ${SSH_OPTIONS} \
        ...
        --release ${VIRL_RELEASE} ${VPP_DEBS_FULL[@]}")
    retval=$?
    if [ ${retval} -ne "0" ]; then
