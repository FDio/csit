...
# we just download the DPDK package from the fast.dpdk.org
wget -q "fast.dpdk.org/rel/dpdk-16.07.tar.xz" || exit
...
case "$TEST_TAG" in
    ...
    PERFTEST_LONG_BRIDGE )
        pybot ${PYBOT_ARGS} \
              -L TRACE \
              -v TOPOLOGY_PATH:${WORKING_TOPOLOGY} \
              -s "dpdk-tests.perf.Long_Bridge_Domain*" \
              dpdk-tests/
        RETURN_STATUS=$(echo $?)
        ;;
    PERFTEST_LONG_IPV4 )
    ...
    PERFTEST_LONG_XCONNECT )
        pybot ${PYBOT_ARGS} \
              -L TRACE \
              -v TOPOLOGY_PATH:${WORKING_TOPOLOGY} \
              -s "dpdk-tests.perf.Long_Xconnect*" \
              dpdk-tests/
        RETURN_STATUS=$(echo $?)
        ;;
    PERFTEST_LONG_XCONNECT_DOT1Q )
        pybot ${PYBOT_ARGS} \
              -L TRACE \
              -v TOPOLOGY_PATH:${WORKING_TOPOLOGY} \
              -s "dpdk-tests.perf.Long_Xconnect_Dot1q*" \
              dpdk-tests/
        RETURN_STATUS=$(echo $?)
        ;;
    PERFTEST_NDR )
