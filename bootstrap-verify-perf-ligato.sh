...
cd .. && git clone --depth 1 ${LIGATO_REPO_URL}/vpp-agent
...
# Install Docker
wget https://download.docker.com/linux/ubuntu/dists/xenial/pool/stable/amd64/${DOCKER_DEB}
...
cd ${SCRIPT_DIR}/../vpp-agent/docker/prod_vpp_agent/ &&\
    ...
    ./shrink.sh
# Export Docker image
docker save prod_vpp_agent_shrink | gzip > prod_vpp_agent_shrink.tar.gz
