...
sudo docker cp ${SCRIPT_DIR}/vpp_download/* agentcnt:/opt/vpp-agent/dev/vpp/build-root/

# Recompile vpp-agent
sudo docker exec -i agentcnt \
    script -qec '. ~/.bashrc; cd $HOME/go/src/github.com/ligato/vpp-agent && make generate && make install'
if [ $? != 0 ]; then
    echo "Failed to build vpp-agent in Docker image."
    exit 1
fi
# Save container state
sudo docker commit `docker ps -q` dev_vpp_agent:latest
