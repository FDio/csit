...
[mk]
    ...
    goodput" in IETF specifications. In addition to
...
The search stops if every goal has reached it precision.
...
Note that smaller Trial Duration values can still be used, for example
selected while focusing on a different Search Goal. Such results
MUST be still accepted by the Load Classification logic.
...
[mk]
    ...
    TODO Does it mean for "implementations to support missing attributes
    with reasonable default values" or "implementations to support missing
    values with reasonable default values"? The latter does not make sense
    to me :)
...
For example, implementations with Goal Initial Trial Duration
may also enable users to specify "how quickly" should Trial Duration increase.
...
[mk]
    ...
    Alterantive phrasing for above paragraph:
    
    In a typical case, Max Load is classified as a Lower Bound,
    making it impossible to increase the Load to continue the Search,
    and it is not clear whether a higher value would be found as a Lower Bound.
...
[mk]
    ...
    Alternative phrasing of above paragraph:

    Any networking measurement setup consisting of
    abstract components satisfying requirements
    for the Measurer, the Controller and the Manager,
    is considered to be compliant with MLRsearch specification.
...
[mk]
    ...
    Any test procedure that involves a call to the Manager of a system based on the
    MLRsearch architecture is said to be compliant with MLRsearch Specification.
...
[mk]
    ...
    The use of "that" and "that" makes it not very clear.

    Can this paragraph be rephrased e.g.
...
Moreover, this also means that if the SUT tested (or the Traffic Generator used) needs a warm-up,
