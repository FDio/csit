...
[mk]
    ...
    Apart from [Lencze-Kovacs-Shima] I haven't found any use of "TCP
    goodput" in IETF specifications.

    VP: Worse. I found https://datatracker.ietf.org/doc/html/rfc2647#section-3.17
    which is in bits per second, contains questionable "minus any bits lost",
    and is not clear on which layer bits are counted.

    In addition to
    [Lencze-Kovacs-Shima] reference I suggest we also add two references
    widely used in the industry for evaluating and describing TCP
    throughput (a.k.a. TCP goodput):

    Framework for TCP Throughput Testing
    https://datatracker.ietf.org/doc/html/rfc6349

    VP: That focuses on TCP equilibrium state. Some users may be interested
    in multiple smaller data transfers (over different connections).
    Re loss ratio it says 5% is too high, not sure what is acceptable today.

    The Macroscopic Behavior of the TCP Congestion Avoidance Algorithm
    https://www.cs.cornell.edu/people/egs/cornellonly/syslunch/fall02/ott.pdf

    VP: Interesting, but some effects may be different for newer algos (e.g. cubic).

    Here proposed edits to the paragraph:
    ...
    measurement, and [Ott-Mathis-Semke-Mahdavi] for models and calculations
    of TCP performance in presence of packet loss.
...
{::comment}
    ...
    TODO Verify MUSTs are not in discussion, or update the paragraph above.
...
If a time evolution of a dynamic quantity is guided by configuration quantities,
such adjectives can be used to distinguish quantities.
For example if the current value of "duration" quantity increases
from "initial duration" to "final duration", all the quoted names
denote separate but related quantites.

{::comment}
[mk]

    Shouldn't first sentence in the above paragraph say:

    If dynamic quantity value's time evolution is guided by configuration
    quantities, such adjectives can be used to distinguish quantities.
...
while stil being a multiple of Load.

{::comment}
[mk]

    Trial Forwarding Ratio => Trial Forwarding Rate?
...
Similarly to Trial Load, single-interface and multi-interface values
can be treated as the same quantity expressed using different unit
for test report purposes. MLRsearch specification will continue
to treat the single-interface value as primary.

{::comment}
[mk]

    What does the last statement mean? MLRsearch tracks aggregate rate
    across all interfaces per earlier paragraph.
...
as in practice that value leads to the smallest variation in overall Search Duration.
...
Informally, this is the smallest Trial Duration the Controller should select
when focusing on the goal.

Note that smaller Trial Duration values can still be used,
...
[mk]
    ...
    I changed s/should Trial Durations/should Trial Duration/ as it is
    related to singular of "Goal Initial Trial Duration". If you meant
    something else here, then let's revert it, but it would require
    explanation of why it's plural :)

    VP: Reverted. The point is that each goal can have different value
    of Goal Initial Trial Duration but also different "how quickly" value.
...
{::comment}
    ...
    Mention "timeout" here?

[mk]

    Thought we decided not to specify "timeout" in this spec. Or do you
    mean adding a sentence in here indicating that strategies for
    dealing with unstable SUT and testing environments are left down to
    implementation, including dealing with prevention of never-ending
    test runs?
...
[mk]
    ...
    This statement bothers me: "two relations can be named using the
    concept of a bound."

    I *think* it means "the relation" between Trial Result instances and
    one Search Goal instance can be expressed using the concept of a
    bound. Yes?

    If yes, above should be rephrased to make it more comprehensible.

    If no, it should be rephrased to make it fully comprehensible.

[/mk]
{:/comment}

In general, such bounds are a current quantity,
even though cases of a load "switching bounds" during search is rare in practice.

{::comment}
[mk]

    Two points here:
    1. What is meant by "load "switching bounds""?
    2. The sentence is a tidbit unclear, and once point 1. is clarified,
...
for an Upper Bound.
Thus, it is not clear whether a higher value would be found
for a Relevant Lower Bound if higher Loads were possible.
...
[mk]
    ...
    Isn't it better to say "a valid Relevant Lower Bound"
    instead of "an effective Relevant Lower Bound"?

    Or does it "collide" with the end of the sentence "result instance
    is irregular"?

    I.e. "effective" vs "irregular" is less of a logic collision "eye
    sore" compared witgh "valid" vs "irregular". :)

[/mk]

    VP: In this case, Max load is a valid Lower Bound,
    but not a regular Relevant Lower Bound.
    I prefer a paragraph explaining thsi situation
    without needing to define yet another specific term.
    I just do not see a good enough possible paragraph yet.
...
{::comment}
    ...
    Clarify those "can be understood as" and "can be seen as" are there
    just to distinguish abstract logical structure
    (implied data upholds requirements) from actual code structure
    (processes, interfaces, specific data structures).
    The latter is irrelevant for compliance, but likely a first impression
    for readers seeing words like "component" and "call".
...
{::comment}
    ...
    TODO: Reformulate an place somewhere:
    ...
    Personally, I believe that any implementation that aims for short search time
    (with fixed controller input) will necessarily also become good
    at repeatability and comparability, but that is only my impression
    based on simulated results when I was tweaking the CSIT implementation.
    Instead of requiring any implementation has to “work correctly”,
    we decided to stay silent on correctness of the methodology in the draft.
...
This also applies when that Trial Load is used
before a specific bound could have become a current Relevant Bound.

Moreover, this also means that if the SUT tested (or the Traffic Generator used)
needs a warm-up, it should be warmed up before starting the Search.

Furthermore, for MLRsearch implementation, it means it is better to measure
...
Many thanks to Alec Hothan of the LFN OPNFV NFVbench project for
