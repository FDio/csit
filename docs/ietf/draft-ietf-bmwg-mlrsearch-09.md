...
For deeper insights, see [FDio-CSIT-MLRsearch] and documents linked there.
...
As Trial Forwarding Rate is a quantity based on Load,
...
As Max Load is a quantity based on Load,
it is ALLOWED to express this quantity using multi-interface values
...
As Min Load is a quantity based on Load,
it is ALLOWED to express this quantity using multi-interface values
...
As Relevant Upper Bound is a quantity based on Load,
it is ALLOWED to express this quantity using multi-interface values
...
As Relevant Lower Bound is a quantity based on Load,
it is ALLOWED to express this quantity using multi-interface values
in test report, e.g. as sum of per-interface loads.
...
As Conditional Throughput is a quantity based on Load,
it is ALLOWED to express this quantity using multi-interface values
...
{::comment}
    ...
    Currently: Loads are large/small, durations short/long, all else (loss, performance) are high/low.
    Reverify in final pass.
...
to me larger than the Relevant Upper Bound.
...
{::comment}
    ...
    TODO-P2: Check capitalization in appendices.
