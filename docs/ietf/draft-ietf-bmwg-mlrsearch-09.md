...
{::comment}
    ...
    Verify MUSTs are not in discussion, or update the paragraph above.
...
The discussions relate the attributes to ideas mentioned in chapter
[Identified Problems](#identified-problems), but even those discussions
section are short, informal, and mostly referencing future sections,
...
multiple trials may be need to be performed at the same load.
...
As multiple trials may be needed for one Load value,
...
A threshold value for a particular ratio
of sums of Trial Effective Duration values.
...
but as shorter Trials are allowed, the full logic is more complicated.

For explainability reasons, the RECOMMENDED value for exceed ratio is 0.5 (50%),
as that value leads to smallest variation in overall Search Duration in practice.
