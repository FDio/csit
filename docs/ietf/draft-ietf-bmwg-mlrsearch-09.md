...
Minimum value for Trial Duration that has to be reached.
...
which may be enough for classification to Upper Bound.
...
a Load value when the results are one-sided,
but also the Search can spend more than Goal Duration Sum measuring a Load
when the results are balance and include
many trials shorter than Goal Final Trial Duration.
...
Minimum value for Trial Duration suggested to use for this goal.
...
perhaps selected when focusing on other Searcg Goasl,
