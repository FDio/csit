...
Traffic types that need to be somehow specified
in Traffic Profile are described in:
...
One search goal can have many different loads classified as an upper bound.
...
performed at that load failed to uphold all the requirements
of the given Search Goal, mainly the Goal Loss Ratio
in combination with the Goal Exceed Ratio.

    LGTM26 - I flagged it as broken logica, but now i see it's okay, very dense though.
...
In more detail, the set of all trial outputs (both short and full-length,
enough of them according to Goal Duration Sum)
performed at that load managed to uphold all the requirements
of the given Search Goal, mainly the Goal Loss Ratio
in combination with the Goal Exceed Ratio.

    LBTM26 - seemingly broken grammar, "managed to uphold all requirements", should be followed by stating what it means.
...
MKP2 As for any other probability function, we can talk about percentiles
