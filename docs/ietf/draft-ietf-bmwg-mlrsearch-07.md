...
<mark>This is a percentage of full-length trials that may be lossy
withou the load being classified as [An Upper Bound](#An-Upper-Bound).</mark>
...
<mark>VP to MK: The CSIT reason is ASTF tests where we want to hit
...
See also (search for) newly added "allowed if makes worse" principle.
</mark>
...
Especially if I say q-quantile when I mean something else entirely. :(
</mark>
...
<mark>Even if the medium allows higher performance,
the SUT interfaces may have their additional own limitations,
e.g. a specific fps limit on the NIC (a very common occurance).</mark>

<mark>[VP] TODO for MK: Edited the marked part. Good now?</mark>
...
<mark>VP to MK: Hmm. Frequently, Conditional Throughput comes
...
but I found few bugs when etsting in simulator).
...
<mark>VP to MK: Separate subsection on probabilistic distributions?</mark>
