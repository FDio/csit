...
+ [RFC2544] (section 24 Trial duration) full trial durations (implies short trials)
...
<mark>mk edit note: What about burst profile,
where the packet load is not evenly distributed in time
but instead is a series of packet bursts (packets sent at linerate in sequence)
separated by inter-burst gap. E.g. see IXIA user guide
[Bursts and the Inter-Burst GAP (IBG)](https://downloads.ixiacom.com/library/user_guides/IxOS/6.30/EA_6.30_Rev_A/IxiaReferenceGuide/GeneralTheory.html#wp1256803).
We don't need to address it now, but if this question comes
we should have an answer e.g. constant load has a burst profile
that can be described by burst size
(number of frames sent back-to-back at linerate) and inter-burst gap.
</mark>

Discussion:

<mark>[VP] TODO for MK: Unmark this paragrah and delete your comment from the definition:
...
<mark> mk edit note: Above sentence does not make sense to me.
Can you provide some examples of cases when
the Goal Duration Sum is > goal final trial duration,
and multiple full-length trials need to be performed as per sentence?
This is fundamental to MLRsearch, hence Priority-Zero, or P0 TODO here.</mark>

See [TST009 Goal](#TST009-Goal) for an example where possibility
of multiple full-length trials at the same load is intended.

<mark>[VP] TODO for MK: Unmark text above and delete your note (also from section title).</mark>
...
<mark>[VP] TODO for MK: Review the linked discussion, the unmark and delete.</mark>
...
One of alternatives to RFC2544 is described in
...
and such more repeatable and comparable.

Only the variant with "z = infinity" is achievable with only
the required attributes of search goal.
...
Simply, the smallest load among upper bounds.
...
<mark>mk edit note: This statement is unclear.
Needs to be reworded, and illustrated by example maybe to aid clarity.
This is fundamental to MLRsearch, hence Priority-Zero, or P0 TODO here.</mark>

In the MLRsearch spefication, the different values are called
[Trial Effective Duration](#Trial-Effective-Duration).

<mark>[VP] TODO for MK: Trial-Effective-Duration is optional.
Trial Duration Sum is fundamental, but now has examples in the specification.
Please review and clear your P0 notes if good enough.
</mark>
...
Even if medium allows higher performance (as in more frames per second
that with withing bandwidth when the frame size is small enough)
...
<mark>[VP] TODO for MK: Re-review after the "hard limit" paragraphs were added.</mark>
...
<mark>mk edit note: is "performance spectrum" an "object" or a "function"? This sentence is a very convoluted way of saying that it is a range of trial forwarding rate values positioned on a scale between two extreme points, is it not?</mark>
...
<mark>P0 TODO: please address the "q-value" edit note points raised earlier in the document.</mark>

<mark>If the quantile touches two trials,</mark>

<mark>mk edit note: What does it mean "quantile touches two trials"? Do you mean two trials are within specific quantile or percentile?</mark>
...
<mark>mk edit note: how is the median "touching" both trials? Isn't median of even set of data samples the average of the two middle data points, in this case the non-imaginary trial and the imaginary one?</mark>
...
<mark>mk edit note: I am confused by "Even if" prefixing each of the above statements. And even more so by your version with "If even".</mark>
...
<mark>mk edit note: "Performance probability function" is this function defined anywhere? Mention in [Performance Spectrum](#Performance Spectrum) is not a complete definition.</mark>
...
- One minus the Goal Exceed Ratio is called the <mark>inceed</mark> ratio. <mark>mk edit note: no "inceed" word in English language. I checked. Need alternative.</mark>
