...
TODO: Unify "the" before "MLRseacrh specification" when text is stable.
...
While any positive real value may be provided, some measurers may limit
possible values, e.g. by rounding down to neared integer in seconds.
In that case, it is RECOMMENDED to give such inputs to the controller
so the controller generated integer trial durations.
...
as defined in [RFC1224] (section 3.5 Data link frame size).
...
must be aware of common set of capabilities, so the traffic profile
uniquely defines the traffic during the search.
The important fact is that all such capabilities may be ignored
by the controller implementations.
...
(assuming lack of resources is still the only cause of lost frames).
...
This is an optional feature. If the measurer does not return
trial effective duration, the controller MUST use trial duration instead.
...
except when optionally passing trial output instance to the manager.
...
(TODO: See the other TODO on tester / TG / TA. Maybe not needed here?)
...
TODO: Deduplicate with MLRsearch Architecture subsection.
