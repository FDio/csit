...
* Intermediate Phases together with the Final Phase are called
  Non-Initial Phases.
* The returned bounds stay within presribed min_rate and max_rate.
  * In this case the returned bounds may be invalid.
  * E.g. upper bound at max_rate still leads to loss ratio not higher
...
1. First trial measures at configured maximum transmit rate (MTR) and
   ...
   * OUT: MRR = measured receive rate.
   ...
   from different metric than intended load. For example, Intended load can be
   in transactions (multiple packets each), but loss ratio is computed
   ...
   If MRR is less than min_rate, that is used.
...
3. Intended count.
   * The number of packets to send during the trial should be
     ...
     * Times a coefficient of loss is calculated from a different metric.
   * But in practice that does not work.
     * It could result in fractional number of packets,
...
4. Attempted count. As the real number of intended packets is not known,
