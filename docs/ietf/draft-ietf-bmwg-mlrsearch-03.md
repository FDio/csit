...
MLRsearch is a search algorithm which improves upon binary search
...
it is useful to describe key properties of MLRsearch
and how do they address the problems from the previous section.
...
The very first phase (instead of bisection the whole load spectrum)
...
Usage of any criteria that allow two or more trials per load (for the zero loss goal)
...
Input parameter (for each criterion separately) then governs what ratio
of overstepped subtrials is still tolerated to declare the whole trial honored
the criterion.
...
MLRsearch can deted a point when no further sub-trial results can possibly
change the decision, so they can be skipped.
Another time save comes from "upgrading" the load from a previous phase,
as sub-trials there remain valid for the next phase.

This assumes the time ovehead between trials is no too great,
as most traffic analyzers need a quiet period, as they cannot distringuish
...
but also give a rule to decide based on overstep frequency.
...
may become the opposite, so one bound is missing for bisection purposes.
MLRseach in that case starts an "external search", which is a variant
of expoentioal search. User can tweak "expansion coefficient"
depending on how close the new valid bound is expected to be.
...
MLRsearch is "conservative" in this situation, at least at the same assigned load,
meaning the high load is never a throughput if there is a smaller load
with non-zero-loss trial result.
The criterion ordering in MLRsearch relies on this design choice
to ensure the previously completer crteria never need to be re-visited.
...
{::comment}
mkonstan TODO: better title "Relation to RFC 2544"?
Based on the latest agreement, highlight that with certain parameter settings mlrsearch results are compliant with RFC 2544 non-zero loss throughput search. ie the final search executed with contigous 60sec trial duration.
{:/comment}
...
{::comment}
mkonstan TODO: Move this section to after [Purpose and Scope](#purpose-and-scope).
Instead of bloated content below, either stick to content from -02 draft or updated it with some of the below content, but reduce the volume of text.
{:/comment}
...
{::comment}
    mkonstan TODO: Why do we need this section here at all? Instead of making it RFC 2544 centric, why don't we make it mlrsearch centric. IOW explain mlrsearch workings, similar to how it was done in -01/-02 version of the draft, and then have a short section describing with which setting mlrsearch is compliant with RFC 2544. And that's it. Thoughts?
{:/comment}
...
{::comment}
mkonstan TODO: Where are the following sections gone, present in -02, but not here:
- MLRsearch Overview
  - Properties
  - Operation
  - Benefits
