...
a number of different packet processing modes and configurations,
drives the requirement of continuous test execution
and bringing down the test execution time.
...
to the noiseless DUT performance), thus indirectly improving
...
the loss ratio goal are though to be affected by SUT noise too much,
so noiseless DUT performance is closed to the remaining sub-trials
(as long as there are enough of them, governed by overstep ratrio).
...
The time savings claim assumes the time ovehead between trials is no too great,
...
In the owrst case, this can make MLRsearch slower than vanilla bisection
...
e.g. it simplifies integration with third-party measurers (or algorithms).
...
as multiple sub-trials per trial are not compliant with RFC 2544).
