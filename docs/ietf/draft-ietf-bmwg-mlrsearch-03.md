...
- *packet_loss_ratio_goal*: ratio of frames dropped to frames transmitted over the trial duration.
- *trial_duration*: amount of time over which packets are transmitted in a single measurement step.
- load: offered frame rate stimulating the SUT and DUT
- throughput: The maximum load at which none of the offered frames are dropped by the device (RFC 1242).
- throughput-with-PLR: The maximum load at which packet-loss-ratio of the offered frames are dropped by the device.
- *lower_bound*: load value denoting lower boundary of search.
- *upper_bound*: load value denoting upper boundary of search.
- *lower_upper_interval*: span between *lower_bound* and *upper_bound* loads.
- *precision_goal*: search stop condition, given as either absolute value in PPS *lower_upper_interval* or relative as ratio ( *lower_upper_bound* / *lower_bound* ).

## Description

MLRsearch approach to address identified problems is based on the following main strategies:

- The main inputs are the overall throughput search goals and parameters
  - A set of one or more *packet_loss_ratio_goals*
    - e.g. NDR with PLR=0 and one (or more) PDR(s) with PLR>0
  - *target_trial_duration* for final measurements
  - *target_precision_goal* for final measurements
- Search is executed as a sequence of phases
  - Initial phases focus on fast finding SUT's load range close to its forwarding rate regardless of PLR
  - Middle search phases narrow down SUT's load ranges close to its forwarding rates for the input set of PLRs, using short trial durations and low precision goals
  - Final search phase uses the main input goals and parameters to find throughput (PLR=0) and forwarding rates compliant with the input set of PLRs
- Each search phase produces following results
  - A valid *phase_lower_bound* and *phase_upper_bound* loads for specific *packet_loss_ratio_goal* such that
    - *phase_lower_bound* load results in loss lower than specific *packet_loss_ratio_goal*
    - *phase_upper_bound* load results in loss higher than specific *packet_loss_ratio_goal*
    - *phase_lower_upper_interval* is less than or equal to *phase_precision_goal*
  - *phase_lower_bound* and *phase_upper_bound* results are stored and re-used across phases if applicable
- Initial phases establish starting values of search bounds satisfying each *packet_loss_ratio_goal*
  - 1st initial phase uses FRMOL load to measure MRR rate 
  - 2nd initial phase uses MRT load to measure MRR2 rate
  - Starting values of phase_lower_bounds are set to MRR2 value for all *packet_loss_ratio_goals*
