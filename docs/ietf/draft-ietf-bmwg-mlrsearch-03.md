...
~~The intention of this document is describe problems with vanilla RFC 2544~~
~~througput bisection, ways they are solved in MLRsearch library,~~
~~and motivations for choosing those solutions over alternatives.~~
...
Ratio search), a throughput bisection approach optimized for software
networking.
...
* Modern DUTs show big spread of discovered throughput
  (single trial results are noisy, noisy neighbor problem).
* The definition of throughput is not clear when trial results are inconsistent
  ~~(loss inversion problem).~~

~~The solutions:~~
MLRsearch aims to address these problems as follows:

* Allow searching with multiple search criteria.
* Each trial result can affect any criterion in principle (trial reuse).
* Be conservative when encountering loss inversion. TODO: clarify.
...
* Requre less strict width condition for earlier phases. TODO: clarify.
* External search when previous bound becomes invalid.
* Use Forwarding rate at maximum offered load ([RFC2285](https://datatracker.ietf.org/doc/html/rfc2285#section-3.6.2)) to initialize early phases.
* Allow uneven interval splits to save some bisects. TODO: clarify.
