...
- *load*: Constant offered load stimulating the SUT and DUT. Consistent
  with [RFC 2285 Offered load](https://datatracker.ietf.org/doc/html/rfc2285#section-3.5.2).
...
- *conditional throughput*: The maximum load at which a list of
  specified conditions are met i.e. loss ratio goal and trial duration.
...
- Middle phases are producing ending bounds by improving upon starting bounds.
  ...
  - The starting bounds are usually the ending bounds from the preceding phase.
    - Unless there are older trial results that are more promising.
  - Each phase operates in a sequence of four actions:
    1. Perform trial at the load between the initial bounds.
      - Depending on the result this becames new valid upper or lower bound.
...
- Final search phase operates in exactly the same way as middle phases.
  ...
  - The ending bounds become the output of MLRsearch.
