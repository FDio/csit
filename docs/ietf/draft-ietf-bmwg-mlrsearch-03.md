...
SUT, consists of a software program processing packets (the under test
...
all programs and operating system running on the server. 

DUT is effectively "nested" within SUT, and its performance can be only
inferred based on direct measurement tests performed on SUT.
...
DUT's network performance. We refer to it as a SUT "noise".
...
(no realistic enough model of the noise is known). Instead, any
proposed throughput search algorithm dealing with software networking
SUT(DUT) setup should employ strategies recognizing the presence of the
SUT "noise" and discovering the maximum throughput of the DUT in the
