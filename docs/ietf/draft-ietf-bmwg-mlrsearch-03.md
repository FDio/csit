...
- Binary search takes too long when most of trials are done far from the
...
{::comment}
Note: Commenting out above line. Without explaining why, above statement
doesn't make sense. Re-adding previous content that got deleted.
{:/comment}
...
{::comment}
Note: Commenting out above line. On its own, this does not relate to long test duration.
{:/comment}
...
{::comment}
Note: Commenting out above paragraph as it does not belong to problem
statement, and indicates that proposed modifications make the problem
worse, without first covering what problems they solve.
{:/comment}
...
{::comment}
Note: Commenting out above paragraph as it is going into a philosophical
realm of how one can measure software performance without hardware
resources. The previous and next paragraphs make it clear enough that
that DUT performance can not be measured directly, at all.
{:/comment}
...
{::comment}
Note: I do not like referring to this DUT behaviour as "DUT noise". It
isn't noise, it is per-design behaviour.

Suggest to remove all reference to DUT noise. They are incorrectly
referring to the undesired DUT behaviour as noise, and pollute the
overall explanation. I went ahead and removed reference to the "DUT
noise", see what you think.
{:/comment}
...
the other end is a noiseful performance, in practice usually affected
by rare but powerful noise spikes.

{::comment}
Note: I don't like the wording here. "rare but powerful noise spikes"
Can you quantify the rareness using relative or absolute metric?
Same for powerful. Powerful enough for DUT to drop one packet, two
packets, more? vs noiseless performance?
{:/comment}

Well-constructed SUT will have the DUT as its performance bottleneck,
so SUT noiseless performance should match DUT performance.
...
at eliminating SUT noise from SUT measurement.
...
{::comment}
Note: I suggest to remove above paragraph.
I am still not convinced packet latency is a good analogy. We have
millions of data points per second with per packet latency, so that
percentiles are meaningful. We will never have millions of data points
for throughput measurements, not in practice.
{:/comment}
...
{::comment}
Note: easier vs harder problem vs "DUT or SUT" problem?
Aren't they all summarized by a single problem of inferring the DUT
throughput, in the presence of SUT noise, vs drilling into various
ideas tackling the easier or harder parts of that same very problem?
{:/comment}
...
{::comment}
Note: Similar to the previous comment. 
I don't like the wording here. "rare but powerful noise spikes"
Can you quantify the rareness using relative or absolute metric?
Same for powerful. Powerful enough for DUT to drop one packet, two
packets, more? vs noiseless performance?
{:/comment}
...
{::comment}
Note: I don't see any value in above paragraph, apart from indirectly
teasing the throughput definition in rfc 2544. Suggest a more
constructive approach to specify somewhere else in the document that
throughput is reported for specific packet loss ratio, as we have been
doing in CSIT for a while.
{:/comment}
...
{::comment}
Note: I can not relate to the last sentence. On what basis can you make
this claim?

And I like the previous version of this paragraph:
...
And an ad-hoc inconsistency resolution could prevent the search from converging.
...
{::comment}
Note: Commenting out last two paragraphs, as they start discussing possible
ideal solution(s), mixing in throughput redefinition and other
repeatibility enhancements, instead of finishing this sub-section with
clear statement.
{:/comment}
...
throughput meeting multiple packet loss ratio criteria, including zero-loss
and non-zero-loss. Following other input parameters are packed
with packet loss ratio goal to form a search "criterion":

- Packet loss ratio goal.
...
{::comment}
Note: Some of above are clear, some are not. They need to be either
covered in terminology section upfront, or briefly described here.
{:/comment}
...
{::comment}
Note: commenting out above. What "fields", i guess you meant criteria :)
They should be explained upfront or here.
{:/comment}

MLRsearch accepts an ordered list of criteria.

{::comment}
Note: what is this order? Is it as listed above? How does it apply?
{:/comment}
...
(the lower bound satisfies the packet loss ratio goal,
...
{::comment}
Note: this needs to be described better. Or if it can't be done without
diving into the detail, removed from here, and covered in MLRsearch
Features section.
{:/comment}
...
{::comment}
Note: what reported values? What is MLRsearch reporting: lower and upper
bounds for what criteria, all of them, or a subset applicable to the
final trial i.e. packet loss ratio, final precsion and final trial? It
is unclear from the current text. Basics need to be covered.
{:/comment}

MLRsearch employs multiple search phases
...
Adjective "final" is used for criteria related specifically for the final
determination phase, to make clear earlier phases may use different criteria.
