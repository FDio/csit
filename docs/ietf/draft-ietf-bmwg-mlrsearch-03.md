...
{::comment}
TODO: remove content in (..) in the problem listing.
{:/comment}
...
commercial off-the-shelf (COTS) compute server hardware (referred to as SUT, System Under Test),
...
interactions with other components of the SUT. But as RFC2544 is a
...
Any alteration to RFC2544 throughput search that improves repeatability
...
Discovering maximum throughput at both zero-loss and
...
While performing throughput search search by executing a sequence of
measurement trials, there is a risk of encountering inconsistencies
between trial results. 

Examples include:

- a trial at the same load (same or different duration) results in
  different packet loss.
- a trial at higher load (same or different duration) results in lower
  packet loss.

In all such situations, throughput search needs to decide how to handle
such inconsistencies.

Presence of high noise is likely to lead to trial inconsistencies. If
throughput search poorly handles trial inconsistencies, this is likely
to lead to poor repeatibility. And poor repeatibility is probably
because there is high noise.

An efficient way to handle trial inconsistencies will lead to better
results repeatability.

{::comment}
poor repeatibility is probably because you have high noise.
if you have high noise it is likely you will also encounter trial inconsistencies.
if your algorithm poorly handles trial inconsistencies, this is likely to lead to poor repeatability
{:/comment}
...
{::comment}
presence of incosistent trial results is likely to lead to longer overall test durations
{:/comment}
...
maximum throughput with multiple loss ratio goals, including zero-loss
and non-zero-loss.

MLRsearch employs multiple search phases to optimize time required to
get to interesting search region, using shorter durations in the early
phases, and the target duration in the final phase.

One or several sub-trials are used to obtain multiple data samples at
given load, that are subsequently used to reduce the dependency on
SUT-DUT noise. The sub-trial is the unit that has to satisfy
restrictions from RFC 2544 (sections 23 and 24).

Search for valid lower and upper throughput bounds is govern by an
ordered list of criteria, including:

- Loss ratio.
- Acceptable percentage of invalid sub-trial samples.
- Trial duration.
- Precision goal.
...
TODO need to capture sub-trials, search phases, order of criteria (loss ratio, % invalid sub-trial samples, trial duration, precision goal)
{:/comment}
