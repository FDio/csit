...
- *trial_duration*: Amount of time over which frames are transmitted in
  a single measurement step.
- *loss_ratio*: Ratio of the count of frames lost to the count of frames
  ...
  In MLRsearch *loss_ratio* is used as a measurement and as a goal
  - *trial_loss_ratio*: Loss ratio measured during a trial.
  - *loss_ratio_goal*: MLRsearch input parameter. If *trial_loss_ratio*
    is smaller or equal to this, the trial satisfies the
    *loss_ratio_goal*.
...
- *conditional-throughput*: The maximum load at which a list of
  specified conditions are met i.e. *loss_ratio*, *trial_duration*, 
  *noise-tolerance*.
- *lower_bound*: One of values tracked by MLRsearch during the search runtime.
  ...
  If the trial satisfies the *loss_ratio_goal*, it is a valid bound (else invalid).
- *upper_bound*: One of values tracked by MLRsearch during the search runtime.
  It is specific to the current trial duration and current loss ratio goal.
  It represents a load value with at least one trial result available.
...
- *precision_goal*: MLRsearch search stop condition, given as either
  absolute value in PPS *lower_upper_interval* or relative as ratio
  ( *lower_upper_bound* / *lower_bound* ).
