...
Depending on DUT behavior, different benchmark teams
can report significantly different throughput values,
even when using identical DUT and test equipment,
just because of minor differences in search algorithm used
...
The goal for the throughput search algorithm (including values for
tweakable parameters) should be to leave less freedom for the benchmark
teams to encounter the repeatability and comparability problem.
...
In order to better assess the actual throughput of DUT, both zero and
(small) non-zero packet loss ratios should be used. Intuitively both
values should be not far from each other. Repeated wide gap between NDR
and PDR indicates a fundamental problem with the DUT system.

The goal for the throughput search algorithm should be to optimize
finding throughput for multiple packet loss ratio criteria.
...
tests, limited amount of physical server resources to execute those
...
The goal for the throughput search algorithm should be to reduce the
overall test duration, and if multiple loss ratio criteria are desired,
optimize finding multiple throughput rates corresponding to those
criteria.
