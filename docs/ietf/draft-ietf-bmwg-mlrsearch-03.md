...
and offers a simple interface. The upper level component is purely in software,
it is also called MLRsearch (as a library) and it transforms input parameters
into return values based, calling Measurer to get data it needs.

## Measurer

The process of performing a trial is abstracted as a call to Measurer.
...
Measurer is supposed to start the traffic at constant intended load
and sustain it for the inteded duration. But Measurer may do more,
for example it may need to wait some time, as required by RFC 2544 section 23.
...
needed to return from Measurer (intended trial time plus any wait time
...
In those cases, the set their results is called a "load statistic".
...
{::comment}
TODO: Mention re-measurement and upgrade here?
{:/comment}
...
The sum of trial costs within a load statistic is called load cost.
...
The code doing this decision is abstracted as a "classifier".
...
If the load cost is larger than or equal to the classifier's assigned cost,
...
If the load cost is smaller than the classifier's assigned cost,
...
to no longer be below classifier's assigned cost.
...
than the achieved precision of the throughput search is known.
{:/comment}

## Return values

{::comment}
TODO: Place somewhere, or delete if redundant.
...
Frequently, a terminology from binary search is used, so the former is called
an upper bound and the latter a lower bound.
