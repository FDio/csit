...
total test duration and searching for multiple loss ratios in a single search.
...
- Multiple phases within one loss ratio goal search, early ones need
  to spend less time on trials.
  - Earlier phases also aim at lesser precision.
...
- Aply several load selection heuristics to save even more time
...
In the context of describing particular DUT's network performance, this
...
eventual throughput (so the do not affect the end result,
thus can be replaced by shorter trials).
...
(small) loss ratio as the criterion for a "throughput rate".
...
The following descriptions intentionally leave out important implementation
details. This is both to hide complexity that is not important for overall
undestanding, and to allow future improvements in the implementation.

## Terminology

- *trial duration*: Amount of time over which frames are transmitted
  (towards DUT) in a single measurement step.
- *loss ratio*: Ratio of the count of frames lost to the count of frames
  ...
  to [RFC 1242 definition of packet loss rate](https://datatracker.ietf.org/doc/html/rfc1242#section-3.6).
...
- *load*: Constant intended load (RFC 2285) stimulating the SUT and DUT.
  - The implementation works with intended load instead,
    as it cannot deal with situations where the offered load
    is considerably different than intended load.
...
- *conditional throughput*: The forwarding rate at the maximum load
  at which a list of specified conditions are met i.e. loss ratio goal
  and trial duration.
...
- *precision goal*: MLRsearch input parameter, acting as a search stop condition,
  given as either absolute or relative width goal.
  An interval meets a precision goal if:
...
- The main inputs include the following search goals and parameters:
  ...
  - "Final" trial duration (e.g. 60s for unconditional compliance with RFC 2544),
    possibly different for each loss ratio goal.
  - Precision goal, possibly different for each loss ratio goal.
- Search is executed as a sequence of phases:
  - *Initial phase* initializes (perhaps invalid) bounds for the next phase.
  - *Middle phase*s improve the bounds, using decreasingly short trial durations
    and lower precision goals. Several middle phases can precede each final phase.
...
- Each search phase produces its *ending* upper bound and lower bound.
  ...
  - Before a middle or final phase starts, it needs (at least) two values
    to act as *starting* bounds (may be invalid).
    Other trial results from previous phases may also be re-used.
  - Each phase may perform several trial measurements,
    until the phases' ending conditions are all met.
- The single initial phase establishes the starting values for bounds, using
  forwarding rate (FR, RFC 2285) of few trials of minimal duration
  (perhaps corrected if the first loss ratio goal is not zero):
  - 1st trial is done at *max load*, its FR is FRMOL (also RFC 2285).
  - 2nd trial is done at *FRMOL*, its FR is FRFRMOL (newly defined here).
  ...
  - By default, FRMOL is used as an upper bound, FRFRMOL as a lower bound.
    - Values can be manipulated here, e.g. when 2nd trial got zero loss,
      or if FRFRMOL is too close to FRMOL.
- Middle phases are producing ending bounds by improving upon starting bounds.
  - Each middle phase uses the same loss ratio goal as the final phase it precedes.
    - Called *current loss ratio goal* for upper/lower bound purposes.
  - Each phase has its own *current trial duration* (for bound purposes)
    and *current precision goal* parameters, computed from main MLRsearch inputs.
    As phases progress, these parameters approach the main MLRsearch input values.
  ...
  - The starting bounds are usually the ending bounds from the preceding phase.
    - Unless there are older trial results, (loads of) which are more promising.
  - Each phase operates in a sequence of four actions:
    1. Perform trial at the load between the initial bounds.
      - Depending on the trial result,
        this becames the first new valid upper or lower bound.
...
- Final search phase operates in exactly the same way as middle phases.
  ...
  - The current trial duration and current precision goal are taken directly
    from MLRsearch main inputs (not computed from them).
  - The forwarding rates of the ending bounds become the output of MLRsearch.
    - Specifically, the final lower bound is the conditional throughput
      for given loss ratio goal.

## Multiple trials per load

Newer versions on MLRsearch implementation allow for *noise tolerance* parameter.
...
This feature would make the description above considerably more complicated,
so this version of this document only describes earlier MLRsearch
implemented versions.

# How the problems are addressed

Configurable loss ratio goals are a direct support for non-zero-loss througput.
Indirectly, the conditional throughput results' stability
increases with the loss ratio goal in practice.

Multiple trials with noice tolerance parameter will also indirectly
increase result stability, and it will allow MLRsearch
to add all the benefits of Binary Search with Loss Verification,
as recommended in RFC 9004.
...
between short overall search time, result stability,
and how simple the definition of the returned contidional throughput can be.
