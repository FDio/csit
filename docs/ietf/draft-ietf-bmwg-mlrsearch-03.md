...
{::comment}
Last two sentences about the ordered list of search criteria are not clear.
{:/comment}
...
{::comment}
This list now belongs to text describing the ordered search criteria, similar to how it was covered in the previous patch(es).
{:/comment}
...
{::comment}
It is here where I suggest the explanation about the ordered search criteria is added.
{:/comment}
...
{::comment}
How is this different from "Acceptable percentage of sub-trials with loss higher than the goal."?
{:/comment}
...
{::comment}
Still missing explanation of ordered criteria, so that above statement with "previously completed criteria" can be understood fully by the reader.
{:/comment}
...
{::comment}
What is "subtrial overall duration"? Subtrial actual duration?
{:/comment}
...
{::comment}
How do the "route updates" fit in here?
{:/comment}
