...
While some benefits of MLRsearch (e.g. overall search duration)
depend on many implementation details, some other benefits (e.g. comparability)
can be described in a more abstract level.
Here I start describing what the output of MLRsearch is.
TODO: Turn the comment into official text.
{:/comment}

## Return values

{::comment}
Here is how the RFC 2544 throughput can be defined as a search return value.
Several trials were performed. Only those with duration of 60s
do matter in the following text.
Each intended load was examined only once, using a single trial.
Each trial either had zero loss observed, or a non-zero loss.
Assuming each non-zero loss trial was done at a load larger than
the load of any zero-loss trial,
one of the zero-loss trials had the highest intended load.
That load is the throughput to return.
RFC 2544 does not specify any stopping condition for the search.
But bisection can as easily report the upper bound for zero loss
(the smallest load which goe non-zero loss).
If both bounds are reported (the lower boun is the throughput),
than the achieved precision of the througput search is known.
{:/comment}

MLRsearch repeatedly chooses an intended load to examine,
examines the load using trials, and classifies the load based on a "classifier".
Some of the loads get returned as parts of MLRsearch output,
as the smallest load not satisfying a classifier,
or the largest load below that satisfying the classifier.

A classifier (as a logical component) takes the set of trial results as an input,
and outputs "does satisfy", "does not satisfy" or "not enough data".
The classification logic is always the same, but is governed by 4 parameter values,
so sometimes this document refers to the 4-tuple of parameter values
...
There is only one acceptable trial duration
for the trials in the classifier input set,
and that is the minimum of investigation time and max trial duration.
Only acceptable trials are considered, the others are forgotten.
Classifier input of zero trial results is always classified as "not enough data".

For each trial, its observed frame loss ratio is compared to
the classifier's loss ratio goal. If the goal is the same or higher,
the trial is "accepted", otherwise the trial is "overstepped".
If the sum of trials' investigation durations is larger than or equal to
the classifier's investigation time, the ratio of accepted trials
is compared to the classifier's acceptance ratio.
If the acceptance ratio is larger or equal, the classifer output is "does satisfy",
otherwise it is "does not satisfy".

If the sum of trials' investigation durations is smaller than
the classifier's investigation time, two hypothetical trial result sets
are created by adding a phantom trials. Their investigation durations
are set to the arithmetic average of the real trials' investigation duration.
Their number is the minimum to get the sum to no longer be below
classifier's investigation time.
One hypothetical set, called pessimistic, adds overstepped phantom trials.
...
it is returned as the result of the original set.
If the hypothetical results do not match, the real set is classified
as "not enough data".
