...
## Software DUT and Server SUT

RFC 2544 specifies a black-box (integrated) system as the testing
target, the DUT.

In case of software networking, we posit that the target DUT is a
software program dedicated to processing packets and running on
shared server hardware using operating system functions as
appropriate. 

To satisfy the requirement of black-box testing, we introduce a concept
of System Under Test (SUT) denoting the overall compute server system
that is accessible from outside through network interfaces and
encompassing server hardware, operating system, DUT software and any
other software running on this server and sharing its resources. DUT
is "nested" within SUT.
...
software running on the same server. This "noise" can negatively affect
DUT's network performance. We refer to it as a SUT-DUT "noise".
...
allocation by operating system during runtime, as well as any
interrupts or other interference with software threads involved in
frame processing. In practice this means low repeatability of
throughput results with specified precision goal.

With the goal of measuring the throughput of the DUT and considering
the "external" interference as noise, it is worth to define the DUT
boundaries.

{::comment} TODO
(DUT defined as a software program running on specific server
hardware, with interference (noise) coming from OS and other software
running on that hardware. Referred to as SUT-DUT noise).
TODO Does this belong here, or in terminology section?
{:/comment}
