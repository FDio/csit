...
* Bi-directional throughput tests: involve packets/frames flowing in
  ...
  reported as aggregate for both directions and/or separately for each
  measured direction.
...
* Packet Throughput Rate: maximum packet rate sustained by DUT/SUT at
  specified Packet Loss Ratio (PLR). In many cases the rate depends on
  the frame size processed bvy DUT/SUT. Hence packet throughput rate
  MUST be quoted with specific frame size as received by DUT/SUT during
  the measurement. For bi-directional tests, packet throughput rate can
  be reported as aggregate for both directions and/or separately for
  each measured direction. Measured in packets-per-second (pps) or
  frames-per-second (fps), equivalent metrics.
* Bandwidth Throughput Rate: a secondary metric calculated from packet
  ...
  bandwidth throughput rate can be reported as aggregate for both
  directions and/or separately for each measured direction. Expressed in
  bits-per-second (bps).
...
* Maximum Receive Rate (MRR): packet/bandwidth throughput rate
  ...
  bits-per-second (bps).
* Trial: a single throughput measurement step.
...
traditional binary search, while guaranteeing the same or similar
results.
...
adopted to discover more throughput rates with different pre-defined
PLRs.
...
flat or increasing with increased offered load. It may not be as
...
errors present e.g. out-of-sequence frames.
...
1. Main loop:
   ...
   a. IN: trial_duration for the current phase. Set to
      ...
      initial_strial_duration and final_trial_duration.
...
2. New transmit rate (or exit) calculation (for point 1.d.):
   ...
   b. If interval width is higher than the current phase goal:
      * Else, IF NDR interval does not meet the current phase width
        goal, prepare for internal search. The new transmit rate is (NDR
        lower bound + NDR upper bound) / 2.
      * Else, IF PDR interval does not meet the current phase width
        goal, prepare for internal search. The new transmit rate is (PDR
        lower bound + PDR upper bound) / 2.
