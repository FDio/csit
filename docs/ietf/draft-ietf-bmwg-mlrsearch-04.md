...
MLRsearch integrally derives a set of search targets.
...
The conditional throughput is found when focusing on teh final target.
...
to form a chain ending in the final target corresponding to the search goal.
...
If the (measured) trial duration is at least as long
as the target trial duration, the trial is called long.
...
A composite of 8 quantities is useful when defining how does
...
If there was a bad long trial, optimistic load exceed ratio
...
Note that is the missing duration sum is zero,
optimistic load exceed ratio becomes equal to pessimistic load exceed ratio.
...
is always bigger than the optimistic load exceed ratio.
...
In that case, MLRsearch does not need to measure more trial
...
Other parts of MLRsearch view only relevant bounds,
...
If the target exceed ratio is zero, and a load has at least one short trial
(short or long), the load becomes an upper bound.

###### Lower bound

A load is classified as an lower bound for a target,
if and only if both optimistic load exceed ratio
...
If the target exceed ratio is zero, all trials at load at or below
...
relevant bounds have width no larget than the goal width.
...
During its lifetime, selector proceedes through the following modes.
In order, but some modes may be skipped.

Each mode has its own way of determining the candidate load (if any).
...
Selector uses a flag to avoid entering this mode
once it finished measuring the halved load.

####### Upgrading

This mode activates when only one bound for the current target is available.
...
Initial target selectors start at this mode.
...
as it knows no other selectors may have changes the relevant loads unexpectedly.

#### Controller output

The output object the controller return to the manager
...
There MAY be several way how to communicate the fact a conditional output
...
The conditional throughput is the average of relative forwarding rates
...
Configurable loss ratio in MLRsearch search goals are ther
