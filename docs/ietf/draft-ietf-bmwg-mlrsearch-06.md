...
MLRsearch offers several ways to address these challenges, giving the user
...
In the context of characterizing particular DUT's network performance,
...
But, due to the logarithmic nature of bisection, even small improvement
in the overall search duration needs a relatively big sacrifice
in the precision of the discovered throughput.
...
Due to the shared multi-tenant nature of SUT, DUT is subject to
...
what other engineering disciplines define as noise.
Thus, all such effects are called SUT noise.
...
the other end can be called a noiseful performance. In practice, trial result
...
among trial results. Also, the extreme noiseless end of the SUT spectrum
...
In practice that is not possible, as based on the author's experience
...
as the noiseless end of the SUT performance spectrum. At least for
...
In this document, we reduce the DUT in the SUT problem to estimating
the noiseless end of the SUT performance spectrum from a limited number of
trial results.
...
loss ratio as the goal for their load search.
...
The presence of other components (mainly the SUT) is also implied.

While the manager and the measurer can be seen as abstractions
...
The creation of reports of appropriate format can also be understood
...
who want to control the overall search duration,
not just the traffic part of it. The manager MUST report
...
Note that this is very similar, but not identical to the Forwarding Rate
...
is based on frame counts aggregated over all SUT output interfaces.
...
happened to use the whole goal duration sum (using full-length trials),
...
comes from the worst trial still not exceeding the goal loss ratio.
...
The search result is a single composite object returned from the controller
to the manager.
...
The subsections define attributes for a regular goal result.
...
at the end of the search as an upper bound (see Appendix A) for this goal.
...
as a lower bound (see Appendix A) for this goal.
This is a REQUIRED attribute.
...
as evaluated at the relevant lower bound at the end of the search.
...
This document aspires to prescribe the MLRsearch specification
...
simply calling them bounds. But the old values still B remain valid bounds,
...
The next difference is in the goals of the search. [RFC2544] has a single goal,
...
especially with a non-zero goal exceed ratio.
...
lower bound. In the future, other intuitive values may become popular,
...
But, this kind of bias is unlikely in MLRsearch implementations.
The reason is precisely in the desire to have short searches.
Even splits are the best way to achieve the desired precision,
...
The two areas of focus in this chapter are load classification
...
is larger than the goal loss ratio. The trial that is not bad is called good.
...
the worst case may still happen, so the conditional throughput
...
Note that Appendix B may partially take into account also short trial results.
By construction, all such results are sorted to come after the quantile.
In practice, the increased duration sum is still smaller
than the goal duration sum, so there is no difference.
...
Scenario one. The user had their reason for not configuring a shorter goal
...
good full-length trials. In extreme cases, there are no good short trials.
...
Scenario one. If the duration is too short, we expect to see a higher frequency
of good short trials. This could lead to a negative excess sum,
...
Scenario two. Settings with a small exceed ratio
...
Scenario three. Here excess sum is small in absolute value, as the balancing sum
...
TODO: Here will be a chapter summarizing how the MLRsearch library
addresses the problems from the Identified Problems chapter.
...
Any intended load value can be classified, according to any given search goal.
