...
The MLRsearch library employs the following enhancements to address these issues:
...
the measurer multiple times, receiving the trial result from each call.
After exit condition is met, the controller returns
the overall search results back to the manager.

The only required argument in the call to the controller
is a list of search goals.
For the structure of the search result,
...
It then hands over the execution
...
As MLRsearch has no reason to focus on a load when the load is classified,
that logic also implies that usually the conditional throughput value
ends up being based on the worst trial that does not exceed the goal loss ratio.
Unless another search goal motivates additional trials
at the relevant lower bound, of course.
...
First example.
For zero exceed ratio, when goal duration sum
has been reached.
The conditional throughput is the smallest forwarding
...
is zero.
This is not reported to the user, as this load
