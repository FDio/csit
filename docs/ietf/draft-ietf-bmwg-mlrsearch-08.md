...
The Measurer is an abstract system component that,
when called with a [Trial Input] (#Trial-Input) instance,
performs one [Trial] (#Trial) and returns
a [Trial Output] (#Trial-Output) instance.
...
a lossless trial becomes a new lower bound.
The span of values between the tightest lower bound
and the tightest upper bound (including both values)
...
But the old values still remain valid bounds, just not as tight as the new ones.
...
computation. One of the ways relies on performance spectrum.
...
The code works correctly even when there are no trial results at a given load.
