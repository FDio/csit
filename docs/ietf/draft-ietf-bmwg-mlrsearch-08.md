...
- Insert multiple coarse targets for each Search Goal, earlier ones need
  ...
  - Use Forwarding Rate (FR) at maximum offered load
    [RFC2285] (Section 3.6.2) to initialize initial targets.
...
throughput for non-zero Goal Loss Ratio value, while being precise enough
...
MLRsearch specifies Relevant Lower Bound to address this problem.
...
Any such compliant test procedure is called an MLRsearch Implementation.
...
each Search Goal, thus becoming
a Lower Bound or an Upper Bound for that Search Goal.

When the relevant bounds are at Loads that are close enough to each other
...
MLRsearch specification uses a number of specific measurement quantities,
...
TODO2: MK - not sure what you mean here?
...
This quantity does not include any preparation or waiting
...
TODO2: MK there should be a reference about the last TODO in one of 1242 2285 or 2544.
...
Goal Final Trial Duration value. Otherwise
...
TODO2: MK not sure i like the use of "intuitions behind" statement
above. We also have section "Trials with Longer Duration".
...
Goal Loss Ratio (lossy trials or noisy trials) is tolerated at a Lower Bound.
...
TODO2: MK MUST is fine here IMV.
...
TODO2: MK or delete the 1st sentenece altogether as it is stating the obvious.
...
TODO: Create subsection for sample? MLRsearch Implementation.
TODO2: MK what do you want to cover there - if just a pointer to LFN FD.io implementation, then that's okay. else?

The meaning of listed attributes is formally given only by their indirect effect
on the search results.

TODO2: MK i don't understand this sentence. it's not saying anything. remove?

See [Compliance ](#compliance) for important Search Goal examples.

#### Goal Initial Trial Duration

TODO2: MK this should be moved up before the "Search Goal" section, and
at the same section level as "Goal Width" section.
...
Min Load is mainly useful for saving time by failing early, e.g. when
...
but are computable before the Search ends.

The adjective **current** marks a quantity that is computable
before the Search ends, and the computed value may change during the Search.
...
TODO2: MK not needed, as Glossary is moving to Index back matter.
...
is called a **short trial**.

#### Full-Length Trial

A trial with Trial Duration equal to the Goal Final Trial Duration
is called a **full-length trial**.
...
TODO2: MK above note doesn't apply anymore. Fixed by adding Long Trial
subsection. Otherwise the term full-lenth trial is overloaded and
confusing.
...
It is the maximum value in a range being searched, together with Lower
...
In more detail, the set of all trial outputs (short, full-length and long)
...
The following subsections define its attributes first,
...
first attribute, Goal Final Trial Duration, makes it unconditionally
compliant.
