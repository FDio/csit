...
{::comment}
    ...
    stopping conditions
    multiple trials (for exceed or short trials)
    short trials for upper bound
    ...
    other search time optimizations (e.g. load selection) unspecified
    effective trial duration?
{:/comment}
...
Those bounds are "hard", in the sense that all futher load values
are smaller than the upper bound and larger than the lower bound.
...
based on classifying full-length trials as either zero-loss or non-zero-loss.
...
are smaller than any upper bound and larger than any lower bound.
...
may still be a Lower Bound for another Search Goal, and due to this other goal
...
Only single-goal search algorithms can have hard bounds
that shield them from loss inversion.

TODO: Capitalize Loss Inversion?
...
what ratio of trial seconds) can have a [Trial Loss Ratio](#Trial-Loss-Ratio)
...
TODO question: why is this relevant here? Inclusion of wait times is
already described in [Trial Effective Duration](#Trial-Effective-Duration),
why the repeat here.
...
But if Max Load is set higher than what interface can receive or transmit,
