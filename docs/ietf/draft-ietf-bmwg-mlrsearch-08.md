...
{::comment}
    ...
    ## Performance Spectrum
     (theoretic performance distinguished from measurement results)

    ### Behavior Descriptions
     (continuing DUT In SUT discussion in minimal detail needed for scenarios)

    ### Example SUT Behaviors
     (basis for following scerarios)

    ## Scenario Intuitions
     (selected examples where we have intuition)

    ### Scenario 1 Intuitions
     Same Duration Best Case scenario.

    ### Scenario 2 Intuitions
     Same Duration Worst Case scenario.

    ### Scenario 3 Intuitions
     Mixed Duration False Good scenario.

    ### Scenario 4 Intuitions
     Mixed Duration True Bad scenario.

    ### Scenario 5 Intuitions
     Mixed Duration Balanced scenario.

    ### Scenario 6 Intuitions
     Mixed With Long Durations scenario.

    ## Algorithms
     (pseudocode translated into senteces)
    ### Load Classification Algorithm
    ### Conditional Throughput Algorithm

    ## Scenario Analysis
     (same scenarios as in intuitions, but now following the algorithms)
    ### Scenario 1 Analysis
    ### Scenario 2 Analysis
    ...
...
In all cases, Effective Trial Duration is equal to Trial Duration.
...
- Load is low, most trials are zero-loss.
...
- 1 more 1-second trial, probably zero-loss.
...
SUT behavior at the current load:
- Load is high, most trials are lossy.
- Exceed Probability is almost one.

Trials measured so far at the current load:
- Three 1-second trials, each of zero loss.
- This is very unlikely situation for the SUT behavior.

Intuition:
- Still needs measuring, as Goal Duration Sum is not reached yet.

Likely future knowing SUT behavior:
- 4 more 1-second trials, probably all of them lossy.
...
SUT behavior at the current load:
- Load is high, but Trial Loss Ratios depend on Trial Durations.
...
SUT behavior at the current load:
- Load is very high, Trial Loss Ratios depend on Trial Durations,
...
- It is somewhat unlikely to see not low-loss trial,
...
Comments:
- This shows short trials may be good replacement for missing full-length trials.
...
SUT behavior at the current load:
- Load is moderate, Trial Loss Ratios do not depend on Trial Durations,
...
- MLRsearch may assume high-loss short trials are reliable
  (as they were in Scenario 4) but their Exceed Frequency is not high enough.
