...
a Lower Bound or an Upper Bound for that goal.
...
or when some goals are proven to have only irregular results.
...
{::comment}
    [Not mentioning this as it is not clerarly a per-interface quantity.]

    Input Frame Rate of [RFC2544] (section 23. Trial description)
...
TODO: Mention dusplicate, previous-trial and other "more than expected"
...
of multiple trials at the same load is intended.
...
TODO: Link to subsection in Problems chapter discussing why non-zero is ok.
...
TODO: Point to explanation subsection when stabilized.
...
TODO: Link to Explanations section when stabilized.
...
TODO: Link to the compliance section for the two goal examples.
...
TODO: Bump Goal Initial Trial Duration to a separate subsection (as Min Load)?
...
arriving at an Irregular Search Result when Min Load is classified
as an Upper Bound.
...
TODO: Recall "the Search" from overview? No, use glossary instead.
...
TODO: Link to explanation sections?
...
{::comment}
    + simple binary search (to compare with)
    + stopping conditions
    + loss inversion (with multiple goals)
    - multiple trials and exceed ratio
     + content is there
     - deduplicate
     - separate effective trial duration?
    + short trials
    + genaralized throughput (mainly for non-zero goal loss ratio)
    - other search time optimizations (trial selection) unspecified
    - warmup, low-first and other implementaton recommendations
     + mentioned
     - not placed properly
    - (multiple TODOs on moving Load Classification details out)
{:/comment}
...
That means, the search needs bot Max Load and Min Load values,
...
For given pair of Max Load and Min Load values,
...
TODO: Define "confirmed bounds"?

## Stopping Conditions and Precision

As MLRsearch specification requires listing both Relevant Bounds for each
Search Goal, the difference between the bounds implies
the result precision achieved,
making it unnecessary to report the specific stopping condition used.

MLRsearch implementations may use [Goal Width](#Goal-Width)
...
Due to various possible time optimizations, there is no longer a scrict
...
Each bound is [RFC2544] simple binary search is "hard",
...
Goal Final Trial Duration, thus no longer behaving
as a pure single-goal search.

### Multiple Goals and Loss Inversion

As MLRsearch supports multiple goals, any implementation of the search procedure
is more complicated (compared to binary search with a single goal),
but most of the complications do not affect the final results much.
Except for one phenomenon: Loss Inversion.

In MLRsearch, a Load that is classified as an Upper Bound for one Search Goal
...
As a consequence of the way the Relevant Bounds are defined,
...
TODO: Define bound invalidation?
...
whose Effective Trial Durations when summed up give a value at least as big
...
TODO: Deduplicate all the above as ,ost is in specification chapter already.

TODO: Find proper place for:
...
## Short Trials and Shortcuts

TODO: Not shortcuts. Duration Selection perhaps?
...
While MLRsearch implementation are free to use any logic to select
Trial Input values, comparability between MLRsearch implementation
is only assured when the Load Classification logic
handles any set of Trial Results in the same way.

The presence of short trial results complicates
the load classification logic, somewhat weakening the intuitions
behind Load Classification algorithm.

While the Load Classification algorithm is designed to avoid any unneeded Trials,
for exaplainability reasons it is RECOMMENDED for users to use
such Controller Input instances that lead to all Trial Duration values
selected by Controller fo be the same,
e.g. by setting any Goal Initial Trial Duration to be a single value
also used in all Goal Final Trial Duration attributes.

TODO note: below to be removed once [Load Classification] and [Load Classification Logic] are done.
...
than Relevant Lover Bound for comparability purposes.
...
## WiP: Reducing Search Time

TODO Not sure we need this section at all. If we do, it should be a
standalone one, covering high-level description of search strategies
used by MLRsearch to explore the performance spectrum of SUT.

TODO: Merge with "shortcuts" part of Short Trials?
TODO: Perhaps "Trial Selection"?

MLRsearch has been designed to reduce the time
