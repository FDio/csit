...
- Balancing sum (T) is the short low-loss sum multiplied by the exceed
  coefficient.
...
- High-loss sum (T) is the full-length high-loss sum plus the positive
  excess sum.
- Measured sum (T) is the high-loss sum plus the full-length low-loss
  sum.
- Whole sum (T) is the larger of the measured sum and the Goal Duration
  Sum.
- Missing sum (T) is the whole sum minus the measured sum.



- Pessimistic high loss sum (T) is the high-loss sum plus the missing
  sum.
- Optimistic exceed ratio (T) is the high-loss sum divided by the whole
  sum.
- Pessimistic exceed ratio (T) is the pessimistic high-loss sum divided
  by the whole sum.
