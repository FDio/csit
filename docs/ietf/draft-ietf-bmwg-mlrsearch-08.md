...
TODO-P0: Mention definitions are not informative? e.g. Goal Final Trial
Duration and Goal Initial Trial Duration have the same Definition
text. (note that these are already fixed for now.)
...
TODO-P0: review updated definition, check if any informal explanation is
needed.

Definition:

A minimum value for Trial Duration required for classifying the load as
a Lower Bound.
...
This section describes two pieces of MLRsearch logic, Load
Classification and the Conditional Throughput, using informal language
reflecting formal pseudocode representation present in [Appendix A: Load Classification](#appendix-a-load-classification) 
and [Appendix B: Conditional Throughput](#appendix-b-conditional-throughput).
This is followed by sample scenarios.
...
TODO-P1: Make more readable, e.g. by distinguishing inputs from temporary variables.
...
TODO-P0: Move somewhere else?

By construction, Conditional Thruoughput algorithm results in a value
that represents Trial Loss Ratio at mest equal to Goal Loss Ratio.
...
TODO-P2: Lucky if less than expected number of high-loss trials?
...
TODO-P2: Mention how ER relates to EP here?
...
TODO-P2: Define loss spike? Mention loss spikes when discussing noise?
...
In the following example, the SUT is well-behaved
in the sense that all Exceed Probability values are constant during the Search
(the Trial Results are independent).

The example is related to a Search that was started with multiple Search Goals.
Several points in time are chosen, to show how to algorithms work
with specific sets of Trial Result available.

In all Trials, the Effective Trial Duration is equal to Trial Duration.

Only one Trial Load value is in focus,
Trial Results at other Loads are not mentioned.
In practice, they would be present, e.g. MLRsearch will look for a Lower Bound
smaller than any Upper Bound found.

In all points in time, only one Search Goal instance is marked as "in focus".
That explains Trial Duration of the new Trials,
but is otherwise unrelated to the algorithms.

{::comment}

TODO-P1: Expand all into proper text.
...
- code 60f120s0l50o
 - (TST009)
 - Goal Final Trial Duration = 60s
 - Goal Duratiobn Sum = 120s
 - Goal Loss Ratio = 0%
 - Goal Exceed Ratio = 50%
- code 1f120s.5l50o
 - (similar to TST009 but for PDR and using 1s trials)
 - Goal Final Trial Duration = 1s
 - Goal Duratiobn Sum = 120s
 - Goal Loss Ratio = 0.5%
 - Goal Exceed Ratio = 50%
- code 60f60s0.5l20o
 - (show nontrivial exceed ratio and classification flip due to more trials)
 - Goal Final Trial Duration = 60s
 - Goal Duratiobn Sum = 60s
 - Goal Loss Ratio = 0.5%
 - Goal Exceed Ratio = 20%

Added trials as time increases:
- code 59x1s0l
 - 1f120s.5l50o goal was in focus
 - Initial 59 trials, each 1 second and zero loss.
 - (last totally undecided)
- code 59x1s0l+1x1s100l
 - 1f120s.5l50o goal was in focus
 - Added one 1s trial of say 1% loss.
 - (first ub: rfc2544)
- code 59x1s0l+60x1s100l
 - 1f120s.5l50o goal was in focus
 - Added 59 more 1s trials of say 1% loss.
 - (last undecided for 1f120s.5l50o)
- code 60x1s0l+60x1s100l
 - 1f120s.5l50o goal was in focus
 - Added one more 1s trial with zero loss.
 - (first lb 1f120s.5l50o)
- code 60x1s0l+60x1s100l+1x60s1l
 - 60f120s0l50o goal was in focus
 - Added one 60s trial with 0.1% loss.
 - (60f60s0.5l20o flips to lb)
- code 60x1s0l+60x1s100l+1x60s1l+1x60s0l
 - 60f120s0l50o goal was in focus
 - Added one 60s trial with zero loss.
 - (tst009 becomes lb)


Tables:

After 59x1s0l (last totally undecided):

Quantity                  | 60f60s0l0o  | 60f120s0l50o | 1f120s.5l50o | 60f60s0.5l20o
--------------------------|-------------|--------------|--------------|--------------
...
After 59x1s0l+1x1s100l (first ub: rfc2544):
...
Pessimistic exceed ratio  | 100%        | 100%         | 50.833%      | 100%
Classification            | Upper Bound | Undecided    | Undecided    | Undecided

After 59x1s0l+60x1s100l (last undecided for 1f120s.5l50o):
...
Pessimistic exceed ratio  | 100%        | 100%         | 50.833%      | 100%
Classification            | Upper Bound | Undecided    | Undecided    | Upper Bound

After 60x1s0l+60x1s100l (first lb 1f120s.5l50o):
...
Classification            | Upper Bound | Undecided    | Lower Bound  | Upper Bound

First Conditional Throughput: 1f120s.5l50o

TODO-P0: Show algorithm.

After 60x1s0l+60x1s100l+1x60s1l (60f60s0.5l20o flips to lb):
...
Classification            | Upper Bound | Undecided    | Lower Bound  | Lower Bound

Second Conditional Throughput: 60f60s0.5l20o

TODO-P0: Show algorithm for both?

After 60x1s0l+60x1s100l+1x60s1l+1x60s0l (tst009 flips to lb):
...
Classification            | Upper Bound | Lower Bound  | Lower Bound  | Lower Bound

Third Conditional Throughput: 60f120s0l50o

TODO-P0: Show algorithm for all?
