...
Again, this is conceptual, real procedure might not have explicit messages.
When comparing distinc allowedt behaviors, the discussion
...
&nbsp;
: SUT is the only entity that can be benchmarked, even though
only the performance of some sub-components are of interest.
...
{::comment}
    ...
    VP: TODO: Put bursty and other non-constant loads outside of the scope?

{:/comment}

{::comment}

    VP: TODO: What about multiple-interface loads if not equal among interfaces?

    VP: TODO: What about interfaces with different medium capacity (bandwidth or pps)?
