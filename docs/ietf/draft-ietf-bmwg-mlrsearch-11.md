...
#          (Expired.)
#
#    VP: TODO201: Delete or update or replace the active item.
#
#{:/comment}
  Lencze-Kovacs-Shima:
...
{::comment}
    ...
    VP: Informative context in introduction. Normative summary in Scope.
...
{::comment}
    ...
    MK: But section 24 of RFC 2544 does list "binary search", and it is
        this that has been adopted as a defacto standard. Re-adding it back
        in :)
...
{::comment}
    ...
    TODO202: Either say the list talks about CSIT implementation,
...
{::comment}
    ...
    MK: TODO203. We do not have any section in this document covering
        implementation details, as these are out of scope. Shall we add a
        note to that regard?
...
{::comment}
    ...
    TODO204: Update the subsection above when the subsections below are complete enough.
...
{::comment}
    ...
    VP: DONE: Verify the .txt render is still ok.
...
{::comment}
    ...
    VP: TODO205: Mention conditional requirements if not clear from usage.
...
There is no separate terminology section.
When a term is introduced, its definition
is immediately followed by methodology information.
Each term is defined in a subsection
of some sections with "Terms" in the section title.
Subsections of other sections still contain methodology information
that does not bind to a singular term.

{::comment}

    VP: DONE: Explain there is no separate list for terminology,
...
{::comment}
    ...
    VP: TODO206: Should we explicitly mention that
    the table of contents also acts as a terminology list?
...
{::comment}
    ...
    VP: Describe this informal principle in official text?
    VP: DONE: Not in draft, maybe in e-mail.

    VP: Distinguish requirements from other discussions?
    VP: DONE: Not in draft11, maybe in darf12?
...
{::comment}
    ...
    DONE: Calls/invocations, interfaces.
...
MLRsearch Architecture describes a set of abstract interacting components.
The transfer of information between the components
is described in imperative programming style,
as function "calls" with arguments as "inputs" and return values as "outputs".
Again, this is conceptual, real procedures might not rely on explicit messages.
When comparing distinct allowed behaviors, the discussion
talks about "implementations" of the component.

{::comment}

    VP: TODO207: Re-check usage of Implementation.

{:/comment}

A test pocedure is said to comply with MLRsearch Specification
if it can be conceptually divided into conceptual components,
...
by calling Conntroller once for each benchmark.
...
{::comment}
    ...
    VP: Imperative programming introduced, "calls" is not correct.
...
they have different requirements and their stopping conditions are out of scope.
...
{::comment}
    ...
    Repeating the same search is possible, this is about single search.
    DONE by mentioning repeated benchmarks.
...
Instead, normative requirements on the content of the Test Report
...
Some discussed aspects are important and specific to MLRsearch Specification,
that is why these terms get full subsections, instead of just external references.

{::comment}

    VP: TODO208: Downgrade the last sentence from draft to e-mail?
...
{::comment}
    ...
    VP: Contruct appropriate "conditional requirement" sentence.
    DONE: Sentence improved.

{:/comment}

{::comment}

    VP: DONE: No time to mention "allowed if worse" principle in draft11 cycle.
...
{::comment}
    ...
    VP: DONE: Emphasize that this is a single trial.
...
(as opposed to a single recurring Trial).

## Trial Terms

{::comment}

    TODO209: Separate short descriptions from further discussions. Everywhere.
...
{::comment}
    ...
    VP: DONE: Put bursty and other non-constant loads outside of the scope?

{:/comment}

{::comment}

    VP: DONE: What about multiple-interface loads if not equal among interfaces?

    VP: DONE: What about interfaces with different medium capacity (bandwidth or pps)?
...
{::comment}
    ...
    VP: DONE: Re-check the current text.

{:/comment}

&nbsp;
: Similarly to Trial Duration, some Measurers MAY limit the possible values
of Trial Load. Contrary to Trial Dduration,
...
{::comment}
    ...
    VP: DONE: Use normative MAY somewhere.
...
&nbsp;
: For example, tests with symetric bidirectional traffic
can report load-related values as "bidirectional load"
(double of "single direction load").
...
{::comment}
    ...
    VP: TODO210: Mention the YANG draft as a possible avenue?
...
{::comment}
    ...
    VP: Multiple traffic profiles (at least frame sizes) in RFC2544,
    this is about single SUT+config+profile benchmark.
    DONE: I thihnk the current sentences are good enough.
...
{::comment}
    ...
    VP: DONE: Apply stricter conditional requirements.
    E-mail: explain conditional requirements.
...
{::comment}
    ...
    VP: DONE: Add the missing sentence.
...
aggregate counter-based approach.
...
(which is based on single-interface load).

&nbsp;
: Consequently, for symmetric bidirectional Traffic Profiles (section 14
of [RFC2544], the Trial Forwarding Rate value is equal to the arithmetic average
...
{::comment}
    ...
    VP: DONE: Checked, it is average.

{:/comment}

&nbsp;
: Given that Trial Forwarding Rate is a quantity based on Load,
Test Report MAY express it using multi-interface values
(becoming the sum of per-interface forwarding rate values).
...
{::comment}
    ...
    VP: DONE: After Trial is stable, simplify this sentence.
...
designed to manage the overall Ssearch duration,
...
{::comment}
    ...
    VP: Done: Stricter conditional requirements not needed.
...
{::comment}
    ...
    While Controller implementations SHOULD NOT include additional attributes
...
{::comment}
    ...
    MK: Edited this one. Applied to subsections in Trial Terms and
          Goal Terms as appropriate. TODO211 check if more places need this.
...
{::comment}
    ...
    VP: DONE: Check if Specification does not mention "implementation".
...
{::comment}
    ...
    VP: DONE: Use MUST NOT to make Controller behavior constrained, conditionally?
...
{::comment}
    ...
    VP: DONE: Reformulate to avoid the "we" construct.

    VP: DONE: Do we need Invalidation as a separate term? I guess no.
...
In practice, the Load frequently becomes a [Upper Bound](#upper-bound) instead.
...
{::comment}
    ...
    VP: DONE low priority: Regular end, irregular exit, user abort.
...
{::comment}
    ...
    VP: DONE elsewhere: Emphasize one controller call gives one benchmark.
...
the following subsections finalize their definitions
...
{::comment}
    ...
    VP: DONE: Nested "definition list" does not work. Use quotes here?
...
{::comment}
    ...
    MK: TODO213. This is not a bad idea. A section that in summary table
    ...
    VP: I think too low priority for draft11.
...
{::comment}
    ...
    DONE: Recheck specification minimizes user/iplementation discussions.

    DONE low priority: Add those discussions here somewhere is useful.
...
{::comment}
    ...
    VP: DONE: Reformulate to avoid "we" construct.
...
{::comment}
    ...
    MK: Disagree. Can we have it in a proper code block instead?

    VP: DONE: block with tags.
...
{::comment}
    ...
    MK: Not table, it's code. Can we have it in a proper code
          block instead?
...
{::comment}
    ...
    MK: Move to Appendix A, before the pseudocode Appendices.
          Keeping it here for now to finish editing with clean change
          tracking in gerrit.

    VP: DONE: Appendix C now. A and B are for pseudocode as that is more important.
...
{::comment}
    ...
    VP: WONTFIX as currently not possible without coding effort:
    https://github.com/gettalong/kramdown/issues/593
...
{::comment}
    ...
    TODO214: There are long lines.

{:/comment}

{::comment}

    VP: DONE: Fix warnings from kramdown.
...
{::comment}
    ...
    <mark>VP TODO215 Final Checks. Only mark as done when there are no active todos above.</mark>

    <mark>VP Rename chapter/sub-/section to better match their content.</mark>

    <mark>MKP3 VP TODO216: Recheck the definition dependencies go bottom-up.</mark>

    <mark>VP TODO217: Unify external reference style (brackets, spaces, section numbers and names).</mark>

    <mark>MKP2 VP TODO218: Capitalization of New Terms: useful when editing and reviewing,
    ...
    <mark>VP TODO219: If time permits, keep improving formal style (e.g., using AI).</mark>
