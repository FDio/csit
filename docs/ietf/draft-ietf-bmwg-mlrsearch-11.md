...
{::comment}
    ...
    the new methodology is independent from (while possible to combine with) RFC2544.
...
This document does not include a stand-alone "Terminology" section.
Instead, each term is defined where it first appears, immediately
followed by the relevant methodology guidance. Sections titled "Terms"
contain these paired definitions and methodology for multiple terms,
while other sections present methodological guidance that is not tied
to a single term.
...
{::comment}
    ...
    MK: Edited. Reworded to avoid repetition of subsections and such.
...
{::comment}
    ...
    MK: I suggest we refrain from introducing and using DAG or other
        analogies from graph theory to explain how terms and terminology
        are managed in this document.
...
imperative-programming idiom: one component “calls” another, supplying
...
Instead, normative requirements of the content of the Test Report
...
Trial Forwarding Rate, by contrast, uses a single set of aggregate
frame counts and therefore yields one value that represents the whole
system, while still preserving the direct link to the per-interface
load.

&nbsp;
: Consequently, for symmetric bidirectional Traffic Profiles (section 14
ofWhen the Traffic Profile is symmetric and bidirectional, as defined in
Section 14 of [RFC2544], the Trial Forwarding Rate is numerically equal
to the simple average of the individual per-interface forwarding rates
...
&nbsp;
: Because this rate is anchored to the load defined for each interface,
a test report MAY show it either as the single aggregate figure just
described or, when that presentation is clearer, as the sum of the
separate per-interface forwarding rates.
