...
author:
   -
    ...
    fullname: Maciek Konstantynowicz
    org: Cisco Systems
    email: mkonstan@cisco.com
   -
    ins: V. Polak
    fullname: Vratko Polak
    org: Cisco Systems
...
informative:
  ...
  TST009:
    ...
    title: "Network Functions Virtualisation (NFV) Release 3; Testing; Specification of Networking Benchmarks and Measurement Methods for NFVI"
    author:
      - org: ETSI
    date: 2020-12
    refcontent: ETSI GS NFV-TST 009 V3.4.1
  Y.1564:
    target: https://www.itu.int/rec/dologin_pub.asp?lang=e&id=T-REC-Y.1564-201602-I!!PDF-E&type=items
    title: "Ethernet service activation test methodology"
    author:
      - org: ITU-T
    seriesinfo:
      ITU-T Recommendation: Y.1564
    date: 2016-02
  FDio-CSIT-MLRsearch:
  ...
  PyPI-MLRsearch:
    author:
      - org: Python Package Index
    target: https://pypi.org/project/MLRsearch/1.2.1/
    title: "MLRsearch 1.2.1"
    date: 2023-10
  I-D.lencse-bmwg-rfc2544-bis:
    display: Lencze-Shima
  Lencze-Kovacs-Shima:
    seriesinfo:
      International Journal of Advances in Telecommunications, Electrotechnics, Signals and Systems,: vol.
        9, no. 2, pp. 10-17
      DOI: 10.11601/ijates.v9i2.288
    title: Gaming with the Throughput and the Latency Benchmarking Measurement Procedures
      of RFC 2544
    author:
    - name: Gábor Lencse
      ins: G. Lencse
  ...
  Ott-Mathis-Semke-Mahdavi:
    ...
    author:
      - name: Matthew Mathis
        ins: M. Mathis
        org: Pittsburgh Supercomputing Center, Teunis Ott, Bellcore
      - name: Jeffrey Semke
        ins: J. Semke
        org: Pittsburgh Supercomputing Center, Teunis Ott, Bellcore
      ...
      - name: Teunis Ott
        ...
        org: Bellcore
    date: 1997-07
    refcontent: ACM SIGCOMM Computer Communication Review, vol. 27, no. 3, pp. 67-82

  I-D.ietf-bmwg-network-tester-cfg:
    display: Vassilev

  IEEE.802.3df:
    target: https://standards.ieee.org/ieee/802.3df/11107/
    title: "IEEE Standard for Ethernet Amendment 9: Media Access Control Parameters for 800 Gb/s and Physical Layers and Management Parameters for 400 Gb/s and 800 Gb/s Operation"
    date: 2024-03
    author:
      org: IEEE
    seriesInfo: 
...
This document describes an alternative to throughput in "Benchmarking Methodology for
...
(COTS) CPU hardware vs. purpose-built Application-Specific Integrated Circuit (ASIC), Network Processing Unit (NPU), and Field-Programmable Gate Array (FPGA) hardware.
...
- Software DUTs show noisy Trial Results,
  leading to a big spread of throughput values that could be discovered.
- Throughput requires a loss of exactly zero frames, but the industry best practices
  frequently allow for tolerance of low but non-zero losses (see [Y.1564] and test-equipment manuals for more information).
- The definition of throughput is not clear when Trial Results are inconsistent
  ...
  throughput metric in {{RFC1242}} and {{RFC2544}} can no longer be pinned to a single, unambiguous
  value.)
...
3. Insert multiple coarse targets for each Search Goal; earlier ones need
   ...
   - Use Forwarding Rate at Maximum Offered Load (FRMOL), as defined
...
class of benchmarks that fits within the wider framework of {{RFC2544}};
see {{scope}} ("Scope").
...
mainly the [Binary Search](#binary-search) for unconditionally compliant {{RFC2544}} Throughput.
...
"Trial duration" ({{Section 24 of RFC2544}}) mentions binary search only in quotes,
without providing specifics.
In this document, we call that algorithm the Binary Search,
as that is the title of Section 12.3.2 of {{TST009}}, which describes a variant of it.

Here is a simplified description of the algorithm:

* Initialize the lower-bound variable to a line-rate Load value.
...
* If loss is zero, set the lower-bound to a midpoint value;
  else, set the upper bound to a midpoint value.
...
The description in {{TST009}} has two more requirements
...
First, a significant amount of time is spent on Trials
...
SUT:
: The collective set of network devices to which
stimulus is offered as a single entity and response measured ({{Section 3.1.2 of RFC2285}}).
...
lowers the observed SUT performance as observed in Trial Results.
...
will also help with the repeatability problem.
...
For more information, see {{Section 5 of I-D.lencse-bmwg-rfc2544-bis}}
(and the references there) for a few synthetic examples, confirming that
each protocol and application can have different realistic loss ratio values.
...
which is not evident from the result of a single goal load search procedure.
...
example, goodput of TCP connection (TCP throughput {{RFC6349}}), but the relationship
...
Ideally, there will be a definition of a new quantity that both generalizes
...
it uses the normative language of BCP 14 {{RFC2119}} {{RFC8174}} to provide an unambiguous specification.
...
This section provides all technical definitions
needed for evaluating whether a particular test procedure
complies with the MLRsearch Specification.
...
Each term-defining subsection contains a short "Definition" paragraph
...
* An abstract architecture consisting of functional components
  (Manager, Controller, and Measurer) and the requirements for their inputs and outputs.

* The required structure and attributes of the Controller Input,
  including one or more Search Goal instances.

* The required logic for Load Classification, which determines whether a given Load
...
The MLRsearch Specification is an independent methodology
and does not change or obsolete any part of {{RFC2544}}.

This specification permits deviations from the Trial procedure
as described in {{RFC2544}}. Any deviation from the procedure in {{RFC2544}}
must be documented explicitly in the Test Report,
and such variations remain outside the scope of the original benchmarks in {{RFC2544}}.
...
can be adapted to also satisfy MLRsearch requirements for a specific Search Goal.
...
* The potential for, and the effects of, interference between different Search Goal instances
  within a multi-goal search are considered outside the normative scope of this specification.

## Architecture Overview

Although the normative text only references terminology that has already
...
by calling the Controller once for each benchmark.

The Manager calls a Controller once,
and the Controller then invokes the Measurer repeatedly
until the Controller decides it has enough information to return outputs.
...
The MLRsearch Specification prescribes [Regular Goal Results](#regular-goal-result)
...
When the Relevant Lower Bound is close enough to the Relevant Upper Bound
...
paragraphs in {{RFC2544}} are still requirements on Test Reports
if they apply to an MLRsearch benchmark.

### Behavior Correctness

The MLRsearch Specification by itself does not guarantee that
...
The MLRsearch Specification
uses a number of specific quantities;
some of them can be expressed in several different units.

In general, the MLRsearch Specification does not require particular units to be used,
...
are also computable before a Search ends.
...
value of "duration" is expected to be equal to the "final duration" value.
...
- "Benchmarking Terminology for LAN Switching Devices" {{RFC2285}} includes
...
Definitions of some central terms from the above documents are copied and
...
even though only the performance of some sub-components are of interest.
  ...
  For example, {{Section 6.1 of RFC2544}} decribes a single SUT
...
on commodity x86/ARM CPUs (vs. purpose-built ASICs, NPUs, and FPGAs).

  When MLRsearch specification mentions SUT configuration,
it usually implies adequate configuration of constituent DUTs,
subjected to requirements from {{Section 7 of RFC2544}}.

  A well-designed SUT should have the primary DUT as their performance bottleneck.
The ways to achieve that are outside the scope of the MLRsearch Specification.
...
: The traffic is sent only in phase c) and received in phases c) and d).
  ...
  It is useful to consider the traffic
...
as defined in BCP 14 {{RFC2119}} {{RFC8174}} to signify the strength of the requirements.
For the purposes of the MLRsearch Specification,
the test procedure MAY deviate from the description in {{RFC2544}},
...
compared to those described in phases a), b), d), and e).
  ...
  {{RFC2544}} seems to treat phase b)
...
{{Appendix C.2.4.1 of RFC2544}} lists another example: ARP with a wait time of 5 seconds.
...
This section defines new terms and redefines existing terms for quantities
relevant as inputs or outputs of a Trial, as used by the Measurer component.
This also includes any derived quantities related to results of one Trial.
...
the nearest integer in seconds. In that case, it is RECOMMENDED to give
...
as long as the intuition of a load intended against a single interface can be applied.

  It MAY be possible to use a Trial Load value to describe non-constant traffic
...
MUST explicitly mention exactly how non-constant the traffic is
and how it reacts to a Trial Load value.
...
MAY be described by a single Trial Load value (e.g., using the largest load among interfaces),
...
"Bidirectional traffic" ({{Section 14 of RFC2544}}),
Trial Load is the data rate per direction, half of the aggregate data rate.
...
undocumented) in practice.
  ...
  The Controller MAY select the Trial Load and Trial Duration values in a way
...
units. Each reported Trial Load value MUST unambiguously state whether
...
with the prevailing practice described in {{RFC1242}}, {{RFC2544}}, and {{RFC2285}}.
...
that are needed for the unique determination of the Trial to be performed.
...
are outside of the scope of this document.
An example standardization effort is described in {{I-D.ietf-bmwg-network-tester-cfg}}.
  ...
  * Symmetric bidirectional traffic:
  ...
  Other traffic properties that need to somehow be specified
...
However, if this is not the case, the Test Report MUST describe the Traffic Profile
in sufficient enough detail to imply how the Trial Forwarding Ratio should be calculated.

  The Trial Forwarding Ratio MAY be expressed in other units
...
does not forward at all, the Trial Forwarding Ratio would be 0.5 (50%).
  ...
  In future extensions, more general ways to compute the Trial Forwarding Ratio
...
is equal to 100% minus the Trial Forwarding Ratio.
  ...
  This is almost identical to Frame Loss Rate in {{Section 3.6 of RFC1242}}.
...
3.6.1 of RFC2285}}. Under the method described in {{RFC2285}}, each output interface is
...
that would be produced by the procedure described in {{RFC2285}}.

  For more complex traffic patterns, such as many-to-one, as mentioned
in "Partially meshed traffic" ({{Section 3.3.2 of RFC2285}}),
...
and the Trial Forwarding Rate is 0.7 million fps.
...
: The Trial Effective Duration is a time quantity related to a Trial.
By default, it is equal to the Trial Duration.
...
: When referring to more than one Trial, the plural term "Trial Outputs" is
...
they are not familiar with,
  ...
  An example of an optional attribute is
...
: When referring to more than one Trial, the plural term "Trial Results" is
...
["Overview of RFC 2544 Problems"](#overview-of-rfc-2544-problems), but these discussion
...
classified as a Lower Bound.
  ...
  The Controller may choose shorter Trial Durations;
results of those Trials may be enough for classification as an Upper Bound.
...
: This is a threshold value for a particular sum of Trial Effective Duration values.
The value MUST be positive.
...
(of the same Search Goal) could reduce Search time but is NOT RECOMMENDED,
as the time savings come at the cost of decreased repeatability.

  In practice, the Search can spend less than the Goal Duration Sum measuring
a Load value when the Trial Results are particularly one-sided,
...
Trials shorter than the Goal Final Trial Duration.
...
: This is a threshold value for Trial Loss Ratio values.
The value MUST be non-negative and smaller than one.

Discussion:

: A Trial with the Trial Loss Ratio larger than this value
...
of the Trial Loss Ratio to the Goal Loss Ratio.
...
: This is a threshold value for a particular ratio of sums
of Trial Effective Duration values.
The value MUST be non-negative and smaller than one.

Discussion:

: Informally, up to this proportion of the Trial Results
...
The actual full logic is more complicated, as shorter Trials are allowed.
  ...
  For explainability reasons, the RECOMMENDED value for the exceed ratio is 0.5 (50%),
...
: This is a threshold value for deciding whether two Trial Load values are close enough.
...
The Search stops if every Search Goal has reached its precision.
...
but implementations may choose a different way to specify width.
  ...
  It is RECOMMENDED to express the Goal Width as a relative difference and
...
: This is the minimal value for the Trial Duration suggested to use for this goal.
...
Such Trial Results MUST be still accepted by the Load Classification Logic.

  The Goal Initial Trial Duration is a mechanism for a user to discourage
...
some of which are required.
  ...
  The required attributes are Goal Final Trial Duration, Goal Duration Sum, Goal
...
may also require users to specify "how quickly" Trial Durations should increase.
...
required by some implementations of MLRsearch.
  ...
  If the Max Load value is provided, the Controller MUST NOT select
...
as opposed to using a different value for each Search Goal.
  ...
  If the Min Load value is provided, the Controller MUST NOT select
...
and the implementation can apply the relative Goal Width safely.

  Given that Min Load is a quantity based on Load,
Test Report MAY express this quantity using multi-interface values,
as the sum of per-interface minimal Loads.
...
A Trial with a Trial Loss Ratio larger than a Goal Loss Ratio value
is called a **High-Loss Trial**, with respect to the given Search Goal
(or lossy Trial, if the Goal Loss Ratio is zero).

#### Low-Loss Trial

If a Trial is not a High-Loss Trial, it is called a **Low-Loss Trial**
(or zero-loss Trial, if the Goal Loss Ratio is zero).

#### Short Trial

A Trial with a Trial Duration shorter than the Goal Final Trial Duration
is called a **Short Trial** (with respect to the given Search Goal).

#### Full-Length Trial

A Trial that is not a Short Trial is called a **Full-Length Trial**.

Note that this includes Trial Duration values larger than the Goal Final Trial Duration.

#### Long Trial

A Trial with a Trial Duration longer than the Goal Final Trial Duration
...
as such by a [Load Classification Code](#load-classification-code)
...
Trial Results within the time remaining till the Goal Duration Sum.
...
for example, when Max Load is classified as a Lower Bound.
  ...
  Conversely, when the Relevant Upper Bound does exist,
it is not affected by the Max Load value.

  Given that the Relevant Upper Bound is a quantity based on Load,
Test Report MAY express this quantity using multi-interface values,
as the sum of per-interface Loads.
...
But the algorithm can be applied at any time on any Lower Bound,
...
Test Report MAY express this quantity using multi-interface values,
as the sum of per-interface forwarding rates.

### Goal Results

The MLRsearch Specification is based on a set of requirements
...
: Implementations MAY add their own attributes.
  ...
  Test Report MUST display the Relevant Lower Bound.
Displaying the Relevant Upper Bound is RECOMMENDED,
...
for example, if two Search Goals differ only in the Goal Width value,
...
: The MLRsearch implementation MAY return additional data in the Controller Output,
e.g., the number of Trials performed and the total Search Duration.

## Architecture Terms

The MLRsearch architecture consists of three main system components:
...
e.g., the Trial Forwarding Ratio not being larger than one.
...
between the Intended Load and the Offered Load into the increased Trial Loss Ratio.

  Neither of these two recommendations are made into mandatory requirements,
...
: Informally, the Controller has considerable freedom in selection of Trial Inputs,
...
: The Manager initializes the SUT (constituent DUTs) and the Measurer
...
details, including the description of a "default" DUT configuration
common for most tests and configuration changes if required by a specific test.
...
of the 802.3 IEEE standard {{IEEE.802.3df}}. In this case, it is acceptable
...
is compliant with the MLRsearch Specification.
...
the MLRsearch architecture is said to be compliant with the MLRsearch Specification.
...
The Goal Loss Ratio and Goal Exceed Ratio attributes
...
The Goal Duration Sum prevents MLRsearch
...
A non-zero exceed ratio is not strictly disallowed, but it could
needlessly prolong the Search when Short Low-Loss Trials are present.
...
The rationale of such a Search is to repeat High-Loss Trials, hoping for zero loss on the second try,
...
For example, for the "max(r) = 2" variant, the following Search Goal instance
...
so a third Full-Length Trial is never needed.
...
single-ratio [Binary Search](#binary-search) in {{RFC2544}} and walks through the
...
and one Trial is used to confirm Min Load is a Lower Bound.

Then, Trial Load is chosen as the mean of the current tightest upper bound
...
The MLRsearch Specification requires listing both
Relevant Upper Bound and Relevant Lower Bound
for each Search Goal, and the difference between the bounds implies
whether the precision needed for Regual Goal Result is achieved.
...
The MLRsearch Specification supports multiple Search Goals, making the Search procedure
more complicated compared to [Binary Search](#binary-search) with a single goal,
...
This introduces questions any multi-goal Search algorithm has to address, such as:
...
Does it mean the later Low-Loss Trials do not count toward a Lower Bound?
...
is considered to be a fluke, at least when computing the Goal Result.

This is formalized using the definitions of
[Relevant Upper Bound](#relevant-upper-bound) and
[Relevant Lower Bound](#relevant-lower-bound).

The Relevant Upper Bound (for a specific Search Goal) is the smallest Load classified
...
and the Relevant Lower Bound are used in the output.

### Consequences

The consequence of the way the Relevant Upper Bound and Relevant Lower Bound are defined
...
For MLRsearch implementations, this means that it is better to measure
...
by infrequent effects, causing unsatisfactory repeatability of {{RFC2544}} Throughput results.
Refer to ["DUT in SUT"](#dut-in-sut) for a discussion about noiseful and noiseless ends
...
A zero exceed ratio means all Trials must have a Trial Loss Ratio
...
Therefore, each Search Goal also has an attribute called Goal Duration Sum.
...
Even if the bandwidth of a medium allows higher traffic forwarding performance,
...
Consider that the hard limit is at one hundred million frames per second (100 Mfps),
Max Load is larger, and the Goal Loss Ratio is 0.5%.
If DUT has no additional losses, 0.5% Trial Loss Ratio will be achieved
at the Relevant Lower Bound of 100.5025 Mfps.

Reporting a throughput that exceeds the SUT's verified hard limit is
counterintuitive. Accordingly, the {{RFC2544}} Throughput metric should
...
In the hard limit example, a 100.5025 Mfps Load will still have
only a 100.0 Mfps forwarding rate, nicely confirming the known limitation.
...
values simply due to DUT performance variability.
...
values may have a relative difference up to the Goal Loss Ratio.
...
This section uses informal language to describe two aspects
...
A detailed description of an example Search is in
...
- Derive initial thresholds:
  ...
  - Subceed ratio (T) is one minus the Goal Exceed Ratio (I).
  - Remaining sum (T) is initially the Full-Length sum multiplied by the subceed
    ratio.
...
- Compute Conditional Throughput:
  ...
  - Current forwarding ratio (T) is one minus the current Trial Loss Ratio.
...
In ["DUT in SUT"](#dut-in-sut), the notion of noise is introduced.
This section uses new terms
to describe possible SUT behaviors more precisely.

From a measurement point of view, noise is visible as Inconsistent Trial Results.
...
the expert can assess the probability of each outcome.

### Exceed Probability

When the Controller selects a new Trial Duration and Trial Load,
...
Exceed Probability is very low for Short Trials but very high for Full-Length Trials.
...
This is the main reason users may want to set a large Goal Final Trial Duration.

#### Mild Increase

Short Trials are slightly less likely to exceed the Goal Loss Ratio,
...
Short Trials have a larger Exceed Probability than Full-Length Trials.
...
which is not commonly seen in practice.
...
Benchmarking activities as described in this document are limited to the
...
Thanks to {{{Gábor Lencse}}}, {{{Giuseppe Fioccola}}}, {{{Carsten Rossenhövel}}}, and BMWG
...
{{{Alec Hothan}}} of the OPNFV NFVbench project for a thorough review and
...
{{{David Dong}}}, {{{Nabeel Cocker}}}, {{{Lars Eggert}}}, {{{Jen Linkova}}},
{{{Mike Bishop}}}, and {{{Éric Vyncke}}} for their reviews and valuable comments.
...
and variables holding effective quantities start with "effect_"
...
The block at the end of this appendix holds pseudocode
that computes a value stored as the `conditional_throughput` variable.
...
one hypothetical run of the Search part of MLRsearch test procedure
...
and the number of times identical Trial Results were added.
...
- Similarly, when a name contains "long", it means the added Trial
  ...
  but a Long Trial for the "1s final" goal.

- When a name contains "good", it means the added Trial is a Low-Loss Trial
  for all the Search Goals.

- When a name contains "short bad", it means the added Trial is a High-Loss Trial
  for all the Search Goals.

- When a name contains "long bad", it means the added Trial
  is a High-Loss Trial for goals "RFC2544" and "TST009",
  but it is a Low-Loss Trial for the two other Search Goals.
...
The code for available Trial Results is: 59x1s0l.
...
The code for available Trial Results is: 59x1s0l+1x1s1l.
...
Due to zero Goal Loss Ratio, the "RFC2544" goal must have a mild or strong increase
...
is classified as an Upper Bound for the "RFC2544" goal this early.
...
The code for available Trial Results is: 59x1s0l+60x1s1l.
...
This is the last point for the "1s final" goal to have this Load still Undecided.
Only one 1-second Trial is missing within the 120-second Goal Duration Sum,
but its Trial Result will decide the classification result.
...
The code for available Trial Results is: 60x1s0l+60x1s1l.
...
The code for available Trial Results is: 60x1s0l+60x1s1l+1x60s.1l.
...
As designed for the "TST009" goal, one Full-Length High-Loss Trial can be tolerated.
...
so the optimistic exceed ratio is still only 50%.

But the 0.1% Trial Loss Ratio is lower than the "20% exceed" Goal Loss Ratio,
...
The code for available Trial Results is: 60x1s0l+60x1s1l+1x60s.1l+1x60s0l.
...
as the three Search Goals will want an Upper Bound (unless this Load is Max Load).
...
calculations. By contrast, the other three Search Goals treat the same Load as
...
The Conditional Throughput is computed from a sorted list
...
The "1s final" has a Goal Final Trial Duration of 1 second,
...
The Conditional Trhoughput value does not depend on the order of 0% loss Trials.
...
Due to a stricter Goal Exceed Ratio, this Conditional Throughput
is smaller than Conditional Throughput of the other two Search Goals.
