...
- Set multiple preceding targets for each search goal, earlier ones need
...
the rest of the enhancements are treated as implementation details,
...
mainly a binary search for RFC 2544 compliant throughput.

The last chapter will summarize how the problems are addressed,
the middle chapter providing explanations and definitions needed for that.
...
increased both the number of performance tests (for a single DUT)
...
But, due to an exponential behavior of bisection, even small improvement
...
interference coming from the operating system and any other
...
These effect affect DUT's network performance negatively.
...
DUT can also exhibit fluctuating performance itself, e.g. due to pauses
for some internal stateful processing. In many cases this
...
eliminated. Such behavior affects trial results in a way similar to SUT noise.
As the two phenomenons are hard to destinguish,
this document uses the word noise as a shorthand covering both
internal DUT performance fluctuations and genuine SUT noise.
...
The impact of the noise varies in time, sometimes wildly.
The noise can sometimes be negligible, but frequently
it lowers the observed SUT performance in a trial.
...
TODO: Distinguish "potential" performance values from "observed" ones?
...
Also the binary search result heavily depends
on how early in the search such a low performance trial happens.
...
loss ratio as the goal for a "throughput rate".

Motivations are many:
i) Modern protocols tolerate frame loss better
then it was the case at the time when [RFC1242] and [RFC2544] were specified.
ii) Trials nowadays send way more frames within the same duration.
iii) Small bursts of frame loss caused by noise have smaller impact
on the trial result, as during other parts of the same trial
the SUT may work mroe closely to its noiseless performance,
thus perhaps lowering the trial loss ratio below the goal loss ratio value.
v) If an approximation of the SUT noise on the trial loss ratio is known,
...
Especially when users are allowed to specify the goal loss ratio value.
This usefulness is enhanced when users can specify multiple
...
performance better than a single search goal result. Repeated wide gap between
zero and non-zero loss conditional throughputs indicates
the noise has a large impact on the observed performance.
...
This allows the subsequent chapters to focus purely on required details
without digressing into explanations.
...
leaves plenty of space fo future improvements, at the cost
of poor comparability of results of different MLRsearch libraries.
...
so it is not required to report the goal width value.
...
MLRsearch keeps the basic logic of binary search (tracking bounds,
...
and the final reported value specifies both bounds
...
The upper bound is considered real, the lossless trials at higher loads
...
(When they are less than width apart, the relevant bounds are used in output.)

One consequence is that every trial result can have an impact.
That means of your SUT (or your traffic generator) needs a "warmup",
...
and RFC 2544 prescibes wait intervals around trial traffic,
...
than the relevant lower bound (for non-zero goal loss ratio values),
...
In fact, setting the goal width below the goal loss ratio
may cause the conditional throghput for a larger loss ratio to became smaller
than a conditional throughput (from the same search) for a goal
with a smaller goal loss ratio, which is counter-intuitive.
Therefore it is RECOMMENDED to set the goal width to a value larger
...
conditional throughput values are when the interval is split in a very uneven way.
...
of its results will be, assuming the user configuration remain the same.
...
best repeatability at given search duration at different goals,
...
are good for comparability between different implementation.
...
The following search goal ensures unconditional compliance with RFC 2544
throughput search procedure.
...
equal to each other, and the value is the RFC 2544 throughput.
If the 60 second value is replaced by a smaller value,
the two values are still equal and conditionally compliant with RFC 2544 throghput.
...
The definitions here are wordy, implementers can look into next chapter
...
is shorter than what RFC 2544 section 23 specifies.
...
Load is the quantity implied by Constant Load of RFC 1242,
...
Traffic profile is REQUIRED by RFC 2544 to contain some specific quantities
...
This is consistent with RFC 2544 zero loss tolerance motivation.
...
TODO: Adapt and properly introduce the following code.
