...
MLRsearch introduces the concept of width. The sreach stops
...
MLRsearch keeps this basic logic, perhaps with a minor technical clarifications.
...
As a consequence, even if a load does have few trial results,
...
Different goals may have different goal width.
...
MLRsearch will probably perform trials at higher loads.
...
that are lower bounds and also smaller than the relevant upper bound.
...
If more than one trial is needed to classify a load,
...
Zero exceed ratio mean all trials have to have trial loss ratio
...
Presence of sorter trial results complicates load classification.
