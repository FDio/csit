...
- Initialize with multiple coarse targets for each search goal, earlier ones need
  to spend less time on trials.
  - Earlier targets also aim for lesser precision.
  - Use Forwarding Rate (FR) at maximum offered load
    [RFC2285] (section 3.6.2) for the initial targets.
...
and the frequency of running those tests as part of development and integration pipelines (CI/CD).
...
The DUT is effectively nested within the SUT.
...
These adverse effects are hard to predict in general, as they have impact similar to
...
The two phenomenons are hard to destinguish,
...
This chapter contains the technical specification of MLRsearch test procedures.
...
{::comment}
    mkonstan:
    This section should high level describe what are the functions of each component.
    With next level of detail covered in the following per component sub-sections.
{:/comment}
...
Manager is the component that initializes SUT, traffic generator
...
{::comment}
    mkonstan:
    Description here is light on detail.
    Current content implies simply just test environment setup.
    It is not clear what configuration the Manager is responsible for, this should be added
{:/comment}
...
{::comment}
    mkonstan:
    Creation of which reports, test reports?
    Is the Manager responsible for the Presentation layer, or just rendering the results into the appropriate data model?
{:/comment}

### Measurer

Measurer is the component which performs one trial
...
{::comment}
    mkonstan:
    how does traffic generator fit into this architecture?
    which compoent interacts with it?
    intuitively it should be the Measures, but it is not listed.
{:/comment}
...
{::comment}
    mkonstan:
    Measurer definition is very abstract and light.
    It does not indicate any interactions with other components e.g. traffic generator.
    "to uphold any requirements and assumptions present in MLRsearch specification" is too vague.
    "(e.g. trial forwarding rate not being larger than one)" is this really the main requirement that the Measurer needs to comply with?
{:/comment}

### Controller

Controller is the component of MLRsearch architecture
...
{::comment}
    mkonstan:
    Controller definition is very abstract and light.
{:/comment}
