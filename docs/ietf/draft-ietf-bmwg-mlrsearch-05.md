...
and an additional noise. The noiseless performance is assumed to be constant.
...
The noiseful end consists of rare occasions of significantly low performance,
but the long trial duration makes those occasions not so rare on trial level.
...
iii) Small burst of frame loss caused by noise have smaller impact on trial result,
as the noiseless performance in other parts of the same trial
can compensate somewhat by keeping the loss ratio below the goal.
v) If SUT noise with "an ideal DUT" is known, it can be set as the goal loss ratio.
...
the SUT noise has a large impact on the observed performance.
...
Sadly, different implementations may exhibit their sweet spot
...
Aas shortening the overall search duration is the main motivation
...
between MLRsearch implementations.
