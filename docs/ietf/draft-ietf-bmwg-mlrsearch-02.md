...
(1.1 No need to measure, loss count has to be zero.)
1.2 Use the zero load as the current lower bound.
2. Chose max load to be the max value allowed by bandwidth of the medium.
2.1 Perform a trial measurement (at the full length duration) at max load.
2.2 If there is zero trial loss count, return max load as Throughput.
2.3 Use max load as the current upper bound.
3. Repeat unless the difference between lower bound and upper bound is
   larger than the precision goal.
3.1 If it is not larget, return the current lower bound as Throughput.
3.2 Else: Chose new load as the arithmetic average of lower and upper bound.
3.3 Perform a trial measurement (at the full length duration) at this load.
3.4 If the trial loss rate is zero, consider the load as new lower bound.
3.5 Else consider the load as the new upper bound.
3.6 Jump pack to 3.
...
## Popularity of non-zero target loss ratios
...
Also, since those times, new "software DUTs" (traffic forwarding programs
running on on-the-shelf host hardware) appeared, and their Throughput results
frequently exhibit quite small repeatability.

* discovering multiple rates of interest is required for software DUTs
  running on COTS general purpose servers (x86, arm)

  * in general, throughput rates of software DUTs (programs)
    are sensitive to server resources allocated to them by OS during runtime,
    as well as to any system interrupts

    * these are general purpose systems, not purpose built,
      single function systems
    * as such server resources shared across multiple programs
    * real time packet processing makes it sensitive to any interrupts
      or blocking of software threads
