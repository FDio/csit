...
its own operational data. The structure of operational data CAN depend on the
node and its current state. The structure of the operational data MUST be JSON
compatible.
...
The `test-id` must be structured as it is in current version of CSIT, so the
test generator is able to create the the test.
