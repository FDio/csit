...
See an example SUT specification [here](topology_model.json). The
**specification of the topology** or topology model is considered to be an
**input information**. The **unified test interface** is considered to be an
**output information**.
...
Key-value pairs defining the metadata of the model. Not specified yet.
...
CAN be added, or the existing CAN be changed. It can be useful, e.g. when there
are more than one very similar nodes in the topology.
...
Unique ID identifying the resource.
...
The placeholder for the states of the resource. It is filled during the
lifetime of the resource. The structure of operational data depends on the type
of the resource.
...
Unique ID identifying the network.
...
**resource**
 ...
 A list of resources dedicated to this node.
...
- resource --> state,
- network --> node --> state,
- network --> node --> command-history and
...
- **state** - List of states which the tests went through.
...
This is a list of states which the tests went through. They can be e.g.:
