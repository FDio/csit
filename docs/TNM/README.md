...
there are more than one very similar nodes in the topology.
...
A network is defined by [nodes](#node) with
[termination points](#termination-point) connected by [links](#link).

**network-id**: Unique ID identifying the network. 

**attr**: Key-value pairs defining the attributes of the network.

**node**: List of nodes in the network, see [Node](#node).

**link**: List of links connecting the nodes, see [Link](#link).
...
NOTE: Do not read the text below, it will be completely changed
