...
Topology file is a dynamic parameter set when a tastbed is reserved.
...
The attributes in `attr` are set when the resource is allocated during the execution.
Also, new resources can be added to the list if they are dynamically created
while the test is running (either as a part of configuration, re-configuration
or the measurement phase).
