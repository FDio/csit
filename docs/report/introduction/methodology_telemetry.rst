...
counters for different events. CSIT and VPP use the same events for measurement.
Difference between VPP and CSIT measurment is that BPF uses one more syscall
to load BPF program to kernel.
...
How CSIT measures performance counters:
  ...
  Close FD and detach from event ~ 1500ms
