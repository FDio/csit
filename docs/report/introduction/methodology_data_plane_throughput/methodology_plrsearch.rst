...
rate measurements is small, see `soak vs ndr comparison`_.
...
Even though the estimate computation is Bayesian, so its the best it could be
...
but also others, such as time independence of loss ratio.
...
this is not a case of overestimation. Measurements at around the same offered load
have higher probability of zero loss earlier (when performed at lower bound),
but smaller probability later (when performed neat upper bound).
That means it is the performance of the system that decreases (slightly)
over time.
...
This test case shows what looks like a quite broad critical region,
...
but human seeing measures loss ratios agrees that the interval should be
wider than that.
...
Some improvements can be made via a specific workarounds,
...
divided by long periods of zero losses (as happens in Vhost test)
