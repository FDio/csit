...
but also others, such as the assumption of time independence of loss ratio.
...
(when performed at the lower bound), but smaller probability later on
(when performed near the upper bound). That means it is the performance
...
The fitting functions give fairly differing estimates.
...
partly of medium loss measurements, but lacks small loss measurements
which the loss ratio target would imply.
With this result composition, it is expected that the convergence
of the two bounds is slow.

In other words, a human only seeing the graph would expect a narrower interval,
but a human seeing the measured loss ratios agrees that the interval should be
wider than that.
