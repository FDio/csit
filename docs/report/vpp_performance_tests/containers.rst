...
- Unable to propeerly pin k8s pods and containers to cpu cores.
...
implement several optimizations into Dockerfile e.g. skip building of VPP but
rather download existing VPP builds from Nexus storage.
...
2. Build ``dev_vpp_image`` Docker image. (this takes significant amount of time)
...
6. Transfer ``prod_vpp_image_shrink`` image to DUTs.
