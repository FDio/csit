...
DUT instead of vpp-nat44ed. Result of this is test is used as a
reference to establish a safe CPS-MTR value (Maximum Transmit Rate) to
be used for CPS-MRR tests. (Today there is no automated discovery of
CPS-MTR value.)
...
CPS-NDR = max(`trial_cps_rate`) found for `pktloss_ratio` = 0, where
`trial_cps_rate` = TRex ASTF `multiplier`.
