...
regardless of packet loss. Maximum load set to [10GE|25GE|40GE|100GE]
link rate for specified Ethernet frame size.
...
- Ethernet frame sizes: 64B (78B for IPv6 tests) for all tests, IMIX for
  ...
  gap).
- Maximum load offered: 10GE link rate for all tests, actual packet rate
  depends on frame size and transmission overhead.
...
- TMM : Trimmed Moving Median, median across the data set of <N> values
  with all data points, excluding the outliers. Used as a trending value
  and as a reference for anomaly detection.
...
Trend graphs show per test case measured MRR throughput values with
