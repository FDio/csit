...
The info output files have extension .info.json and are archived as a tree
in S3 storage (transparently gzipped) to allow content querying.
The raw output files are moved into compressed tar archive,
to save storage space and hide from querying.
