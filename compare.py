...
ver_map = {
    ...
    "rls2306+VPPv23.06-rc2~0-g5e6bc730e~b15": "",
...
def relative_change_stdev(mean1, mean2, std1, std2):
    ...
    :rtype: float
    """
    # Zero stdev is single sample means not a real iterative result.
...
with open("results.csv", "rt") as fin:
    ...
    for row in reader:
        ...
        if "2vhostvr1" in case_key or "-af-xdp-" in case_key or "-4c" in case_key:
            # TODO: Allow single-worker and single-numa vhosts.
            # TODO: Investigate which 4c results are really bad.
            continue
        if "2n-icx" not in case_key or "-avf-" in case_key:
...
with open("comparison.csv", "wt") as fout:
    fout.write("case_key,")
    for rls in rlsver2column:
        fout.write(f"{rls}_avg,{rls}_stdev,")
    fout.write("c2v2dc6v2_avg,c2v2dc6v2_stdev,c6v2dc6v6r1_avg,c6v2dc6c6r1_stdev\n")
    for case_key in case_keys:
        ...
        diff_avg, diff_std = relative_change_stdev(
            row["c2v2_avg"], row["c6v2_avg"], row["c2v2_std"], row["c6v2_std"]
        )
        fout.write(f"{diff_avg},{diff_std},")
        diff_avg, diff_std = relative_change_stdev(
            row["c6v2_avg"], row["c6v6r1_avg"], row["c6v2_std"], row["c6v6r1_std"]
        )
        fout.write(f"{diff_avg},{diff_std}\n")
