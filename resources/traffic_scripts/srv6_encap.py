...
| | ... | --src_ip ${src_ip} | --dst_ip ${dst_ip} | --dut1_bsid ${dut1_bsid}
| | ... | --dut1_srcsid ${dut1_srcsid} | --dut1_dstsid1 ${dut1_dstsid1}
| | ... | --dut1_dstsid2 ${dut1_dstsid2} | --dut2_srcsid ${dut2_srcsid}
| | ... | --dut2_dstsid1 ${dut2_dstsid1} | --dut2_dstsid2 ${dut2_dstsid2}
| | ... | --decap ${decap}
''''
...
def check_srv6(
    ...
    if pkt_recv[IPv6][0].src != dut1_srcsid:
        raise RuntimeError(
            f"Outer IPv6 packet has invalid source address: "
            f"{pkt_recv[IPv6][0].src} should be: {dut1_srcsid}"
        )

    if pkt_recv[IPv6][0].dst != dut2_dstsid1:
        raise RuntimeError(
            f"Outer IPv6 packet has invalid destination address: "
            f"{pkt_recv[IPv6][0].dst} should be: {dut2_dstsid1}"
        )

    if dut2_dstsid2 == u"None":
        if pkt_recv.haslayer(IPv6ExtHdrSegmentRouting):
            raise RuntimeError(
                f"Segment Routing Header in received packet: {pkt_recv!r}"
            )
        if pkt_recv[IPv6][0].nh != ipv6nh[43]:  # IPv6
            raise RuntimeError(
                f"Outer IPv6 packet has invalid next header: "
                f"{pkt_recv[IPv6][0].nh} should be: {ipv6nh[43]}"
            )
    else:
        ...
        if pkt_recv[IPv6][0].nh != ipv6nh[43]:  # Routing Header
            raise RuntimeError(
                f"Outer IPv6 packet has invalid next header: "
                f"{pkt_recv[IPv6][0].nh} should be: {ipv6nh[43]}"
            )
        if pkt_recv[IPv6ExtHdrSegmentRouting].addresses != [
            dut2_dstsid2, dut2_dstsid1
        ]:
            raise RuntimeError(
                f"Segment Routing Header has invalid addresses: "
                f"{pkt_recv[IPv6ExtHdrSegmentRouting].addresses} should be: "
                f"{[dut2_dstsid2, dut2_dstsid1]}"
            )
        if pkt_recv[IPv6ExtHdrSegmentRouting].segleft != 1:
            raise RuntimeError(
                f"Segment Routing Header has invalid segleft value: "
                f"{pkt_recv[IPv6ExtHdrSegmentRouting].segleft} should be: 1"
            )
    ...
    if pkt_recv[IPv6][1].nh != ipv6nh[59]:  # No Next Header
        raise RuntimeError(
            f"Inner IPv6 packet has invalid destination address: "
            f"{pkt_recv[IPv6][0].dst} should be: {ipv6nh[59]}"
        )
