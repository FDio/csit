...
def check_srv6(
    ...
    if dut2_dstsid2 == u"None":
        ...
        if pkt_recv[IPv6][0].nh != 41:  # ipv6nh[41] == IPv6
            raise RuntimeError(
                f"Outer IPv6 packet has invalid next header: "
                f"{pkt_recv[IPv6][0].nh} should be: 41 ({ipv6nh[41]})"
            )
        ...
        if pkt_recv[IPv6][0].nh != 43:  # ipv6nh[43] == Routing Header
            raise RuntimeError(
                f"Outer IPv6 packet has invalid next header: "
        ...
        if pkt_recv[IPv6ExtHdrSegmentRouting].nh != 41:  # ipv6nh[41] == IPv6
            raise RuntimeError(
                f"Segment Routing Header has invalid next header: "
                f"{pkt_recv[IPv6ExtHdrSegmentRouting].nh} should be: 41 "
                f"({ipv6nh[41]})"
            )
        if pkt_recv[IPv6][2].src != src_ip:
            raise RuntimeError(
                f"Inner IPv6 packet has invalid source address: "
                f"{pkt_recv[IPv6][0].src} should be: {src_ip}"
            )

        if pkt_recv[IPv6][2].dst != dst_ip:
            raise RuntimeError(
                f"Inner IPv6 packet has invalid destination address: "
                f"{pkt_recv[IPv6][0].dst} should be: {dst_ip}"
            )

        if pkt_recv[IPv6][2].nh != 59:  # ipv6nh[59] == No Next Header
            raise RuntimeError(
                f"Inner IPv6 packet has invalid next header: "
                f"{pkt_recv[IPv6][1].nh} should be: 59 ({ipv6nh[59]})"
            )
