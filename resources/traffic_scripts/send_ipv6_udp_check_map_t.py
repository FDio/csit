...
"""Traffic script that sends an empty UDP datagram and checks if IPv4 addresses
...
def _check_udp_chksum(ip):
    new = ip.__class__(str(ip))
    new.show()
    del new['UDP'].chksum
    new = ip.__class__(str(new))
    new.show()
    print 'Computed: {}.'.format(new['UDP'].chksum)
    print 'Received: {}.'.format(ip['UDP'].chksum)
    return new.chksum == ip.chksum
...
def main():  # pylint: disable=too-many-statements, too-many-locals
    ...
    udp = (Ether(dst=tx_dst_mac) /
           IP(src=tx_src_ipv6, dst=tx_dst_ipv6) /
    ...
    if not _check_udp_chksum(ip):
        raise RuntimeError("UDP checksum check failed .")
