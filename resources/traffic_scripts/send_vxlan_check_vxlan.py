...
def main():
    ...
    the other one. Dot1q or Dot1ad tagging of the ethernet frame can be set.
    """
    args = TrafficScriptArg([u'tx_src_mac', u'tx_dst_mac', u'tx_src_ip',
                             u'tx_dst_ip', u'tx_vni', u'rx_src_ip', u'rx_dst_ip',
                             u'rx_vni'])
    ...
    if ether is None:
        raise RuntimeError(f'Packet Rx timeout')
    ip = ether.payload
    ...
    if ip.payload.dport != 4789:
        raise RuntimeError(f'VXLAN UDP port mismatch '
                           f'{ip.payload.dport} != {4789}')
    vxlan_pkt = ip.payload.payload

    if int(vxlan_pkt.vni) != int(rx_vni):
        raise RuntimeError(f'vxlan mismatch')
    rx_pkt_p = vxlan_pkt.payload

    if rx_pkt_p.src != tx_pkt_p.src:
        raise RuntimeError(f'RX encapsulated MAC src mismatch '
                           f'{rx_pkt_p.src} != {tx_pkt_p.src}')
    if rx_pkt_p.dst != tx_pkt_p.dst:
        raise RuntimeError(f'RX encapsulated MAC dst mismatch '
                           f'{rx_pkt_p.dst} != {tx_pkt_p.dst}')
    if rx_pkt_p[u'IP'].src != tx_pkt_p[u'IP'].src:
        raise RuntimeError(f'RX encapsulated IP src mismatch '
                           f'{rx_pkt_p[u'IP'].src} != {tx_pkt_p[u'IP'].src}')
    if rx_pkt_p[u'IP'].dst != tx_pkt_p[u'IP'].dst:
        raise RuntimeError(f'RX encapsulated IP dst mismatch '
                           f'{rx_pkt_p[u'IP'].dst} != {tx_pkt_p[u'IP'].dst}')
