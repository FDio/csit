...
def main():
    ...
    packets."""
    args = TrafficScriptArg(['dst_mac', 'src_mac', 'dst_ip', 'src_ip', 'timeout'])
    ...
    for _ in range(1000):
        pkt = rxq.recv(wait_step)
        if not pkt:
            timeout -= wait_step
            if timeout < 0:
                raise RuntimeError("ICMP echo Rx timeout")
            continue
        if is_icmp_reply(pkt):
            icmp_reply = pkt
            break
    else:
        raise RuntimeError("Loop limit reached")

    if icmp_reply['IP'].src != dst_ip:
        raise RuntimeError("Source address of reply doesn't match")

    if icmp_reply['IP'].dst != src_ip:
        raise RuntimeError("Destination address of reply doesn't match")
