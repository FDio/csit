...
class ConfigBlaster(object):
    ...
    def send_requests(self, tid, stats, core):
        ...
        system("taskset -pc {0} {1}".format(core, getpid()))
        ...
        with self.print_lock:
            ...
            print("        Requests/s: {0} OK, {1} Total".format(
                ok_rps, total_rps))
    ...
    def run_cycle(self, function):
        ...
        for c in range(self.ncycles):
            ...
            with self.print_lock:
                ...
                print("    Elapsed time:    {0}s".format(t.secs))
                print(
                    "    Peak requests/s: {0} OK, {1} Total".format(
                        self.stats.get_ok_rqst_rate,
                        self.stats.get_total_rqst_rate))
                print(
                    "    Avg. requests/s: {0} OK, {1} Total ({2} "
