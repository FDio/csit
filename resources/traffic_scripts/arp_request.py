...
def parse_arguments():
    ...
    """
    args = TrafficScriptArg([u'src_if', u'src_mac', u'dst_mac',
                             u'src_ip', u'dst_ip'])
...
def arp_request_test():
    ...
    arp_request = (Ether(src=src_mac, dst=u'ff:ff:ff:ff:ff:ff') /
    ...
    try:
        ...
        if not ether.haslayer(ARP):
            raise RuntimeError(f'Unexpected packet: does not contain ARP '
                               'header "{ether.__repr__()}"')

        arp = ether[u'ARP']
        arp_reply = 2

        if arp.op != arp_reply:
            raise RuntimeError(f'expected op={arp_reply}, received {arp.op}')
        if arp.ptype != 0x800:
            raise RuntimeError(f'expected ptype=0x800, received {arp.ptype}')
        if arp.hwlen != 6:
        ...
        if arp.hwsrc != dst_mac:
            raise RuntimeError(f'expected hwsrc={dst_mac}, '
                               f'received {arp.hwsrc}')
        if arp.psrc != dst_ip:
            raise RuntimeError(f'expected psrc={dst_ip}, received {arp.psrc}')
        if arp.hwdst != src_mac:
            raise RuntimeError(f'expected hwdst={src_mac}, '
                               f'received {arp.hwdst}')
        if arp.pdst != src_ip:
