...
"""Traffic script that sends an Ethernet+NSH packet
from TG to DUT.
"""
import sys
import time
...
def main():
    """Send Ethernet+NSH packet from TG to DUT.
    ...
    nsh_header = '\x00\x06\x01\x03\x00\x00\xb9\xff' \
                 '\xC0\xA8\x32\x4B\x00\x00\x00\x09' \
                 '\xC0\xA8\x32\x48\x03\x00\x12\xB5'

    raw_data = nsh_header + str(innerpkt)

    pkt_header = (Ether(src=src_mac, dst=dst_mac, type=0x894F) /
              Raw(load=raw_data))
