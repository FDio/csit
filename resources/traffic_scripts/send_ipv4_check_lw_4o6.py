...
def main():
    ...
    src_ipv6 = args.get_arg('src_ipv6')
    dst_ipv6 = args.get_arg('dst_ipv6')
    ...
    udp /= IP(src=tx_src_ipv4, dst=tx_dst_ipv4)
    udp /= UDP(sport=tx_src_udp_port, dport=tx_dst_udp_port)
    ...
    for _ in range(5):
        ...
        raise RuntimeError("IP in IP Rx error.")

    # check ethernet
    if ether.dst != rx_dst_mac:
        raise RuntimeError("Destination MAC error {} != {}.".format(ether.dst, rx_dst_mac))
    print "Destination MAC: OK."

    if ether.src != rx_src_mac:
        raise RuntimeError("Source MAC error {} != {}.".format(ether.src, rx_src_mac))
    print "Source MAC: OK."

    ipv6 = ether.payload

    # check ipv6
    if ipv6.dst != dst_ipv6:
        raise RuntimeError("Destination IP error {} != {}.".format(ipv6.dst,  dst_ipv6))
    print "Destination IPv6: OK."

    if ipv6.src != src_ipv6:
        raise RuntimeError("Source IP error {} != {}.".format(ipv6.src,  src_ipv6))
    ...
    if ipv4.dst != tx_dst_ipv4:
        raise RuntimeError("Destination IP error {} != {}.".format(ipv4.dst, tx_dst_ipv4))
    print "Destination IPv4: OK."

    if ipv4.src != tx_src_ipv4:
        raise RuntimeError("Source IP error {} != {}.".format(ipv4.src,  tx_src_ipv4))
    print "Source IPv4: OK."

    if ipv4.proto != 17:  # UDP
        raise RuntimeError("IP protocol error {} != UDP.".format(ipv4.proto))
    ...
    if udp.dport != tx_dst_udp_port:
        raise RuntimeError("UDP dport error {} != {}.".format(udp.dport, tx_dst_udp_port))
    print "UDP dport: OK."

    if udp.sport != tx_src_udp_port:
        raise RuntimeError("UDP sport error {} != {}.".format(udp.sport, tx_src_udp_port))
    print "UDP sport: OK."
