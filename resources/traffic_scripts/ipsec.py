...
def check_ipv4(pkt_recv, dst_tun, src_ip, dst_ip, sa_in):
    ...
    if pkt_recv['IP'].dst != dst_tun:
        raise RuntimeError(
            'Received packet has invalid destination address {0} should be {1}'
            .format(pkt_recv['IP'].dst, dst_tun))
    ...
    if d_pkt['IP'].dst != dst_ip:
        raise RuntimeError(
            'Decrypted packet has invalid destination address {0} should be {1}'
            .format(d_pkt['IP'].dst, dst_ip))
...
def check_ipv6(pkt_recv, dst_tun, src_ip, dst_ip, sa_in):
    ...
    if pkt_recv['IPv6'].dst != dst_tun:
        raise RuntimeError(
            'Received packet has invalid destination address {0} should be {1}'
            .format(pkt_recv['IPv6'].dst, dst_tun))
