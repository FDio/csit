...
def main():
    args = TrafficScriptArg([u'src_mac', u'dst_mac', u'src_nh_mac', u'dst_nh_mac',
                             u'src_ip', u'dst_ip', u'h_num'], [u'is_dst_tg'])

    src_rxq = RxQueue(args.get_arg(u'tx_if'))
    src_txq = TxQueue(args.get_arg(u'tx_if'))

    src_mac = args.get_arg(u'src_mac')
    dst_mac = args.get_arg(u'dst_mac')
    src_nh_mac = args.get_arg(u'src_nh_mac')
    dst_nh_mac = args.get_arg(u'dst_nh_mac')
    src_ip = args.get_arg(u'src_ip')
    dst_ip = args.get_arg(u'dst_ip')
    hop_num = int(args.get_arg(u'h_num'))

    is_dst_tg = True if args.get_arg(u'is_dst_tg') in [u'True', u''] else False
    dst_rxq = RxQueue(args.get_arg(u'rx_if')) if is_dst_tg else None
    dst_txq = TxQueue(args.get_arg(u'rx_if')) if is_dst_tg else None
    ...
    if is_dst_tg:
        # send ICMPv6 neighbor advertisement message
        pkt_send = (Ether(src=dst_mac, dst=u'ff:ff:ff:ff:ff:ff') /
                    IPv6(src=dst_ip, dst=u'ff02::1:ff00:2') /
    ...
    if is_dst_tg:
        ...
        if ipv6.hlim != (hop_limit - hop_num):
            raise RuntimeError(f'Invalid hop limit {ipv6.hlim}' 
                               f'' should be {hop_num}')

        if not ipv6.haslayer(ICMPv6EchoRequest):
            raise RuntimeError(f'Unexpected packet with no IPv6 ICMP received '
                               f'{ipv6.__repr__()}')
        ...
        if icmpv6.id != echo_id or icmpv6.seq != echo_seq:
            raise RuntimeError(f'Invalid ICMPv6 echo reply'
                               f' received ID {icmpv6.id} '
                               f'seq {icmpv6.seq} should be '
                               f'ID {echo_id} seq {echo_seq}')
    ...
    if not ether.haslayer(IPv6):
        raise RuntimeError(f'Unexpected packet with no IPv6 layer received' 
                           f' {ether.__repr__()}')
    ...
    if ipv6.hlim != hop_limit_reply:
        raise RuntimeError(f'Invalid hop limit {ipv6.hlim}'
                           f' should be {hop_limit_reply}')

    if not ipv6.haslayer(ICMPv6EchoReply):
        raise RuntimeError(f'Unexpected packet with no ' 
                           f'IPv6 ICMP received {ipv6.__repr__()}')
