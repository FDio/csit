...
def main():
    args = TrafficScriptArg(['rx_src_mac', 'tx_src_mac', 'server_ip',
                             'client_ip', 'client_mask'],
                            ['hostname'])

    server_if = args.get_arg('rx_if')
    server_mac = args.get_arg('tx_src_mac')
    server_ip = args.get_arg('server_ip')

    client_mac = args.get_arg('rx_src_mac')
    ...
    if dhcp_request.src != dhcp_discover.dst:
        raise RuntimeError("Src MAC error.")
    print "Src MAC: OK."

    if dhcp_request['IP'].dst != rx_dst_ip or dhcp_request[
        'IP'].src != rx_src_ip:
