...
def dhcp_offer(rx_if, tx_if, tx_dst_ip, server_ip, proxy_ip,
               client_ip, server_mac, option_82):
...
def main():
    ...
    srxq = RxQueue(rx_if)  # Server side Rx queue
    stxq = TxQueue(tx_if)  # Server side Tx queue

    crxq = RxQueue(tx_if)  # Client side Rx queue
    ctxq = TxQueue(rx_if)  # Client side Tx queue
