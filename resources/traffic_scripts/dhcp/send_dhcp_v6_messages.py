...
def solicit(tx_if, rx_if, dhcp_multicast_ip, link_local_ip, proxy_ip, server_ip,
            client_duid, client_mac):
    ...
    dhcp_solicit = Ether(src=client_mac, dst="ff:ff:ff:ff:ff:ff") / \
...
def advertise(rx_if, tx_if, dhcp_multicast_ip, proxy_ip, server_ip, server_mac,
              interface_id):
    ...
    dhcp_advertise = Ether(src=server_mac) / \
                     IPv6(src=server_ip, dst=dhcp_multicast_ip) / \
                     UDP(sport=547, dport=546) / \
                     DHCP6_RelayReply(peeraddr="fe80::1",
                                      linkaddr=proxy_ip) / \
...
def request(tx_if, rx_if, dhcp_multicast_ip, link_local_ip, proxy_ip, server_ip,
            client_duid, client_mac):
...
def reply(rx_if, tx_if, dhcp_multicast_ip, proxy_ip, server_ip, server_mac,
          interface_id):
    ...
    dhcp_reply = Ether(src=server_mac) / \
                 IPv6(src=server_ip, dst=dhcp_multicast_ip) / \
                 UDP(sport=547, dport=546) / \
                 DHCP6_RelayReply(peeraddr="fe80::1",
                                  linkaddr=proxy_ip) / \
...
def main():
    ...
    # SOLICIT
    interface_id = solicit(tx_if, rx_if, dhcp_multicast_ip, link_local_ip,
                           proxy_ip, server_ip, client_duid, client_mac)

    # ADVERTISE
    advertise(tx_if, rx_if, dhcp_multicast_ip, proxy_ip, server_ip, server_mac,
              interface_id)

    # REQUEST
    request(tx_if, rx_if, dhcp_multicast_ip, link_local_ip, proxy_ip, server_ip,
            client_duid, client_mac)

    # REPLY
    reply(tx_if, rx_if, dhcp_multicast_ip, proxy_ip, server_ip, server_mac,
          interface_id)
