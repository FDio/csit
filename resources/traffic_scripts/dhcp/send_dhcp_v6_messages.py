...
def solicit(tx_if, rx_if, dhcp_multicast_ip, proxy_ip, server_ip):
    ...
    dhcp_solicit = Ether() / \
                   ...
                   DHCP6_Solicit() / \
                   DHCP6OptClientId(duid="0800276418d2")
...
def request(tx_if, rx_if, dhcp_multicast_ip, proxy_ip, server_ip):
...
def main():
    ...
    # SOLICIT
    interface_id = solicit(tx_if, rx_if, dhcp_multicast_ip,
                           proxy_ip, server_ip)

    # ADVERTISE
    advertise(tx_if, rx_if, dhcp_multicast_ip, proxy_ip,
              server_ip, interface_id)

    # REQUEST
    request(tx_if, rx_if, dhcp_multicast_ip,
                           proxy_ip, server_ip)
