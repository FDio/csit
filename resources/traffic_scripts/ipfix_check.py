...
def main():
    ...
    verbose = False if count > 1 else True

    print("Details filtered for non-IPFIX packets.")
    ...
    data = None
    while True:
        pkt = rxq.recv(5, ignore=ignore, verbose=verbose)
        if pkt is None:
            continue
    ...
    for item in ("src_port", "tcp_src_port", "udp_src_port"):
        try:
            if int(data[item]) != source_port:
                raise RuntimeError(
                    e.format("Source port", source_port, data[item]))
        except KeyError:
            pass
    for item in ("dst_port", "tcp_dst_port", "udp_dst_port"):
        try:
            if int(data[item]) != destination_port:
                raise RuntimeError(
                    e.format("Source port", destination_port, data[item]))
        except KeyError:
            pass
    # verify protocol ID
