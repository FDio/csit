...
class LispGPEHeader(Packet):
    ...
    name = u"Lisp GPE Header"
    fields_desc = [
        FlagsField(u"flags", None, 8, [u"N", u"L", u"E", u"V", u"I", u"P",\
                                      u"R", u"O"]),
...
def main():
    ...
    if rx_dst_mac == ether[Ether].dst and rx_src_mac == ether[Ether].src:
        ...
        raise RuntimeError(
            f"Matching packet unsuccessful: {ether.__repr__()}")
    ...
    if ot_mode == '6to4':
        if not isinstance(ip, IP):
            raise RuntimeError(
                f"Not an IP packet received {ip.__repr__()}")
    elif ot_mode == '4to6':
    ...
    if src_rloc == ip.src:
        ...
        raise RuntimeError(f'Matching Src RLOC unsuccessful: '
                           f'{src_rloc} != {ip.src}')

    if dst_rloc == ip.dst:
        print(u"Destination RLOC matches configuration.")
    else:
        raise RuntimeError(f'Matching dst RLOC unsuccessful: '
                           f'{dst_rloc} != {ip.dst}')
