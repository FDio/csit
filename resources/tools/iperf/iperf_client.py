...
def simple_burst(args):
    ...
    if args.rate and args.frame_size:
        ...
        bandwidth = mss * args.rate * 8 / args.instances
    ...
    for i in range(args.instances):
        ...
        if args.rate and args.frame_size:
            cmd_list += ["--bandwidth", f"{bandwidth}"]
        ...
        cmd_list += ["--json"]
        processes.append(
            subprocess.Popen(cmd_list, shell=True, stdout=subprocess.PIPE)
        )
    if args.async_start:
        for i in range(args.instances):
    ...
    for i in range(args.instances):
        ...
        stdout, _ = processes[i].communicate(timeout=timeout)
        print(f"from process {i}: {stdout}", file=sys.stderr)
