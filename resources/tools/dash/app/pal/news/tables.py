...
def _table_regressions(regressions: dict) -> dbc.Table:
    """Generates table with regressions.

    :param regressions: Dictionary with regressions and their last occurence.
...
def _table_progressions(progressions: dict) -> dbc.Table:
    """Generates table with progressions.

    :param progressions: Dictionary with progressions and their last occurence.
    :type progressions: dict
    :returns: The table with progressions.
    :rtype: dbc.Table
    """
    return dbc.Table.from_dataframe(
...
#     title = f"Regressions in the last {C.NEWS_TIME_PERIOD} days"
...
#     title = f"Progressions in the last {C.NEWS_TIME_PERIOD} days"
...
#     return [
#         _table_info(job_data),
#         _table_failed(job_data, failed),
#         _table_regressions(regressions),
#         _table_progressions(progressions)
#     ]
...
def table_news(data: pd.DataFrame, job: str, period: int) -> list:
    ...
    if regressions["Last Regression"]:
        r_list.append(_table_regressions(regressions))
    ...
    if progressions["Last Progression"]:
        r_list.append(_table_progressions(progressions))
