...
if do_import:
    ...
    # variable
    path_to_papi_module = None
    for root, dirs, files in os.walk('/usr/lib'):
        for name in files:
            if name == 'vpp_papi.py':
                path_to_papi_module = root
                break
    if path_to_papi_module:
        sys.path.append(path_to_papi_module)
        from vpp_papi import VPP
    else:
        raise PapiError('vpp_papi module not found')
...
def main():
    ...
    for data in json_data:
        ...
        reply_client = dict(client_name=client_name)
        reply_apis = list()
        papi_connect(vpp, client_name)
        for api in data['apis']:
            api_name = api.get('api_name')
            api_args_unicode = api.get('api_args')
            api_reply = dict(api_name=api_name)
            api_args = dict()
            for k, v in api_args_unicode.iteritems():
                value = v if isinstance(v, int) else binascii.unhexlify(v)
                api_args[str(k)] = value
            try:
                r = papi_run(vpp, api_name, api_args)
                api_reply['api_reply'] = process_reply(r)
                reply_apis.append(api_reply)
                ...
                raise PapiCommandError(
                    'PAPI command {api}({args}) error:\n{exc}'.format(
                        api=api_name, args=api_args), exc=repr(err))
        papi_disconnect(vpp)
        reply_client["apis"] = reply_apis
        reply.append(reply_client)
