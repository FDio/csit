...
The first version of this project got packaged as `tober_csit`,
as the original team is not expected to have enugh time for proper maintenance.

If you want to fix bugs or add features,
it is recommended to fork the code and repackage as `tober_{flavor}` with the flavor part
hinting at who made the modifications.

In medium term, the most responsive team can become the official maintainer (dropping flavors).

In long term, it is the [Robot Framework project](https://github.com/robotframework/robotframework)
that is best suited to keep compatibility with its `rebot`.

# Tober Developer Notes

This section explains the design decisions, trade-offs, and utilities (like `rounder.py`) used in the `tober` project.
