...
def get_string(index: Optional[int]) -> str:
    ...
    if s and s.startswith("*"):
        ...
        try:
            ...
            print(
                f"Warning: Failed to decompress string at {index=}"
                f" ('{s[:40]}...'): {e}",
                file=sys.stderr,
            )
            decoded_s = s
...
def create_status_element(
    ...
    :param failure_message_str: The reconstructed failure message (if FAIL).
...
def parse_item_list(item_list: List[List[Any]], parent_xml: ET.Element) -> List[str]:
    """
    ...
    for item_arr in item_list:
        if len(item_arr) < 5:  # It's a message
            ...
            # This keyword's failure message is combined from its children.
...
def parse_suite(
    ...
    if meta_items:
        ...
        for meta_item in meta_items:
            ET.SubElement(meta_xml, "item", name=get_string(meta_item[0])).text = (
                clean_html_tags(get_string(meta_item[1]))
            )
...
def main() -> None:
    ...
    if not all([suite_match, base_millis_match, generated_match, generator_match]):
        exit_with_error(
            "Could not find all required window.output.* variables"
            " or generator meta tag in log.html."
        )
    ...
    if not all_strings:
        exit_with_error(
            "No strings found. The regex for"
            ' `window.output["strings"].concat()` might have failed.'
        )
    ...
    try:
        ...
        exit_with_error(
            "Failed to parse data: Exceeded Python's recursion depth."
            f" The data is too nested. Error: {e}"
        )
