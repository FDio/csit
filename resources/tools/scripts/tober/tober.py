...
decoded_strings_cache: Dict[int, str] = {}
base_millis: int = 0
...
def print_parser_error_context(data_str: str, e: Exception) -> None:
    """Prints context around a JSONDecodeError or SyntaxError.

    :param data_str: The full data string that failed to parse.
    :type data_str: str
    :param e: The exception object (JSONDecodeError or SyntaxError).
    :type e: Exception
    :returns: None
    :rtype: None
    """
...
def get_string(index: Optional[int]) -> str:
    """
    Fetches a string by its index, decompressing and caching it on-demand.

    :param index: The index of the string to retrieve from `all_strings`.
    :type index: Optional[int]
    :returns: The decompressed and unescaped string.
    :rtype: str
    """
    ...
    if s and s.startswith("*"):
        ...
        try:
            ...
            print(
                f"Warning: Failed to decompress string at index {index} ('{s[:40]}...'): {e}",
                file=sys.stderr,
            )
            decoded_s = s
    else:
...
def clean_html_tags(text_str: str) -> str:
    """
    Removes common, simple HTML tags from a string.

    Specifically, removes/replaces `<p>`, `<br>`, `<a>`, and `<b>` tags
    to clean up string content for XML text nodes.

    :param text_str: The string to clean.
    :type text_str: str
...
def format_timestamp(offset_millis: Optional[int]) -> str:
    """
    Converts an offset timestamp from the log into YYYYMMDD HH:MM:SS.mmm format.

    :param offset_millis: The millisecond offset from `base_millis`.
    :type offset_millis: Optional[int]
    :returns: The formatted ISO timestamp string, or "N/A" on failure.
    :rtype: str
    """
    ...
    try:
        ...
        dt_aware = datetime.datetime.fromtimestamp(
...
def create_status_element(
    ...
    status_arr: List[Any],
    failure_message_str: Optional[str] = None,
) -> None:
    """
    Creates a <status> element and appends it to the parent.

    Uses failure_message_str for FAIL, or looks up the message from
    the status array for other statuses.

    :param parent_xml: The parent XML element (e.g., <suite>, <test>, <kw>).
    :type parent_xml: ET.Element
    :param status_arr: The status array from the log data.
    :type status_arr: List[Any]
    :param failure_message_str: The reconstructed failure message (if status is FAIL).
    :type failure_message_str: Optional[str]
    :returns: None
    :rtype: None
    """
    ...
    if status_code == 0:  # FAIL
        ...
        status_text = clean_html_tags(
            get_string(status_arr[3] if len(status_arr) > 3 else None)
        )
...
def format_failure_message(
    failures: List[str], preamble: Optional[str] = None
) -> Optional[str]:
    """
    Combines a list of failures into a single string.

    :param failures: A list of failure message strings.
    :type failures: List[str]
    :param preamble: An optional string to prepend (e.g., "teardown failed:").
    :type preamble: Optional[str]
    :returns: A combined failure message string, or None if list is empty.
    :rtype: Optional[str]
    """
    if not failures:
        return None
...
def separate_body_items(
    body_array: List[List[Any]],
) -> Tuple[List[List[Any]], List[List[Any]]]:
    """
    Splits a body list into (setups_and_body, teardowns).

    :param body_array: A list of body items (keywords or messages).
    :type body_array: List[List[Any]]
    :returns: A tuple containing two lists: (setups_and_body, teardowns).
...
def parse_item_list(
    item_list: List[List[Any]], parent_xml: ET.Element
) -> List[str]:
    """
    Parses a list of body items (which can be KWs or MSGs).
    Returns a list of failure message strings.

    :param item_list: A list of item arrays (keywords or messages).
    :type item_list: List[List[Any]]
    :param parent_xml: The parent XML element to append new items to.
    :type parent_xml: ET.Element
    :returns: A list of failure messages bubbled up from children.
    :rtype: List[str]
    """
    failures = []
...
def parse_suite(
    suite_arr: List[Any], parent_xml: ET.Element, id_index: int = 0
) -> None:
    """
    Parses a suite array and appends it to the parent XML element.

    :param suite_arr: The suite data array.
    :type suite_arr: List[Any]
    :param parent_xml: The parent XML element (either <robot> or <suite>).
    :type parent_xml: ET.Element
    :param id_index: The 1-based index of this suite within its parent.
    :type id_index: int
    :returns: None
    :rtype: None
    """
    ...
    setup_failures = parse_item_list(setups, suite_xml)
    ...
    td_failures = parse_item_list(teardowns, suite_xml)
    ...
    if meta_items:
        ...
        for meta_item in meta_items:
            ET.SubElement(
                meta_xml, "item", name=get_string(meta_item[0])
            ).text = clean_html_tags(get_string(meta_item[1]))
...
def parse_test(
    test_arr: List[Any], parent_xml: ET.Element, parent_id: str, index: int
) -> None:
    """
    Parses a test array and appends it to the parent XML element.

    :param test_arr: The test data array.
    :type test_arr: List[Any]
    :param parent_xml: The parent <suite> XML element.
    :type parent_xml: ET.Element
    :param parent_id: The 'id' string of the parent suite.
    :type parent_id: str
    :param index: The 1-based index of this test within its suite.
    :type index: int
    :returns: None
    :rtype: None
    """
    ...
    body_failures = parse_item_list(setups_and_body, test_xml)
    td_failures = parse_item_list(teardowns, test_xml)
...
def parse_keyword(kw_arr: List[Any], parent_xml: ET.Element) -> Optional[str]:
    """
    ...
    :param kw_arr: The keyword data array.
    :type kw_arr: List[Any]
    :param parent_xml: The parent XML element (suite, test, or kw).
    :type parent_xml: ET.Element
    :returns: A reconstructed failure message, or None if no failure.
    :rtype: Optional[str]
    """
    status_arr = kw_arr[8]
    ...
    if kw_type == "FOR":
        ...
        (setups_and_body, teardowns) = separate_body_items(
            kw_arr[9]
        )  # body = <iter> blocks
    ...
    if tags_text:
        ...
        for tag in tags_text.split(","):
            if tag.strip():
                ET.SubElement(tags_xml, "tag").text = clean_html_tags(
                    tag.strip()
                )
    ...
    if status_arr[0] == 0:  # FAIL
        body_fail_msg = format_failure_message(body_failures)
        td_fail_msg = format_failure_message(
            td_failures, "keyword teardown failed:"
        )
...
def parse_message(msg_arr: List[Any], parent_xml: ET.Element) -> None:
    """
    Parses a message array and appends it to the parent XML element.

    :param msg_arr: The message data array.
    :type msg_arr: List[Any]
    ...
    if (
        msg_arr[1] == 2 and msg_text.startswith("<b>") and "</b>" in msg_text
    ):  # INFO level and bold
...
def main() -> None:
    ...
    Reads log.html, parses JS data, and builds the XML tree.

    :returns: None
    :rtype: None
    """
    global base_millis, all_strings
    ...
    for match in re.finditer(
        r"window\.sPart(\d+)\s*=\s*(\[.*?\]);", log_content, re.DOTALL
    ):
        sparts[match.group(1)] = match.group(2)

    suite_match = re.search(
        r'window\.output\["suite"\]\s*=\s*(\[.*?\]);', log_content, re.DOTALL
    )
    base_millis_match = re.search(
        r'window\.output\["baseMillis"\]\s*=\s*(\d+);', log_content
    )
    generated_match = re.search(
        r'window\.output\["generated"\]\s*=\s*(\d+);', log_content
    )
    strings_matches = re.finditer(
        r'window\.output\["strings"\]\.concat\((\[.*?\])\);',
        log_content,
        re.DOTALL,
    )
    generator_match = re.search(
        r'<meta content="(Robot Framework.*?)" name="Generator">', log_content
    )

    if not all(
        [suite_match, base_millis_match, generated_match, generator_match]
    ):
        exit_with_error(
            "Could not find all required window.output.* variables or generator meta tag in log.html."
        )
    ...
    def sPart_replacer(match: re.Match) -> str:
        """Callback for re.sub to replace window.sPartX.

        :param match: The regex match object.
        :type match: re.Match
        :returns: The replacement string (the sPart content or "null").
        :rtype: str
        """
    ...
    while re.search(r"window\.sPart\d+", suite_data_str) and i < limit:
        suite_data_str = re.sub(
            r"window\.sPart(\d+)", sPart_replacer, suite_data_str
        )
        i += 1
    ...
    try:
        ...
        exit_with_error(
            'Failed to parse window.output["strings"].concat() data.'
        )

    if not all_strings:
        exit_with_error(
            'No strings found. The regex for `window.output["strings"].concat()` might have failed.'
        )
    ...
    try:
        ...
        exit_with_error(
            f'Failed to parse window.output["suite"] data as JSON: {e}'
        )
    except RecursionError as e:
        exit_with_error(
            f"Failed to parse data: Exceeded Python's recursion depth. The data is too nested. Error: {e}"
        )

    generated_ts_seconds = (base_millis + generated_millis) / 1000.0
    # Create datetime object, explicitly interpreting the timestamp as UTC
    generated_dt_aware = datetime.datetime.fromtimestamp(
        generated_ts_seconds, tz=datetime.timezone.utc
    )
    # Create a "naive" object (no tzinfo) from the UTC time
    ...
    root_xml.set("schemaversion", "5")  # Hardcoded based on provided output.xml
