...
def main() -> None:
    ...
    try:
        ...
        print(
            f"Error: Failed to parse XML in {input_file}: {e}", file=sys.stderr
        )
        sys.exit(1)
    ...
    print(
        "Rounding timestamps, converting <status>, removing <doc>, <statistics>, <errors>, line numbers, and source_names..."
    )
        ...
        try:
            if el.tag == "robot":
                ...
                if start_str and elapsed_str:
                    ...
                    el.set(
                        "endtime", endtime_dt.isoformat(timespec="milliseconds")
                    )
