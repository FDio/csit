...
VER="RELEASE"
echo "determine VPP dependency"
if [ "${OS}" == "centos7" ]; then
    HC_VER=`rpm -qpR honeycomb*.rpm | grep 'vpp ' | cut -d ' ' -f 3`
    if [ "${HC_VER}" != "" ]; then
        VER=${HC_VER}.x86_64
    fi
else
    HC_VER=`dpkg -I honeycomb*.deb | grep -oP 'vpp \(= \K[^\)]+'`
    if [ "${HC_VER}" != "" ]; then
        VER=${HC_VER}_amd64
    fi
fi
echo "Downloading vpp artifacts: ${VPP_ARTIFACTS}(${PACKAGE}), VER=${VER}"
