...
for test_index in range(num_tests):
    ...
    for iteration_index in range(len(parent_iterations)):
        parent_bursts.append(
            json.loads(parent_iterations[iteration_index][test_index]))
        new_bursts.append(
            json.loads(new_iterations[iteration_index][test_index]))
        parent_max = BitCountingMetadataFactory.find_max_value(parent_bursts[-1])
        new_max = BitCountingMetadataFactory.find_max_value(new_bursts[-1])
    ...
    for iteration_index in range(len(parent_bursts)):
        parent_values.extend(parent_bursts[iteration_index])
        new_values.extend(new_bursts[iteration_index])
