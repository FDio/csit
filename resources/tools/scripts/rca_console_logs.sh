...
jobname="${1}"
build_pattern="${2}"
# TODO: Detect last run and gor backward?
for i in {1..999}; do
    ...
    if fgrep " FAIL" "console.log" > "fails.txt"; then
        echo "${i}: has $(wc -l < 'fails.txt') failures, look at it."
        continue
    fi
    if ! fgrep " PASS" "console.log" > "passes.txt"; then
        echo "${i}: no passes? suspicious."
        continue
    fi
    echo "${i}: no failures, $(wc -l < 'passes.txt') passes, skip."
    continue
done
