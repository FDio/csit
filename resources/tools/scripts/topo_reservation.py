...
def diagnose(node, res_dir):
    ...
    :type cmd: str
    """
    command = "ls --full-time -ac '{d}' | tail -n 1".format(d=res_dir)
    ret, stdout, _ = exec_cmd(node, command)
    print("DEBUG command: {c}".format(c=command))
    print("DEBUG RC: {r}".format(r=ret))
    print("DEBUG stdout: {s}".format(s=stdout))
    print("DEBUG stderr: {s}".format(s=_))
    if not ret:
        print("Reservation dir not found, the testbed appears to be free.")
        return
    try:
        _, filename = stdout.rsplit(" ", 1)
    except ValueError:
        print("ls output is not perseable. More info:")
