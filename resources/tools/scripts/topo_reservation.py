...
def main():
    ...
    if args.cancel:
        ret, _, err = exec_cmd(tg_node, "rm -r {}".format(RESERVATION_DIR))
        if ret:
    ...
    diag_cmd(tg_node, "ls -cld '{dir}'/*".format(dir=RESERVATION_DIR))
    diag_cmd(tg_node, "head -1 '{dir}/run.url'".format(dir=RESERVATION_DIR))
    ...
    ret, _, err = exec_cmd(tg_node, "mkdir '{dir}'".format(dir=RESERVATION_DIR))
    ...
    ret2, _, err = exec_cmd(
        tg_node, "touch '{dir}/{runtag}' && ( echo '{url}' > '{dir}/run.url' )"\
