...
def create_streams(traffic_options, frame_size=64):
    ...
    if (p1_src_start_udp_port is None) or (p1_dst_start_udp_port is None) or \
            (p2_src_start_udp_port is None) or (p2_dst_start_udp_port is None):
    ...
    print("#" * 80)
    print("p1_src_start_ip: {}".format(p1_src_start_ip))
    print("p1_src_end_ip: {}".format(p1_src_end_ip))
    print("p1_src_start_udp_port: {}".format(int(p1_src_start_udp_port)))
    print("p1_src_end_udp_port: {}".format(int(p1_src_end_udp_port)))
    print("p2_dst_start_ip: {}".format(p2_dst_start_ip))
    print("p2_dst_start_ip: {}".format(p2_dst_start_ip))
    print("p2_dst_start_udp_port: {}".format(int(p2_dst_start_udp_port)))
    print("p2_dst_end_udp_port: {}".format(int(p2_dst_end_udp_port)))
    print("#" * 80)
    if p1_src_start_udp_port != p1_src_end_udp_port:
        vm1 = STLScVmRaw([
            # define tuple gen
            STLVmTupleGen(ip_min=p1_src_start_ip,
                          ip_max=p1_src_end_ip,
                          port_min=int(p1_src_start_udp_port),
                          port_max=int(p1_src_end_udp_port) + 1,
                          name="tuple"),
        ...
        vm2 = STLScVmRaw([
            ...
            STLVmTupleGen(ip_min=p2_dst_start_ip,
                          ...
                          port_max=int(p2_dst_end_udp_port) + 1,
                          name="tuple"),
            # write ip to packet IP.src
