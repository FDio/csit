...
def create_packets_v6(traffic_options, frame_size=78):
    ...
    base_pkt_a = Ether()/IPv6(src=p1_dst_start_ip, dst=p1_src_start_ip)
    base_pkt_b = Ether()/IPv6(src=p2_dst_start_ip, dst=p2_src_start_ip)

    base_pkt_a.show()
    base_pkt_b.show()

    vm1 = CTRexScRaw([STLVmFlowVar(name="ipv6_src", min_value=2, max_value=254,
                                   size=2, op="inc"),
                      STLVmWrFlowVar(fv_name="ipv6_src", pkt_offset="IPv6.dst",
                                     offset_fixup=14)
                     ]
                     , split_by_field="ipv6_src")

    vm2 = CTRexScRaw([STLVmFlowVar(name="ipv6_src", min_value=2, max_value=254,
                                   size=2, op="inc"),
                      STLVmWrFlowVar(fv_name="ipv6_src", pkt_offset="IPv6.dst",
                                     offset_fixup=14)
                     ]
