...
LAST_MODIFIED_BEGIN=utc.localize(datetime.strptime("2021-12-12", "%Y-%m-%d"))
...
def process_json_to_dataframe(schema_name, paths):
    ...
    # sanitize
    sdf = sdf.withColumn("message", regexp_replace("message", "[\\r\\n]", ""))
...
for part_type in ["mrr", "ndrpdr", "soak"]:
    ...
    out_sdf \
        ...
        .repartition(1) \
        .write \
        .partitionBy("result_type", "year", "month", "day") \
        .mode("append") \
