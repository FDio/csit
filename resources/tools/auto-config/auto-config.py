...
def autoconfig_grub_apply(node, ssh):
    ...
    if answer == 'n':
        ...
        if diffs != '':
            ...
            print diffs
            print "\n The actual application of the changes is not ",
            print "enabled yet!!!!"
            answer = autoconfig_yn(
                "\nAre you sure you want to apply these changes [y/N]? ",
                'n')
...
5) Dry Run from ./auto-config.yaml (will not ask questions).\n\
6) Install/Uninstall VPP.\n\
7) Install QEMU patch (Needed when running openstack).\n\
9 or q) Quit'
