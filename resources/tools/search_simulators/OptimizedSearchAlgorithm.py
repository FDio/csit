...
class OptimizedSearchAlgorithm(AbstractSearchAlgorithm):
    ...
    def __init__(self, rate_provider, final_duration=30.0, final_width=0.005, duration_coefficient=3.1):
        """FIXME"""
    ...
    def narrow_down_ndr_and_pdr(self, fail_rate, line_rate, allowed_drop_fraction):
        ...
        max_lo = line_rate * (1.0 - self.final_width)
    ...
    def double_relative_width(relative_width):
        ...
        return 2 * relative_width - relative_width * relative_width

    @staticmethod
    def double_absolute_width(relative_width, upper_bound):
        """Return absolute width corresponding to double logarithmic width."""
        return upper_bound * OptimizedSearchAlgorithm.double_relative_width(relative_width)
    ...
    @staticmethod
    def half_absolute_width(relative_width, upper_bound):
        """Return absolute width corresponding to half logarithmic width."""
        return upper_bound * OptimizedSearchAlgorithm.half_relative_width(relative_width)
        ...
        while 1:
            ...
            if ndr_lo.drop_fraction > 0.0:
                if ndr_lo.target_tr > state.fail_rate:
                    new_width = self.double_absolute_width(ndr_rel_width, ndr_lo.target_tr)
                    new_tr = max(ndr_lo.target_tr - new_width, state.fail_rate)
            ...
            if ndr_hi.drop_fraction <= 0.0:
                if ndr_hi.target_tr < state.line_rate:
                    new_width = self.double_absolute_width(ndr_rel_width, ndr_hi.target_tr)
                    new_tr = min(ndr_hi.target_tr + new_width, state.line_rate)
            ...
            if pdr_lo.drop_fraction > state.allowed_drop_fraction:
                if pdr_lo.target_tr > state.fail_rate:
                    new_width = self.double_absolute_width(pdr_rel_width, pdr_lo.target_tr)
                    new_tr = max(pdr_lo.target_tr - new_width, state.fail_rate)
            ...
            if pdr_hi.drop_fraction <= state.allowed_drop_fraction:
                if pdr_hi.target_tr < state.line_rate:
                    new_width = self.double_absolute_width(pdr_rel_width, pdr_hi.target_tr)
                    new_tr = min(pdr_hi.target_tr + new_width, state.line_rate)
            ...
            if max(ndr_rel_width, pdr_rel_width) > state.width_goal:
                ...
                if ndr_rel_width >= pdr_rel_width:
                    abs_half_width = self.half_absolute_width(ndr_rel_width, ndr_hi.target_tr)
                    new_tr = ndr_hi.target_tr - abs_half_width
                    state = self._measure_and_update_state(state, duration, new_tr)
                    continue
                else:
                    abs_half_width = self.half_absolute_width(pdr_rel_width, pdr_hi.target_tr)
                    new_tr = pdr_hi.target_tr - abs_half_width
                    state = self._measure_and_update_state(state, duration, new_tr)
                    continue
