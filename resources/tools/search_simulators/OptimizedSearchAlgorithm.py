...
class OptimizedSearchAlgorithm(AbstractSearchAlgorithm):
    ...
    def __init__(self, rate_provider, final_trial_duration=30.0, final_width=0.0033, intermediate_phases=2, initial_trial_duration=1.0):
        """FIXME"""
    ...
    def narrow_down_ndr_and_pdr(self, fail_rate, line_rate, allowed_drop_fraction):
        ...
        state = self.ndrpdr(state, self.final_trial_duration, self.intermediate_phases)
        return state.result
    ...
    def ndrpdr(self, state, duration, intermediate_phases):
        """Iterate to improve bounds. When time is up, return current result."""
        if intermediate_phases > 0:
            # We need to finish intermediate phases first, recurse.
            shorter_in_ph = int(intermediate_phases) - 1
            duration_multiplier = duration / self.initial_trial_duration
            phase_exponent = float(shorter_in_ph) / intermediate_phases
            shorter_duration = self.initial_trial_duration * math.pow(duration_multiplier, phase_exponent)
            ...
            state = self.ndrpdr(state, shorter_duration, shorter_in_ph)
