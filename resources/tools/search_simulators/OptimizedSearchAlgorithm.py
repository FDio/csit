...
class OptimizedSearchAlgorithm(AbstractSearchAlgorithm):
    ...
    def ndrpdr(self, state, duration, intermediate_phases):
        ...
        if intermediate_phases > 0:
            ...
            duration_multiplier = duration / self.initial_duration
            phase exponent = float(shorter_in_ph) / intermediate_phases
            shorter_duration = self.initial_duration * math.pow(duration_multiplier, phase_exponent)
