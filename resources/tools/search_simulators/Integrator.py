"""FIXME."""

import copy
import math
import random
import time

from log_plus import log_plus


Integrator__HALF_PI = math.asin(1)
Integrator__DOUBLE_PI = 4 * Integrator__HALF_PI

def integrate_nd(n, value_lweight_f, duration=1.0, scale_factor=2.0):
    """FIXME."""

    len_top = n * (n + 1)
    top = list()
    sum_1 = 0.0
    lsum_w = None
    x_avg = [0.0 for first in range(n)]
    v_avg = 0.0
    v_lrm2 = None
    A = [[0.0 for first in range(second + 1)] for second in range(n)]
    bias_xa = [0.0  for first in range(n)]
    bias_A = [[1.0 if first == second else 0.0 for first in range(second + 1)]
              for second in range(n)]
    time_stop = time.time() + duration
    while 1:
        # Compute biased values.
        if len(top) < len_top:
            a = copy.deepcopy(bias_A)
            xa = list(x_avg)
        else:
            bias_lw = top[0][0]
            lw = log_plus(lsum_w, bias_lw)
            rb = math.exp(bias_lw - lw)
            rw = math.exp(lsum_w - lw)
#            print "bias_lw", bias_lw, "lsum_w", lsum_w, "rb", rb, "rw", rw
            xa = [rw * x_avg[first] + rb * bias_xa[first]  for first in range(n)]
            a = [[rw * A[first][second] + rb * bias_A[first][second]
                  for first in range(second + 1)] for second in range(n)]
        # FIXME!!!
        c = math.sqrt(ayy)
        b = axy / c
        a = math.sqrt(axx - b * b)
#        print "a", a, "b", b, "c", c
        while 1:
            # FIXME!!!
            T = math.tan(Integrator__HALF_PI * random.random())
            r = math.sqrt(T)
            phi = Integrator__DOUBLE_PI * random.random()
            Y = r * math.sin(phi)
            y = ya + scale_factor * c * Y
            if y >= 1.0 or y <= -1.0:
                continue
            X = r * math.cos(phi)
            x = xa + scale_factor * (a * X + b * Y)
            if x >= 1.0 or x <= -1.0:
                continue
            break
        lrarity = log_plus(0.0, 2 * math.log(T))
        sum_1 += 1.0
        value, lweight = value_lweight_f(*x)
#        print "value", value, "lweight", lweight
        if len(top) < len_top:
            top12.append((lweight, x))
        # Hack: top[-1] is either smallest, or just appended to len_top-1 item list.
        if len(top) >= len_top and lweight >= top[-1][0]:
            top = top[:-1]
            top.append((lweight, x))
            top.sort(key=lambda item: -item[0])
#            print "DEBUG top", repr(top)
            # top has changed, recompute biases
            bias_xa = top[0][1]
            bias_A = [[0.0 for first in range(second + 1)] for second in range(n)]
            bias_lsw = 0.0
            for vv, xx in top[1:]:
                lsw_old = bias_lsw
                bias_lsw = log_plus(lsw_old, 0.0)
                rw_old = math.exp(lsw_old - bias_lsw)
                rw_new = math.exp(0.0 - bias_lsw)
                dx = [xx[first] - bias_xa[first] for first in range(n)]
                # Do not move center from biggest point
                for second in range(n):
                    for first in range(second + 1):
                        bias_A[first][second] += dx[first] * dx[second] * rw_new
                        bias_A[first][second] *= rw_old
        lsw_old = lsum_w
        lsum_w = log_plus(lsw_old, lweight)
#        print "new lsum_w", lsum_w
        if lsw_old is None:
            x_avg = list(x)
            v_avg = value
        else:
            rw_old = math.exp(lsw_old - lsum_w)
            rw_new = math.exp(lweight - lsum_w)
            dx = [x[first] - x_avg[first] for first in range(n)]
            dv = value - v_avg
            for first in range(n):
                x_avg[first] += dx[first] * rw_new
            v_avg += dv * rw_new
            for second in range(n):
                for first in range(second + 1):
                    A[first][second] += dx[first] * dx[second] * rw_new
                    A[first][second] *= rw_old
            ad = abs(dv)
            if ad > 0.0:
                v_lrm2 = log_plus(v_lrm2, 2 * math.log(ad) + lweight - lsum_w)
            if v_lrm2 is not None:
                v_lrm2 += lsw_old - lsum_w
        if time.time() >= time_stop:
            stdev = math.exp(v_lrm2 / 2.0)
#            print "Integrated v_avg", v_avg, "stdev", stdev
            print "DEBUG integrator used", sum_1, "samples"
            return v_avg, stdev


def integrate_2d(value_lweight_f, duration=1.0, scale_factor=2.0):
    """FIXME."""

    top12 = list()
    sum_1 = 0.0
    lsum_w = None
    x_avg = 0.0
    y_avg = 0.0
    v_avg = 0.0
    v_lrm2 = None
    Axx = 0.0
    Axy = 0.0
    Ayy = 0.0
    bias_xa = 0.0
    bias_ya = 0.0
    bias_Axx = 1.0
    bias_Axy = 0.0
    bias_Ayy = 1.0
    time_stop = time.time() + duration
    while 1:
        # Compute biased values.
        if len(top12) < 12:
            axx = bias_Axx
            axy = bias_Axy
            ayy = bias_Ayy
            xa = x_avg
            ya = y_avg
        else:
            bias_lw = top12[0][0]
            lw = log_plus(lsum_w, bias_lw)
            rb = math.exp(bias_lw - lw)
            rw = math.exp(lsum_w - lw)
#            print "bias_lw", bias_lw, "lsum_w", lsum_w, "rb", rb, "rw", rw
            xa = rw * x_avg + rb * bias_xa
            ya = rw * y_avg + rb * bias_ya
            axx = rw * Axx + rb * bias_Axx
            axy = rw * Axy + rb * bias_Axy
            ayy = rw * Ayy + rb * bias_Ayy
#        print "xa", xa, "ya", ya, "axx", axx, "axy", axy, "ayy", ayy
        c = math.sqrt(ayy)
        b = axy / c
        a = math.sqrt(axx - b * b)
#        print "a", a, "b", b, "c", c
        while 1:
            T = math.tan(Integrator__HALF_PI * random.random())
            r = math.sqrt(T)
            phi = Integrator__DOUBLE_PI * random.random()
            Y = r * math.sin(phi)
            y = ya + scale_factor * c * Y
            if y >= 1.0 or y <= -1.0:
                continue
            X = r * math.cos(phi)
            x = xa + scale_factor * (a * X + b * Y)
            if x >= 1.0 or x <= -1.0:
                continue
            break
#        print "x", x, "y", y
        lrarity = log_plus(0.0, 2 * math.log(T))
        sum_1 += 1.0
        value, lweight = value_lweight_f(x, y)
#        print "value", value, "lweight", lweight
        if len(top12) < 12:
            top12.append((lweight, x, y))
        # Hack: top12[-1] is either smallest, or just appended to 11-item list.
        if len(top12) >= 12 and lweight >= top12[-1][0]:
            top12 = top12[:-1]
            top12.append((lweight, x, y))
            top12.sort(key=lambda item: -item[0])
#            print "DEBUG top12", repr(top12)
            # top12 has changed, recompute biases
            bias_xa = top12[0][1]
            bias_ya = top12[0][2]
            bias_Axx = 0.0
            bias_Axy = 0.0
            bias_Ayy = 0.0
            bias_lsw = 0.0
            for vv, xx, yy in top12[1:]:
                lsw_old = bias_lsw
                bias_lsw = log_plus(lsw_old, 0.0)
                rw_old = math.exp(lsw_old - bias_lsw)
                rw_new = math.exp(0.0 - bias_lsw)
                dx = xx - bias_xa
                dy = yy - bias_ya
                # Do not move center from biggest point
                #bias_xa += dx * rw_new
                #bias_ya += dy * rw_new
                bias_Axx += dx * dx * rw_new
                bias_Axy += dx * dy * rw_new
                bias_Ayy += dy * dy * rw_new
                bias_Axx *= rw_old
                bias_Axy *= rw_old
                bias_Ayy *= rw_old
#            print "new biases: xa", bias_xa, "ya", bias_ya,
#            print "Axx", bias_Axx, "Axy", bias_Axy, "Ayy", bias_Ayy
        lsw_old = lsum_w
        lsum_w = log_plus(lsw_old, lweight)
#        print "new lsum_w", lsum_w
        if lsw_old is None:
            x_avg = x
            y_avg = y
            v_avg = value
        else:
            rw_old = math.exp(lsw_old - lsum_w)
            rw_new = math.exp(lweight - lsum_w)
            dx = x - x_avg
            dy = y - y_avg
            dv = value - v_avg
            x_avg += dx * rw_new
            y_avg += dy * rw_new
            v_avg += dv * rw_new
            Axx += dx * dx * rw_new
            Axy += dx * dy * rw_new
            Ayy += dy * dy * rw_new
            Axx *= rw_old
            Axy *= rw_old
            Ayy *= rw_old
            ad = abs(dv)
            if ad > 0.0:
                v_lrm2 = log_plus(v_lrm2, 2 * math.log(ad) + lweight - lsum_w)
            if v_lrm2 is not None:
                v_lrm2 += lsw_old - lsum_w
        if time.time() >= time_stop:
            stdev = math.exp(v_lrm2 / 2.0)
#            print "Integrated v_avg", v_avg, "stdev", stdev
            print "DEBUG integrator used", sum_1, "samples"
            return v_avg, stdev
