...
def integrate_2d(value_lweight_f, duration=1.0, scale_factor=20.0):
    ...
    while 1:
        ...
        if record_lw is None:
            record_lw = lweight
        elif record_lw < lweight:
            record_lw = lweight
            # Big jumps in record create too narrow generator peaks.
            # TODO: Find a better solution than just inflating bias arbitrarily.
            arbitrary = lweight - 1.0
            if lsum_w < arbitrary:
                bias_lw += arbitrary - lsum_w
                print "bias_lw bumped to", bias_lw
#                lsum_w = arbitrary
        lsw_old = lsum_w
