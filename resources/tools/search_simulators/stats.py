...
def stat(s):
    ...
    for x in np.arange(0, numberOfRows):
        result = s.narrow_down_intervals(min_rate=20000, max_rate=37000000, packet_loss_ratios=[0.0, 0.005])
        ...
        p.reset()
    #print(f"min {df.min()!r}")
    #print(f"median {df.median()!r}")
    #print(f"max {df.max()!r}")
    print(u"mean:")
    print(f"{df.mean()}")
    print(u"standard deviation:")
    print(f"{df.std()}")
    #print(f"skew {df.skew()!r}")
    #print(f"kurtosis {df.kurt()!r}")

#print(u"basic binary search, width 20000, 10s final duration, single loss measurer")
...
print(u"optimized search, 30s final duration, single loss measurer")
