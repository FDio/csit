...
class TwoPhaseMeasurer(AbstractMeasurer):
    ...
    def __init__(
        self,
        first_phase_duration=0.002,
        first_rr_lo=1950000.0,
        first_rr_hi=2100000.0,
        second_rr_lo=2150000.0,
        second_rr_hi=2180000.0,
        shiftness=5.0,
        sensitivity=0.02,
    ):
    ...
    def measure(self, intended_duration, intended_load):
        ...
        first_phase_duration = min(intended_duration, self.first_phase_duration)
        first_phase_tx = int(first_phase_duration * intended_load)
        ...
        if first_phase_duration >= intended_duration:
            return MeasurementResult(
                first_phase_duration, intended_load, first_phase_tx, first_phase_dx
            )
        second_phase_duration = intended_duration - first_phase_duration
        second_phase_tx = int(second_phase_duration * intended_load)
        second_rr_lo, second_rr_hi = self.second_rr_lo, self.second_rr_hi
        if intended_load > second_rr_lo:
            second_rr_range = second_rr_hi - second_rr_lo
            overload_coeff = intended_load / second_rr_range * self.sensitivity
