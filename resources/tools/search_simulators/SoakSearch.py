...
#            transmit_rate = min(max_rate, max(min_rate, average))
    ...
    @staticmethod
    def fit(mrr, koeff, load):
        """FIXME."""
        print "fitting mrr", mrr, "koeff", koeff, "load", load
        loss_average = load * math.exp(
            - mrr / load - mrr * mrr * koeff / 2.0 / load / load)
        print "fitted loss average", loss_average
        return loss_average

    @staticmethod
    def find_critical_rate(mrr, koeff, lps):
        """FIXME."""
        print "finding for mrr", mrr, "koeff", koeff, "lps", lps
        rate = mrr
        loss = SoakSearch.fit(mrr, koeff, rate)
        if loss == lps:
            return rate
        if loss > lps:
            rate_hi = rate
            while 1:
                rate_lo = rate_hi / 2.0
                loss = SoakSearch.fit(mrr, koeff, rate_lo)
                if loss > lps:
                    rate_hi = rate_lo
                    continue
                if loss == lps:
            ...
            while 1:
                rate_hi = rate_lo * 2.0
                loss = SoakSearch.fit(mrr, koeff, rate_hi)
                if loss < lps:
                    rate_lo = rate_hi
                    continue
                if loss == lps:
        ...
        while rate_hi != rate_lo:
            ...
            loss = SoakSearch.fit(mrr, koeff, rate)
            if rate == rate_hi or rate == rate_lo or loss == lps:
                print "found", rate
                return rate
            if loss > lps:
                rate_hi = rate
    ...
    def lprob(self, mrr, koeff, trial_result_list):
        ...
        for result in trial_result_list:
            alps = self.fit(mrr, koeff, result.target_tr)
