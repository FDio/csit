...
then
    ...
    echo specify argument -- probably is centos-7-1511
    exit 1
fi
...
RPM_WANTLIST_INFRA="nfs-utils cloud-init pkgconfig"
RPM_WANTLIST_CSIT="python-devel python-pip python-virtualenv git strongswan"
...
RPM_WANTLIST_JAVA="openjdk-8-jdk-headless"
...
REPOS_TMPDIR=`mktemp -d`

###
### Get copy of yum repos into this server.
###
sshpass -p $SSH_PASS scp -r ${SSH_USER}@{ip}/etc/yum.repos.d $REPOS_TMPDIR

### Install qemu ($RPM_WANTLIST_NESTED) separately from PPA
for i in ${RPM_WANTLIST_NESTED};  do
    echo http://cbs.centos.org/kojifiles/packages/qemu-kvm-ev/2.3.0/31.el7_2.21.1/x86_64/$i >> $RPM_TEMPFILE
done
###
...
#
### Get the complete RPM list.
#
do_ssh rpm -qa >> $RPM_TEMPFILE
