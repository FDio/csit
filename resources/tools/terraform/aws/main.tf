...
# Create a subnet association for the management
...
# Register a key-pair (public key) for logging in through SSH
# Replace the public key here with the one that you own
...
################################### AMI ###################################
## Find the official Ubuntu 18.04 AMI
data "aws_ami" "ubuntu" {
  most_recent = true
...
################################### Instances ###################################
# Create a placement group for the instances
# This (strategy: cluster) provides a higher network throughput
...
################################### Routes ###################################
# Add a route entry for the default route
