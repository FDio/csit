...
2. Configuration file
   - logging_level
   - sleep_interval
   - re_run_cancelled (optional) - run again cancelled runs
...
class AppConfiguration:
    ...
    def _validate_conf(self, conf_source: str):
        ...
        if conf_source == "env":
            ...
            # Path to operational data.
            if not self._conf["app"].get("path_oper", None):
                logging.debug(
                    f"No path to operational data defined, "
                    f"'{C.DEFAULT_OPER_DATA}' will be used."
                )
                self._conf["app"]["path_oper"] = C.DEFAULT_OPER_DATA
        ...
        # Validate parameters read from the configuration file.
            ...
            temp = self._conf["app"].get("oper_data", None)
            if not temp:
                logging.debug(
                    f"No file for operational JSON data defined, "
                    f"'{C.DEFAULT_OPER_DATA}' will be used."
                )
                self._conf["app"]["oper_data"] = \
                    f"{self._conf['app']['path_oper']}/{C.DEFAULT_OPER_DATA}"
            else:
                self._conf["app"]["oper_data"] = \
                    f"{self._conf['app']['path_oper']}/{temp}"
            # CSV data file.
            temp = self._conf["app"].get("csv_data", None)
            if not temp:
                logging.debug(
                    f"No file for operational CSV data defined, "
                    f"'{C.DEFAULT_CSV_DATA}' will be used."
                )
                self._conf["app"]["csv_data"] = \
                    f"{self._conf['app']['path_oper']}/{C.DEFAULT_CSV_DATA}"
            else:
                self._conf["app"]["csv_data"] = \
                    f"{self._conf['app']['path_oper']}/{temp}"
            # CSV testbed file.
            temp = self._conf["app"].get("testbed_data", None)
            if not temp:
                logging.debug(
                    f"No file for testbed CSV data defined, "
                    f"'{C.DEFAULT_TESTBED_DATA}' will be used."
                )
                self._conf["app"]["testbed_data"] = \
                    f"{self._conf['app']['path_oper']}/{C.DEFAULT_TESTBED_DATA}"
            else:
                self._conf["app"]["testbed_data"] = \
                    f"{self._conf['app']['path_oper']}/{temp}"
            ...
            # Re-runs.
            if not self._conf["app"].get("re_run_cancelled", None):
                self._conf["app"]["re_run_cancelled"] = C.DEFAULT_RE_RUN_CANCELLED
    ...
    def read_conf(self):
        ...
        self._conf["app"]["path_oper"] = \
            os.getenv("PATH_OPER", C.DEFAULT_PATH_OPER)
    ...
    @property
    def re_run_cancelled(self) -> bool:
        """If True re-run the cancelled runs.
        :rtype: bool
        """
        return self._conf["app"]["re_run_cancelled"]

    @property
    def nr_of_re_runs(self) -> int:
        """Number of re-runs of the cancelled runs.
        :rtype: str
        """
