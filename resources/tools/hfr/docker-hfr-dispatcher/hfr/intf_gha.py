...
class Gha:
    ...
    def _get(self, url: str) -> tuple:
        ...
        try:
            resp = requests.get(url=url, headers=self._gh_header,
                                timeout=C.GITHUB_REQUEST_TIMEOUT)
        except requests.exceptions.RequestException as err:
            logging.warning(f"GET requests to '{url}' failed:\n{err}")
            return False, None
        ...
        if resp.ok:
            try:
                ...
                logging.warning(
                    ...
                    f"Cannot decode the response.\n{err}"
                )

        return resp.ok, content

    def _post(self, url: str, params: dict=dict()) -> tuple:
        ...
        try:
            resp = requests.post(url=url, json=params, headers=self._gh_header,
                                 timeout=C.GITHUB_REQUEST_TIMEOUT
            )
        except requests.exceptions.RequestException as err:
            logging.warning(f"POST requests to '{url}' failed:\n{err}")
            return False, None
    ...
    def get_run_status(self, run: Run) -> tuple:
        ...
        if run.data.gha_data and run.data.gha_data["id"]:
            # Get directly info about the run
            return self._get(
                f"{self._gh_url}/actions/runs/{run.data.gha_data['id']}"
            )
        else:
            # Get info about all runs and find the right one based on gha_name
            status, content = self._get((
                f"{self._gh_url}/actions/workflows/{run.conf.workflow}.yml/"
                f"runs"
            ))
    ...
    def cancel_run(self, run: Run, force: bool=False) -> tuple:
        """Cancel the run.
        ...
        return self._post(url = (
            f"{self._gh_url}/actions/runs/{run.data.gha_data['id']}/{method}"
        ))
