...
4. started -> cancelled
   - If the run was cancelled either by GHA, HFR or manually.
...
The run has been cancelled either by GHA, HFR or manually.
...
class RunManagement:
    ...
    def _get_free_tb(self, node: str) -> Testbed:
        ...
        :rtype: Testbed
        """
        SHARED_TGS = ("2n-emr", "3n-emr", "2n-gnr", "3n-gnr")
        for tb in self._tbs.testbeds:
            if node == tb.node and tb.status == "available":
                # Check the testbeds with shared TGs.
                if node in SHARED_TGS:
                    for stb in self._tbs.testbeds:
                        if stb.node in SHARED_TGS and \
                                stb.node[-4:] == tb.node[-4:] and \
                                stb.name != tb.name and \
                                stb.tg["host"] == tb.tg["host"] and \
    ...
    def _print_run(run: Run, prev_state: str=str()):
        """Print the elementary information abut the run to the log.
        """
        if run.data.hfr_id:
    ...
    def _is_gha_replying(self, gha_run: Run) -> bool:
        """Check if GHA replies. If not, finish the run.

        :param run: The run to be manipulated.
        :type run: Run
        """
        if gha_run.data.gha_retries == self._app_conf.max_gha_retries:
            gha_run.data.nr_of_runs += 1
            gha_run.data.hfr_conclusion = "gha_failure"
            gha_run.data.hfr_status = "finished"
            return False
        else:
            return True
    ...
    def _queued(self, run: Run):
        ...
        run.conf.inputs["hfr_run_id"] = run.data.hfr_id
    ...
    def _started(self, run: Run):
        ...
        The run is queued in GHA waiting for an executor and testbed.
        ...
        if not self._is_gha_replying(run):
            return
        ...
        else:  # The run has not been started yet
            run.data.hfr_wait_for_run -= 1
            if run.data.hfr_wait_for_run == 0:
                self._to_cancel(run)
                run.data.hfr_status = "cancelled"
            # else: The run remains in the state "started".
    ...
    def _cancelled(self, run: Run):
        """HFR run state: cancelled (by GHA, HFR or manually).
        ...
        status to "queued" and increase the counter of runs. Otherwise change
        ...
        if not self._app_conf.re_run_cancelled or \
            ...
            run.data.hfr_id = str()
            run.data.hfr_wait_for_run = \
                int(C.DEFAULT_MAX_TIME_FOR_RUN / self._app_conf.sleep_interval)
    ...
    def _finished(self, run: Run):
        ...
        # If there is no (negative) conclusion, it must be success.
        if not run.data.hfr_conclusion:
            run.data.hfr_conclusion = "success"
