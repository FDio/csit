...
def load_topology(args):
    """Load topology filed referenced to by parameter passed to this script.
...
def ssh_no_error(ssh, cmd):
    """Execute a command over ssh channel, and if it fails log and exit.
...
def update_mac_addreses_for_node(node):
    ...
    for port_name, port in node['interfaces'].items():
        if not port.has_key('driver'):
            err_msg = '{0} port {1} has no driver element, exitting'.format(
                    node['host'], port_name)
...
def update_nodes_mac_addreses(topology):
    ...
    for node in topology['nodes'].values():
        update_mac_addreses_for_node(node)
...
def main():
    ...
    update_nodes_mac_addreses(topology)
