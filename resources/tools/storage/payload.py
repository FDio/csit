...
if __name__ == "__main__":
    ...
    df = glue_context.create_dynamic_frame.from_options(
        "s3",
        {'paths': [ PATH ], 'compression': 'gzip', 'recurse':True},
        format="json"
    )
    df.toDF().show()
    ...
    df_ndrpdr = spark.createDataFrame([], schema_ndrpdr)
    df_soak = spark.createDataFrame([], schema_soak)
    df_mrr = spark.createDataFrame([], schema_mrr)
