...
df.repartition(1).write.parquet(u"/tmp/ndrpdr.parquet", partitionBy=[u"test_type"])
df = process_dataframe(u"soak", path_list).toDF()
df.show(truncate=False)
df.repartition(1).write.parquet(u"/tmp/soak.parquet", partitionBy=[u"test_type"])
df = process_dataframe(u"mrr", path_list).toDF()
df.show(truncate=False)
df.repartition(1).write.parquet(u"/tmp/mrr.parquet", partitionBy=[u"test_type"])
