...
LAST_MODIFIED_BEGIN=utc.localize(datetime.strptime(u"2021-11-25", u"%Y-%m-%d"))
...
def process_dataframe(schema_name, paths):
    ...
    drop_subset = [
        u"test_id", u"test_name", u"test_type", u"status", u"sut_type",
        u"sut_version", u"version"
    ]
    ...
    for path in filtered:
        ...
        df = df.unionByName(df_loaded, allowMissingColumns=True)

    # drop rows with all nulls
    df = df.na.drop(how=u"all")
    df = df.na.drop(how=u"any", thresh=None, subset=drop_subset)
