...
def s3_path_get_bucket(path):
    """ """
    return path.split(u"/")[2]
...
def process_dataframe(schema_name, paths):
    ...
    # filter list
    filtered = filter(lambda test_type: schema_name in test_type, paths)
...
if __name__ == "__main__":
    ...
    process_dataframe(u"ndrpdr", path_list).toDF().show(DBG_MAX_DF, truncate=DBG_TRUNCATE)
    process_dataframe(u"soak", path_list).toDF().show(DBG_MAX_DF, truncate=DBG_TRUNCATE)
    process_dataframe(u"mrr", path_list).toDF().show(DBG_MAX_DF, truncate=DBG_TRUNCATE)
    #dyf_dropfields.toDF().repartition(1).write.parquet(u"/tmp/soak.parquet", partitionBy=[u"test_type"])