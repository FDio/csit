...
class Storage:
    ...
    def get_matching_s3_length(
            self, key):
        """
        Gets the file size of S3 object. If the file is gzip'd the packed size
        is reported.
        ...
        try:
            ...
            return unpack("I", last_four_bytes.read(4))[0]
        except exceptions.EndpointConnectionError:
            return 0

    def s3_file_processing(
            self, prefix, suffix, expression):
        """
        ...
        for key in key_iterator:
            if self.get_matching_s3_length(key=key[u"Key"]) <= S3_API_LIMIT:
                print(key[u"Key"])
                yield loads(
                    self.get_matching_s3_content(
