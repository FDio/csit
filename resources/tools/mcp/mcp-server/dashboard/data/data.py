...
class Data:
    ...
    def _create_dataframe_from_parquet(
        ...
        try:
            wr.config.botocore_config = botocore.config.Config(
                ...
                retries={
                    "max_attempts": C.S3_READ_MAX_ATTEMPTS,
                    "mode": "adaptive",
                },
            )
            if C.AWS_ENDPOINT_URL:
                wr.config.s3_endpoint_url = C.AWS_ENDPOINT_URL
            for attempt in range(1, C.S3_READ_MAX_ATTEMPTS + 1):
                try:
                    df = wr.s3.read_parquet(
                        path=path,
                        path_suffix="parquet",
                    ...
                    if attempt >= C.S3_READ_MAX_ATTEMPTS:
                        raise
                    delay = C.S3_READ_RETRY_BASE_SECONDS * (2 ** (attempt - 1))
                    logging.warning(
                        "Retrying S3 parquet read after %s (attempt %s/%s, "
                        "delay %ss).",
                        type(err).__name__,
                        attempt,
    ...
    def read_all_data(
        ...
        selected_types = set(data_types or self._data)
        data_lists = {key: list() for key in selected_types if key in self._data}

        logging.info("\n\nReading data:\n" + "-" * 13 + "\n")
        for data_set in self._data_spec:
            if data_set["data_type"] not in selected_types:
                continue
            logging.info(
        ...
        for key in data_lists:
            ...
            # Release partition-level frames as soon as the combined dataset is
            # owned by the cache reader. This materially reduces load-time RSS.
        ...
        return {key: self._data[key] for key in data_lists}
