...
class CoverageService:
    ...
    def _ensure_index(
        ...
        with self._lock:
            ...
            self._rows = compact_semantic_frame(
                rows,
                categorical_columns=(
                    "release", "dut", "dut_version", "area", "area_label",
                    "infra", "nic", "driver", "suite", "accordion_title",
                    "test_name", "test", "framesize", "cores", "job",
                    "test_id", "throughput_unit",
                ),
            )
            self._unclassified_row_count = unclassified
            self._histogram_decode_error_count = decode_errors
            self._memory_bytes = dataframe_memory_bytes(self._rows)
...
def _build_rows(data: pd.DataFrame) -> tuple[pd.DataFrame, int, int]:
    ...
    unclassified = 0
    decode_errors = 0
    for source in dataframe_records(data):
...
def _sort_rows(data: pd.DataFrame) -> pd.DataFrame:
    ...
    ordered = data.copy()
    ordered["_group_key"] = [
        natural_key(str(value)) for value in ordered["accordion_title"]
    ]
    ordered["_test_key"] = [
        natural_key(str(value)) for value in ordered["test_name"]
    ]
