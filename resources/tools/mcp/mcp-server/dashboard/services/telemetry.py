...
def indexing_telemetry_status(
        telemetry_sources: Mapping[str, pd.DataFrame]
    ) -> dict[str, dict[str, Any]]:
    """Return status metadata while telemetry indexing is in progress."""

    status = empty_telemetry_status(telemetry_sources)
    for dataset, frame in telemetry_sources.items():
        if frame.empty:
            continue
        source_row_count = _telemetry_source_row_count(frame)
...
def failed_telemetry_status(
    ...
    source_row_count = _telemetry_source_row_count(data)
...
def _telemetry_source_row_count(data: pd.DataFrame) -> int:
    """Count source rows without attaching telemetry to the public dataframe."""

    if data.empty or "telemetry" not in data.columns:
        return 0
    return sum(1 for value in data["telemetry"] if _telemetry_values(value))
...
def normalize_dataset_telemetry(
    ...
    has_attached = "telemetry" in data.columns
    has_sidecar = (
        telemetry_source is not None and
        not telemetry_source.empty and
        "telemetry" in telemetry_source.columns
    )
    if data.empty or not (has_attached or has_sidecar):
        return (
            data,
    ...
    if has_sidecar:
        raw_by_source = telemetry_source.set_index("_source_row")["telemetry"]
        candidate_positions = sorted({
            int(position)
            for position in raw_by_source.index
            if (
                pd.notna(position) and 0 <= int(position) < len(working_data) and
                _telemetry_values(raw_by_source.get(position))
            )
        })
    else:
        raw_by_source = None
    ...
    for source_position in selected_positions:
        ...
        raw_value = (
            raw_by_source.get(source_position)
            if raw_by_source is not None else row.get("telemetry")
        )
        decoded = decode_telemetry_cell(raw_value)
    ...
    telemetry_data = compact_semantic_frame(
        telemetry_data,
        categorical_columns=(
            "source_dataset", "test_type", "dut_type", "dut_version",
            "tg_type", "job", "test_id", "release", "metric_name",
            "label_key", "type", "unit",
        ),
    )
    return (
    ...
    for index, item in enumerate(values):
        try:
            ...
            remaining = (
                ...
                else max_decoded_bytes - decoded_bytes
            )
            if remaining is not None and remaining <= 0:
                limit_exceeded = True
                errors.append(
                    f"decode[{index}]: decoded telemetry exceeds byte budget"
                )
                break
            decoded = _bounded_zlib_decompress(compressed, remaining)
            decoded_bytes += len(decoded)
            decoded_parts.append(decoded.decode("utf-8", errors="replace"))
        except OverflowError as err:
            limit_exceeded = True
            errors.append(f"decode[{index}]: {err}")
            break
...
def _bounded_zlib_decompress(
    ...
    while pending:
        ...
        if remaining < 0:
            raise OverflowError("decoded telemetry exceeds byte budget")
...
def telemetry_metrics_payload(
    ...
    return {
        ...
        "dataset": dataset,
        "telemetry": True,
        "index": _representative_index_metadata(status, dataset),
...
def _parse_labels(label_text: str) -> dict[str, str]:
    ...
    for item in _split_label_items(label_text):
        ...
        opening_quote = raw_value[0] if raw_value[0] in {'"', "'"} else None
        closing_quote = raw_value[-1] if raw_value[-1] in {'"', "'"} else None
        if opening_quote != closing_quote:
            raise ValueError("label value has mismatched quotes")
        if opening_quote is not None:
            labels[key] = _unescape_label_value(
                raw_value[1:-1], quote=opening_quote
            )
        else:
...
def _unescape_label_value(value: str, *, quote: str | None = None) -> str:
...
def canonical_telemetry_label(value: Any) -> str:
    """Return one canonical label value for matching and presentation."""
    ...
    if (
        ...
        quote = normalized[0]
        normalized = _unescape_label_value(normalized[1:-1], quote=quote)
    return normalized
...
def _representative_index_metadata(
        status: Mapping[str, Any], dataset: str
    ) -> dict[str, Any]:
    """Describe the bounded diagnostic index behind legacy telemetry tools."""

    telemetry = status.get("telemetry", {})
    dataset_status = (
        telemetry.get(dataset, {})
    ...
    return {
        ...
        "source_row_count": dataset_status.get("source_row_count"),
        "indexed_source_row_count": dataset_status.get("indexed_row_count"),
        "sample_count": dataset_status.get("sample_count"),
        "truncated": dataset_status.get("truncated"),
    }
