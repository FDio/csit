...
class ComparisonService:
    ...
    def catalog_payload(
        ...
        return {
            ...
            "total_row_count": len(self._source_rows),
    ...
    def _ensure_index(
        ...
        with self._lock:
            if self._cache_key == cache_key:
                return
            points, source_rows, unclassified = _build_index(data)
            self._points = points
...
def _build_index(
    ...
    source_rows = data.to_dict(orient="records")
    unclassified = 0
    ...
    return pd.DataFrame(points, columns=_POINT_COLUMNS), source_rows, unclassified
