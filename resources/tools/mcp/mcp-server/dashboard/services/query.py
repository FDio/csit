...
PRIVATE_RESULT_COLUMNS = frozenset({"telemetry"})
...
def result_tool_payload(
    ...
    requested_private_columns = sorted(
        PRIVATE_RESULT_COLUMNS.intersection(parsed_columns or ())
    )
    if requested_private_columns:
        errors.append({
            "field": "columns",
            "message": (
                "Raw telemetry is not available through generic result tools. "
                "Use telemetry_timeseries for bounded telemetry analysis."
            ),
            "value": requested_private_columns,
        })
        parsed_columns = [
            column for column in parsed_columns or []
            if column not in PRIVATE_RESULT_COLUMNS
        ]

    public_data = data.drop(
        columns=list(PRIVATE_RESULT_COLUMNS),
        errors="ignore",
    )
    filtered_data = public_data
    for column, value in text_filters:
...
def _filter_by_text(
    ...
    if column not in data.columns:
        ...
        return data
    if not isinstance(value, (list, tuple, set)) and not (
            data[column].dtype == object and
            _series_contains_list_values(data[column])
        ):
        expected = str(json_safe_value(value))
        return data.loc[data[column].astype("string") == expected]
    return data.loc[
