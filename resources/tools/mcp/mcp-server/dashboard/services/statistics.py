...
class StatisticsService:
    """Cache the run/result join once for each published cache generation."""
    ...
    def enriched_data(
        ...
        cache_key = (
            id(statistics), id(trending), status.get("last_success_at"),
            len(statistics), len(trending),
        )
        ...
        with self._lock:
            ...
            self._enriched = compact_semantic_frame(
                enriched,
                categorical_columns=("dut", "test_type", "cadence", "testbed"),
            )
            self._memory_bytes = dataframe_memory_bytes(self._enriched)
...
def enrich_job_statistics(
    ...
    enriched = statistics.copy(deep=False)
    jobs = (
        enriched["job"]
    ...
    result_rows = trending.loc[:, result_columns].copy(deep=False)
