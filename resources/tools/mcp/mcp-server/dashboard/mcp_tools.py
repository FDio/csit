...
def register_mcp_tools(mcp, data_cache: DataCacheService, settings: AppSettings) -> None:
    ...
    iterative_service = IterativeService()
    ...
    def telemetry_trend_summary(
        ...
        payload = telemetry_trend_summary_payload(
            dataset=dataset,
            data=data,
            status=data_cache.status_snapshot(),
            metric_name=metric_name,
    ...
    def telemetry_anomalies(
        ...
        payload = telemetry_anomalies_payload(
            dataset=dataset,
            data=data,
            status=data_cache.status_snapshot(),
            metric_name=metric_name,
    ...
    @mcp.tool(
        ...
        description=(
            "Provides filtered trending data for CSIT tests with response metadata."
        ),
    )
    def trending(
            test_type: str | None = None,
...
def _telemetry_data_or_error(
    ...
    try:
        source_data = data_cache.get_parquet(query=dataset)
        telemetry_data = data_cache.get_telemetry(dataset)
    except DataCacheError as err:
        return None, cache_error_payload(err, data_cache)

    if "telemetry" not in source_data.columns:
        return None, validation_error_payload(
            [{
                "field": "dataset",
                "message": (
                    f"dataset '{dataset}' has no telemetry column in the "
                    "cached data."
                ),
                "value": dataset,
            }],
