"""Iterative export formatting and serialization helpers."""
...
def default_iterative_export_filename(now: datetime | None = None) -> str:
    """Return the required UTC timestamped filename stem."""

    current = now or datetime.now(tz=UTC)
    if current.tzinfo is None:
        current = current.replace(tzinfo=UTC)
    return f"iterative-{current.astimezone(UTC).strftime('%Y-%m-%d %H:%M:%S')}"


def sanitized_iterative_export_filename(
        value: Any,
        default: str,
        extension: str,
    ) -> str:
    ...
    stem = _filename_component(raw) or _filename_component(default) or "iterative"
    return f"{stem[:120]}.{extension}"


def iterative_export_table(
        records: Sequence[Mapping[str, Any]],
    ) -> tuple[list[str], list[list[Any]]]:
    """Return Iterative sample rows in chronological order."""

    indexed_records = list(enumerate(records))
    indexed_records.sort(key=lambda item: _record_sort_key(item[1], item[0]))
    rows = [_iterative_export_row(record) for _, record in indexed_records]
    return list(ITERATIVE_EXPORT_HEADERS), rows
...
def xlsx_export_bytes(headers: Sequence[str], rows: Sequence[Sequence[Any]]) -> bytes:
    ...
    worksheet = workbook.add_worksheet("Iterative")
...
def _iterative_export_row(record: Mapping[str, Any]) -> list[Any]:
    return [
