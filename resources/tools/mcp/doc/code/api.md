...
| --- | --- | --- |
| `server://info` | Public | Name, version, 29-tool count/list, telemetry/query limits, one public resource, private resource list, and full data status. |
...
| `telemetry_catalog` | `(dataset="trending", dut=None, area=None, test=None, infra=None, testbed=None, framesize=None, cores=None, test_type=None, offset=0, limit=1000)` |
| `telemetry_timeseries` | `(dataset, series, metric, node_names=None, testbed=None, days=30, start_time=None, end_time=None, passed=True, aggregation="per_run", offset=0, limit=1000)` |

The first four tools query the bounded long-form diagnostic index. Their
responses include an `index` object with representative scope, parser version,
source-row coverage, sample count, and truncation state.
`telemetry_catalog` reads only the compact source locator; it performs no
OpenMetrics decode and no JumpAvg analysis.
`telemetry_timeseries` currently accepts `dataset="trending"`, resolves selected
logical series through a compact locator, and reads private raw sidecar rows
only for the requested window. It supports `per_run` and `daily_mean`, reports
completeness plus encoded/decoded byte counts and stage-specific selection
diagnostics. Empty results distinguish no rows in the window, a missing source
metric, missing requested nodes, and semantic-label-policy rejection. It
refuses partial results when a source-row, sample, encoded-byte, decoded-byte,
or response budget is exceeded.
`metric="cycles_per_packet"` maps supported direct VPP metrics to
`cycles/packet`; direct samples take priority over active runtime fallback
samples, and returned records preserve thread identity. Exact raw metric names
are also accepted.

The generic `trending`, `iterative`, and `coverage` tools never return raw
telemetry. Requesting `columns="telemetry"` returns `validation_error`; the
private parquet preview omits that column as well.
...
| `server_busy` | A telemetry or shared heavy-query concurrency/memory gate rejected new work. |
| `query_timeout` | A targeted telemetry query exceeded its execution deadline. |
| `telemetry_worker_failed` | The isolated decode process failed or exceeded its memory budget; FastMCP remains available. |
| `query_too_large` | A targeted query exceeded a source-row, sample, encoded/decoded-byte, or response-byte budget; no partial result is returned. |
