...
For historical telemetry, use `telemetry_catalog` to resolve telemetry-capable series and
then send all selected series and VPP nodes in one `telemetry_timeseries` call.
Use `telemetry_metrics` only for a bounded diagnostic preview. Require
`completeness.complete=true` before drawing conclusions; do not fan out
concurrent raw telemetry calls.

## Capability Boundaries

- All 29 public tools are read-only.
