...
See doc/wrk_lld.rst for the structure of a wrk traffic profile.
"""
...
class WrkTrafficProfile(object):
    ...
    def __init__(self, profile_name):
        ...
        if self.traffic_profile:
            print("\nThe wrk traffic profile '{0}' is valid.\n".
                  format(self.profile_name))
            print("wrk traffic profile '{0}':".format(self.profile_name))
            pprint(self.traffic_profile)
        else:
    ...
    def _validate_traffic_profile(self):
        ...
        if not isinstance(self.traffic_profile, dict):
            print("  The wrk traffic profile must be a dictionary.")
        ...
        for param in self.MANDATORY_PARAMS:
            if not self.traffic_profile.get(param, None):
                print("  The parameter '{0}' in mandatory.".format(param))
                is_valid = False
        if not is_valid:
            self.traffic_profile = None
            return
        ...
        if not isinstance(self.traffic_profile["urls"], list):
            print("  The parameter 'urls' must be a list.")
            is_valid = False
        ...
        try:
            ...
            print("  The parameter 'cpus' must be an integer greater than 1.")
            is_valid = False

        # Level 3: Mandatory params: Check if first-cpu is a valid integer:
        try:
            first_cpus = int(self.traffic_profile["first-cpu"])
            if cpus < 0:
                raise ValueError
            self.traffic_profile["first-cpu"] = first_cpus
        except ValueError:
            print("  The parameter 'first-cpu' must be an integer greater than "
                  "1.")
            is_valid = False
        ...
        try:
            ...
            print("  The parameter 'duration' must be an integer greater than "
                  "1.")
            is_valid = False
        ...
        try:
            ...
            print("  The parameter 'nr-of-threads' must be an integer greater "
                  "than 1.")
            is_valid = False
        ...
        try:
            ...
            print("  The parameter 'nr-of-connections' must be an integer "
                  "greater than 1.")
            is_valid = False
        ...
        if script is not None:
            if not isinstance(script, str):
                print("  The path to LuaJIT script in invalid")
                is_valid = False
            else:
                if not isfile(script):
                    print("  The file '{0}' in not present.".format(script))
                    is_valid = False
        else:
            print("  The optional parameter 'LuaJIT script' is not defined. No "
                  "problem.")
            ...
            if not (isinstance(header, dict) or isinstance(header, str)):
                ...
                if isinstance(header, dict):
                    ...
                    if header_lst:
                        ...
                        print("  The parameter 'header' is defined but empty.")
                        is_valid = False
        else:
            print("  The optional parameter 'header' is not defined. "
                  "No problem.")
        ...
        if latency is not None:
            try:
                ...
                print("  The parameter 'latency' must be boolean.")
                is_valid = False
        else:
            print("  The optional parameter 'latency' is not defined. "
                  "No problem.")
        ...
        if timeout:
            try:
                ...
                print("  The parameter 'timeout' must be integer greater than "
                      "1.")
                is_valid = False
        else:
            print("  The optional parameter 'timeout' is not defined. "
                  "No problem.")
        ...
        if self.traffic_profile["cpus"] % len(self.traffic_profile["urls"]):
            print("  The number of CPUs must be a multiplication of the number "
                  "of URLs.")
            self.traffic_profile = None
