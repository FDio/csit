...
- Inventory file is created with same or similar content as `lf_inventory`.
- Group variables in `ansible/group_vars/all.yaml` are adjusted per needs.
  Special attention to `proxy_env` variable.
- Host variables in `ansible/host_vars/x.x.x.x.yaml` are defined.
