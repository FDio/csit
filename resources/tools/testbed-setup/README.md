...
### Prepare the PXE bootstrap server when there is no http server
  ...
  - the configuration is in one of the files in /etc/nginx/nginx.conf, /etc/nginx/conf.d/ or /etc/nginx/sites-enabled/ under server/root
  - save the path to NGINX_ROOT
