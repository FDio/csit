...
from pal_utils import get_last_successful_build_nr, \
    ...
    def _parse_elements(self):
        ...
        for element in self._cfg_yaml:
            ...
            if element[u"type"] == u"table":
                ...
                try:
                    element[u"template"] = self._replace_tags(
                        element[u"template"],
                        self._specification[u"environment"][u"paths"])
                except KeyError:
                    pass
                ...
                try:
                    for item in (u"reference", u"compare"):
                        if element.get(item, None):
                            data_set = element[item].get(u"data", None)
                            if isinstance(data_set, str):
                                element[item][u"data"] = \
                                    self.configuration[u"data-sets"][data_set]
                            data_set = element[item].get(u"data-replacement",
                                                         None)
                            if isinstance(data_set, str):
                                element[item][u"data-replacement"] = \
                                    self.configuration[u"data-sets"][data_set]

                    if element.get(u"history", None):
                        for i in range(len(element[u"history"])):
                            data_set = element[u"history"][i].get(u"data", None)
                            if isinstance(data_set, str):
                                element[u"history"][i][u"data"] = \
                                    self.configuration[u"data-sets"][data_set]
                            data_set = element[u"history"][i].get(
                                "udata-replacement", None)
                            if isinstance(data_set, str):
                                element[u"history"][i][u"data-replacement"] = \
                                    self.configuration[u"data-sets"][data_set]

                except KeyError:
                    raise PresentationError(
                        f"Wrong data set used in {element.get(u'title', u'')}."
                    )

                self._specification[u"tables"].append(element)
                count += 1
                ...
                layout = element[u"layout"].get(u"layout", None)
                if layout is not None:
                    element[u"layout"].pop(u"layout")
                    try:
                        for key, val in (self.configuration[u"plot-layouts"]
                                         [layout].items()):
                            element[u"layout"][key] = val
                    except KeyError:
                        raise PresentationError(
                            f"Layout {layout} is not defined in the "
                            f"configuration section."
                        )
                self._specification[u"plots"].append(element)
                count += 1

            elif element[u"type"] == u"file":
                logging.info(f"  {count:3d} Processing a file ...")
                try:
                    element[u"dir-tables"] = self._replace_tags(
                        element[u"dir-tables"],
                        self._specification[u"environment"][u"paths"])
                except KeyError:
                    pass
                self._specification[u"files"].append(element)
                count += 1
                ...
                self._specification[u"cpta"] = element
                count += 1
