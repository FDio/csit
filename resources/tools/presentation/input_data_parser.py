...
class InputData:
    ...
    def process_local_directory(self, local_dir, replace=True):
        ...
        root, dirnames, filenames = next(walk(local_dir))
        print(root)
        print(dirnames)
        print(filenames)
        ...
        # local_builds: key: dir (job) name, value: list of filenames (builds)
        local_builds = dict()
        ...
        for job, files in local_builds.items():
            for build_nr, local_file in enumerate(files):
                self.process_local_file(local_file, job, build_nr)
