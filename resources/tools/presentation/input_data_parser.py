...
class InputData:
    ...
    def print_oper_data(self):
        ...
        print(self._input_data[u"tests"])
        for test_name, test in self._input_data[u"tests"]:
            print(f"{test_name}")
            for dut in test.get(u"show-run", dict()):
                print(f"Host IP: {dut.get(u'host', '')}, "
                      f"Socket: {dut.get(u'socket', '')}")
                for thread_nr, thread in dut.get(u"threads", dict()):
                    th_name = \
                        u"main" if thread_nr == 0 else f"worker_{thread_nr + 1}"
                    print(f"DUT{thread_nr + 1}, {th_name}")
                    txt_table = prettytable.PrettyTable(tbl_hdr)
                    for row in thread:
                        txt_table.add_row(row)
                    print(txt_table.get_string())
                    print()
