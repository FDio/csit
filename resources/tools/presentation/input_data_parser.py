...
class ExecutionChecker(ResultVisitor):
    ...
    def start_test(self, test):
        ...
        data[u"test_id"] = self._test_id.replace(u" ", u"_")
        ...
        if u"PERFTEST" in data[u"tags"]:
            ...
            for tag in data[u"tags"]:
                groups = re.search(REGEX_TC_TAG, tag)
                if groups:
                    tag_count += 1
                    tag_tc = tag.lower()
            if tag_count == 1:
                self._test_id = re.sub(
                    REGEX_TC_NAME_NEW, f"-{tag_tc}-", self._test_id, count=1
                )
                data[u"test_name"] = re.sub(
                    REGEX_TC_NAME_NEW, f"-{tag_tc}-", data[u"test_name"],
                    count=1
                )
            else:
                data[u"status"] = False
        ...
        elif u"MRR" in data[u"tags"] or u"FRMOBL" in data[u"tags"] or\
                u"BMRR" in data[u"tags"]:
...
class InputData:
    ...
    def _parse_json_setup(self, j_file):
        ...
        try:
            ...
            duration = data.get(u"duration", 0.0)
            s_id = data.get(u"suite_name", u"").lower()
    ...
    def _parse_json_teardown(j_file):
        ...
        :param j_file: The full path to the JSON file to be parsed.
        :type j_file: str
        :returns: Duration of the teardown phase in seconds.
        :rtype: float
        """
        duration = 0.0
    ...
    def _parse_json_test(self, j_file):
        ...
        :rtype: tuple(bool, str, dict, float)
        """
        duration = 0.0
        status = False
        t_id = u""
        ...
        try:
            ...
            data[u"end_time"] = dt.fromisoformat(data.get(u"end_time", None))
            duration = data.get(u"duration", 0.0)
            t_id = data.get(u"test_id", u"").lower()
            if u"PERFTEST" in data[u"tags"]:
                # Replace -nc- by -mtnc- in test id and test name:
                tag_count = 0
                tag_tc = u""
                for tag in data[u"tags"]:
                    groups = re.search(REGEX_TC_TAG, tag)
                ...
                if tag_count == 1:
                    t_id = re.sub(
                        REGEX_TC_NAME_NEW, f"-{tag_tc}-", t_id, count=1
                    )
                    data[u"test_name"] = re.sub(
                        REGEX_TC_NAME_NEW, f"-{tag_tc}-", data[u"test_name"],
                        count=1
                    ...
                    data[u"status"] = u"FAIL"
            data[u"status"] = True if data.get(u"status", u"") == u"PASS" \
                else False
            if self._for_output == u"trending":
                try:
                    del(data[u"log"])
    ...
    def _parse_json(self, job, build):
        ...
        metadata = {
            ...
            u"tests_passed": 0,
            u"tests_failed": 0,
            u"elapsedtime": 0.0,  # sum of all intervals [ms]
            u"generated": u""
        }
        ...
        # For backward compatibility keep it in milliseconds as integer
        # (for now).
        # TODO: Change format to second as float ???
        metadata[u"elapsedtime"] = int(metadata[u"elapsedtime"] * 1e3)  # ms
