...
class InputData(object):
        ...
        try:
            for job, builds in data_to_filter.items():
                ...
                for build in builds:
                    ...
                    try:
                        data_itms = self.data[job][str(build)][data_set].items()
                        logger.info(data_itms)
                    except KeyError:
                        if continue_on_error:
                            continue
                        else:
                            return None
                    for test_ID, test_data in data_itms:
    ...
    def filter_tests_by_name(self, element, params=None, data_set="tests",
        ...
        if not include:
            logger.warn("No tests to include, skipping the element.")
            return None
