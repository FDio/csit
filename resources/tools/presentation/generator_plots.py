...
def generate_plots(spec, data):
    ...
    generator = {
        "plot_nf_reconf_box_name": plot_nf_reconf_box_name,
        "plot_perf_box_name": plot_perf_box_name,
        "plot_lat_err_bars_name": plot_lat_err_bars_name,
        "plot_tsa_name": plot_tsa_name,
        "plot_http_server_perf_box": plot_http_server_perf_box,
        "plot_nf_heatmap": plot_nf_heatmap
    }
    ...
    for index, plot in enumerate(spec.plots):
        try:
            ...
            generator[plot["algorithm"]](plot, data)
            logging.info("  Done.")
...
def plot_perf_box_name(plot, input_data):
    ...
    logging.info(
        f"    Creating data set for the {plot.get('type', '')} {plot_title}."
    )
    data = input_data.filter_tests_by_name(
    ...
    try:
        ...
        if layout.get("title", None):
            layout["title"] = f"<b>Throughput:</b> {layout['title']}"
        if y_max:
...
def plot_lat_err_bars_name(plot, input_data):
    ...
    for job in data:
        for build in job:
            for test in build:
                try:
                    logging.debug(f"test['latency']: {test['latency']}\n")
                ...
                try:
                    if test["type"] in ("NDRPDR", ):
                        ...
                        logging.warning(f"Invalid test type: {test['type']}")
                        continue
    ...
    try:
        ...
        layout = deepcopy(plot["layout"])
        if layout.get("title", None):
            layout["title"] = f"<b>Latency:</b> {layout['title']}"
...
def plot_tsa_name(plot, input_data):
    ...
    if not y_vals:
        logging.warning(f"No data for the plot {plot.get('title', '')}")
        return
        ...
        try:
            for idx in range(len(val["val"])):
                ...
                if isinstance(val["val"][idx], float):
                    htext += (
                        f"No. of Runs: {val['count'][idx]}<br>"
                        f"Mean: {val['val'][idx]:.2f}Mpps<br>"
                    )
                if isinstance(val["diff"][idx], float):
                    htext += f"Diff: {round(val['diff'][idx]):.0f}%<br>"
                if isinstance(val["rel"][idx], float):
                    htext += f"Speedup: {val['rel'][idx]:.2f}"
            ...
            traces.append(
                plgo.Scatter(
                    ...
                    line=dict(
                        ...
                        width=2,
                        dash="dash"),
                    text=[f"Perfect: {y:.2f}Mpps" for y in val["ideal"]],
                    hoverinfo="text"
                )
            )
            cidx += 1
    ...
    try:
        ...
        layout = deepcopy(plot["layout"])
        if layout.get("title", None):
            layout["title"] = f"<b>Speedup Multi-core:</b> {layout['title']}"
...
def plot_http_server_perf_box(plot, input_data):
    ...
    try:
        ...
        # Export Plot
        logging.info(
            f"    Writing file {plot['output-file']}{plot['output-file-type']}."
        )
        ploff.plot(plpl, show_link=False, auto_open=False,
                   filename=f"{plot['output-file']}{plot['output-file-type']}")
    except PlotlyError as err:
        logging.error(f"   Finished with error: {repr(err)}".replace("\n", " "))
        return
            ...
            for test in build:
                ...
                try:
                    if plot["include-tests"] == "MRR":
                        result = test["result"]["receive-rate"]
                ...
                if result:
                    vals[chain][node]["vals"].append(result)
        ...
        for node, _ in enumerate(txt_nodes):
            if data[chain][node] is not None:
                ...
                hover_line.append(text.format(
                    name=vals[txt_chains[chain]][txt_nodes[node]]["name"],
                    nr=vals[txt_chains[chain]][txt_nodes[node]]["nr"],
                    val=data[chain][node],
                    stdev=vals[txt_chains[chain]][txt_nodes[node]]["stdev"]))
