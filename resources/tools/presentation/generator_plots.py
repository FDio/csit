...
def plot_service_density_heatmap(plot, input_data):
    ...
    for c in chains:
        for n in nodes:
            try:
                ...
                val = None
            data[c - 1].append(val)
            data2[c - 1].append(val + randint(0, 10)/10) if val else data[c - 1].append(None)
            diff[c - 1].append(((val + randint(0, 10)/10) - val) * 100 / val if val else None)
    ...
    layout["annotations"] = annotations + annotations2 + annotations_diff
