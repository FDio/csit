...
def plot_service_density_heatmap_compare(plot, input_data):
    ...
    #         "StDev: {stdev}")
    text = ("Test: {name}<br>"
            "  1c: Thput: {val1c}; StDev: {stdev1c}; Runs: {nr1c}<br>"
            "0.5c: Thput: {val05c}; StDev: {stdev05c}; Runs: {nr05c}<br>"
    ...
    for c in range(len(txt_chains)):
        ...
        for n in range(len(txt_nodes)):
            if data[c][n] is not None:
                ...
                annotations1c.append(deepcopy(data_point))
                data_point["text"] = str(data2[c][n])
                annotations05c.append(deepcopy(data_point))
