...
def plot_statistics(plot, input_data):
    ...
    for job, builds in plot[u"data"].items():
        for build_nr in builds:
            ...
            date = datetime(
                int(meta[u"generated"][0:4]),
                int(meta[u"generated"][4:6]),
                int(meta[u"generated"][6:8]),
                int(meta[u"generated"][9:11]),
                int(meta[u"generated"][12:])
            )
            data_x.append(date)
            data_y_pass.append(meta[u"tests_passed"])
            data_y_fail.append(meta[u"tests_failed"])
            minutes = meta[u"elapsedtime"] // 60000
            duration = f"{(minutes // 60):02d}:{(minutes % 60):02d}"
            data_y_duration.append(minutes)
            hover_text.append(hover_str.format(
                date=date,
                passed=meta[u"tests_passed"],
                failed=meta[u"tests_failed"],
                duration=duration,
                sut=u"vpp" if u"vpp" in job else u"dpdk",
                build=meta[u"version"],
