                ...
                try:
                    if test["type"] in ("NDRPDR", ):
                        if "-pdr" in plot_title.lower():
                            ...
                            logger.warn("Invalid test type: {0}".
                                        format(test["type"]))
                            continue
...
def plot_throughput_speedup_analysis_name(plot, input_data):
    ...
    if not y_vals:
        logger.warn("No data for the plot '{}'".format(plot.get("title", "")))
        return
    ...
    for test_name, test_vals in y_vals.items():
        try:
            ...
            logger.warn("No data for '{0}'".format(test_name))
            logger.warn(repr(err))
    ...
    for name, val in vals.items():
        ...
        try:
            ...
            logger.warn("No data for '{0}'".format(name))
            logger.warn(repr(err))
