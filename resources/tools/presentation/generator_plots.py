...
def plot_perf_box_name(plot, input_data):
    ...
    TODO: Rewrite only for soak and hoststack
    ...
    for ttype in plot.get(u"test-type", list()):
        for core in plot.get(u"core", list()):
                        ...
                        for test_id, test in build.iteritems():
                            ...
                            try:
                                ...
                                data_names.append(
                                    ...
                                    f"({len(data_y[-1]):02d} "
                                    f"run"
                                    f"{u's' if len(data_y[-1]) > 1 else u''}) "
                                    f"{name}"
                                )
            ...
            for idx in range(len(data_x)):
                traces.append(
                    plgo.Box(
                        ...
                        y=[y / 1e6 if y else None for y in data_y[idx]],
                        name=data_names[idx],
                        hoverinfo=u"y+name"
                    )
                )
            ...
            try:
                ...
                if layout.get(u"title", None):
                    layout[u"title"] = \
                        f"{layout[u'title'].format(core=core)}-{ttype}"
                ...
                # Export Plot
                file_name = (
                    f"{plot[u'output-file'].format(core=core)}"
                    f"-{ttype}"
                    f".html"
                )
                logging.info(f"    Writing file {file_name}")
