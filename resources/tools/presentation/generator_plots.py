...
def plot_hdrh_lat_by_percentile(plot, input_data):
    ...
    for test in data[0][0]:
        try:
            ...
            desc = {
                u"LAT0": u"No load",
                u"PDR10": f"10% PDR background traffic ({(pdr * 0.1):.2f}Mpps)",
                u"PDR50": f"50% PDR background traffic ({(pdr * 0.5):.2f}Mpps)",
                u"PDR90": f"90% PDR background traffic ({(pdr * 0.9):.2f}Mpps)",
                u"PDR": f"100% PDR background traffic ({pdr:.2f}Mpps)",
                u"NDR10": f"10% NDR background traffic ({(ndr * 0.1):.2f}Mpps)",
                u"NDR50": f"50% NDR background traffic ({(ndr * 0.5):.2f}Mpps)",
                u"NDR90": f"90% NDR background traffic ({(ndr * 0.9):.2f}Mpps)",
                u"NDR": f"100% NDR background traffic ({ndr:.2f}Mpps)"
            }
