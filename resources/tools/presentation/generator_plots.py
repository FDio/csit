...
def plot_throughput_speedup_analysis(plot, input_data):
    ...
    for test_name, test_vals in throughput.items():
        if test_vals["1"]:
            ...
            if len(name) > 30:
                name_lst = name.split('-')
                name = "-".join(name_lst[:len(name_lst)/2]) + '-\n' + \
    ...
    y_vals_rel = [y_vals_rel_1, y_vals_rel_2, y_vals_rel_4]
    y_vals = [y_vals_1, y_vals_2, y_vals_4]

    y_vals_zipped = zip(names, y_vals_rel, y_vals)
    ...
    for val in y_vals_zipped:
        traces.append(plgo.Bar(y=x_vals,
                               ...
                               text=val[2],
                               hoverinfo='x'))
    plot["layout"]["height"] = len(x_vals) * 100 + 100
    try:
        # Create plot
        logging.info("    Writing file '{0}{1}'.".
                     format(plot["output-file"], plot["output-file-type"]))
        ...
        ploff.plot(plpl,
                   ...
                   filename='{0}{1}'.format(plot["output-file"],
                                            plot["output-file-type"]))
        with open("{0}.rst".format(plot["output-file"]), 'w') as rst_file, \
            open("{0}{1}".format(plot["output-file"],
                                 plot["output-file-type"]), 'r') as html_file:
            rst_file.write(".. raw:: html\n\n\t")
            rst_file.write(html_file.read())
            rst_file.write("\n\n")
