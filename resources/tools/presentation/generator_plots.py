...
def plot_soak_bars(plot, input_data):
        ...
        for build in job:
            for test in build:
                ...
                try:
                    if test["type"] in ("NDRPDR", ):
                        if "-pdr" in plot_title.lower():
                            y_vals[test["parent"]] = test["throughput"]["PDR"]
                        elif "-ndr" in plot_title.lower():
                            y_vals[test["parent"]] = test["throughput"]["NDR"]
                        else:
                            continue
                    else:
                        continue
    ...
    for test_name, test_data in y_sorted.items():
        idx += 1
        name = "{nr}. {name}".\
            format(nr=idx, name=test_name.lower().replace('-ndrpdr', ''))
        if len(name) > 50:
            name_lst = name.split('-')
            name = ""
            split_name = True
            ...
            name = name[:-1]
        # x_name.append(name)
        # x_vals.append(str(idx) + '.')
        # idx += 1
        # y_vals.append(test_data.get("UPPER", None) / 1000000)
        # y_base.append(test_data.get("LOWER", None) / 1000000)
    ...
    #     traces.append(plgo.Bar(x=x_vals[i],
    #                            y=y_vals[i],
    #                            base=y_base[i],
    #                            name=x_name[i]))
    # try:
