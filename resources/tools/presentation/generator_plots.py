...
def plot_error_bars_latency(plot, input_data):
    ...
    for key, val in y_sorted.items():
        ...
        y_mins.append(mean(val[1]) - mean(val[0]))
        y_maxs.append(mean(val[2]) - mean(val[1]))
        x_vals.append(key)  # dir 2
        y_vals.append(mean(val[4]))
        y_mins.append(mean(val[4]) - mean(val[3]))
        y_maxs.append(mean(val[5]) - mean(val[4]))
    ...
    # x_axis = ["TGint1-to-SUT1-to-SUT2-to-TGint2",
    #           "TGint2-to-SUT2-to-SUT1-to-TGint1"] * (len(x_vals) / 2)
...
def plot_line_throughput_speedup_analysis(plot, input_data):
    ...
    traces.append(plgo.Scatter(x=x_vals,
                               y=[nic_limit / 1000000, ] * len(x_vals),
                               showlegend=False,
                               mode="lines",
                               line=dict(
                                   color="Grey",
                                   width=2),
                               text="NIC Limit: {0:.2f}Mpps".
                               format(nic_limit / 1000000),
                               hoverinfo="text"
                               ))
    y_max.append(10 ** floor(log10(nic_limit)) * (int(nic_limit / 10) + 1))
    traces.append(plgo.Scatter(x=x_vals,
                               y=[link_limit / 1000000, ] * len(x_vals),
                               showlegend=False,
                               mode="lines",
                               line=dict(
                                   color="Grey",
                               ...
                               format(link_limit / 1000000),
                               hoverinfo="text"
                               ))
    y_max.append(10 ** floor(log10(link_limit)) * (int(link_limit / 10) + 1))
    traces.append(plgo.Scatter(x=x_vals,
                               y=[pci_limit / 1000000, ] * len(x_vals),
                               showlegend=False,
                               mode="lines",
                               line=dict(
                                   color="Grey",
                                   width=2),
                               text="PCIe Limit: {0:.2f}Mpps".
                               format(pci_limit / 1000000),
                               hoverinfo="text"
                               ))
