...
def plot_throughput_speedup_analysis(plot, input_data):
        ...
        for build in job:
            for test in build:
                name = re.sub(REGEX_NIC, "", test["parent"].
                              replace('-ndrpdr', '').replace('2n1l-', ''))
                if y_vals.get(name, None) is None:
                    y_vals[name] = {"1": list(), "2": list(), "4": list()}
                    y_tags[name] = test.get("tags", None)
                try:
                    if test["type"] in ("NDRPDR",):
                        if "-pdr" in plot_title.lower():
                            ttype = "PDR"
    ...
    if not y_vals:
        logging.warning("No data for the plot '{0}'".
    ...
    for name, test_vals in y_vals.items():
        try:
            if test_vals["1"][1]:
                vals[name] = dict()
                ...
                vals[name]["ideal"] = [y_1c_max[name],
                                       y_1c_max[name] * 2,
                                       y_1c_max[name] * 4]
                vals[name]["diff"] = [(y_val_1 - y_1c_max[name]) * 100 /
                                      y_val_1, None, None]
    ...
    if (pci_limit < threshold and
        (pci_limit < nic_limit * 0.95 or pci_limit > nic_limit * 1.05)):
        traces.append(plgo.Scatter(
            x=x_vals,
