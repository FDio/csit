...
def plot_service_density_heatmap(plot, input_data):
    ...
    text = ("Network Functions: {nodes}<br>"
            "Chains of NF: {chains}<br>"
            "Packet Throughput: {val}Mpps")

    for c in range(len(txt_chains)):
        for n in range(len(txt_nodes)):
            val = data[c][n] if data[c][n] else "Not measured"
            hovertext.append(text.format(nodes=txt_nodes[n],
                                         chains=txt_chains[c],
                                         val=val))
            if data[c][n]:
                annotations.append(dict(
                    ...
                    xanchor="center",
                    yanchor="middle",
                    text="{val}Mpps".format(val=data[c][n]),
                    font=dict(
                        size=16,
                    ),
                    align="center",
