...
def plot_error_bars_latency(plot, input_data):
    ...
    for idx in range(len(x_vals)):
        ...
        hovertext = ("Test: {test}<br>"
                     ...
                     "Max: {max:.2f}uSec<br>"
                     "Avg: {avg:.2f}uSec<br>"
                     "Min: {min:.2f}uSec".format(test=x_vals[idx],
                                                 dir=direction,
                                                 max=y_maxs[idx],
                                                 avg=y_vals[idx],
                                                 min=y_mins[idx]))
...
def plot_line_throughput_speedup_analysis(plot, input_data):
    ...
    if nic_limit < threshold:
        traces.append(plgo.Scatter(
            ...
            name="NIC Limit: {0:.2f}Mpps".format(nic_limit),
            showlegend=False,
            mode="lines",
            line=dict(
               dash="dot",
    ...
    if lnk_limit < threshold:
        traces.append(plgo.Scatter(
            ...
            name="Link Limit: {0:.2f}Mpps".format(lnk_limit),
            showlegend=False,
            mode="lines",
            line=dict(
               dash="dot",
        ...
        annotations.append(dict(
            ...
            xanchor="left",
            yanchor="bottom",
            text="Link Limit: {0:.2f}Mpps".format(lnk_limit),
            font=dict(
                size=14,
            ),
            align="left",
        ...
        y_max.append(10 ** floor(log10(lnk_limit)) * (int(lnk_limit / 10) + 1))
    ...
    if pci_limit < threshold:
        traces.append(plgo.Scatter(
            ...
            name="PCIe Limit: {0:.2f}Mpps".format(pci_limit),
            showlegend=False,
            mode="lines",
            line=dict(
               dash="dot",
    ...
    try:
        ...
        logging.info("Layout Annotations: {0}\n".format(layout["annotations"]))
        plpl = plgo.Figure(data=traces, layout=layout)
