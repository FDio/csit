...
def plot_service_density_heatmap_compare(plot, input_data):
    ...
    #         "StDev: {stdev}")
    text = ("Test: {name}<br>"
            "1c Thput/StDev/Runs: {val1c}/{stdev1c}/{nr1c}<br>"
            "0.5c Thput/StDev/Runs: {val05c}/{stdev05c}/{nr05c}<br>"
    ...
    for c in range(len(txt_chains)):
        ...
        for n in range(len(txt_nodes)):
            if data[c][n] is not None:
                ...
                annotations1c.append(data_point)
                data_point["text"] = str(data2[c][n])
                annotations05c.append(data_point)
                data_point["text"] = "{diff:.1f}%".format(diff=diff[c][n])
                annotations_diff.append(data_point)

                hover_line.append(text.format(
                    name=vals[txt_chains[c]][txt_nodes[n]]["name"],
                    val1c=data[c][n],
                    val05c=data2[c][n],
                    stdev1c=vals[txt_chains[c]][txt_nodes[n]]["stdev"],
                    stdev05c = vals[txt_chains[c]][txt_nodes[n]]["stdev"],  # change
                    nr1c=vals[txt_chains[c]][txt_nodes[n]]["nr"],
                    nr05c=vals[txt_chains[c]][txt_nodes[n]]["nr"],  # change
                ...
                dict(
                    ...
                    args=[
                        ...
                        },
                        {
                            "colorscale": [my_green, ],
                            "reversescale": False,
                            "annotations": annotations + annotations1c,
                        },
                    ]
                ),
                dict(
                    ...
                    args=[
                        ...
                        },
                        {
                            "colorscale": [my_blue, ],
                            "reversescale": False,
                            "annotations": annotations + annotations05c,
                        },
                    ]
                ),
                dict(
    ...
    layout["annotations"] = annotations + annotations1c
    layout['updatemenus'] = updatemenus
