...
def plot_service_density_heatmap_compare(plot, input_data):
    ...
    text = "Test: {name}{text_r}{text_c}{text_diff}"
    text_r = "<br>1c: Thput: {val_r}; StDev: {stdev_r}; Runs: {nr_r}"
    text_c = "<br>0.5c: Thput: {val_c}; StDev: {stdev_c}; Runs: {nr_c}"
    ...
    for c in range(len(txt_chains)):
        ...
        for n in range(len(txt_nodes)):
            if data_r[c][n] is not None:
                ...
                data_point["text"] = str(data_c[c][n]) if data_c[c][n] is not None else ""
                annotations_c.append(deepcopy(data_point))
                data_point["text"] = str(diff[c][n]) if diff[c][n] is not None else ""
                ...
                hover_line.append(text.format(
                    ...
                    text_r=text_r.format(
                        ...
                        stdev_r=vals[txt_chains[c]][txt_nodes[n]]["stdev_r"],
                        nr_r=vals[txt_chains[c]][txt_nodes[n]]["nr_r"]
                    ) if data_r[c][n] is not None else "",
    ...
    updatemenus = list([
        dict(
            ...
            buttons=list([
                dict(
                    label=plot["reference"]["name"],  # "VNF-1c"
                    method="update",
                    args=[
                        {
                ...
                dict(
                    label=plot["compare"]["name"],  # "VNF-0.5c"
                    method="update",
                    args=[
                        {
