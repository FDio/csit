...
def execute_command(cmd):
    ...
    if stdout:
        logging.info(stdout.encode('utf-8'))
    if stderr:
        logging.info(stderr.encode('utf-8'))

    if proc.returncode != 0:
        logging.error("    Command execution failed.")
    return proc.returncode, stdout.encode('utf-8'), stderr.encode('utf-8')
...
def archive_input_data(spec):
    ...
    try:
        ...
        raise PresentationError(
            "Not possible to archive the input data.",
            repr(err)
        )
