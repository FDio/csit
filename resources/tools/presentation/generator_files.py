    ...
    for data_set, table_set in zip(data_sets, table_sets):
        ...
        for suite_longname, suite in suites.items():
            ...
            if _tests_in_suite(suite[u"name"], tests):
                groups = re.search(REGEX_NIC, suite[u"name"])
                nic = groups.group(1) if groups else None
                if nic is None:
                    continue
                file_name = f"{join(table_set, chapter_l1)}.rst"
                if chapters[chapter_l1][chapter_l2].get(nic, None):
                    chapters[chapter_l1][chapter_l2][nic] = list()
                chapters[chapter_l1][chapter_l2][nic].append(file_name)
