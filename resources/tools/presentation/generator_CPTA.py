...
def _evaluate_results(trimmed_data, window=10):
    ...
    - regress: less than trimmed moving average - 3 * stdev
    - normal: between trimmed moving average - 3 * stdev and median + 3 * stdev
    - progress: more than trimmed moving average + 3 * stdev
    ...
    if len(trimmed_data) > 2:
        ...
        results = [0.66, ]
        tma = trimmed_data.rolling(window=win_size, min_periods=2).mean()
        ...
        for build_nr, value in trimmed_data.iteritems():
            if first:
                ...
                continue
            if (np.isnan(value) \
                    or np.isnan(tma[build_nr]) \
                    or np.isnan(tmstd[build_nr])):
                results.append(0.0)
            elif value < (tma[build_nr] - 3 * tmstd[build_nr]):
                results.append(0.33)
            elif value > (tma[build_nr] + 3 * tmstd[build_nr]):
                results.append(1.0)
            else:
        ...
        results = [0.0, ]
        try:
            tma = np.mean(trimmed_data)
            tmstd = np.std(trimmed_data)
            if trimmed_data.values[-1] < (tma - 3 * tmstd):
                results.append(0.33)
            elif (tma - 3 * tmstd) <= trimmed_data.values[-1] <= (
                    tma + 3 * tmstd):
                results.append(0.66)
            else:
                results.append(1.0)
...
def _generate_trending_traces(in_data, build_info, period, moving_win_size=10,
    ...
    """Generate the trending traces:
     - samples,
     - trimmed moving average (trending line)
    ...
    if show_trend_line:
        data_y_trimmed = [val for val in t_data.values() if not isnan(val)]
        data_trend_y = pd.Series(data_y).rolling(
            window=moving_win_size, min_periods=2).mean()
