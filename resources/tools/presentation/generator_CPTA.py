...
def _generate_all_charts(spec, input_data):
    ...
    def _generate_chart(graph):
        ...
        if groups:
            ...
            for tst_name, test_data in chart_data.items():
                if not test_data:
                    logs.append(("WARN", "No data for the test '{0}'".
                                 format(tst_name)))
                    continue
                message = "index: {index}, test: {test}".format(
                    index=index, test=tst_name)
        ...
        if traces:
            ...
            try:
                ...
                logs.append(("WARN", "No data for the plot. Skipped."))

        for level, line in logs:
            logger.write(line, level=level)
            ...
            for row in csv_content:
                if txt_table is None:
                    ...
                    try:
                        ...
                        logger.warn("Error occurred while generating TXT "
                                    "table:\n{0}".format(err))
                line_nr += 1
