...
def table_weekly_comparison(table, in_data):
    ...
    for job_name, job_data in data.items():
        for build_nr, build in job_data.items():
            ...
            if tb_ip and tb_tbl:
                ...
                testbed = u""
            header[0].insert(1, build_nr)
            header[1].insert(1, testbed)
            header[2].insert(
    ...
    for idx, cmp in enumerate(table.get(u"comparisons", list())):
        ...
        if idx_ref is None or idx_cmp is None:
            continue
        header[0].append(u"Diff")
        header[1].append(header[-1][idx_ref - idx])
        header[2].append(u"vs")
        header[3].append(header[-1][idx_cmp - idx][0])
    ...
    # Generate txt table:
    # txt_file_name = f"{table[u'output-file']}.txt"
    # txt_table = prettytable.PrettyTable(hdr_str)
    # for row in tbl_lst:
