...
def _tpc_generate_html_table(header, data, out_file_name, legend=u"",
    ...
    with open(f"{path}{file_name}.rst", u"wt") as rst_file:
        ...
        rst_file.write(
            ...
            u'        function getDocHeight(doc) {\n'
            u'            doc = doc || document;\n'
            u'            var body = doc.body, html = doc.documentElement;\n'
            u'            var height = Math.max( body.scrollHeight, '
            u'body.offsetHeight,\n'
            u'                html.clientHeight, html.scrollHeight, '
            u'html.offsetHeight );\n'
            u'            return height;\n'
            u'        }\n'
            u'        function setIframeHeight(id) {\n'
            u'            var ifrm = document.getElementById(id);\n'
            u'            var doc = ifrm.contentDocument? '
            u'ifrm.contentDocument:\n'
            u'                ifrm.contentWindow.document;\n'
            u'            ifrm.style.visibility = "hidden";\n'
            u'            ifrm.style.height = "10px";\n'
            u'            ifrm.style.height = getDocHeight( doc ) + 4 + "px";\n'
            u'            ifrm.style.visibility = "visible";\n'
            u'        }\n'
            u'    </script>\n\n'
        )
        rst_file.write(
            ...
            f'width="1500" height="1000" '
            f'src="../..{out_file_name.replace(u"_build", u"")}_in.html">'
            f'</iframe>\n\n'
            # f'    <iframe id="id00" onload="setIframeHeight(this.id)" '
            # f'frameborder="0" scrolling="no"  width="1500" '
            # f'src="../..{out_file_name.replace(u"_build", u"")}_in.html">'
            # f'</iframe>\n\n'
        )
        if legend:
            rst_file.write(legend.replace(u"\n", u" |br| ")[1:])
        if footnote:
