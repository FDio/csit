...
def table_details(table, input_data):
    ...
    for suite in suites.values:
        ...
        for test in data[job][build].keys():
            if data[job][build][test][u"parent"] in suite_name:
                row_lst = list()
                for column in table[u"columns"]:
                    try:
...
def table_merged_details(table, input_data):
    ...
    for suite in suites.values:
        ...
        for test in data.keys():
            if data[test][u"parent"] in suite_name:
                row_lst = list()
                for column in table[u"columns"]:
                    try:
...
def table_perf_comparison(table, input_data):
    ...
    try:
        ...
        history = table.get(u"history", None)
        if history:
            for item in history:
                header.extend(
                    [
        ...
        for build in builds:
            for tst_name, tst_data in data[job][str(build)].items():
                ...
                try:
                    ...
                    if result:
                        if table[u"reference"][u"nic"] in tst_data[u"tags"]:
                            tbl_dict[tst_name_mod][u"ref-data"].append(result)
                        elif table[u"compare"][u"nic"] in tst_data[u"tags"]:
        ...
        for build in builds:
            for tst_name, tst_data in data[job][str(build)].items():
                ...
                if tst_name_mod in tests_lst:
                    try:
                        if tst_data[u"type"] in (u"NDRPDR", u"MRR", u"BMRR"):
                            if table[u"include-tests"] == u"MRR":
                                result = tst_data[u"result"][u"receive-rate"]
                            elif table[u"include-tests"] == u"PDR":
                                result = \
...
def _generate_url(base, testbed, test_name):
    ...
    if u"lbdpdk" in test_name or u"lbvpp" in test_name:
        ...
        feature = u"-base-scale"
            ...
            or u"l2dbbasemaclrn" in test_name or u"l2dbscale" in test_name:
        file_name = u"l2"
...
def table_perf_trending_dash_html(table, input_data):
    ...
    testbed = table.get(u"testbed", None)
    if testbed is None:
        logging.error(
    ...
    for r_idx, row in enumerate(csv_lst[1:]):
        ...
        trow = ET.SubElement(dashboard, u"tr", attrib=dict(bgcolor=background))

        # Columns:
        for c_idx, item in enumerate(row):
            alignment = u"left" if c_idx == 0 else u"center"
            tdata = ET.SubElement(trow, u"td", attrib=dict(align=alignment))
            # Name:
            if c_idx == 0:
                url = _generate_url(u"../trending/", testbed, item)
                ref = ET.SubElement(tdata, u"a", attrib=dict(href=url))
                ref.text = item
            else:
                tdata.text = item
    try:
