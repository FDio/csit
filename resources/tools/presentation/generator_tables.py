...
def _tpc_generate_html_table(header, data, output_file_name, legend=u"",
    ...
    ploff.plot(
        ...
        show_link=False,
        auto_open=False,
        filename=f"{output_file_name}_in.html"
    )
    ...
    file_name = output_file_name.split(u"/")[-1]
    if u"vpp" in output_file_name:
...
.. raw:: html
    ...
    <iframe frameborder="0" scrolling="no" src="{output_file_name}_in.html">
    </iframe>

"""
        )
        if legend:
            rst_file.write(legend)
        if footnote:
            rst_file.write(footnote)
