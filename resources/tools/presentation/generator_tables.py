...
def _tpc_generate_html_table(header, data, output_file_name):
    ...
    df_sorted = [df.sort_values(
        by=[key, header], ascending=[True, True]
        if key != header else [False, True]) for key in header]
    df_sorted_rev = [df.sort_values(
        by=[key, header], ascending=[False, True]
        if key != header else [True, True]) for key in header]
