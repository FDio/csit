...
def table_last_failed_tests(table, input_data):
    ...
    logging.info("input data version")
    logging.info(input_data.metadata("csit-vpp-perf-mrr-daily-master", "965")["version"])
    ...
    for job, builds in table["data"].items():
        for build in builds:
            ...
            try:
                ...
                logging.info(input_data.metadata(job, build))
                logging.info(type(input_data.metadata(job, build)))
                logging.info("version")
                logging.info(version)
                logging.info(type(version))
            except KeyError:
