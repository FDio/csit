...
def table_oper_data_html(table, input_data):
    ...
    def _generate_html_table(tst_data):
        ...
        trow = ET.SubElement(tbl, u"tr", attrib=dict(bgcolor=colors[u"body"]))
        tcol = ET.SubElement(
        ...
        for dut_name, data in tst_data[u"show-run"].items():
            ...
            trow = ET.SubElement(
                tbl, u"tr", attrib=dict(bgcolor=colors[u"body"])
            )
            tcol = ET.SubElement(
                trow, u"td", attrib=dict(align=u"left", colspan=u"6")
            )
            tcol.text = (
                f"Host IP: {data.get(u'host', '')}, "
