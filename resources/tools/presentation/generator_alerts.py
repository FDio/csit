...
class Alerting:
    ...
    def _get_compressed_failed_tests(self, alert, test_set, sort=True):
            ...
            with open(file_path, u'r') as f_txt:
                for idx, line in enumerate(f_txt):
                    ...
                    for e_msg in self.error_msgs:
                        if SequenceMatcher(None, str(e_msg), str(error_msg)).ratio() > 0.5:
                            error_msg = e_msg
                            break
                    ...
                    if failed_tests.get(name, {}).get(error_msg_index) is None:
                        failed_tests[name][error_msg_index] = dict(nics=list(), framesizes=list(), cores=list())

                    if test[0] not in failed_tests[name][error_msg_index][u"nics"]:
                        failed_tests[name][error_msg_index][u"nics"].append(test[0])
                    if test[1] not in failed_tests[name][error_msg_index][u"framesizes"]:
                        failed_tests[name][error_msg_index][u"framesizes"].append(test[1])
                    check_core = test[2] + f"[{str(error_msg_index)}]"
                    if check_core not in failed_tests[name][error_msg_index][u"cores"]:
                        failed_tests[name][error_msg_index][u"cores"].append(test[2]
                                                                             + "["
                                                                             + str(error_msg_index)
                                                                             + "]")
        ...
        if sort:
            ...
            return build, version, passed, failed, sorted_failed_tests, self.error_msgs

        return build, version, passed, failed, failed_tests, self.error_msgs
    ...
    def _generate_email_body(self, alert):
        ...
        for idx, test_set in enumerate(alert.get(u"include", list())):
            ...
            build, version, passed, failed, failed_tests, self.error_msgs = \
            ...
            for test, message in failed_tests.items():
                for e_message, params in message.items():
                    failed_tests[test][e_message][u"nics"] = u" ".join(sorted(params[u"nics"]))
                    ...
                    if len(failed_tests[test][e_message][u"nics"]) > max_len.nics:
                        max_len.nics = len(failed_tests[test][e_message][u"nics"])
                    if len(failed_tests[test][e_message][u"framesizes"]) > max_len.frmsizes:
                        max_len.frmsizes = len(failed_tests[test][e_message][u"framesizes"])
                    if len(failed_tests[test][e_message][u"cores"]) > max_len.cores:
                        max_len.cores = len(failed_tests[test][e_message][u"cores"])
