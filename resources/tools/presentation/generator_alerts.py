...
class Alerting():
    ...
    def _list_gressions(self, alert, idx, header, type):
        ...
        :param type: 'regression' or 'progression'.
        :type alert: dict
        :type idx: int
        :type header: str
        :type type: str
        ...
        if type not in (u"regressions", u"progressions"):
            return

        in_file = (
            f"{self.configs[alert[u'way']][u'output-dir']}/"
            f"cpta-{type}-{alert[u'urls'][idx].split(u'/')[-1]}.txt"
        )
        out_file = (
            f"{self.configs[alert[u'way']][u'output-dir']}/"
            f"trending-{type}.txt"
    ...
    def _generate_email_body(self, alert):
        ...
        out_dir = f"{self.configs[alert[u'way']][u'output-dir']}"
        text = u""
        ...
        file_name = f"{out_dir}/{self.configs[alert[u'way']][u'output-file']}"
