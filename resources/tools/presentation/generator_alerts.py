...
class Alerting(object):
    ...
    def __init__(self, spec):
        ...
        :param spec: The specification of alerts.
        :type spec: dict
        """
        ...
        if not self.configs:
            raise AlertingError("No alert configutation is specified.")
        ...
        for alert, alert_data in self.alerts.iteritems():
            ...
            if not alert_data.get("failed-tests", None) in self._ALERTS:
    ...
    def _send_email(server, addr_from, addr_to, subject, text=None, html=None):
        ...
        try:
            ...
            logging.info("SMTP Server: {0}".format(server))
            logging.info("From: {0}".format(addr_from))
            logging.info("To: {0}".format(", ".join(addr_to)))
            logging.info("Message: {0}".format(msg.as_string()))
    ...
    @staticmethod
    def _create_alert_message(alert):
