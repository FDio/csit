...
class Alerting():
    ...
    def _generate_email_body(self, alert):
        ...
        config = self.configs[alert[u"way"]]
        ...
        for idx, test_set in enumerate(alert.get(u"include", [])):
            ...
            max_len_name, max_len_nics, max_len_frmsizes, max_len_cores = \
                (0, 0, 0, 0)
            for name, params in failed_tests.items():
                ...
                if len(failed_tests[name][u"nics"]) > max_len_nics:
                    max_len_nics = len(failed_tests[name][u"nics"])
                if len(failed_tests[name][u"framesizes"]) > max_len_frmsizes:
                    max_len_frmsizes = len(failed_tests[name][u"framesizes"])
                if len(failed_tests[name][u"cores"]) > max_len_cores:
                    max_len_cores = len(failed_tests[name][u"cores"])

            for name, params in failed_tests.items():
                text += (
                    f"{name + u' ' * (max_len_name - len(name))}  "
                    f"{params[u'nics']}"
                    f"{u' ' * (max_len_nics - len(params[u'nics']))}  "
                    f"{params[u'framesizes']}"
                    f"{u' ' * (max_len_frmsizes-len(params[u'framesizes']))}  "
                    f"{params[u'cores']}"
                    f"{u' ' * (max_len_cores - len(params[u'cores']))}\n"
                )

            # Add list of regressions:
            file_name = (
                f"{config[u'output-dir']}/"
                f"cpta-regressions-{alert[u'urls'][idx].split(u'/')[-1]}.txt"
            )

            try:
                with open(file_name, u'r') as txt_file:
                    file_content = txt_file.read()
                    reg_file_name = \
                        f"{config[u'output-dir']}/trending-regressions.txt"
                    with open(reg_file_name, u'a+') as reg_file:
                        reg_file.write(regression_hdr)
                        if file_content:
                            reg_file.write(file_content)
                        else:
                            reg_file.write(u"No regressions")
            ...
            file_name = (
                f"{config[u'output-dir']}/"
                f"cpta-progressions-{alert[u'urls'][idx].split(u'/')[-1]}.txt"
            )
            try:
                with open(file_name, u'r') as txt_file:
                    file_content = txt_file.read()
                    pro_file_name = \
                        f"{config[u'output-dir']}/trending-progressions.txt"
                    with open(pro_file_name, u'a+') as pro_file:
                        pro_file.write(regression_hdr)
                        if file_content:
                            pro_file.write(file_content)
                        else:
                            pro_file.write(u"No progressions")
            except IOError as err:
                logging.warning(repr(err))

        text += f"\nFor detailed information visit: {alert[u'url-details']}\n"
        file_name = f"{config[u'output-dir']}/{config[u'output-file']}"
