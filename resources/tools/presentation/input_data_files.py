...
def download_and_unzip_data_file(spec, job, build, pid):
    ...
    for source in spec.environment.get(u"data-sources", tuple()):
        if not source.get(u"enabled", False):
            continue
        if source.get(u"type", None) == u"json":
            success, downloaded_name = _download_json(
                source,
                job,
                build,
        ...
        elif source.get(u"type", None) == u"xml":
            success, downloaded_name = _download_xml(
                source,
                job,
                build,
                spec.environment[u"paths"][u"DIR[WORKING,DATA]"],
        ...
        elif source.get(u"type", None) == u"xml-docs":
            success, downloaded_name = _download_xml_docs(
                source,
                job,
                build,
                spec.environment[u"paths"][u"DIR[WORKING,DATA]"],
