...
def plot_performance_box(plot, input_data):
    ...
    # Transform the data
    data = input_data.filter_tests_data(plot)

    print(plot)
    print()
    print(data["csit-vpp-perf-1707-all"]["17"])
    ...
    for i, col in enumerate(df.columns):
        ...
        traces.append(plgo.Box(y=df[col],
                               x=[str(i + 1) + '.'] * len(df[col]),
                               name=name,
                               **plot["traces"]))
