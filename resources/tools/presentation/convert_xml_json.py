...
class JSONData:
    ...
    def dump(self, file_out, indent=u"    "):
...
def _export_test_from_xml_to_json(tid, in_data, out, template, metadata):
    ...
    execution = {
        ...
        # u"msg": in_data.get(u"msg", u"")  will be generated from data
    }
    ...
    if in_sh_run:
        for key, val in in_sh_run.items():
            ...
            log_item[u"source"][u"host"] = val.get(u"host", u"")
            log_item[u"source"][u"socket"] = val.get(u"socket", u"")
            log_item[u"msg-type"] = u"operational"
            log_item[u"msg"] = u"show-runtime"
            log_item[u"data"] = val.get(u"runtime", list())
            data.add_element(log_item, p_log)
        ...
        for build_nr, build in builds.items():
            ...
            for test_id, test_data in build[u"tests"].items():
                ...
                if structure == "tree":
                    ...
                    file_name = \
                        f"{join(build_dir, job, build_nr, *dirs, test_id)}" \
                        f".json"
                else:
                    file_name = join(
                        build_dir,
                ...
                _export_test_from_xml_to_json(
                    test_id, test_data, file_name, template, metadata
                )

    # tar.gz the json files:
    if structure == "tree":
        for job, builds in data.data.items():
            for build_nr, build in builds.items():
                path = join(build_dir, job, build_nr)
                files = get_files(path, u"json")
                tar_file = \
                    f"{join(build_dir, job, build_nr, u'output')}.tar.gz"
                with tarfile.open(tar_file, u"w:gz") as file_handler:
                    for file in files:
                        file_handler.add(file, arcname=file.replace(path, u""))
                rmtree(join(path, u"tests"))
    else:
        files = get_files(build_dir, u"json")
        with tarfile.open(join(build_dir, u"output.tar.gz"), u"w:gz") \
                as file_handler:
            for file in files:
                file_handler.add(file, arcname=file.replace(build_dir, u""))
                os.remove(file)
