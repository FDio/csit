...
def _export_test_from_xml_to_json(tid, in_data, out, template, metadata):
    ...
    if in_sh_run:
        for val in in_sh_run.values():
            val.pop(u"threads", None)
        data.add_element({u"node": in_sh_run}, p_results)
    ...
    if t_type == u"DEVICETEST":
        ...
        results = {
            u"result": in_data.get(u"result", dict())
        }
    elif t_type == "TCP":  # Not used ???
        results = {
            u"result": in_data.get(u"result", float(u"NaN"))
        }
    elif t_type == "RECONF":
        results = {
            u"result": {
    ...
    for job, builds in data.data.items():
        ...
        for build_nr, build in builds.items():
            ...
            for test_id, test_data in build[u"tests"].items():
                ...
                if structure == "tree":
                    file_name = \
                        f"{join(build_dir, job, build_nr, test_id)}.json"
                else:
                    file_name = join(
                        build_dir,
    ...
    if structure == "tree":
        for job, builds in data.data.items():
            for build_nr, build in builds.items():
                ...
                zip_file = \
                    f"{join(build_dir, job, build_nr, u'output')}.json.zip"
                with zipfile.ZipFile(zip_file, u"w") as file_handler:
                    for file in files:
                        file_handler.write(file)
                        os.remove(file)
    else:
        files = get_files(build_dir, u"json")
        with zipfile.ZipFile(join(build_dir, u"output.json.zip"), u"w") \
                as file_handler:
            for file in files:
                file_handler.write(file)
                os.remove(file)
