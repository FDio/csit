...
def _generate_trending_traces(in_data, job_name, build_info,
    ...
    data_y_pps = [float(item[u"receive-rate"]) for item in data_y]
    data_y_mpps = [float(item[u"receive-rate"]) / 1e6 for item in data_y]
    data_y_stdev = [float(item[u"receive-stdev"]) / 1e6 for item in data_y]
