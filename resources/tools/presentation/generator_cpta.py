        ...
        for job_name, job_data in anomaly_classifications.items():
            ...
            for test_name, classification in job_data.items():
                if classification != u"normal":
                    ...
                    # ltc = line.split("|")[4]
                    if len(str(ltc)) > max_len.re_pro:
                        max_len.re_pro = len(str(ltc))
                    if classification == u'regression':
                        test_reg_list.append(tst)
                        nic_reg_list.append(nic)
                        frmsize_reg_list.append(frmsize)
                        trend_reg_list.append(trend)
                        number_reg_list.append(number)
                        ltc_reg_list.append(ltc)
                    elif classification == u'progression':
                        test_prog_list.append(tst)
                        nic_prog_list.append(nic)
                        frmsize_prog_list.append(frmsize)
                        trend_prog_list.append(trend)
                        number_prog_list.append(number)
                        ltc_prog_list.append(ltc)

                    if classification in (u"regression", u"outlier"):
                        result = u"FAIL"

            text = u""
            for index in range(len(test_reg_list)):
                text += (f"{test_reg_list[index]}"
                         f"{u' ' * (max_len.tst - len(test_reg_list[index]))}  "
                         f"{nic_reg_list[index]}"
                         f"{u' ' * (max_len.nic - len(nic_reg_list[index]))}  "
                         f"{frmsize_reg_list[index]}"
                         f"{u' ' * (max_len.frmsize - len(frmsize_reg_list[index]))}  "
                         f"[ {trend_reg_list[index]}M |"
                         f"{u' ' * (max_len.trend - len(str(trend_reg_list[index])))}  "
                         f"#{number_reg_list[index]} |"
                         f"{u' ' * (max_len.run - len(str(number_reg_list[index])))}  "
                         f"{ltc_reg_list[index]}% ]"
                         f"{u' ' * (max_len.re_pro - len(str(ltc_reg_list[index])))}  "
                         f"\n")
            ...
            text = u""
            for index in range(len(test_prog_list)):
                text += (f"{test_prog_list[index]}"
                         f"{u' ' * (max_len.tst - len(test_prog_list[index]))}  "
                         f"{nic_prog_list[index]}"
                         f"{u' ' * (max_len.nic - len(nic_prog_list[index]))}  "
                         f"{frmsize_prog_list[index]}"
                         f"{u' ' * (max_len.frmsize - len(frmsize_prog_list[index]))}  "
                         f"[ {trend_prog_list[index]}M |"
                         f"{u' ' * (max_len.trend - len(str(trend_prog_list[index])))}  "
                         f"#{number_prog_list[index]} |"
                         f"{u' ' * (max_len.run - len(str(number_prog_list[index])))}  "
                         f"{ltc_prog_list[index]}% ]"
                         f"{u' ' * (max_len.re_pro - len(str(ltc_prog_list[index])))}  "
                         f"\n")
