...
from BitCountingClassifier import BitCountingClassifier as classifier


N = 10
n = 4
p = provider(1000, 10, 2.0, 0.0)
c = classifier()

def process(prov, clas, seed, samples, pseudosamples):
    group_list = p.get_runs(N, n, seed, samples, pseudosamples, False)
...
for _ in range(4):
    ...
    if ob2 > ib2:
        print "input is encoded by {bits} bits less, partition is bad.".format(
            bits=(ob2 - ib2))
        detected19 += 1
    else:
        print "input is encoded by {bits} bits more, partition is good.".format(
            bits=(ib2 - ob2))
        undetected19 += 1
