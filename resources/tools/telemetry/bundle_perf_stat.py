    ...
    def attach(self, duration=1):
        ...
        try:
            ...
            text = subprocess.getoutput(
                ...
                '{{cpu/event={hex(self.events[u"eventcode"])},\
                umask={hex(self.events[u"umask"])}/u}}'\
                -a --per-thread\
                sleep {duration}"""
            )
        ...
        if text == u"":
            getLogger("console_stdout").info(self.events[u"name"])
        elif text.count(u";") < 6:
            getLogger("console_stdout").info(
                f"Could not get counters for event "\
                f"{self.events[u'name']}. "\
            ...
            for line in text.splitlines():
                item = dict()
                labels = dict()
                item[u"name"] = self.events[u"name"]
