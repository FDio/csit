...
M_PMB_PF = (
    r"(?P<thread_name>\S+)\s+\("
    r"(?P<thread_id>\S+)\)\s+\S+\s+"
    r"(?P<minor_page_faults>\S+)\s+"
    r"(?P<major_page_faults>\S+)"
)
M_PMB_THREAD = (
    r"(?P<thread_name>\S+)\s+\("
    r"(?P<thread_id>\S+)\)\s*"
...
M_PMB_IC_NODE = (
    ...
    r"(?P<calls>\S+)\s+"
    r"(?P<packets>\S+)\s+"
    r"(?P<packets_per_call>\S+)\s+"
    r"(?P<clocks_per_packets>\S+)\s+"
    r"(?P<instructions_per_packets>\S+)\s+"
    r"(?P<ipc>\S+)"
)
M_PMB_CM_HEADER = (
...
class BundleVpp:
        ...
        try:
            call = run(
                ["ss", "--listening", "--processes",
                 "--socket=unix_stream", "--no-header"],
                stdout=PIPE, stderr=DEVNULL, text=True, check=True
            )
            for line in call.stdout.splitlines():
                if "pid={pid}".format(pid=pid) and "api.sock" in line:
                    server_address = line.split()[3]
                    break
            else:
                getLogger(__name__).error(u"Failed to detect socket from PID!")
                sys.exit(1)
        except FileNotFoundError:
            getLogger(__name__).error(u"Failed to execute socket search!")
            sys.exit(1)
    ...
    def process_data(self):
        ...
        for command in zip(self.api_command_list, self.api_replies_list):
            ...
            try:
                getLogger(__name__).info(command[1].reply)
                self_fn = getattr(self, self_fn.replace(" ", "_"))
    ...
    def show_interface(self, reply):
        ...
        for line in reply.splitlines():
            ...
            if fullmatch(M_INT_BEGIN, line):
                interface = fullmatch(M_INT_BEGIN, line).groupdict()
                snke = interface[u"counter"].replace(" ", "_").replace("-", "_")
                item[u"name"] = snke
                item[u"value"] = interface[u"count"]
            if fullmatch(M_INT_CONT, line):
                interface_cont = fullmatch(M_INT_CONT, line).groupdict()
                snke = interface[u"counter"].replace(" ", "_").replace("-", "_")
                item[u"name"] = snke
                item[u"value"] = interface_cont[u"count"]
            if fullmatch(M_INT_BEGIN, line) or fullmatch(M_INT_CONT, line):
                labels[u"name"] = interface[u"name"]
                labels[u"index"] = interface[u"index"]
                item[u"labels"] = labels
                self.metric_open[item[u"name"]].labels(**labels).inc(
                    float(item[u"value"])
                )
    ...
    def show_runtime(self, reply):
        ...
        for line in reply.splitlines():
            ...
            if fullmatch(M_RUN_NODES, line):
                ...
                for metric in self.metric_open:
                    item = dict()
                    labels = dict()
                    item[u"name"] = metric
