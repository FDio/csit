...
# FIXME: Add "|| die" everywhere.

for gen in $(find ./tests -type f -executable -name '*.py'); do
    directory=$(dirname "$gen")
    filename=$(basename "$gen")
    # TODO: Is there something more elegant than subshell?
    #   The goal is to ensure working directory is restored on failure.
    ( set -exu && cd "$directory" && PYTHONPATH="$PYTHONPATH" ./"$filename" )
done
