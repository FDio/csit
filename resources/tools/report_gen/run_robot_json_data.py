...

{
    ...
    "data": {
        "ID": {
            ...
            "tolerance": "tolerance"  # Only for PDR
        },
        "ID" {
            # next test
        }
...
.. note:: ID is the full path to the test.
    ...
    def start_test(self, test):
        ...
        if "NDRPDRDISC" in tags:
            if test.status == "PASS":
                ...
                test_result["throughput"]["value"] = int(
                    rate_value.split('.')[0])
                test_result["throughput"]["unit"] = rate_unit
                test_result["latency"] = self._get_latency(test.message,
                                                           test_type)
                if test_type == "PDR":
                    test_result["tolerance"] = str(re.search(
