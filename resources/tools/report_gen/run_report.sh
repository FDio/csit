...
if [[ ${LATEX} -eq 1 ]] ;
then
    sudo apt-get -y librsvg2-bin install texlive-latex-recommended \
        texlive-fonts-recommended texlive-latex-extra latexmk wkhtmltopdf
...
then
    ...
    rsvg-convert -f pdf -o fdio.pdf fdio.svg
    ...
    cd ${DIR[BUILD,LATEX]} && \
        pdflatex --interaction nonstopmode csit.tex && \
        pdflatex --interaction nonstopmode csit.tex
