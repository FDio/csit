...
def parse_data_pps(args):
    ...
    for i, file in enumerate(sorted(select_files_in_folder(args.input))):
        ...
        if sel:
            ...
            try:
                vdevice = xml_tree.getroot().attrib['vdevice'].split('~')
                xdata.append(vdevice[0])
            except IndexError:
                xdata.append(str(i)+":"+xml_tree.getroot().attrib['vdevice'])
...
def main():
    ...
    if xdata and ydata:
        ...
        for suite in ydata:
            if args.latency:
                y = []
                [y.extend([i, i]) for i in ydata[suite][1::2][1].split('/')]
                traces.append(plgo.Box(
                    x=['Flow 1', 'Flow 1', 'Flow 1', 'Flow 1', 'Flow 1', 'Flow 1',
                       'Flow 2', 'Flow 2', 'Flow 2', 'Flow 2', 'Flow 2', 'Flow 2'],
                    y=y,
                    name=suite.lower(),
                    jitter=0,
                    boxmean=False,
        ...
        layout = plgo.Layout(
            ...
            yaxis=dict(
                ...
                showticklabels=True,
                tickcolor='rgb(238, 238, 238)',
                title='uSec' if args.latency else 'Packets Per Second',
                zeroline=False,
            ),
            boxmode='group',
            boxgroupgap=0.5,
            ...
            showlegend=True,
            legend=dict(
                orientation='v' if args.latency else 'h',
                y=-0.5,
            ),
            width=700,
            height=700,
