    ...
    def skip_by_filename(self, filename):
        ...
        if self.test_code:
            if u"-trex-" in self.test_code:
                ...
                substrings = (u"-l2xcbase-testpmd-", u"-ip4base-l3fwd-")
                if not any(substr in filename for substr in substrings):
                    return True
            elif u"-device-" in self.test_code:
                if nodeness != 1:
                    return True
            # Else assumed to be VPP perf test, covered by nodeness already.
            # Not skipping due dpdk/trex/perf/device.
        return False
