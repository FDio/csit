...
class EditState:
    ...
    def __init__(
        ...
        print(f"{self} {self.filename} state created, nic_driver_split {self.nic_driver_split}")
    ...
    def without_nic_driver_split(self):
        ...
        """
        print(f"{self.filename} attempt to remove nic driver from {self}")
        if not self.nic_model_split:
            print(f"{self.nic_model_split!r}")
    ...
    def check_suite_tag_and_add_suite_id(self, suite_ids):
        ...
        """
        _, suite_id, suite_tag = self.get_iface_and_suite_ids(self.filename)
