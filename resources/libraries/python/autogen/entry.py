...
def generate_recursively(verbose=False):
    ...
    for visit_dir, _, files in os.walk(cwd):
        for filename in files:
            if not filename.endswith(u".robot"):
                continue
            if verbose:
            ...
            try:
                os.chdir(visit_dir)
                classify_and_handle(filename, prolog, suite_ids)
            finally:
                os.chdir(cwd)
    if verbose:
...
def classify_and_handle(filename, prolog, suite_ids):
    ...
    print(f"{filename} subtype {subtype}")
