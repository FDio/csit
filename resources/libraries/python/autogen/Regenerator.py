...
class Regenerator(object):
    ...
    def regenerate_glob(self, pattern, protocol="ip4", tc_kwargs_list=None):
        ...
        primary_type = "ndrpdr"
        primary_keyword = Constants.PERF_TYPE_TO_KEYWORD[primary_type]
        suite_types = Constants.PERF_TYPE_TO_KEYWORD.keys()
        ...
        in_filename = "Filename not set yet."

        def replace_defensively(whole, to_replace, replace_with, how_many, msg):
        ...
        for in_filename in glob(pattern):
            ...
            for suite_type in suite_types:
                tmp_filename = replace_defensively(
                    ...
                    "File name should contain suite type once.")
                suite_keyword = Constants.PERF_TYPE_TO_KEYWORD[suite_type]
                tmp_prolog = replace_defensively(
                    in_prolog, primary_type.upper(), suite_type.upper(), 1,
                    "Suite type should appear once in uppercase (as tag).")
                tmp_prolog = replace_defensively(
                    tmp_prolog, primary_keyword, suite_keyword, 1,
                    "Main search keyword should appear once in suite.")
                tmp_prolog = replace_defensively(
                    tmp_prolog, Constants.PERF_TYPE_TO_SUITE_DOC_VER[
                        primary_type], Constants.PERF_TYPE_TO_SUITE_DOC_VER[
                        suite_type], 1, "Exact suite type doc not found.")
                tmp_prolog = replace_defensively(
                    tmp_prolog, Constants.PERF_TYPE_TO_TEMPLATE_DOC_VER[
                        primary_type], Constants.PERF_TYPE_TO_TEMPLATE_DOC_VER[
                        suite_type], 1, "Exact template type doc not found.")
                ...
                for nic_name in nic_names:
                    ...
                    out_filename = replace_defensively(
                        ...
                        "File name should contain NIC code once.")
