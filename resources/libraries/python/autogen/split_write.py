...
def write_files(state, suite_ids):
    ...
    if state.test_type_split:
        split_on_test_type(state, suite_ids)
...
def split_on_test_type(state, suite_ids):
...
def split_on_dpdk_nic_driver(state, suite_ids):
    ...
    :type suite_ids: Set[str]
    """
    print(f"{state} splitting nic driver for dpdk")
    for nic_driver in Constants.DPDK_NIC_NAME_TO_DRIVER[state.nic_model]:
        print(f"state inside dpdk split {state} {state.nic_driver_split}")
...
def split_on_vpp_nic_driver(state, suite_ids):
    ...
    :type suite_ids: Set[str]
    """
    print(f"{state} splitting nic driver for vpp")
    for nic_driver in Constants.NIC_NAME_TO_DRIVER[state.nic_model]:
        # Each iteration needs to be based on state, not tmp_state.
        print(f"state inside vpp split {state} {state.nic_driver_split}")
        tmp_state = state.without_nic_driver_split()
        tmp_state = tmp_state.with_prolog_replaced(
