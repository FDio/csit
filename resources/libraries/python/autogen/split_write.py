...
def write_files(state, suite_ids):
    ...
    :param suite_ids: List of suite IDs encountered so far.
    :type state: EditState
    :type suite_ids: Set[str]
    """
    if state.suite_type_split:
        split_on_suite_type(state, suite_ids)
    elif state.nic_model_split:
        split_on_nic_model(state, suite_ids)
    elif state.nic_driver_split:
        if u"DPDK" in state.prolog:
            split_on_dpdk_nic_driver(state, suite_ids)
        else:
            split_on_vpp_nic_driver(state, suite_ids)
    else:
        state.check_suite_tag_and_add_suite_id(suite_ids)
...
def split_on_suite_type(state, suite_ids):
    """Edit suite type, call write_files without suite_type_split.
    ...
    for suite_type in Constants.PERF_TYPE_TO_KEYWORD:
        ...
        tmp_state = tmp_state.with_prolog_replaced(
            ...
            Constants.PERF_TYPE_TO_SUITE_DOC_VER[suite_type], 1,
            u"Exact suite type doc not found."
        )
        tmp_state = tmp_state.with_prolog_replaced(
            Constants.PERF_TYPE_TO_TEMPLATE_DOC_VER[u"ndrpdr"],
            Constants.PERF_TYPE_TO_TEMPLATE_DOC_VER[suite_type], 1,
            u"Exact template type doc not found.",
        )
        write_files(tmp_state, suite_ids)

def split_on_nic_model(state, suite_ids):
...
def split_on_dpdk_nic_driver(state, suite_ids):
    ...
    for nic_driver in Constants.DPDK_NIC_NAME_TO_DRIVER[state.nic_model]:
        apply_common_nic_driver_edits(nic_driver, tmp_state, suite_ids)

def split_on_vpp_nic_driver(state, suite_ids):
    """Edit nic driver, call write_files without nic_driver_split.
...
def apply_common_nic_driver_edits(nic_driver, state, suite_ids):
    ...
    :type suite_ids: Set[str]
    """
    _, old_suite_id, _ = state.get_iface_and_suite_ids()
