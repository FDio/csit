...
class AutoConfig(object):
            ...
            if 'devices' in self._nodes[key]:
                ...
                for j in range(len(devices)):
                    ...
                    for k in range(len(macs)):
                        if k > 0:
                            interfaces[p]['mac_address'] += ',{}'.format(macs['l2_addr'][k])
    ...
    Configure the PCI devices interactivly.
    '''
    ...
    def devices_interactive(self):
        ...
        for i in self._nodes.items():
            ...
            if len(devices_used) > 0:
                ...
                if a == 'y':
                    self.devices_update(node, devices_used)
                    return

            # Reset the devices
            devices_to_rebind = devices_used + vpp.get_other_devices()
            vpp.rebind_vpp_devices(node, devices_to_rebind)

            # Get the new device list
            vpp = VppPCIUtil(node)
            devices_used = vpp.get_kernel_devices()

            print("These devices are available:\n")
            vpp.show_vpp_devices(devices_used)

            devices, devices_removed = self._get_new_vpp_device_list(self, devices_used)
