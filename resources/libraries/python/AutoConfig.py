...
class AutoConfig(object):
    ...
    def apply_grub(self):
        ...
        self.apply_grub_cmdline()

        # reboot
        for i in self._nodes.items():
            node = i[1]

            if node['grub']['vpp_cmdline'] != '':
                a = self._ask_user_yn("The node {} been reconfigured are you ready to reboot ".
                                      format(node['host']))
                if a == 'y':
    ...
    @staticmethod
    def devices_update(node, vpp, devices_to_be_used):
    ...
    def patch_qemu(self):
        ...
        for i in self._nodes.items():
            ...
            qu = QemuUtils()
            qu.build_qemu(node, force_install=False, apply_patch=True)
