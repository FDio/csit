            ...
            if node['type'] == NodeType.DUT:
                try:
                    ...
                    if status_code == HTTPCodes.OK:
                        ...
                        raise HoneycombError('Honeycomb on node {0} is '
                                             'shutting down'.format(node['host']),
                                             enable_logging=False)
                    else:
                        raise HoneycombError('unexpected return code: {0}'.
                                             format(status_code))
            ...
            if node['type'] == NodeType.DUT:
                ...
                try:
                    payload = data.format(
                        ...
                        honeycomb_user=node['honeycomb']["honeycomb_user"],
                        honeycomb_passwd=node['honeycomb']["honeycomb_passwd"])
