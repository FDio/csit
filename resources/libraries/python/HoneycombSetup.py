...
from ssh import SSH
from HTTPRequests import HTTPRequests, HTTPRequestError
...
class HoneycombSetup(object):
        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                try:
                    ...
                    raise HoneycombError('Insufficient information about node'
                                         ' {} in topology'.format(node['host']))
    ...
    def start_honeycomb(node):
        ...
        (ret_code, stdout, stderr) = \
            ssh.exec_command_sudo('{}start'.format(C.REMOTE_HC_DIR))
        if 0 != int(ret_code):
    ...
    def stop_honeycomb_on_all_duts(nodes):
        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                ...
                ssh.connect(node)
                (ret_code, stdout, stderr) = \
                    ssh.exec_command_sudo('{}stop'.format(C.REMOTE_HC_DIR))
                if 0 != int(ret_code):
                    logger.debug('stdout: {0}'.format(stdout))
                    logger.debug('stderr: {0}'.format(stderr))
                    errors.append(node['host'])
                    continue
                logger.info("Honeycomb on node {} was successfully stopped.".
                            format(node['host']))
        if errors:
    ...
    def check_honeycomb_startup_state(nodes):
        ...
        :param nodes: nodes in topology
        :type nodes: dict
        :return: True if all GETs returned code 200
        :rtype bool
        """

        with open("{}/vpp_version.hc".format(C.RESOURCES_TPL_HC)) as f:
            data = f.readline()

        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                status_code, resp = HTTPRequests.get(
                        node,
                        data,
                        timeout=10,
                ...
                if status_code == 200:
                    pass
                elif status_code in (401, 403, 404, 503):
    ...
    @staticmethod
    def add_vpp_to_topology(nodes, headers):
        """Add vpp node to topology in Honeycomb
        ...
        :rtype: tuple
        :raises HoneycombError: if a node was not added to topology
        ...
        with open("{}/config_topology_node.hc".
                          format(C.RESOURCES_TPL_HC)) as f:
            path = f.readline()

        xml_data = xml.parse("{}/add_vpp_to_topology.xml".
                ...
                try:
                    ...
                    if sc != 200:
                        raise HoneycombError(
                            "VPP {} was not added to topology. "
                            "Status code: {}".format(node["host"], sc))
