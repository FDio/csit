...
class HoneycombSetup(object):
    ...
    def check_honeycomb_startup_state(nodes):
        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                status_code, resp = HTTPRequest.get(node, data, timeout=10,
    ...
    def check_honeycomb_shutdown_state(nodes):
        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                try:
                    status_code, resp = HTTPRequest.get(node, '/index.html',
                                                        timeout=5,
                                                        enable_logging=False)
                    if status_code == HTTPCodes.OK:
    ...
    def add_vpp_to_honeycomb_network_topology(nodes, headers):
        ...
        url_file = os.path.join(C.RESOURCES_TPL_HC, "config_topology_node.url")
        with open(url_file) as f:
            path = f.readline()

        xml_file = os.path.join(C.RESOURCES_TPL_HC, "add_vpp_to_topology.xml")
        try:
