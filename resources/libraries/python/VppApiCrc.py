...
class VppApiCrcChecker(object):
    ...
    ordinary Python2 (bytes) str, so str() is used when input is
    ...
    def _register_collection(self, collection_name, name_to_crc_mapping):
        ...
        if collection_name in self._expected:
            raise RuntimeError("Collection {cl!r} already registered.".format(
                cl=collection_name))
        mapping = {str(k): str(v) for k, v in name_to_crc_mapping.items()}
        logger.debug("Converted collection {col!r}".format(col=mapping))
    ...
    def _register_all(self):
        ...
        logger.debug("filepath {fp!r}".format(fp=file_path))
        with open(file_path, "r") as file_in:
            collections_dict = yaml.load(file_in.read())
            logger.debug("yaml: {data!r}".format(data=collections_dict))
        for collection_name, name_to_crc_mapping in collections_dict.items():
            logger.debug("registering {col!r}".format(col=collection_name))
    ...
    def _process_crc(self, api_name, crc):
        ...
        :type crc: str
        """
        logger.debug("processing {api!r} {crc!r}".format(api=api_name, crc=crc))
    ...
    def _check_dir(self, directory):
        ...
        logger.info("Surviving collections: {col!r}".format(
