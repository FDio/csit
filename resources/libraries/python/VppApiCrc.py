...
class VppApiCrcChecker
            ...
            for filename in files:
                ...
                for msg_obj in msgs:
                    ...
                    if msg_name not in cls.VPP_API_CRC_VERIFIED:
                        ...
                        continue
                    if msg_crc in cls.VPP_API_CRC_VERIFIED[msg]:
                        continue
                    raise RuntimeError(
                        "Found message with unsupported crc: "
                        "{root} {filename} {msg} {crc}".format(
                            root, filename, msg, msg_crc))
    ...
    def check_message(cls, message):
        ...
        raise RuntimeError(
            ...
            "candidate CRCs found: {crcs}".format(message, candidates))
