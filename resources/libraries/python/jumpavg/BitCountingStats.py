...
class BitCountingStats(AvgStdevStats):
    ...
    def __init__(
        ...
        if self.size < 1:
            return
        if avg is None:
            raise ValueError("Avg is None: {s!r}".format(s=self))
        if max_value is None or max_value <= 0.0:
            raise ValueError("Invalid max value: {s!r}".format(s=self))
    ...
    def __repr__(self):
        ...
        """
        return (
            "BitCountingStats(size={size!r},avg={avg!r},stdev={stdev!r},"
