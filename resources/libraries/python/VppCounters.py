...
class VppCounters(object):
    ...
    def vpp_show_errors(node):
        ...
        # TODO: Use VPPStats when issue with different results is solved.
        # cmd = "vpp-stats"
        # args = dict(path='^/err')
        # with PapiExecutor(node) as papi_exec:
    ...
    @staticmethod
    def vpp_show_runtime(node, log_zeros=False, log_table=False):
        """Run "show runtime" CLI command.

        :param node: Node to run command on.
        :param log_zeros: Log also items with zero values.
        :param log_table: Log also the results as a pretty table.
        :type node: dict
        :type log_zeros: bool
        :type log_table: bool
        """
        ...
        for idx in range(len(runtime)):
            ...
            if (sum(calls_th) or sum(vectors_th) or
                ...
                runtime_non_zero.append(runtime[idx])
        ...
        if log_zeros:
            ...
            if log_table:
                logger.info("Runtime:\n{runtime}".format(
                    runtime=VppCounters._pprint_runtime(runtime_non_zero)))
