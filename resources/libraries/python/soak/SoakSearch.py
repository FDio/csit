...
#        zeros = 0
        ...
        while 1:
            ...
            measurement, average, stdev, b_avg, b_a = self.measure_and_compute(
                trial_duration, transmit_rate, trial_result_list, max_rate,
                b_avg, b_a)
            logging.info("Trial {count} computed avg {avg} stdev {stdev}".format(
...
#        print "returning log_weight", log_weight
    ...
    def measure_and_compute(
            ...
            trial_result_list, max_rate, b_avg, b_a):
        """FIXME."""
...
#            print "mrr", mrr, "a", a
        ...
        random.seed(0)
        control_q = Queue.Queue(1)
        result_q = Queue.Queue(1)
        worker = threading.Thread(
            target=Integrator.integrate_nd, args=(control_q, result_q))
        worker.setDaemon(True)
        worker.start()
        control_q.put((n, value_lweight_f, b_avg, b_a))
        measurement = self.measurer.measure(trial_duration, transmit_rate)
        control_q.put((None))
        # TODO: Make sure ctrl+c breaks correctly.
        avg, stdev, b_avg, b_a = result_q.get(True, 1.0)
        avg = math.exp(avg)
        stdev *= avg
        return measurement, avg, stdev, b_avg, b_a
