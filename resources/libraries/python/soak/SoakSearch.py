...
#                trial_result_list = trial_result_list[1:-2]
...
#        print "load", load, "mrr", mrr, "a", a, "x", x
        if x > 0:
            loga = math.log(load - mrr + log_plus(0, -x) / a)
...
#        print "log_weight", log_weight
    ...
    def compute(self, trial_result_list, max_rate, trial_duration):
        ...
        def value_lweight_f(*x):
            ...
            # 100.0 is there to satisfy some approximations in lfit.
            mrr = 100.0 + max_rate * math.tan(X[0] * Integrator.Integrator__HALF_PI)
            a = 1.0 / (1.0 + mrr / 100.0 * X[1])
#            print "mrr", mrr, "a", a
