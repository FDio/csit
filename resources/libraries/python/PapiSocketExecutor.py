...
class AsyncState:
    ...
    Data classes are not available in Python 3.6 currently used by CSIT,
    plain classes do not pass review (people do not like zero-method classes),
    ...
    """
    local_list: list
    """Copy of commands to execute."""
    err_msg: str
    """Text to use when raising an error."""
    vpp_instance: "vpp_papi.VPPApiClient"
    """Connected client instance for sending and receiving."""
    ret_list: list
    """Responses gathered so far."""
    receive_index: int
    """Index of the next response to be read."""
    send_index: int
    """Index of the next command to be send."""
    lll: int
    """Length of Local List."""
    one_stop: int
    """Phase one ends when send index reaches this value.
    Depends on max_inflight and length of local list."""
...
class PapiSocketExecutor:
    ...
    def _execute_async(self, local_list, err_msg=u"Undefined error message"):
        ...
        vpp_instance = self.get_connected_client()
        state = AsyncState(
            vpp_instance=vpp_instance, local_list=local_list, err_msg=err_msg,
            ret_list=list(), send_index=0, receive_index=0,
            lll=None, one_stop=None
        )
        # The phases do not need self, everything (vpp_instance) is in state.
        # Phase zero: Initialize.
        state = PapiSocketExecutor._phase_zero(state)
        # Phase one: Only sending commands.
        state = PapiSocketExecutor._phase_one(state)
        # Phase two: Receive one, send one; repeat.
        state = PapiSocketExecutor._phase_two(state)
        # Phase three: Only receiving.
        state = PapiSocketExecutor._phase_three(state)
        return state.ret_list

    @staticmethod
    def _phase_zero(state):
        """Compute dependent state fields and other setup actions.

        :param state: Container holding the current execution state.
        :type state: AsyncState
        :returns: The same state container with updated values.
        :rtype: AsyncState
        """
        ...
        state.lll = len(state.local_list)
        state.one_stop = min(max_inflight, state.lll)
        return state

    @staticmethod
    def _phase_one(state):
        """Keep sending commands without reading replies, until one_stop.
        ...
        api_object = state.vpp_instance.api
        while state.send_index < state.one_stop:
    ...
    def _phase_two(state):
        ...
        vpp_instance = state.vpp_instance
        # Just to save some CPU work inside loop.
