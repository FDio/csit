...
class PapiSocketExecutor:
    ...
    class AsyncState:
        ...
        def __init__(
                ...
                ret_list=None, lll=None, one_stop=None, send_iterator=None,
    ...
    def _phase_zero(self, state):
        ...
        state.one_stop = min(max_inflight + 2, state.lll)
    ...
    def _phase_two(self, state):
        ...
        while state.send_index < state.lll:
            ...
            if state.fast_receive:
                ...
                # Pylint complains. Rightfully, touching private parts is rude.
                # But we need to do that in order to be able to test current
                # (and older) VPP builds, without wasting tens of minutes
                # without fast receive. Improvement for VPP, is planned,
                # but we still need to support few releases without it.
                transport._read()
            else:
