...
#            logging.getLogger("vpp_papi.vpp_serializer").setLevel(logging.INFO)
    ...
    def __enter__(self):
        ...
        try:
            vpp_instance.connect_sync(u"csit_socket")  # , do_async=True)
    ...
    def _execute(self, err_msg=u"Undefined error message", exp_rv=0):
        ...
        for command in local_list:
            ...
            logger.trace(f"calling papi command {command!r}")
            ...
            logger.trace(f"raw responses {reply!r}")
            # *_dump commands return list of objects, convert, ordinary reply.
            if not isinstance(reply, list):
                reply = [reply]
            for item in reply:
                logger.trace(f"checking response item {item!r}")
                dict_item = dictize(item)
                logger.trace(f"dictized to {dict_item!r}")
