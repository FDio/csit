...
class LXCUtils(object):
    ...
    def __init__(self, container_name='vpp-slave'):
        # LXC container name
    ...
    def _lxc_create(self, distro='ubuntu', release='xenial', arch='amd64'):
        """Creates a system object where is stored the configuration information
        and where can be stored user information.
    ...
    def _lxc_start(self):
        ...
        if int(ret) != 0:
            raise RuntimeError('Failed to start LXC container {}.'
                               .format(self._container_name))
    ...
    def lxc_cpuset_cpus(self, container_cpu):
        ...
        logger.info("Previous cpuset.cpus of container {0} is:".format(
            self._container_name))
        self._lxc_cgroup(state_object='cpuset.cpus')
        self._lxc_cgroup(state_object='cpuset.cpus', value=container_cpu)

        logger.info("Current cpuset.cpus of container {0} is:".format(
            self._container_name))
        self._lxc_cgroup(state_object='cpuset.cpus')

    def lxc_mount_dir(self):
