...
class KubernetesUtils(object):
    ...
    def setup_kubernetes_on_node(node):
        ...
        :param node: DUT node.
        :type node: dict
        :raises RuntimeError: If failed to setup Kubernetes on node.
        """
        ssh = SSH()
        ssh.connect(node)
    ...
    def apply_kubernetes_resource_on_node(node, yaml_file, **kwargs):
        ...
        :type yaml_file: str
        :type kwargs: dict
        :raises RuntimeError: If failed to apply Kubernetes template.
        """
        ssh = SSH()
        ssh.connect(node)
    ...
    def create_kubernetes_cm_from_file_on_node(node, name, key, src_file):
        ...
        :type key: str
        :type src_file: str
        :raises RuntimeError: If failed to create Kubernetes ConfigMap.
        """
        ssh = SSH()
        ssh.connect(node)
    ...
    def delete_kubernetes_resource_on_node(node, rtype='po,cm', name=None):
        ...
        :type rtype: str
        :type name: str
        :raises RuntimeError: If failed to delete Kubernetes resource.
        """
        ssh = SSH()
        ssh.connect(node)
    ...
    def describe_kubernetes_resource_on_node(node, rtype='po,cm'):
        ...
        :type node: dict
        :type rtype: str
        :raises RuntimeError: If failed to describe Kubernetes resource.
        """
        ssh = SSH()
        ssh.connect(node)
    ...
    def reset_kubernetes_on_node(node):
        ...
        :param node: DUT node.
        :type node: dict
        :raises RuntimeError: If failed to reset Kubernetes.
        """
        ssh = SSH()
        ssh.connect(node)
