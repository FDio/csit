...
class KubernetesUtils(object):
    ...
    def apply_kubernetes_resource_on_node(node, yaml_file, **kwargs):
        ...
        with open('{tpl}/{yaml}'.format(tpl=Constants.RESOURCES_TPL_K8S,
                                        yaml=yaml_file) as src_file:
            stream=src_file.read()
        data = reduce(lambda a, kv: a.replace(*kv), kwargs.iteritems(), stream)
    ...
    def wait_for_kubernetes_pods_on_node(node, nspace):
        ...
        :param node: DUT node.
        :param nspace: Kubernetes namespace. (Default: all-namespaces)
        :type node: dict
        :type nspace: str
