...
class KubernetesUtils(object):
    ...
    def apply_kubernetes_resource_on_node(node, yaml_file, **kwargs):
        ...
        for data in yaml.load_all(stream):
            ...
            if int(ret_code) != 0:
                raise Exception('Failed to apply Kubernetes template {yaml} '
    ...
    def create_kubernetes_cm_from_file_on_node(node, name, key, src_file):
        ...
        (ret_code, _, _) = ssh.exec_command_sudo(cmd, timeout=120)
        if int(ret_code) != 0:
            raise Exception('Failed to create Kubernetes ConfigMap {name} '
                            'on {node}.'.format(name=name, node=node['host']))
    ...
    def delete_kubernetes_resource_on_node(node, name=None):
        ...
        cmd = 'kubectl delete -n csit pods,services,configmaps {name}'\
            .format(name=name)
        (ret_code, _, _) = ssh.exec_command_sudo(cmd, timeout=120)
        if int(ret_code) != 0:
    ...
    @staticmethod
    def describe_kubernetes_pods_on_node(node):
        """Describe Kubernetes PODs on node.
        ...
        cmd = 'kubectl describe -n csit pods'
        (ret_code, _, _) = ssh.exec_command_sudo(cmd, timeout=120)
        if int(ret_code) != 0:
            raise Exception('Failed to describe Kubernetes PODs on {node}.'
                            .format(node=node['host']))

    @staticmethod
    def describe_kubernetes_pods_on_all_duts(nodes):
        """Describe Kubernetes PODs on all DUTs.
        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                KubernetesUtils.describe_kubernetes_pods_on_node(node)
    ...
    def wait_for_kubernetes_pods_on_node(node):
        ...
        cmd = 'kubectl get pods -n csit --no-headers'
        for i in range(12):
    ...
    def create_kubernetes_vswitch_startup_config(self, **kwargs):
    ...
    def create_kubernetes_vnf_startup_config(self, **kwargs):
