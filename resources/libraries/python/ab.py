...
def run_ab(tg_node, ip_addr, tls_tcp, ciphers, files_num, rps_cps,
           protocol="TLS1.3"):
    """ Run ab test.
    ...
    python_dir = Constants.RESOURCES_LIB_PY
    port = u"80"
    qnum = u"1000000"

    cmd = f"{Constants.REMOTE_FW_DIR}/{python_dir}/abfork.py" \
          f" --clients 2000 --ip {ip_addr}" \
          f" --files {files} "
    if tls_tcp == u"tls":
        port = u"443"
        qnum = u"40000"
        cmd = f"{cmd} --cipher {ciphers}  --protocol {protocol}"

    cmd = f"{cmd}  --port {port} --requests {qnum}"
    ...
    exec_cmd(tg_node, "nproc", timeout=180, sudo=True)

    for i in range(111):
        cmd = f"taskset --cpu-list {i} ab -n 9091 -c 18 -k   " \
              f"http://{ip_addr}:{port}/{files}"
        ret2, _, stderr = exec_cmd(tg_node, cmd, timeout=180, sudo=True)
