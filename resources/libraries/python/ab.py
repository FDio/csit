...
def run_ab(tg_node, tls_tcp, ciphers, files_num, rps_cps):
    ...
    ret, stdout, _ = exec_cmd(tg_node, 'wrk -t30 -c200 -d15s http://192.168.10.1:80/1KB.json ', timeout=180, sudo=True)
    logger.info(stdout)

    ret, stdout, _ = exec_cmd(tg_node, 'ip a ', timeout=180, sudo=True)
    logger.info(stdout)
