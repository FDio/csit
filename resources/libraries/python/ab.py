...
def run_ab(tg_node, ip_addr, tg_addr, tls_tcp, ciphers, files_num, rps_cps,
           r_total, port, protocol="TLS1.3"):
    """ Run ab test.
    ...
    :param r_total: ab test requests total.
    :param port: http/https 80/443.
    ...
    cmd = f"{Constants.REMOTE_FW_DIR}/{python_dir}/abfork.py" \
          f" --clients 2000 --ip {ip_addr} --tip {tg_addr}" \
          f" --files {files} "

    if tls_tcp == u"tls":
        cmd = f"{cmd} --cipher {ciphers}  --protocol {protocol}"

    cmd = f"{cmd}  --port {port} --requests {r_total}"
