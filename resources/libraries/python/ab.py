...
def run_ab(tg_node, tls_tcp, ciphers, files_num, rps_cps):
    ...
    ret, stdout, _ = exec_cmd(tg_node, 'curl -v -k http://192.168.10.1:80/1KB.json', timeout=180, sudo=True)
    logger.info(stdout)

    ret, stdout, _ = exec_cmd(tg_node, 'ifconfig ', timeout=180, sudo=True)
    logger.info(stdout)
