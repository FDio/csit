...
class DropRateSearch(object):
    ...
    def __init__(self):
        ...
        #last step of the binary search, unit: RateType (self._rate_type)
        self._rate_binary_step = 0
    ...
    def binary_search(self, b_min, b_max, traffic_type):
        """Bichnary search of rate with loss below acceptance criteria
        ...
        rate_diff = abs(self._rate_binary_step - rate)

        # convergence criterium
        if float(rate_diff) < float(self._binary_convergence_threshold):
            logger.trace("convergence comparing: {} < {} ".format(rate_diff,
                         self._binary_convergence_threshold))
