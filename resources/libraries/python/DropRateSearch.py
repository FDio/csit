...
class OptimizedSearchAlgorithm(AbstractSearchAlgorithm):
    ...
    class ProgressState(object):
        ...
        def __init__(self, result, phases, duration, width_goal, allowed_drop_fraction, fail_rate, line_rate):
            ...
            self.result = result
            """NdrPdrResult holding current bounds."""
            self.phases = int(phases)
            """How many intermediate phases to execute before the current one."""
            self.duration = float(duration)
            """Trial duration to be uset at this phase."""
            self.width_goal = float(width_goal)
            """Goal relative width relevant for current phase."""
            self.allowed_drop_fraction = float(allowed_drop_fraction)
            """Drop fraction allowed for current PDR search."""
    ...
    def __init__(self, rate_provider, final_trial_duration=30.0, final_width=0.005, intermediate_phases=2, initial_trial_duration=1.0):
        ...
        """Final values come from trial of final phase, performed at this duration."""
        ...
        """Trial duration to be used at first intermediate phase."""
