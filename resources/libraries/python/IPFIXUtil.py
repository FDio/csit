...
with IPFIX packets.
 ...
 Option template sets are not supported.
  """
...
class IPFIXHandler(object):
    ...
    def update_template(self, packet):
        ...
        :type packet: scapy.layers.l2.Ether object
        """
        template_list = packet['IPFIX template'].Template
        template_id = packet['IPFIX template'].TemplateID

        IPFIXData.fields_desc = []
        for item in template_list[::2]:
            IPFIXData.fields_desc.append(
                self.template_elements[item])
        bind_layers(IPFIXHeader, IPFIXData, Set_ID=template_id)
        # if the packet doesn't end there, assume it contains another data set
