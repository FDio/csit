...
1. Job name. The test suites are generated for this job. Only one job can be
   specified.

The optional or mandatory input information, depending on the job type, is:
2. Test set (on-demand jobs (iterative, coverage, bisect, ...)). Specifies the
...
3. Test type (some of on-demand jobs). Only one test type from this list can be
   specified:
   - mrr,
   - ndrpdr,
   - soak.
...
4. Output directory (string). Directory to store generated files. If not given,
   the default directory is used.
5. Output file name (string). The name of output file(s):
...
7. Create the job specification as an MD file (boolean).
8. Logging level (string). One of "NOTSET", "DEBUG", "INFO", "WARNING", "ERROR",
   "CRITICAL".
...
- The testbed `2n-icx` is defined for this job but without a test set. The test
  ...
  set.
- The testbed `2n-spr` is defined for this job with a test set. This tes set can
...
(specified in the job specification) and (optionally) using parameter
from command line.
...
- started by a gerrit trigger, examples (currently used):
  ...
  - verify jobs:
    - csit-2n-zn2-perftest mrrANDnic_i*ANDdrv_avfAND1cAND64bANDeth-l2patch
  - bisect jobs:
    - bisecttest-2n-spr d35f7f098 nic_intel-e810cqANDdrv_vfio_pciAND1cAND0bANDeth-ip4tcphttp-ldpreload-nginx-1_21_5ANDtcp_cps
- job name e.g.:
  - report:
...
Hints:
clear; ./suite_generator.py --job csit-dpdk-perf-report-iterative-2402-2n-spr --create-json --test-type mrr --test-set 2n-spr-dpdk-iterative
clear; ./suite_generator.py --job csit-dpdk-perf-report-iterative-2402-2n-spr --create-json
clear; ./suite_generator.py --job csit-vpp-perf-mrr-daily-master-x-2nu-c1-2xl --create-json
"""
