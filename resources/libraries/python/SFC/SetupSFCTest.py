...
def install_sfc_test(node):
    ...
    if_pci1 = Topology.get_interface_pci_addr(node, if_name_list[0])
    if_pci2 = Topology.get_interface_pci_addr(node, if_name_list[1])
    ...
    (ret_code, _, stderr) = ssh.exec_command(
        ...
        .format(con.REMOTE_FW_DIR, if_pci1, if_pci2), timeout=600)

    if ret_code != 0:
        logger.error('Install the NSH SFC error: {0}'.format(stderr))
        raise Exception('Install the NSH SFC failed')
    else:
