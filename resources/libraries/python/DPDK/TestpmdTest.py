...
class TestpmdTest:
    ...
    def start_testpmd_on_all_duts(
        ...
        for iteration in range(5):
            ...
            all_ready = True
            for dut in duts:
                if not TestpmdTest.is_testpmd_ready(nodes[dut], iteration):
                    all_ready = False
                    break
            for dut in duts:
    ...
    def start_testpmd(
        ...
        if node[u"type"] == NodeType.DUT:
            ...
            testpmd_args = DpdkUtil.get_testpmd_args(
                ...
                # including up to 9s wait at startup for links going up,
                # thus simplifying readiness check logic.
                pmd_disable_link_check=False,
                # Testing readiness negative case:
    ...
    def is_testpmd_ready(node, iteration):
        ...
        :param node: DUT node.
        :param iteration: Wait time is (in minutes) two to the power of this.
        :type node: dict
        :type iteration: int
