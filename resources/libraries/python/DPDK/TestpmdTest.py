...
class TestpmdTest:
                ...
                for check in range(120):
                    for dut in duts:
                        if not TestpmdTest.is_testpmd_ready(nodes[dut]):
                            break
                    else:  # All duts are ready for traffic.
                        return
                    time.sleep(1)
                    # Continue with next check iteration.
    ...
    @staticmethod
    def start_testpmd(
        nodes, dut, topology_info, nb_cores, jumbo_frames,
    ):
        ...
        testpmd_args = DpdkUtil.get_testpmd_args(
            ...
            eal_driver=False,
            eal_pci_whitelist0=topology_info[f"{dut.lower()}_if1_pci"][0],
            eal_pci_whitelist1=topology_info[f"{dut.lower()}_if2_pci"][0],
            eal_in_memory=True,
            pmd_num_mbufs=32768,
            pmd_fwd_mode=u"io",
            pmd_nb_ports=u"2",
    ...
    def is_testpmd_ready(node):
        ...
        rc, _, _ = exec_cmd(node, command, message=message)
        return rc == 0
