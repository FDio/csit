...
class TestpmdTest:
    ...
    def start_testpmd_on_all_duts(
        ...
        for node in nodes:
            if u"DUT" in node:
                testpmd_started = False
                for i in range(3):
                    testpmd_started = TestpmdTest.check_testpmd(nodes[node])
                    if testpmd_started:
                        break
                    else:
                        TestpmdTest.start_testpmd(
                ...
                message = f"Failed to start testpmd state at node " \
                          f"{node['host']}"
    ...
    def check_testpmd(node):
        ...
        if node[u"type"] == NodeType.DUT:
            ...
            rc, _, _ = exec_cmd(node, command, timeout=1800)
