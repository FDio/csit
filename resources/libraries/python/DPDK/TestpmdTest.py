    ...
    def start_testpmd_on_all_duts(
        ...
        for node in nodes:
            if u"DUT" in node:
                ...
                pci1 = topology_info[f"{node}_pf1_pci"][0]
                pci2 = topology_info[f"{node}_pf2_pci"][0]
                InterfaceUtil.set_interface_state_pci(nodes[node], pci1,
                                                      state="down")
                InterfaceUtil.set_interface_state_pci(nodes[node], pci2,
                                                      state="down")
                DPDKTools.initialize_dpdk_framework(nodes[node], if1, if2,
                                                    "vfio-pci")
