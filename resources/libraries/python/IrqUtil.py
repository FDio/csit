...
class IrqUtil:
    ...
    @staticmethod
    def set_pci_interface_irqs_affinity(node, pci_addr, cpu_list):
        ...
        IrqUtil.set_interface_irqs_affinity(node, interface, cpu_list)
    ...
    def set_interface_irqs_affinity(node, interface, cpu_skip_cnt=0, cpu_cnt=1):
        ...
        for irq, cpu in zip(irq_list, cpu_list):
            if int(cpu) < 32:
                ...
                groups = int(int(cpu)/32)
                mask_fill = u""
                for _ in range(groups):
                    mask_fill = f"{mask_fill},00000000"
                mask = 1 << (int(cpu) - (32 * groups))
