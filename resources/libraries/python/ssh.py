    ...
    def handle(self):
        ...
        try:
            ...
            raise SSHException('Incoming request to {host}:{port} failed:'
                               '\n{err}'.
                               format(host=self.chain_host,
                                      port=self.chain_port, err=repr(err)))
        if chan is None:
        ...
        print('Tunnel open: {req} -> {chan} -> {r_host}:{r_port}'.
...
def exec_cmd(node, cmd, timeout=600, sudo=False):
    ...
    if node.get('host_port') is not None:
        ...
        if local_port_hash is not None:
            ...
            print('Connecting to ssh host {host}:{port} ...'.
                         format(host=node['host'], port=node['host_port']))
            try:
                client.connect(node['host'], port=node['host_port'],
            ...
            print('Forwarding port {l_port} to {r_host}:{r_port} ...'.
