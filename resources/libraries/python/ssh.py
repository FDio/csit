...
class SSH(object):
    ...
    def exec_command(self, cmd, timeout=10, log_stdout_err=True):
        ...
        stdout = BytesIO()
        stderr = BytesIO()
        stdout_wrapper = TextIOWrapper(buffer=stdout)
        stderr_wrapper = TextIOWrapper(buffer=stderr)
        try:
        ...
        while not chan.exit_status_ready() and timeout is not None:
            if chan.recv_ready():
                stdout_wrapper.write(chan.recv(self.__MAX_RECV_BUF))

            if chan.recv_stderr_ready():
                stderr_wrapper.write(chan.recv_stderr(self.__MAX_RECV_BUF))

            if time() - start > timeout:
                raise SSHTimeout(
                    f"Timeout exception during execution of command: {cmd}\n"
                    f"Current contents of stdout buffer: "
                    f"{stdout_wrapper.read()}\n"
                    f"Current contents of stderr buffer: "
                    f"{stderr_wrapper.read()}\n"
                )
        ...
        if log_stdout_err or int(return_code):
            logger.trace(
                f"return STDOUT {stdout_wrapper.read()}"
            )
            logger.trace(
                f"return STDERR {stderr.getvalue()}"
            )
        return return_code, stdout_wrapper.read(), stderr_wrapper.read()
