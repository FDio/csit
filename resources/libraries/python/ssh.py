...
class SSH(object):
    ...
    def exec_command(self, cmd, timeout=10, log_stdout_err=True):
        ...
        while not chan.exit_status_ready() and timeout is not None:
            if chan.recv_ready():
                output = chan.recv(self.__MAX_RECV_BUF)
                logger.trace(f"stdout: {output}")
                stdout += (output.decode())

            if chan.recv_stderr_ready():
                err = chan.recv_stderr(self.__MAX_RECV_BUF)
                logger.trace(f"err: {err}")
                stderr += (err.decode())
        ...
        while chan.recv_ready():
            stdout += (chan.recv(self.__MAX_RECV_BUF).decode())

        while chan.recv_stderr_ready():
            stderr += (chan.recv_stderr(self.__MAX_RECV_BUF).decode())
