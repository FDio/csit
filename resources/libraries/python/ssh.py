...
class SSH(object):
    ...
    def exec_command(self, cmd, timeout=10, log_stdout_err=True):
        ...
        while not chan.exit_status_ready() and timeout is not None:
            ...
            if time() - start > timeout:
                raise SSHTimeout(
                    ...
                    f"{stdout.getvalue().decode(encoding='utf-8')}\n"
                    f"Current contents of stderr buffer: "
                    f"{stderr.getvalue().decode(encoding='utf-8')}\n"
                )
        ...
        if log_stdout_err or int(return_code):
            logger.trace(
                f"return STDOUT {stdout.getvalue().decode(encoding='utf-8')}"
            )
            logger.trace(
                f"return STDERR {stderr.getvalue().decode(encoding='utf-8')}"
            )
        return return_code, stdout.getvalue().decode(encoding='utf-8'),\
               stderr.getvalue().decode(encoding='utf-8')
    ...
    def interactive_terminal_exec_command(self, chan, cmd, prompt):
        ...
        tmp = buf.decode(encoding='utf-8').replace(cmd.replace(u"\n", u""), u"")
        for item in prompt:
            tmp.replace(item, u"")
        return tmp
