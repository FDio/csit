...
class SSH(object):
    ...
    def _reconnect(self, attempts=1):
        ...
        while attempts >= 0:
            ...
            raise IOError('Cannot connect to {host}'.format(host=node['host']))
...
def exec_cmd_no_error(node, cmd, timeout=600, sudo=False,
    ...
    if not message:
        message = 'Command execution failed: "{cmd}"\nRC: {rc}\n{stderr}'
        message = message.format(cmd=cmd, rc=ret_code, stderr=stderr)
    logger.info(message)
