...
class DMAUtil:
    ...
    def enable_dma_device_on_dut(node, dma_device_name, wq_count):
        ...
        for i in range(wq_count):
            ...
            cmd = f"sudo -E -S accel-config config-engine " \
        ...
        for i in range(wq_count):
            ...
            cmd = f"sudo -E -S " \
                f" accel-config config-wq {dma_device_name}/{wq_name} " \
    ...
    def show_lspci(node):
        ...
        stdout, _ = exec_cmd_no_error(
            node, cmd, sudo=True,
            message=u"Failed to get accel-config list on DUT.",
            retries=3)
        logger.debug(stdout)

        cmd = "grep -H . /sys/bus/dsa/devices/wq*/ats_disable"
        stdout, _ = exec_cmd_no_error(
            node, cmd, sudo=True,
            message=u"Failed to get ats_disable on DUT.",
            retries=3)
        logger.debug(stdout)

        cmd = "grep -H . /sys/bus/dsa/devices/wq*/block_on_fault"
        stdout, _ = exec_cmd_no_error(
            node, cmd, sudo=True,
            message=u"Failed to get block_on_fault on DUT.",
            retries=3)
        logger.debug(stdout)

        # cmd = 'echo "/usr/lib64" >> /etc/ld.so.conf.d/x86_64-linux-gnu.conf'
        # stdout, _ = exec_cmd_no_error(
        #         node, cmd, sudo=True,
        #         message=u"Failed to echo lib64 on DUT.",
        #         retries=3)
        # logger.debug(stdout)
        ...
        # stdout, _ = exec_cmd_no_error(
        #         node, cmd, sudo=True,
        #         message=u"Failed to ldconfig on DUT.",
        #         retries=3)
        # logger.debug(stdout)

        # cmd = 'find / -name "libaccel-config.so.1"'
        # stdout, _ = exec_cmd_no_error(
        #         node, cmd, sudo=True,
        #         message=u"Failed to find libaccel on DUT.",
        #         retries=3)
        # logger.debug(stdout)

        cmd = "ls /usr/lib/libaccel-config.so.1 2> /dev/null | wc -l"
        stdout, _ = exec_cmd_no_error(
                node, cmd, sudo=True,
                message=u"Failed to ls on DUT.",
                retries=3)
        logger.debug(stdout)

        if int(stdout.strip()) == 0:
            cmd = "ln -s -f /usr/lib64/libaccel-config.so.1 \
                /usr/lib/libaccel-config.so.1"
            stdout, _ = exec_cmd_no_error(
                node, cmd, sudo=True,
                message=u"Failed to ln on DUT.",
                retries=3)
            logger.debug(stdout)

        # file_name = f"intel_dsa_sample.c"
        cmd = f"cd {Constants.REMOTE_FW_DIR}; \
            sudo -E -S make intel_dsa_sample LDLIBS=-laccel-config; \
            sudo -E -S ./intel_dsa_sample; echo over"
        stdout, _ = exec_cmd_no_error(
                node, cmd,
