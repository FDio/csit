...
class DUTSetup(object):
    ...
    def verify_vpp_on_dut(node):
        ...
        ret_code, _, _ = ssh.exec_command_sudo("service vpp restart")
        if int(ret_code) != 0:
            raise RuntimeError('Failed to restart VPP on host: {}'.
                               format(node['host']))

        ret_code, _, _ = ssh.exec_command_sudo("vppctl show version verbose")
        if int(ret_code) != 0:
            raise RuntimeError('Failed to get VPP version on host: {}'.
                               format(node['host']))

        ret_code, _, _ = ssh.exec_command_sudo("vppctl show int")
        if int(ret_code) != 0:
