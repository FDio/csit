...
class DUTSetup:
    ...
    def restart_service(node, service, lock_path=None):
        ...
        if lock_path:
            command = f"flock -w 300 {lock_path} -c '{command} && sleep 3'"
    ...
    @staticmethod
    def verify_program_installed(node, program, lock_path=None):
        ...
        """
        if lock_path:
            cmd = f"flock -w 300 {lock_path} -c {program} && sleep 3"
        else:
