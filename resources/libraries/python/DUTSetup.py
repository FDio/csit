        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                ...
                try:
                    ...
                    command = 'yum -y remove "*vpp*"'
                    message = 'Failed to remove previous VPP'\
                        'installation on host {host}!'.\
                        format(host=node['host'])
                    exec_cmd_no_error(node, command, timeout=120, sudo=True,
                                      message=message)

                    command = 'rpm -ivh {dir}*.rpm'.format(dir=vpp_pkg_dir)
                    ...
                    command = 'dpkg -i --force-all {dir}*.deb'.\
