...
class DUTSetup:
    ...
    def get_huge_page_free(node, huge_size):
        ...
        command = f"cat /sys/kernel/mm/hugepages/hugepages-{huge_size}kB/"\
            f"free_hugepages"
        for _ in range(3):
            free_hugepages, _ = exec_cmd_no_error(node, command)
            try:
                huge = int(free_hugepages)
            except ValueError:
                logger.trace(u"Reading huge pages information failed!")
            else:
                break
        else:
            raise RuntimeError(u"Getting huge pages information failed.")
        return huge
    ...
    def get_huge_page_overcommit(node, huge_size):
        ...
        for _ in range(3):
            ...
            try:
                huge = int(nr_overcommit_hugepages)
            except ValueError:
                logger.trace(u"Reading huge pages information failed")
            else:
                break
