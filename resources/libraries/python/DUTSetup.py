...
class DUTSetup(object):
    ...
    def crypto_device_init(node):
        ...
        ret_code, _, _ = ssh.exec_command(
            "sudo sh -c 'echo {0}:{1}:00.0 | "
            "tee sys/bus/pci/devices/{1}\:{2}\:00.0/driver/unbind'"
            .format(cryptodev.split(':')[0], cryptodev.split(':')[1]))
        ...
        ret_code, _, _ = ssh.exec_command(
            "sudo sh -c 'echo {0}:{1}:00.0 | "
            "tee /sys/bus/pci/drivers/dh895xcc/bind'"
            .format(cryptodev.split(':')[0], cryptodev.split(':')[1]))
        ...
        ret_code, _, _ = ssh.exec_command_sudo(
            r"sudo sh -c 'echo 32 | "
            r"tee /sys/bus/pci/devices/{0}\:{1}\:00.0/sriov_numvfs'"
            .format(cryptodev.split(':')[0], cryptodev.split(':')[1]))
