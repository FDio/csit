...
class QemuNode(object):
    ...
    def qemu_add_vhost_user_if(self, socket, server=True, mac=None):
        ...
        if mac is None:
            mac = '52:54:00:{0:02x}:04:{1:02x}'.format(self._id, self._vhost_id)
    ...
    def _qemu_qga_flush(self):
        ...
        if 0 != int(ret_code):
            ...
            raise RuntimeError('QGA execute "{0}" failed on {1}'.format(qga_cmd,
                self._node['host']))
        logger.trace(stdout)
        if not stdout:
            return {}
