...
from resources.libraries.python.NdrPdrResult import NdrPdrResult
from resources.libraries.python.ReceiveRateInterval import ReceiveRateInterval
from resources.libraries.python.ReceiveRateMeasurement import ReceiveRateMeasurement
        ...
        while 1:
            ...
            if ndr_hi.drop_fraction <= 0.0:
                if ndr_hi.target_tr < state.line_rate:
                    new_tr = min(state.line_rate, self.double_step_up(ndr_rel_width, ndr_hi.target_tr))
            ...
            if pdr_hi.drop_fraction <= state.allowed_drop_fraction:
                if pdr_hi.target_tr < state.line_rate:
                    new_tr = min(state.line_rate, self.double_step_up(pdr_rel_width, pdr_hi.target_tr))
