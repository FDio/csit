...
class Topology:
    ...
    def _add_node_item(node, value, path):
        ...
        """
        if len(path) == 1:
            logger.trace(f"_add_node_item leaf: subnode {node} key {path[0]} value {value}")
        ...
        if path[0] not in node:
            ...
            logger.trace(f"_add_node_item inner str: subnode {node} key {path[0]} value {sub_node}")
            node.protected_setitem(path[0], sub_node)
        logger.trace(f"_add_node_item inner down: subnode {node} key {path[0]} rest {path[1:]}")
        Topology._add_node_item(node[path[0]], value, path[1:])
        logger.trace(f"_add_node_item inner up: subnode {node} key {path[0]} rest {path[1:]}")
