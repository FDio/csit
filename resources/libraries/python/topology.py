...
class Topology:
    ...
    def _add_node_item(node, value, path):
        ...
        """
        if len(path) == 1:
            logger.trace(f"_add_node_item before adding leaf: subnode {node!r} key {path[0]!r} value {value!r}")
        ...
        if path[0] not in node:
            ...
            logger.trace(f"_add_node_item inner str: subnode {node!r} key {path[0]!r} value {sub_node!r}")
            node.protected_setitem(path[0], sub_node)
        logger.trace(f"_add_node_item inner down: subnode {node!r} key {path[0]!r} rest {path[1:]!r}")
        Topology._add_node_item(node[path[0]], value, path[1:])
        logger.trace(f"_add_node_item inner up: subnode {node!r} key {path[0]!r} rest {path[1:]!r}")
