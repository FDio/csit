...
def _replace_nodes(new_nodes, reason="unknown", log_level=u"TRACE"):
    ...
    logger.write(
        msg=f"Going to update nodes. Reason: {reason}\n{new_nodes}",
        level=log_level,
    )
    ...
    logger.trace(u"attempt to set dict")
    set_global_variable(u"\\&{nodes}", DICT__nodes)
    logger.trace(u"attempt to get as scalar back")
    var_set = get_variable(u"\\${nodes}", u"not_set")
    logger.trace(f"dict got back: {var_set!r}")
    logger.trace(u"attempt to set scalar")
    set_global_variable(u"\\${nodes}", DICT__nodes)
    logger.trace(u"attempt to get scalar back")
    var_set = get_variable(u"\\${nodes}", u"not_set")
    logger.trace(f"scalar got back: {var_set!r}")
