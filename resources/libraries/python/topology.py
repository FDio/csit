...
class SubNodeDict(FrozenDict):
    ...
    Also tuples and None are allowed.
...
def _get_nodes():
    ...
    if isinstance(DICT__nodes, dict):
        # FrozenDict is a Mapping, but not a dict.
...
class Topology:
    ...
    def add_node_item(node, value, path, reason=u"unknown", log_level=u"TRACE"):
        ...
        if not path:
            raise KeyError(u"Empty path!")
        nodes = _get_nodes()
        for node_key, node_struct in nodes.items():
            if node_struct == node:
                break
        else:
    ...
    def add_new_port(node, ptype):
        ...
        :type node: topology.NodeDict
        :type ptype: str
        :returns: Node structure after update and port key or None.
        :rtype: Tuple[topology.NodeDict, Optional[str]]
        """
        max_ports = 1000000
        iface = None
        for i in range(1, max_ports):
            if node[u"interfaces"].get(str(ptype) + str(i)) is None:
                ...
                node = Topology.add_node_item(
                    node, dict(), path, u"add_new_port"
                )
                break
        return node, iface
    ...
    def update_interface_sw_if_index(node, iface_key, sw_if_index):
        ...
        :param iface_key: Topology key of the interface.
        :param sw_if_index: Internal index to store, None if unknown.
        :type node: topology.NodeDict
        :type iface_key: str
    ...
    def update_interface_mtu(node, iface_key, mtu):
        ...
        :rtype: topology.NodeDict
        """
        path = [u"interfaces", iface_key, u"mtu"]
        return Topology.add_node_item(
            node, int(mtu), path, u"update_interface_mtu"
        )
    ...
    def set_interface_numa_node(node, iface_key, numa_node_id):
        ...
        No-op if interface is not found.
        ...
        """
        try:
            # Make the last level mutable, to avoid TypeError.
            node[u"interfaces"][iface_key].copy()[u"numa_node"] = numa_node_id
            # No KeyError, meaning it is safe to update real nodes.
            path = [u"interfaces", iface_key, u"numa_node"]
            return Topology.add_node_item(
                node, int(numa_node_id), path, u"set_interface_numa_node"
            )
        except KeyError:
            return node
    ...
    def set_node_arch(node, arch):
        ...
        """
        path = [u"arch"]
        return Topology.add_node_item(node, arch, path, u"set_node_arch")
    ...
    def set_node_cpuinfo(node, cpuinfo):
        ...
        :type node: topology.NodeDict
        :type arch: Iterable[Iterable[int]]
        :returns: Node structure after update, useful for subsequent calls.
        :rtype: topology.NodeDict
        """
        path = [u"cpuinfo"]
        return Topology.add_node_item(node, cpuinfo, path, u"set_node_cpuinfo")
    ...
    def clean_sockets_on_all_nodes(nodes):
        """Remove any existing sockets from topology file.
        ...
        new_nodes = {
            ...
            node_key: node_struct.excluding(u"sockets", do_raise=False)
