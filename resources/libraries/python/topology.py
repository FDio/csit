...
from resources.libraries.python.protected_structure import ProtectedDict
...
class Topology:
    ...
    def _add_node_item(node, value, path):
        ...
        if path[0] not in node:
            ...
            sub_node = dict() if node[path[0]] == u"" \
                else SubNodeDict({node[path[0]]: u""})
    ...
    def update_interface_mac_address(node, iface_key, mac_address):
        ...
        _notify_nodes(f"update_interface_mac_address address {address}")
