...
def pre_serialize_recursive(data):
    ...
    if isinstance(data, IntFlag):
        return repr(data)
    # VAT2 needs string name for enums (and intenums).
    if isinstance(data, (IntEnum, Enum)):
        return data.name
    # VAT2 sees AddressUnion as an union.
    if isinstance(data, AddressUnion):
    ...
    if isinstance(data, (AddressUnion, Address, AddressWithPrefix)):
        return str(data)
