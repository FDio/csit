...
"""Library holding Robot friendly form of "raise from" expression."""

from robot.api import logger


def raise_from(raising, excepted, level=u"WARN"):
    """Logging analogue of "raise from" syntax.

    Unfortunately, Robot Framework does not show the cause chain,
    so using "raise from" from Python 3 is currently not enough.

    Current implementation just logs the excepted error, and raises the new one.
    For the allowed log level values, see:
