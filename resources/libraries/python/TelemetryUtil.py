...
class TelemetryUtil:
    ...
    @staticmethod
    def telemetry(node, profile, pid=None):
        ...
        cd_cmd = f"cd '{Constants.REMOTE_FW_DIR}/{Constants.RESOURCES_TOOLS}'"
        bin_cmd = f"python3 -m telemetry -c {config}"

        exec_cmd(node, f"{cd_cmd} && {bin_cmd}", sudo=True)

    @staticmethod
    def telemetry_on_all_duts(nodes, profile, pid=None):
        ...
        for node in nodes.values():
            if node[u"type"] == NodeType.DUT:
                TelemetryUtil.telemetry(node, profile=profile, pid=pid)
