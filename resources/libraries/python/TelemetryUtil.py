...
class TelemetryUtil:
    ...
    @staticmethod
    def run_telemetry(node, profile, pid=None):
        ...
        bin_cmd = f"python3 -m telemetry -c {config}\""

        cmd_opts = OptionString(prefix=u"--")
        cmd_opts.add_with_value(u"config", config)
        cmd_opts.add_with_value(u"pid", pid)

        exec_cmd(node, f"{cd_cmd} && {bin_cmd}", sudo=True)
        exec_cmd(node, f"cat /tmp/metric.prom", sudo=True)
        exec_cmd(node, f"cat /tmp/metric.json", sudo=True)

    @staticmethod
    def run_telemetry_on_all_duts(nodes, profile, pid=None):
        ...
        for node in nodes.values():
            if node[u"type"] == NodeType.DUT:
                TelemetryUtil.run_telemetry(node, profile=profile, pid=pid)
