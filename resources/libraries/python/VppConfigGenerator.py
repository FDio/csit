...
class VppConfigGenerator(object):
    ...
    def set_node(self, node):
        ...
        if node['type'] != NodeType.DUT:
            raise RuntimeError('Startup config can only be applied to DUT node.')
        self._node = node
    ...
    def add_uio_driver(self, value):
        """Add uio-driver configuration.

        :param value: uio-driver configuration.
        :type value: str
        """
        path = ['uio-driver']
        self.add_config_item(self._nodeconfig, value, path)
