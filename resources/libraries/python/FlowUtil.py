...
class FlowUtil:
    ...
    @staticmethod
    def vpp_show_lsmod(node):
        cmd = u"lsmod"
        err_msg = u"Failed to show mod {node[u'host']}"
        ret, _, _ = exec_cmd(node, cmd, sudo=True)
        ...
        cmd = u"find / -name ice.pkg"
        err_msg = u"Failed to show mod {node[u'host']}"
        ret, _, _ = exec_cmd(node, cmd, sudo=True)

        cmd = u"ls  /sys/bus/pci/drivers/ice/"
        err_msg = u"Failed to show mod {node[u'host']}"
        ret, _, _ = exec_cmd(node, cmd, sudo=True)


        cmd = u"ls  /sys/bus/pci/drivers/i40e/"
        err_msg = u"Failed to show mod {node[u'host']}"
        ret, _, _ = exec_cmd(node, cmd, sudo=True)
        return ret
