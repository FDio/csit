...
class VppGrubUtil(object):
    ...
    def apply_cmdline(self, node):
        ...
        if ret != 0:
            logger.debug(stderr)
            if stdout.strip('\n') != ofilename:
                (ret, stdout, stderr) = ssh.exec_command('sudo cp {} {}'.format(filename, ofilename))
                if ret != 0:
                    logger.debug('stdout: {}'.format(stdout))
                    logger.debug('stderr: {}'.format(stderr))
                    raise RuntimeError('Executing cp command failed to node {}'.format(self._node['host']))

        # Get the contents of the current grub config file
        (ret, stdout, stderr) = ssh.exec_command('cat {}'.format(filename))
        if ret != 0:
            raise RuntimeError('Executing cat command failed to node {}'.format(self._node['host']))
