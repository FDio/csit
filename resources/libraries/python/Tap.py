...
class Tap(object):
    ...
    def add_tap_interface(node, tap_name, mac=None):
        ...
        :rtype: int
        """
        if isinstance(unicode, tap_name):
        ...
        if mac is None:
            mac = InterfaceUtil.vpp_get_interface_mac(node, tap_name)
    ...
    @staticmethod
    def vpp_get_tap_interface_mac(node, host_if_name):
        ...
        :rtype: str
        """
        return Tap.tap_dump(node, host_if_name).get('host_mac_addr')
    ...
    def tap_dump(node, name=None):
        ...
        def process_tap_dump(tap_dump):
            ...
            tap_dump['host_ip4_addr'] = \
                ip_address(unicode(tap_dump['host_ip4_addr']))
            tap_dump['host_ip6_addr'] = \
                ip_address(unicode(tap_dump['host_ip6_addr']))
            return tap_dump
