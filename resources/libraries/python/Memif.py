...
class Memif(object):
    ...
    @staticmethod
    def _memif_create(node, mid, sid, rxq=1, txq=1, role='slave'):
    ...
    def create_memif_interface(node, filename, mid, sid, rxq=1, txq=1,
        ...
        # Create socket
        rsp = Memif._memif_socket_filename_add_del(node, True, filename, sid)

        # TODO: Check the response, raise ValueError if not successful
        # It might not be necessary as retval is checked by verify_reply

        # Create memif
        rsp = Memif._memif_create(node, mid, sid, rxq=1, txq=1, role='slave')

        # TODO: Extract sw_if_index, raise ValueError if not successful
