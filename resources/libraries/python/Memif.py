...
class Memif(object):
    ...
    def bin_to_mac(mac_bin):
        ...
        TODO: When https://gerrit.fd.io/r/#/c/18360 is merged, use the method
              from L2Util
    ...
    def vpp_get_memif_interface_name(node, sw_if_idx):
        ...
        for item in dump:
            if item["memif_details"]["sw_if_index"] == sw_if_idx:
                print("if_name")
                print(item["memif_details"]["if_name"].rstrip('\x00'))
                return item["memif_details"]["if_name"].rstrip('\x00')
        return None
    ...
    def vpp_get_memif_interface_mac(node, sw_if_idx):
        ...
        for item in dump:
            if item["memif_details"]["sw_if_index"] == sw_if_idx:
                print("hw_addr")
                print(Memif.bin_to_mac(item["memif_details"]["hw_addr"]))
                return Memif.bin_to_mac(item["memif_details"]["hw_addr"])
        return None
