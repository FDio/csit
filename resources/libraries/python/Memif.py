...
class Memif(object):
    ...
    def vpp_get_memif_interface_name(node, sw_if_idx):
        ...
        :rtype: str
        """
        with VatTerminal(node, json_param=False, non_json_output=True) as vat:
            response = vat.vat_terminal_exec_cmd_from_template('memif_dump.vat')
        logger.trace("response:\n".format(response))
        memif_data = Memif.parse_memif_dump_data(str(response[0]))
        logger.trace("memif_data:\n".format(memif_data))
        for item in memif_data:
            if memif_data[item]['sw_if_index'] == sw_if_idx:
                return item
            return None
    ...
    def vpp_get_memif_interface_mac(node, sw_if_idx):
        ...
        with VatTerminal(node, json_param=False, non_json_output=True) as vat:
            response = vat.vat_terminal_exec_cmd_from_template('memif_dump.vat')
        memif_data = Memif.parse_memif_dump_data(response)
        for item in memif_data:
            if memif_data[item]['sw_if_index'] == sw_if_idx:
    ...
    def vpp_get_memif_interface_socket(node, sw_if_idx):
        ...
        :type node: dict
        :type sw_if_idx: int
        :return: Memif interface socket path.
        :rtype: str
        """
        with VatTerminal(node, json_param=False, non_json_output=True) as vat:
            response = vat.vat_terminal_exec_cmd_from_template('memif_dump.vat')
        memif_data = Memif.parse_memif_dump_data(response[0])
        for item in memif_data:
            if memif_data[item]['sw_if_index'] == sw_if_idx:
    ...
    def vpp_get_memif_interface_id(node, sw_if_idx):
        ...
        for item in memif_data:
            if memif_data[item]['sw_if_index'] == sw_if_idx:
                return memif_data[item].get('id', None)
