...
class Classify(object):
    ...
    def get_classify_session_data(node, table_index, session_index=None):
        ...
        :rtype: list or dict
        """
        PapiExecutor.dump_and_log(node, ["classify_session_dump", ])
    ...
    @staticmethod
    def add_replace_acl_multi_entries(node, acl_idx=0, rules=None):
    ...
    def vpp_log_macip_acl_settings(node):
        ...
        :type node: dict
        """
        # TODO: Remove when bug in data processing is fixed
        try:
            VatExecutor.cmd_from_template(
