...
class Classify(object):
    ...
    def vpp_log_plugin_acl_settings(node):
        ...
        try:
            with VatTerminal(node, json_param=False) as vat:
                vat.vat_terminal_exec_cmd_from_template(
                    "acl_plugin/acl_dump.vat")
        except Exception:
            pass
        except:
            raise RuntimeError("Retrieving of configured ACLs failed "
    ...
    def vpp_log_plugin_acl_interface_assignment(node):
        ...
        try:
            with VatTerminal(node, json_param=False) as vat:
                vat.vat_terminal_exec_cmd_from_template(
                    "acl_plugin/acl_interface_dump.vat")
        except Exception:
            pass
        except:
            raise RuntimeError("Retrieving of ACL interface list failed "
                               "on node {0}".format(node['host']))

    @staticmethod
    def set_acl_list_for_interface(node, interface, input_acl_idx=None,
                                   output_acl_idx=None):
        """Set the list of input and/or output ACLs applied to the interface. It
        unapplies any previously applied ACLs.

        :param node: VPP node to set ACL on.
        :param interface: Interface name or sw_if_index.
        :param input_acl_idx: Index(ies) of ingress ACLs to be applied on
          the interface. (Optional)
        :param output_acl_idx: Index(ies) of egress ACLs to be applied on
          the interface. (Optional)
        :type node: dict
        :type interface: str or int
        :type input_acl_idx: list
        :type output_acl_idx: list
        ...
        input_acl = 'input ' + ' '.join(str(idx) for idx in input_acl_idx) \
            if input_acl_idx else ''

        output_acl = 'output ' + ' '.join(str(idx) for idx in output_acl_idx) \
            if output_acl_idx else ''

        try:
            with VatTerminal(node, json_param=False) as vat:
                vat.vat_terminal_exec_cmd_from_template(
                    "acl_plugin/acl_interface_set_acl_list.vat",
                    interface=sw_if_index, input_acl=input_acl,
