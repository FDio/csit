...
class Classify(object):
    ...
    def _classify_add_del_session(
            node, is_add, table_index, match, match_len,
    ...
    def vpp_configures_classify_session_l3(
        ...
        # Prepend match with l2 part
        match = 14 * '\0' + match

        match_len = (match_n + skip_n) * 16

        # Pad match to match skip_n_vector + match_n_vector size
        match = match + (match_len - len(match) if len(match) < match_len
                         else 0) * '\0'
