...
class Classify(object):
    ...
    @staticmethod
    def _build_ip6_match(nh=0, src_ip='', dst_ip='',
                         src_port=0, dst_port=0):
    ...
    @staticmethod
    def _classify_add_del_table(node,
                                is_add,
                                mask,
                                match_n_vectors=1,
        ...
        :param nbuckets:
        :param memory_size:
        :param skip_n_vectors:
        :param next_table_index:
        :param miss_next_index:
        :param current_data_flag:
    ...
    @staticmethod
    def _classify_add_del_session(node,
                                  is_add,
                                  table_index,
                                  match,
    ...
    @staticmethod
    def _create_classify_session(node, table_index, match, pbr_option=0,
                                 vrfid=0, is_add=1):
        ...
        """
        return Classify._classify_add_del_session(
            node,
            is_add=is_add,
            table_index=table_index,
    ...
    def vpp_creates_classify_table_l3(node, ip_version, direction, ip=""):
        ...
        mask_ip = dict(
            ip4=binascii.hexlify(inet_aton(ip)),
            ip6=binascii.hexlify(inet_pton(AF_INET6, ip))
        )
    ...
    def vpp_configures_classify_session_l3(node, acl_method, table_index,
        ...
        Classify._create_classify_session(
            node,
            is_add=1,
            table_index=table_index,
            match=match)
