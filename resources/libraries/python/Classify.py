...
class Classify(object):
    ...
    def add_macip_acl_multi_entries(node, rules=""):
        ...
        perf
        /home/tibor/ws/csit/tests/vpp/perf/l2/10ge2p1x710-eth-l2bdbasemaclrn-macip-iacl10sl-100flows-ndrpdr.robot
        csit-3n-hsw-perftest mrrANDnic_intel-x710ANDl2bdmaclrnANDmacipANDiaclANDacl_statelessANDacl10AND100k_flowsAND64bAND2c
       ...
       TODO: Create a list of rules
        ...
        rules_l = rules.split(", ")
        logger.debug("rules_l:\n{rules}".format(rules=rules_l))
        ...
        for rule in rules_l:
            ...
            mac = str(groups.group(1).split(' ')[1].replace(':', ''))
            acl_rule["src_mac"] = binascii.hexlify(mac)

            groups = re.search(reg_ex_mask, rule)
            mask = str(groups.group(1).split(' ')[1].replace(':', ''))
            acl_rule["src_mac_mask"] = binascii.hexlify(mask)

            groups = re.search(reg_ex_ip, rule)
            grp = groups.group(1).split(' ')[1].split('/')
            acl_rule["src_ip_addr"] = binascii.hexlify(inet_pton(
                AF_INET6 if acl_rule["is_ipv6"] else AF_INET, str(grp[0])))
            acl_rule["src_ip_prefix_len"] = int(grp[1])

            acl_rules.append(acl_rule)

        logger.debug("acl_rules:\n{rules}".format(rules=acl_rules))
