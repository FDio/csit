...
def main():
    ...
    for res in results:
        stats = res.get()
        if stats:
            listn = [a + b for a, b in zip(listn, stats)]

    # Output results.
    print(f"Rate: {round(listn[6], 2)} [Kbytes/sec]")
    print(f"Latency: {round(listn[4]/8, 2)} ms")
    print(f"Processing:{round(listn[5]/8, 2)} ms")
    print(f"Connection rate:{round(listn[3], 2)} per sec")
    print(f"Total transferred: {round(listn[2])} bytes")
    print(f"Complete requests: {round(listn[0])} ")
    print(f"Failed requests: {round(listn[1])} ")


def one(cpu, requests, clients, cipher, protocol, ipaddress, files, port):
    """Run one test.
    ...
    cmd = f"taskset --cpu-list {cpu} ab -n {requests} -c {clients}" \
          f" -Z {cipher} -f {protocol}" \
          f" https://{ipaddress}:{port}/{files} 2>/dev/null"
