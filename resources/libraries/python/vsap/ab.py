...
def check_ab(tg_node):
    ...
    ret, _, _ = ssh.exec_command(
        "command -v ab")
    if int(ret) != 0:
        raise RuntimeError('AB is not installed on TG node.')
...
def run_ab(tg_node, ciphers, files_num):
    ...
    python_dir = u"resources/libraries/python/vsap"
    cmd = f"{Constants.REMOTE_FW_DIR}/{python_dir}/abfork.py" \
          f" --port 443 --clients 2000 --ip {ip_address}  --cipher {ciphers}" \
          f" --files {files} --requests 40000 --protocol TLS1.2"

    ret, stdout, retc = exec_cmd(tg_node, cmd, timeout=180, sudo=True)
