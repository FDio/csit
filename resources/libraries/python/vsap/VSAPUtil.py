        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                ...
                if stdout.strip() == u"Ubuntu":
                    ...
                    message = u"Nginx install failed!"

                    exec_cmd_no_error(
                        node, f"apt-get purge -y 'nginx' || true", \
                        timeout=120, sudo=True
                    )

                    exec_cmd_no_error(
                        node, f"dpkg -i --force-all {pkg_dir}nginx*.deb", \
                        timeout=120, sudo=True, message=message
                    )

                    exec_cmd_no_error(node, f"dpkg -l | grep nginx", sudo=True)

                    script_dir = u"tests/vsap/vsap_scripts"
