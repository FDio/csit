        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                ...
                if stdout.strip() == u"Ubuntu":
                    message = u"VPP uninstall failed!"
                    ...
                    message = u"Nginx uninstall failed!"

                    exec_cmd_no_error(
                        node, u"apt-get purge -y 'nginx' || true", \
                        timeout=120, sudo=True
                    )

                    logger.console(u"Openssl3.0.0 install on DUT... ")
                    message = u"Openssl3.0.0 install failed!"
                    ...
                    message = u"VSAP install failed!"

                    exec_cmd_no_error(
                        node, f"apt-get purge -y 'vsap' || true", \
                        timeout=120, sudo=True
                    )
                    ...
                    script_dir = u"resources/libraries/bash/entry"
                    command = f"{Constants.REMOTE_FW_DIR}/{script_dir}" \
                            f"/run_nginx.sh export"
                    message = u"Export path failed!"
