...
class IPTopology:
    ...
    @staticmethod
    def initialize_ipv4_forwarding(nodes, count=1, paralel_links=1):
        """
        ...
        :param paralel_links: Number of paralel links to configure.
        :type nodes: dict
        :type count: int
        :type paralel_links: int
        """
        topology = BuiltIn().get_variable_value("&{topology_info}")
        dut = topology["duts"][-1]
        ifl = BuiltIn().get_variable_value("${int}")

        for l,i in zip(range(paralel_links), range(1, paralel_links*2, 2)):
            ...
            IPUtil.vpp_add_ip_neighbor(
                nodes["DUT1"], DUT1_int1, f"1.{l}.1.1",
            ...
            if dut == "DUT2":
                ...
                IPUtil.vpp_add_ip_neighbor(
                    nodes["DUT1"], DUT1_int2, f"3.{l}.3.2", DUT_mac1
                )
                DUT_mac2 = BuiltIn().get_variable_value(
                    f"${{DUT1_{ifl}{i+1}_mac}}[0]"
                )
                IPUtil.vpp_add_ip_neighbor(
                    nodes["DUT2"], DUT_int1, f"3.{l}.3.1", DUT_mac2
                )
            IPUtil.vpp_add_ip_neighbor(
                nodes[dut], DUT_int2, f"2.{l}.2.1",
                topology[f"TG_pf{i}_mac"][0]
            )

            IPUtil.vpp_interface_set_ip_address(
                nodes["DUT1"], DUT1_int1, f"1.{l}.1.2", 30
            )
            if dut == "DUT2":
                IPUtil.vpp_interface_set_ip_address(
                    nodes["DUT1"], DUT1_int2, f"3.{l}.3.1", 30
                )
                IPUtil.vpp_interface_set_ip_address(
                    nodes["DUT2"], DUT_int1, f"3.{l}.3.2", 30
                )
            IPUtil.vpp_interface_set_ip_address(
                nodes[dut], DUT_int2, f"2.{l}.2.2", 30
            )

            IPUtil.vpp_route_add(
                nodes["DUT1"], f"{i}0.0.0.0", 32, gateway=f"1.{l}.1.1",
                interface=DUT1_int1, count=count
            )
            if dut == "DUT2":
                IPUtil.vpp_route_add(
                    nodes["DUT1"], f"{i+1}0.0.0.0", 32, gateway=f"3.{l}.3.2",
                    interface=DUT1_int2, count=count
                )
                IPUtil.vpp_route_add(
                    nodes["DUT2"], f"{i}0.0.0.0", 32, gateway=f"3.{l}.3.1",
                    interface=DUT_int1, count=count
                )
            IPUtil.vpp_route_add(
                nodes[dut], f"{i+1}0.0.0.0", 32, gateway=f"2.{l}.2.1",
