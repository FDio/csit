...
class IPUtil:
    ...
    def linux_interface_has_ip(
        ...
        cmd += " | grep 'inet ' | awk -e '{print $2}'"
        cmd += f" | grep '{ip_addr_with_prefix}' || true"
        stdout, _ = exec_cmd_no_error(node, cmd, timeout=5, sudo=True)

        has_ip = stdout.rstrip()
        logger.trace(f"has_ip = '{has_ip}' Type: {type(has_ip)}\n"
            f"ip = '{ip_addr_with_prefix}' Type: {type(ip_addr_with_prefix)}")
