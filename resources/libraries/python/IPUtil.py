...
def address_increment(initial_network):
    """Factory function creating ObjIncrement for IP addresses.

    The input is an IP network to ensure the address is aligned
    with respect to the prefix length.
    ...
    :type initial_address: Union[ipaddress.IPv4Network, ipaddress.IPv6Network]
    :type prefix_length: int
    """
    initial_address = initial_network.network_address
    increment = 1 << (initial_network.max_prefixlen - initial_network.prefixlen)
...
class IPUtil:
    ...
    def vpp_route_add(node, network, prefix_len, **kwargs):
        ...
        if count > 100:
            ...
            netiter = NetworkIncrement(ip_network(f"{network}/{prefix_len}"), 1)
        ...
        netiter = address_increment(ip_network(f"{network}/{prefix_len}"))
        with PapiSocketExecutor(node) as papi_exec:
