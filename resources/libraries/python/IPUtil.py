...
class IPUtil:
    ...
    def compose_vpp_route_structure(node, network, prefix_len, **kwargs):
        ...
        network_and_plen = AddressWithPrefix.of(network, prefix_len)

        paths = list()
        n_hop = dict(
            address=AddressUnion.of(gateway),
        ...
        path = dict(
            ...
            proto=getattr(
                ...
                if network_and_plen.is_ip6() else u"FIB_PATH_NH_PROTO_IP4"
            ).value,
            nh=n_hop,
            n_labels=0,
