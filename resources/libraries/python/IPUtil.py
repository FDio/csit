...
class AbstractNetworkIncrement(ObjIncrement):
    """
    An iterator object which accepts an IPv4Network or IPv6Network and
    returns a new network, address part incremented by the increment
    number of networks each time it is iterated, or when inc_fmt is called.
    The increment may be positive (default 1), negative or 0
    (in which case the network is always the same).

    This is an abstract base class, subclasses differ in formatting.
    """
    def __init__(self, initial_value, increment=1):
        """
    ...
    def _str_fmt(self):
        ...
        raise NotImplementedError(u"AbstractNetworkIncrement does not format.")


class DashNetworkIncrement(AbstractNetworkIncrement):
    """A variant of network increment, formatting as:
    '<ip_address_start> - <ip_address_stop>'
    """
    # Most methods are inherited, including constructor.
...
class SlashNetworkIncrement(AbstractNetworkIncrement):
    """A variant of network increment, formatting as:
    '<ip_address_start>/<prefix_length>'
    """
    # Most methods are inherited, including constructor.
...
class IPUtil:
    ...
    def vpp_route_add(node, network, prefix_len, **kwargs):
        ...
        netiter = SlashNetworkIncrement(ip_network(f"{network}/{prefix_len}"))
        ...
        if count > 100:
            ...
            with open(tmp_filename, u"w") as tmp_file:
                for _ in range(count):
                    command_parts[1] = f"{netiter.inc_fmt()}"
        ...
        with PapiSocketExecutor(node) as papi_exec:
            for i in range(count):
                args[u"route"] = IPUtil.compose_vpp_route_structure(
                    node, next(netiter).network_address, prefix_len, **kwargs
                )
