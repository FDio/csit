...
class NetworkIncrement(ObjIncrement):
    ...
    each time it's iterated or when inc_fmt is called.
    The increment may be positive, negative or 0
    (in which case the network is always the same).

    The formatted output is suitable for some CLI commands.
    For other uses, it may be easier to track prefix length and network address
    in two separate objects, the latter created by address_increment.
    """
...
def address_increment(initial_address, prefix_length):
    ...
    The increment amount is defined via prefix length,
    as addresses for continuous string of networks is the main use case.
    Consequently, this can be used also for (the address part of) IP networks,
    useful if NetworkIncrement formatting is less fitting.
    Use ObjIncrement directly if your address increment is not a power of two.

    The alignment of the address with respect to prefix is not checked,
    we have devicetests with non-aligned addresses.
    Increment is computed from prefix length
    to create consecutive networks without gaps nor overlaps.

    :param initial_address: The initial IP address.
    :param prefix_length: Defines the size of the network.
