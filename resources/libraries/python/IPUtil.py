...
class IPUtil(object):
    ...
    def vpp_route_add(node, network, prefix_len, **kwargs):
        ...
        if gateway:
            gt_addr = ip_address(unicode(gateway))
        ...
        paths = list()
        if gateway:
            # nh = dict(address=union_addr(gt_addr))
            nh = str(gt_addr)
            path = dict(
                sw_if_index=InterfaceUtil.get_interface_index(node, interface)
        ...
        with PapiExecutor(node) as papi_exec:
            for i in xrange(kwargs.get('count', 1)):
                ...
                # prefix['address'] = addr
                # route['prefix'] = prefix
                route['prefix'] = '{addr}/{addr_len}'.format(addr=str(net_addr + i), addr_len=int(prefix_len))
                papi_exec.add(cmd, route=route, **args)
