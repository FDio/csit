...
class CpuUtils:
    ...
    def get_affinity_vswitch(
            node, phy_cores, rx_queues=None, rxd=None, txd=None):
        """Get affinity
        ...
        cpu_node = Topology.get_interface_numa_node(node, dut_pf_keys)
        smt_used = CpuUtils.is_smt_enabled(node[u"cpuinfo"])
        skip_cnt = Constants.CPU_CNT_SYSTEM
        cpu_main = CpuUtils.cpu_list_per_node_str(
            node, cpu_node, skip_cnt, cpu_cnt
        )
        skip_cnt = Constants.CPU_CNT_SYSTEM + Constants.CPU_CNT_MAIN
        if cpu_count_int:
            cpu_wt = CpuUtils.cpu_list_per_node_str(
                node, cpu_node, skip_cnt, cpu_count_int
            )
        fp_count_int = \
            int(fp_cores_count) * 2 if smt_used else int(fp_cores_count)
        dp_count_int = \
            int(dp_cores_count) * 2 if smt_used else int(dp_cores_count)
        ...
        compute_resource_info[u"cpu_alloc_str"] = u",".join(cpu_main, cpu_wt)
        compute_resource_info[u"cpu_count_int"] = cpu_count_int
