...
class CpuUtils(object):
    ...
    def cpu_slice_of_list_for_nf(**kwargs):
        ...
        """
        if kwargs['nf_chain'] - 1 >= kwargs['nf_chains']:
            raise RuntimeError("ChainID is higher than total number of chains!")
        if kwargs['nf_node'] - 1 >= kwargs['nf_nodes']:
            raise RuntimeError("NodeID is higher than chain nodes!")

        smt_used = CpuUtils.is_smt_enabled(
            kwargs['node']['cpuinfo'])
        cpu_list = CpuUtils.cpu_list_per_node(
            kwargs['node'], kwargs['cpu_node'], smt_used)
        cpu_list_len = len(cpu_list)
        sib = cpu_list_len / CpuUtils.NR_OF_THREADS

        if not smt_used and not isinstance(kwargs['nf_dtc'], int):
            raise RuntimeError("Cannot allocate if SMT is not enabled.")
        dtc = 1 if not isinstance(kwargs['nf_dtc'], int) else kwargs['nf_dtc']

        nf_total = kwargs['nf_chains'] * kwargs['nf_nodes']
        mt_req = (nf_total + kwargs['nf_mtcr'] - 1) / kwargs['nf_mtcr']
        dt_req = (nf_total + kwargs['nf_dtcr'] - 1) / kwargs['nf_dtcr']
        ...
        offset = (kwargs['nf_node'] - 1) + (kwargs['nf_chain'] - 1)\
            * kwargs['nf_nodes']
        try:
        ...
        if smt_used:
            ...
            if isinstance(kwargs['nf_dtc'], int):
                dt_list = \
    ...
    def get_affinity_nf(nodes, node, nf_chains=1, nf_nodes=1, nf_chain=1,
        ...
        :param vs_dtc: Amount of Dataplane threads of vswitch.
        :param nf_dtc: Amount of Dataplane threads of NF.
        :param nf_mtcr: NF main thread core ratio.
        :param nf_dtcr: NF dataplane thread core ratio.
        :type nodes: dict
        :type node: dict
        :type nf_chains: int
        :type nf_nodes: int
