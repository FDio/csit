...
class IPv6Management(object):
    ...
    def set_management_interface_address(node, interface, address, prefix):
        ...
        """
        cmd0 = "sudo sysctl -a | grep ipv6"
        # Enable IPv6 for the specified interface
        cmd1 = "sudo sysctl net.ipv6.conf.{0}.disable_ipv6=0".format(interface)

        # Configure IPv6 address on the interface
        cmd2 = "sudo ip address add {address}/{prefix} dev {interface}".format(
                interface=interface,
                address=address,
                prefix=prefix)

        # Set the interface up
        cmd3 = "sudo ip link set {interface} up".format(interface=interface)

        ssh = SSH()
        ssh.connect(node)
        ret_code, _, _ = ssh.exec_command(cmd0)
        if ret_code != 0:
            raise RuntimeError("Could not configure IP address on interface.")
        ret_code, _, _ = ssh.exec_command(cmd1)
        if ret_code != 0:
            raise RuntimeError("Could not configure IP address on interface.")
        ret_code, _, _ = ssh.exec_command(cmd2)
        if ret_code != 0:
            raise RuntimeError("Could not configure IP address on interface.")
        ret_code, _, _ = ssh.exec_command(cmd3)
        if ret_code != 0:
            raise RuntimeError("Could not configure IP address on interface.")
