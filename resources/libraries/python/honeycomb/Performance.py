...
class Performance(object):
    ...
    @staticmethod
    def configure_honeycomb_cpu_affinity(node, cores, reserved=2):
        """Add CPU affinity and FIFO scheduler to Honeycomb startup script.

        :param node: Honeycomb node.
        :param cores: List or number of CPU cores to use.
        ...
        cores = "{first}-{last}".format(first=reserved, last=cores-1)

        find = 'java'

        # Use alternate scheduler, Ubuntu's default can't load-balance
        # over isolcpus
        replace = 'chrt -f 99 taskset -c {0} java'.format(cores)
