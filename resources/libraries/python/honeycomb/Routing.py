...
class RoutingKeywords(object):
    ...
    def configure_interface_slaac(node, interface, slaac_data=None):
        """Configure routing advertisement on the specified interfaces. 

        :param node: Honeycomb node.
        :param interface: Interface to configure routing advertisement.
        ...
        if not slaac_data:
            status_code, _ = HcUtil.delete_honeycomb_data(
                node, 'config_routing_table_slaac', path)
        else:
            data = {
                'interface': [
                    {
            ...
            status_code, _ = HcUtil.put_honeycomb_data(
                node, 'config_routing_table_hc2vpp', data, path)
    ...
    def get_interface_slaac_oper_data(node, interface):
        ...
        status_code, resp = HcUtil.\
            get_honeycomb_data(node, "config_routing_table_slaac", path)
        ...
        try:
            ...
            dict_of_str = {k:str(v) for k,v in dict_of_str.items()}
            return dict_of_str
