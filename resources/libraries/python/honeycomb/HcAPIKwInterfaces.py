...
class InterfaceKeywords(object):
    ...
    def compare_data_structures(data, ref, _path=''):
        ...
        if data == ref:
            ...
            for key in ref:
                if key not in data:
                    raise HoneycombError("Key {key} is not present in path "
                                         "{path}. Keys in path:"
                                         "{data_keys}".format(
                                            key=key,
                                            path=_path,
                ...
                if data[key] != ref[key]:
                    if isinstance(data[key], list) \
                        ...
                        raise HoneycombError("Data mismatch, key {key} in path"
                                             "{path} has value {data}, but"
                                             " should be {ref}".format(
                                                key=key,
                                                path=_path,
                                                data=data[key],
                                                ref=ref[key]))
            ...
            for item in ref:
                if item not in data:
                    if isinstance(item, dict):
                        ...
                        raise HoneycombError("Data mismatch, list item {index}"
                                             " in path {path} has value {data},"
                                             " but should be {ref}".format(
            ...
            raise HoneycombError("Type mismatch. Data is of type {data_type},"
                                 " but should be {ref_type}".format(
                                    data_type=type(data),
                                    ref_type=type(ref)))
        else:
