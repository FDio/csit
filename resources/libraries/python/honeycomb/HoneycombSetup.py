...
class HoneycombSetup(object):
    ...
    @staticmethod
    def check_honeycomb_startup_log(node):
        ssh = SSH()
        ssh.connect(node)
        cmd = "cat /var/log/honeycomb/honeycomb.log | grep '{0}'"
        _, _, stdout = ssh.exec_command(cmd.format("started successfully!"))
        if "no such file" in stdout:
            ssh.exec_command("tail -n 250 /var/log/syslog")
        else:
            ret_code, _, _ = ssh.exec_command(cmd.format("Exception"))
            if ret_code != 0:
                ssh.exec_command(cmd.format("Caused by:"))
