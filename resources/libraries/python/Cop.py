...
class Cop(object):
    ...
    def cop_add_whitelist_entry(node, interface, ip_format, fib_id):
        ...
        err_msg = 'Failed to add COP whitelist on interface '{interface}' ' \
                  with '{ip_format}'.format(interface,ip_format)
        args_in = dict(
            is_add=True,
            in_sw_if_index=int(sw_if_index),
            in_ipformat=inet_pton(AF_INET,str(ip_format)),
    ...
    def cop_interface_enable_or_disable(node, interface, state):
        ...
        if state in ('enable', 'disable'):
            ...
            err_msg = 'Failed to enable or disable on interface '{interface}' ' \
                  with '{state}'.format(interface,state)
