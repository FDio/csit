...
class NginxManage:
    def __init__(self, node, version, nginx_conf_name):
        """
        ...
        self.path = f'{Constants.NGINX_INSTALL_DIR}/nginx-{self.version}'
...
class NginxUtil:
    ...
    @staticmethod
    def copy_nginx_tmp_conf(nodes, nginx_version, conf_name):
        """Copy NGINX-TMP conf on node.
        ...
        for node in nodes.values():
            if node[u"type"] == NodeType.DUT:
                cmd = f'cp {Constants.REMOTE_FW_DIR}/' \
                      ...
                      f' {Constants.NGINX_INSTALL_DIR}/nginx-{nginx_version}' \
                      f'/conf/{conf_name}'
                exec_cmd_no_error(node, cmd, sudo=True, disconnect=True,
