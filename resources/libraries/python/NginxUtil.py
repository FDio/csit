...
class NginxUtil:
    ...
    @staticmethod
    def set_nginx_worker_processes(node, workers):
    ...
    def taskset_idle_cores_to_nginx_pid(node, cpu_idle_str):
        ...
        pids = NginxUtil.get_nginx_pids(node)
    ...
    @staticmethod
    def get_nginx_pids(node):
        cmd = "ps -ef | grep 'nginx: worker process' |" \
              " grep -v grep | awk '{print $2}'"
        pids_str, _ = exec_cmd_no_error(node, cmd, sudo=True, disconnect=True,
                                        message='start nginx failed')
        if pids_str == '':
            raise RuntimeError('Node type is not a DUT.')
        pids_list = [i for i in pids_str.split('\n') if i]
        return pids_list

    @staticmethod
    def get_vcl_conf_cmd():
        vcl_path = f"{Constants.REMOTE_FW_DIR}" \
                   f"/{Constants.RESOURCES_TPL}/vcl/vcl_iperf3.conf"
        cmd = f"export VCL_CONFIG={vcl_path}"
        return cmd

    @staticmethod
    def nginx_start(node, nginx_mode, **kwargs):
        """Start NGINX cmd app on node.

        :param node: Node to start cmd on.
        :param nginx_mode: ldp or vcl.
        :type node: dict
        :returns: nothing
        """
        kwargs['filename'] = f"{NGINX_PATH}/conf/nginx-tmp.conf"
        cmd_options = NginxUtil.get_cmd_options(**kwargs)
        vcl_cmd = NginxUtil.get_vcl_conf_cmd()
        if nginx_mode == "ldp":
            ld_options = OptionString()
            ld_options.add(f"LD_PRELOAD={Constants.VCL_LDPRELOAD_LIBRARY}")
            cmd_options = ld_options.extend(cmd_options)
        exec_cmd_no_error(node, f'{vcl_cmd};{cmd_options} &', disconnect=True,
                          message="start nginx failed")
        exec_cmd_no_error(node, 'env', disconnect=True)
