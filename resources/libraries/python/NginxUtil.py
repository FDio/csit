...
class NginxUtil:
    ...
    def taskset_idle_cores_to_nginx_pid(node, cpu_idle_str):
        ...
        show_conf_cmd = "cat /etc/vpp/startup.conf; " \
                        "cat /tmp/openvpp-testing/download_dir/" \
                        "nginx-1.14.2/conf/nginx-vsap.conf; " \
                        "ps -ef |grep nginx; vppctl show  session verbose "
        exec_cmd_no_error(
            node, show_conf_cmd, sudo=True, timeout=180,
        )
