...
class VPPUtil(object):
    ...
    def vpp_show_version(node, verbose=True):
        ...
        return_version = reply[u"version"].rstrip(b'\0')
        version = f"VPP version:      {return_version}\n"
        if verbose:
            cd = reply[u"build_date"].rstrip(b'\0')
            cl = reply[u"build_directory"].rstrip(b'\0')
            version += f"Compile date:     {cd}\nCompile location: {cl}\n"
        logger.info(version)
