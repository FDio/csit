...
class VPPUtil(object):
    ...
    def vpp_show_version(node, verbose=True):
        ...
        version = ('VPP version:      {ver}\n'.
                   format(ver=reply_obj['version'].rstrip('\0x00')))
        if verbose:
            version += ('Compile date:     {date}\n'
                        'Compile location: {cl}\n '.
                        format(date=reply_obj['build_date'].rstrip('\0x00'),
                               cl=reply_obj['build_directory'].rstrip('\0x00')))
        logger.info(version)
        return reply_obj['version'].rstrip('\0x00')
