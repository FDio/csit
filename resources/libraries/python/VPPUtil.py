    ...
    def install_vpp_on_all_duts(nodes, vpp_pkg_dir):
        ...
        for node in nodes.values():
            ...
            if node["type"] == NodeType.DUT:
                ...
                exec_cmd(node, f"sh -c \"{command}\"", timeout=120, sudo=True)
                command = "echo vfio-pci | tee /sys/bus/pci/devices/0000:17:00.0/driver_override"
                exec_cmd(node, f"sh -c \"{command}\"", timeout=120, sudo=True)
                ...
                exec_cmd(node, f"sh -c \"{command}\"", timeout=120, sudo=True)
                command = "ip link set dev `basename /sys/bus/pci/devices/0000:17:00.0/net/*` up"
                exec_cmd(node, f"sh -c \"{command}\"", timeout=120, sudo=True)
                ...
                exec_cmd(node, f"sh -c \"{command}\"", timeout=120, sudo=True)
                command = "echo vfio-pci | tee /sys/bus/pci/devices/0000:17:00.1/driver_override"
                exec_cmd(node, f"sh -c \"{command}\"", timeout=120, sudo=True)
                ...
                exec_cmd(node, f"sh -c \"{command}\"", timeout=120, sudo=True)
                command = "ip link set dev `basename /sys/bus/pci/devices/0000:17:00.1/net/*` up"
                exec_cmd(node, f"sh -c \"{command}\"", timeout=120, sudo=True)
