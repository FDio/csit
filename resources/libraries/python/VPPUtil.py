...
class VPPUtil:
    ...
    def show_log(node):
        ...
        :type node: dict
        """
        exec_cmd(node, "cat /tmp/asan.log*; rm /tmp/asan.log*", timeout=180, sudo=True)
        exec_cmd(node, "tail -n 999 /var/log/vpp/vpp.log", timeout=180, sudo=True)
        exec_cmd(node, "journalctl --no-pager --lines=100", timeout=180, sudo=True)
        exec_cmd(node, "dmesg | tail -n 999", timeout=180, sudo=True)
