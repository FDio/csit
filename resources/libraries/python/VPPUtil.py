...
class VPPUtil(object):
    ...
    def verify_vpp_installed_on_dut(node):
        ...
        """
        exec_cmd_no_error(
            node, cmd='command -v vpp', message='VPP is not installed!')

    @staticmethod
    def verify_vpp_started_on_dut(node, retries=0):
        ...
        for _ in range(retries + 1):
            ...
            raise RuntimeError(
                'VPP failed to start on host {name}'.format(name=node['host']))
    ...
    def verify_vpp_on_all_duts(nodes):
        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                ...
                VPPUtil.verify_vpp_started_on_dut(node, retries=120)
