    ...
    def get_vcl_test_command(vcl_test_attributes):
        ...
        if role == "client":
            ...
            if vcl_test_attributes.get("uni_direct"):
                ...
                vcl_test_cmd["args"] += " -B"
            # Number of client sessions (-s), default 1
            nclients = vcl_test_attributes.get("nclients", 1)
            if nclients and int(nclients) > 1:
                vcl_test_cmd["args"] += f" -s {nclients}"
            # QUIC streams per session (-q), default 1
            quic_streams = vcl_test_attributes.get("quic_streams", 1)
            if quic_streams and int(quic_streams) > 1:
                vcl_test_cmd["args"] += f" -q {quic_streams}"
