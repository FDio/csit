...
class HoststackUtil:
    ...
    def start_hoststack_test_program(node, namespace, core_list, program):
        ...
        # NGINX used `worker_cpu_affinity` in configuration file; everything
        # else must be pinned via taskset to the caller-provided core_list
        # (already selected on the NIC's NUMA in hoststack.robot). Without
        # this, unpinned test programs get scheduled onto whatever house-
        # keeping CPU Linux picks, which on isolcpus-heavy CI hosts lands
        # cross-socket vs VPP + NIC.
        taskset_cmd = ""
