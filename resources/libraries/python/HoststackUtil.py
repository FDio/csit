...
class HoststackUtil():
    ...
    @staticmethod
    def get_nginx_command(nginx_attributes, nginx_version, nginx_conf_name,
                          nginx_ins_dir):
        ...
        :param nginx_conf_name: nginx conf file name.
        :type nginx_attributes: dict
        :type nginx_ins_dir: nginx install dir
        ...
        nginx_cmd[u"path"] = f"{nginx_ins_dir}/" \
                             f"nginx-{nginx_version}/sbin/"
        nginx_cmd[
            u"args"] = f"-c {nginx_ins_dir}/" \
                       f"nginx-{nginx_version}/conf/{nginx_conf_name} "
        return nginx_cmd
    ...
    def log_vpp_hoststack_data(node):
        ...
        logger.trace(f"VPP show error:\n"
            f"{PapiSocketExecutor.run_cli_cmd(node, u'show error')}\n")
        logger.trace(f"VPP show interface:\n"
            f"{PapiSocketExecutor.run_cli_cmd(node, u'show interface')}\n")
