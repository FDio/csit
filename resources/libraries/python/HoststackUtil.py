...
from resources.libraries.python.model.ExportResult import (
    export_hoststack_iperf3_fail_result, export_hoststack_vpp_echo_fail_result,
    export_hoststack_iperf3_pass_result, export_hoststack_vpp_echo_pass_result,
)
...
class HoststackUtil():
    ...
    def analyze_hoststack_test_program_output(node, role, nsim_attr, program):
        ...
        :raises RuntimeError: If node subtype is not a DUT.
        """
        defer_fail, output = HoststackUtil._analyze_hoststack_output_no_export(
            node=node,
            role=role,
            nsim_attr=nsim_attr,
            program=program,
        ...
        if defer_fail:
            if program[u"name"] == u"iperf3":
                export_hoststack_iperf3_fail_result(output)
            else:
                # Assuming vpp_echo.
                export_hoststack_vpp_echo_fail_result(output)
            return (True, output)
        if program[u"name"] == u"iperf3":
            export_hoststack_iperf3_pass_result(output)
        else:
            # Assuming vpp_echo.
            export_hoststack_vpp_echo_pass_result(output)
