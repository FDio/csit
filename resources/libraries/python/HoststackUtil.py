    ...
    def hoststack_test_program_finished(node, program_pid, program,
        ...
        try:
            ...
            if len(program_stdout) > 0:
                logger.debug(f"{program[u'name']} stdout log:\n"
                             f"{program_stdout}")
            else:
                logger.debug(f"Empty {program[u'name']} stdout log :(")
            ...
            program_stdout, program_stderr = \
                HoststackUtil.get_hoststack_test_program_logs(other_node,
                                                              other_program)
            if len(program_stdout) > 0:
                logger.debug(f"{other_program[u'name']} stdout log:\n"
                             f"{program_stdout}")
            else:
                logger.debug(f"Empty {other_program[u'name']} stdout log :(")
            if len(program_stderr) > 0:
                logger.debug(f"{other_program[u'name']} stderr log:\n"
                             f"{program_stderr}")
            else:
                logger.debug(f"Empty {program[u'name']} stderr log :(")
            raise
