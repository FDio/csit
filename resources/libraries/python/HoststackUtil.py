...
class HoststackUtil(object):
    ...
    def get_vpp_echo_command(vpp_echo_attributes):
        ...
        vpp_echo_cmd = [u"vpp_echo", vpp_echo_attributes[u"role"],
            u"socket-name", vpp_echo_attributes[u"vpp_api_socket"],
    ...
    def start_hoststack_test_program(node, namespace, program, *args):
        ...
        DUTSetup.kill_program(node, program, namespace)
        ...
        cmd = f"nohup {shell_cmd} '{program} {opts} " \
            f">/tmp/{program}_stdout.log 2>/tmp/{program}_stderr.log &'"
        try:
            exec_cmd_no_error(node, cmd, sudo=True)
            return DUTSetup.get_pid(node, program)[0]
        except RuntimeError:
    ...
    def analyze_hoststack_test_program_output(node, role, nsim_attr,
        ...
        no_results = False
        program_cmd = u" ".join([program] + program_args)
