...
class HoststackUtil():
    ...
    def _get_ldpreload_path(node):
        ...
        If the node is aarch64, return the default value for that architecture.
        Otherwise return the default value for x86_64 architecture.

        :param node: Topology node to decide architecture.
        :type node: dict
        :returns: Path to VCL preload library.
        :rtype: str
        """
        if ret := Constants.VCL_LDPRELOAD_LIBRARY:
            return ret
        if Topology.get_node_arch(node) == u"aarch64":
            return u"/usr/lib/aarch64-linux-gnu/libvcl_ldpreload.so"
        return u"/usr/lib/x86_64-linux-gnu/libvcl_ldpreload.so"
