...
class InterfaceUtil:
    ...
    @staticmethod
    def vpp_bring_interface_down_and_set_mtu(node, interface, mtu):
        """Set interface state to down and apply new MTU value.

        VPP (at least for dpdk plugin) refuses to change MTU
        when interface is up.

        As some VPP plugins (or NIC drivers they rely on)
        do not support MTU editing at all, so that part can fail.
    ...
    @staticmethod
    def vpp_set_interface_mtu_and_bring_up(node, interface, mtu):
        """Set MTU on interface and bring interface up.

        The interface is temporarily brought down before MTU can be changed.
        ...
        :type mtu: int
        """
        InterfaceUtil.vpp_bring_interface_down_and_set_mtu(node, interface, mtu)
