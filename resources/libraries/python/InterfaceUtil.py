...
class InterfaceUtil(object):
    ...
    def pci_to_int(pci_str):
        ...
        :rtype: int
        """
        x = list(pci_str.split(':')[0:2])
        x.extend(pci_str.split(':')[2].split('.'))

        return (int(x[0], 16) | int(x[1], 16) << 16 | int(x[2], 16) << 24 |
                int(x[3], 16) << 29)

    @staticmethod
    def get_interface_sw_if_index(node, interface):
    ...
    def update_vpp_interface_data_on_node(node):
        ...
        for if_name, if_data in node['interfaces'].items():
            for ifc in interface_list:
                if ifc['l2_address'] == if_data['mac_address']:
                    if_data['name'] = ifc['interface_name']
                    if_data['vpp_sw_index'] = ifc['sw_if_index']
                    if_data['mtu'] = ifc['mtu'][0]
                    logger.trace('Interface {ifc} found by MAC {mac}'.format(
                        ifc=if_name, mac=if_data['mac_address']))
                    interface_list.remove(ifc)
                    break
            else:
    ...
    def vxlan_dump(node, interface=None):
        ...
        if interface is not None:
            sw_if_index = InterfaceUtil.get_interface_sw_if_index(
                node, interface)
        else:
    ...
    def create_subinterface(node, interface, sub_id, outer_vlan_id=None,
        ...
        cmd = 'create_subif'
        args = dict(sw_if_index=InterfaceUtil.get_interface_sw_if_index(
                        node, interface),
                    sub_id=int(sub_id),
    ...
    def vpp_enslave_physical_interface(node, interface, bond_interface):
        ...
        args = dict(sw_if_index=Topology.get_interface_sw_index(
                        node, interface),
                    bond_sw_if_index=Topology.get_interface_sw_index(
                        node, bond_interface),
                    is_passive=0,
                    is_long_timeout=0)
    ...
    @staticmethod
    def vpp_ip_source_check_setup(node, interface_name):
        """Setup Reverse Path Forwarding source check on interface.

        :param node: Node to setup RPF source check.
        :param interface_name: Interface name to setup RPF source check.
        :type node: dict
        :type interface_name: str
        """
    ...
    def vpp_put_vxlan_and_vlan_interfaces_to_bridge_domain(
        ...
        with PapiExecutor(node) as papi_exec:
            for i in range(0, vxlan_count):
                dst_ip = dst_address_start + i * ip_step
                neighbor = dict(sw_if_index=sw_idx_vxlan,
                                flags=0,
                                mac_address=str(Topology.get_interface_mac(
                                    op_node, op_node_if)),
                ...
                args2 = dict(
                    ...
                    table_id=0,
                    classify_table_index=Constants.BITWISE_NON_ZERO,
                    next_hop_table_id=0,
                    next_hop_id=Constants.BITWISE_NON_ZERO,
                    is_add=1,
                    is_ipv6=is_ipv6,
                    next_hop_weight=1,
                    next_hop_proto=1 if is_ipv6 else 0,
                    dst_address_length=16 if is_ipv6 else 4,
                    dst_address=inet_pton(af_inet, str(dst_ip)),
                    next_hop_address=inet_pton(af_inet, str(dst_ip)),
                    next_hop_via_label=0xfffff + 1,
                    next_hop_out_label_stack=Constants.BITWISE_NON_ZERO)
                bd_id = bd_id_start + i
                subif_id = i + 1
