...
from socket import AF_INET, AF_INET6, inet_ntop, inet_pton, htons, htonl
...
class InterfaceUtil(object):
    ...
    def vpp_create_avf_interface(node, vf_pci_addr, num_rx_queues=None):
        ...
        x = list(vf_pci_addr.split(':')[0:2])
        x.extend(vf_pci_addr.split(':')[2].split('.'))

        cmd = 'avf_create'
        args = dict(pci_addr=htonl(int(x[0], 16) << 16 | int(x[1], 16) << 8 | int(x[2], 16) << 3 | int(x[3], 16)),
                    enable_elog=htons(0),
                    rxq_num=htons(int(num_rx_queues) if num_rx_queues else 0),
                    rxq_size=htons(0),
                    txq_size=htons(0))
