...
class InterfaceUtil(object):
    ...
    def vpp_create_multiple_vxlan_ipv4_tunnels(
        ...
        for i in range(0, vxlan_count):
            ...
            #                            sub_id=sub_id)
            sub_if = 'exec create sub-interfaces {name} sub_id {sub_id}'\
                     .format(name=nonvxlan_if_name, sub_id=sub_id)
            # tmp_f.write(sub_if)
            commands.append(sub_if)
            logger.trace('{cmd}'.format(cmd=sub_if))

        logger.trace('commands listr: {cmd}'.format(cmd=commands))
        with open(tmp_fn, 'w') as tmp_f:
            tmp_f.writelines('{}\n'.format(str(c) for c in commands))
        ...
        with open(tmp_fn, 'w') as tmp_f:
            for i in range(0, vxlan_count):
                ...
                route = 'ip_add_del_route {ip}/32 via {ip} ' \
                        'sw_if_index {sw_idx} resolve-attempts 10 ' \
                        'count 1'.format(ip=dst_ips[i], sw_idx=sw_idx_vxlan)
