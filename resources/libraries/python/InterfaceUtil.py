...
class InterfaceUtil(object):
    ...
    def vpp_create_bond_interface(node, mode, load_balance=None, mac=None):
        ...
        args = dict(id=int(Constants.BITWISE_NON_ZERO),
                    ...
                    mode=int(getattr(LinkBondMode, '{md}'.format(
                        md=mode.replace('-', '_').upper()))),
                    lb=0 if load_balance is None else int(getattr(
    ...
    def vpp_show_bond_data_on_node(node, details=False):
        ...
        data = ('Bond data on node {host}:'.format(host=node['host']))
        with PapiExecutor(node) as papi_exec:
            papi_resp = papi_exec.add(cmd).get_dump(err_msg)

        papi_dump = papi_resp.reply[0]['api_reply']
        for item in papi_dump:
            data += ('{b}'.format(b=item[cmd_reply]['interface_name'].
                                  rstrip('\x00')))
            data += ('  mode: {m}'.
                     format(m=item[cmd_reply]['mode'].rstrip('\x00')))
            data += ('  load balance: {lb}'.
                     format(lb=item[cmd_reply]['lb'].rstrip('\x00')))
            ...
            if details:
                ...
                for slave in slave_data:
                    if not slave['is_passive']:
                        data += ('    {s}'.format(s=slave['interface_name']))
    ...
    def vpp_bond_slave_dump(node, interface):
        ...
        args = dict(sw_if_idx=Topology.get_interface_sw_index(node, interface))
