...
class InterfaceUtil(object):
    ...
    def vpp_create_multiple_vxlan_ipv4_tunnels(
        ...
        :param node: VPP node 1 to create tunnel interfaces.
        ...
        logger.trace('{cmd}:'.format(cmd=tmp_fn))
        commands = list()
        for i in range(0, vxlan_count):
            ...
            logger.trace('{cmd}'.format(cmd=ip_config))
            vni = vni_start + i
            ...
            logger.trace('{cmd}'.format(cmd=vxlan_tunnel))
            ...
            logger.trace('{cmd}'.format(cmd=sub_if))

        logger.trace('commands list: {cmd}'.format(cmd=commands))
        with open(tmp_fn, 'w') as tmp_f:
            tmp_f.writelines(commands)
            
        vat.execute_script(tmp_fn, node, timeout=300, json_out=False,
                           copy_on_execute=True)
        command = 'cat {file}'.format(file=tmp_fn)
        ...
        for i in range(0, vxlan_count):
            ...
            logger.trace('{cmd}'.format(cmd=vxlan_up))
            ...
            logger.trace('{cmd}'.format(cmd=subif_up))
        ...
        for i in range(0, vxlan_count):
            ...
            logger.trace('{cmd}'.format(cmd=l2fib_entry1))
            ...
            logger.trace('{cmd}'.format(cmd=l2fib_entry2))
