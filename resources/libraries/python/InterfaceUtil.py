...
class InterfaceUtil(object):
    ...
    def vpp_get_interface_data(node, interface=None):
        ...
        err_msg = 'Failed to get interface dump on host {host}'.format(
            host=node['host'])
        with PapiExecutor(node) as papi_exec:
            papi_if_dumpp = papi_exec.add(cmd, **args).get_details(err_msg)
    ...
    def vpp_get_interface_ip_addresses(node, interface, ip_version):
        ...
        err_msg = 'Failed to get L2FIB dump on host {host}'.format(
            host=node['host'])
        with PapiExecutor(node) as papi_exec:
            papi_resp = papi_exec.add(cmd, **args).get_details(err_msg)

        data = list()
        for item in papi_resp:
            item['ip'] = inet_ntop(AF_INET6, item['ip']) if is_ipv6 \
                else inet_ntop(AF_INET, item['ip'][0:4])
            data.append(item)
