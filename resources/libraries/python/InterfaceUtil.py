...
class InterfaceUtil(object):
    ...
    @staticmethod
    def vpp_set_interface_rx_placement(node, sw_if_index, queue_id=0,
                                       worker_id=1):
    ...
    @staticmethod
    def vpp_set_rr_rx_placement(node, worker_threads):
        """Set Round Robin interface RX placement on all worker threads
        on node.

        :param nodes: Topology nodes.
        :param worker_threads: Total numer of worker threads on VPP.
        :type nodes: dict
        :type worker_threads: int
        """
        worker_id = 0
        for mapping in InterfaceUtil.vpp_dump_interface_rx_placement(node):
            for ifc in node['interfaces'].values():
                if mapping['sw_if_index'] == ifc['vpp_sw_index'] \
                    and 'VirtualEthernet' in ifc['name'] \
                    or 'Memif' in ifc['name']:
                    InterfaceUtil.vpp_set_interface_rx_placement(
                        node, mapping['sw_if_index'], mapping['queue_id'],
                        worker_id % worker_threads)
                    worker_id += 1

    @staticmethod
    def vpp_set_rr_rx_placement_on_all_duts(nodes, worker_threads):
        """Set Round Robin interface RX placement on all worker threads
        on all DUTs.
        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                InterfaceUtil.vpp_set_rr_rx_placement(node, worker_threads)
    ...
    def vpp_dump_interface_rx_placement(node):
        ...
        :param node: Node to run command on.
        :type node: dict
        :returns: Thread mapping information as a list of dictionary
        :rtype: list
