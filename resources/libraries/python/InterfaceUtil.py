...
class InterfaceUtil(object):
    ...
    def set_interface_state(node, interface, state, if_type='key'):
        ...
        if node['type'] == NodeType.DUT:
            ...
            cmd = 'ip link set {ifc} {state}'.format(ifc=face_name, state=state)
    ...
    def update_vpp_interface_data_on_node(node):
        ...
        for interface in node['interfaces'].keys():
            for if_data in interface_list:
                if if_data['l2_address'] == interface['mac_address']:
                    interface['name'] = if_data['interface_name']
                    interface['vpp_sw_index'] = if_data['sw_if_index']
                    interface['mtu'] = if_data['mtu'][0]
                    logger.trace('Interface {ifc} found by MAC {mac}'.format(
                        ifc=interface, mac=interface['mac_address']))
                    interface_list.remove(if_data)
                    break
            else:
                logger.trace('Interface {ifc} not found by MAC {mac}'.format(
                    ifc=interface, mac=interface['mac_address']))
                interface['vpp_sw_index'] = None
