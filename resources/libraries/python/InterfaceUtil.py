...
class InterfaceUtil(object):
    ...
    def vpp_create_multiple_vxlan_ipv4_tunnels(
            ...
            tg_if_to_vxlan_mac_start, tg_if_to_vlan_mac_start, mac_step,
            mac_limit, bd_id_start):
        """Create multiple VXLAN tunnel interfaces and VLAN sub-interfaces on 
        VPP node.

        Put each pair of VXLAN tunnel interface and VALN sub-interface to 
        ...
        interfaces.
        :param node_vlan_if: VPP node interface to create VLAN sub-interface.
        ...
        :param tg_if_to_vxlan_mac_start: TG interface towards VXLAN tunnel
        interfaces MAC address start.
        :param tg_if_to_vlan_mac_start: TG interface towards VLAN sub-interfaces
        MAC address start.
        :param mac_step: MAC address incremental step.
        :param mac_limit: MAC address limit.
        ...
        :type ip_step: int
        :type ip_limit: str
        :type tg_if_to_vxlan_mac_start: str
        :type tg_if_to_vlan_mac_start: str
        :type mac_step: int
        :type mac_limit: str
        :type bd_id_start: int
        """

        vxlan_count = n_tunnels
        ...
        for i in range(0, vxlan_count):
            ...
            #     tg_if_to_vlan_mac_int = \
            #         tg_if_to_vlan_mac_start_int + i * mac_step
            #     if tg_if_to_vlan_mac_int > mac_limit_int:
            #         mac_limit_reached = True
            ...
            #         tg_if_to_vlan_macs.append(
            #             L2Util.int_to_mac(tg_if_to_vlan_mac_int))
            ...
            ip_config = 'sw_interface_add_del_address sw_if_index {sw_idx} ' \
                        '{ip}/32'.format(sw_idx=sw_idx_vxlan, ip=src_ip)
            commands.append(ip_config + '\n')
            vni = vni_start + i
            vxlan_tunnel = 'vxlan_add_del_tunnel src {src_ip} dst {dst_ip} ' \
                           'vni {vni}'.format(
                            src_ip=src_ip, dst_ip=dst_ip, vni=vni)
            commands.append(vxlan_tunnel + '\n')
            if n_tunnels > 1:
                vlan = i + 1
                # sub_if = 'exec create sub-interfaces {name} {sub_id}'.format(
                #           name=nonvxlan_subif_name, sub_id=sub_id)
                # commands.append(sub_if + '\n')
                vlan_subif = 'create_vlan_subif sw_if_index {sw_idx} vlan ' \
                             '{vlan}'.format(sw_idx=sw_idx_vlan, vlan=vlan)
                commands.append(vlan_subif + '\n')
        ...
        for i in range(0, vxlan_count):
            ...
            vlan_subif_name = '{if_name}.{vlan}'.format(
                if_name=vlan_if_name, vlan=i+1) if n_tunnels > 1 else ''
            vlan_found = False
            vlan_idx = None
            for data in if_data:
                ...
                if not vxlan_found and if_name == vxlan_subif_name:
                    ...
                    vxlan_found = True
                elif n_tunnels > 1 and not vlan_found\
                        and if_name == vlan_subif_name:
                    vlan_idx = data['sw_if_index']
                    vlan_found = True
                if vxlan_found and (n_tunnels == 1 or vlan_found):
                    break
            ...
            vxlan_up = 'sw_interface_set_flags sw_if_index {sw_idx} ' \
                       'admin-up link-up'.format(sw_idx=vxlan_subif_idx)
            commands.append(vxlan_up + '\n')
            if n_tunnels > 1:
                Topology.update_interface_sw_if_index(
                    node, vlan_subif_key, vlan_idx)
                Topology.update_interface_name(
                    node, vlan_subif_key, vlan_subif_name)
                vlan_up = 'sw_interface_set_flags sw_if_index {sw_idx} ' \
                          'admin-up link-up'.format(sw_idx=vlan_idx)
                commands.append(vlan_up + '\n')
        ...
        for i in range(0, vxlan_count):
            arp = 'ip_neighbor_add_del sw_if_index {sw_idx} dst {ip} ' \
                  'mac {mac}'.format(sw_idx=sw_idx_vxlan, ip=dst_ips[i],
                                     mac=op_node_if_mac)
            commands.append(arp + '\n')
            route = 'ip_add_del_route {ip}/32 via {ip} sw_if_index {sw_idx}' \
                    ' resolve-attempts 10 count 1'.format(
                     ip=dst_ips[i], sw_idx=sw_idx_vxlan)
            commands.append(route + '\n')
            bd_id = bd_id_start + i
            subif_id = i + 1
            sw_idx_vxlan_subif = Topology.get_interface_sw_index(
                node, 'vxlan_tunnel{nr}'.format(nr=subif_id))
            vxlan_to_bd = 'sw_interface_set_l2_bridge ' \
                          'sw_if_index {sw_idx} bd_id {bd_id} shg 0 ' \
                          'enable'.format(
                           sw_idx=sw_idx_vxlan_subif, bd_id=bd_id)
            commands.append(vxlan_to_bd + '\n')

            nonvxlan_subif_key = 'vlan_subif{nr}'.format(nr=subif_id) \
                if n_tunnels > 1 else node_vlan_if
            sw_idx_nonvxlan = Topology.get_interface_sw_index(
                node, nonvxlan_subif_key)
            nonvxlan_to_bd = 'sw_interface_set_l2_bridge ' \
                             'sw_if_index {sw_idx} bd_id {bd_id} shg 0 ' \
                             'enable'.format(
                              sw_idx=sw_idx_nonvxlan, bd_id=bd_id)
            commands.append(nonvxlan_to_bd + '\n')
            # if n_tunnels > 1:
            #     tg_if1 = tg_if_to_vlan_macs[i]
            #     l2fib_entry1 = 'l2fib_add_del mac {mac} bd_id {bd_id} ' \
            #                    'sw_if_index {sw_idx}'.format(
            #                     mac=tg_if1, bd_id=bd_id, sw_idx=sw_idx_nonvxlan)
            #     commands.append(l2fib_entry1 + '\n')
            #     tg_if2 = tg_if_to_vxlan_macs[i]
            #     l2fib_entry2 = 'l2fib_add_del mac {mac} bd_id {bd_id} ' \
            #                    'sw_if_index {sw_idx}'.format(
            #                     mac=tg_if2, bd_id=bd_id,
            #                     sw_idx=sw_idx_vxlan_subif)
            #     commands.append(l2fib_entry2 + '\n')
