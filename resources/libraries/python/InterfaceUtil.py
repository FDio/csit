...
class InterfaceUtil(object):
    ...
    def vpp_create_avf_interface(node, vf_pci_addr):
        ...
        logger.trace('Output: {output}'.format(output=output))
        logger.trace('Output: {output}'.format(output=output[0]))
    ...
    def set_linux_interface_mac(node, interface, mac, namespace=None,
        ...
        :type vf_id: int
        """
        mac_str = 'vf {} mac {}'.format(vf_id, mac) if vf_id is not None \
            else 'address {}'.format(mac)
        ns_str = 'ip netns exec {}'.format(namespace) if namespace else ''

        cmd = '{} ip link set {} {}'.format(ns_str, interface, mac_str)
        exec_cmd_no_error(node, cmd, sudo=True)
