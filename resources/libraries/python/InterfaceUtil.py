...
class InterfaceUtil(object):
    ...
    def create_vlan_subinterface(node, interface, vlan):
        ...
        sw_vlan_idx = papi_resp['sw_if_index']
        ...
        return '{ifc}.{vlan}'.format(ifc=interface, vlan=vlan), sw_vlan_idx
    ...
    def create_vxlan_interface(node, vni, source_ip, destination_ip):
        ...
        args = dict(is_add=1,
                    ...
                    vni=int(vni))
        err_msg = 'Failed to create VXLAN interface on host {host}'.format(
            host=node['host'])
        with PapiExecutor(node) as papi_exec:
            papi_resp = papi_exec.add(cmd, **args).get_replies(err_msg).\
                verify_reply(err_msg=err_msg)
    ...
    def vxlan_dump(node, interface=None):
        ...
        for item in papi_vxlan_dump:
            if interface is None:
                ...
                return process_vxlan_dump(item[cmd_reply])
    ...
    def tap_dump(node, name=None):
        ...
        def process_tap_dump(tap_dump):
            ...
            tap_dump['host_mac_address'] = \
                L2Util.bin_to_mac(tap_dump['host_mac_address'])
            tap_dump['host_ip4_addr'] = \
                inet_ntop(AF_INET, tap_dump['host_ip4_addr']['ip'][0:4])
            tap_dump['host_ip6_addr'] = \
                inet_ntop(AF_INET6, tap_dump['host_ip6_addr']['ip'])
            return tap_dump
        ...
        for item in papi_tap_dump:
            if name is None:
                ...
                return process_tap_dump(item[cmd_reply])
    ...
    def create_subinterface(node, interface, sub_id, outer_vlan_id=None,
        ...
        no_tags = 1 if type_subif == 'no_tags' else 0
        one_tag = 1 if type_subif == 'one_tag' else 0
        two_tags = 1 if type_subif == 'two_tags' else 0
        dot1ad = 1 if type_subif == 'dot1ad' else 0  # 0=dot1q, 1=dot1ad
        exact_match = 1 if type_subif == 'exact_match' else 0
        default_sub = 1 if type_subif == 'default_sub' else 0
