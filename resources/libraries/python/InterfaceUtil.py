...
class InterfaceUtil:
    ...
    def vpp_sw_interface_set_rx_placement(
        ...
        logger.debug(f"sw_interface_set_rx_placement args: {args}")
        with PapiSocketExecutor(node) as papi_exec:
            papi_exec.add(cmd, **args).get_reply(err_msg)
        ...
        for placement in InterfaceUtil.vpp_sw_interface_rx_placement_dump(node):
            for interface in node[u"interfaces"].values():
                if placement[u"sw_if_index"] == interface[u"vpp_sw_index"] \
                    ...
                    id_counter += 1
        placement = InterfaceUtil.vpp_sw_interface_rx_placement_dump(node)
        logger.debug(f"new placement str: {placement}")
        logger.debug(f"new placement repr: {placement}")
        return worker_ids
    ...
    def vpp_round_robin_rx_placement_on_all_duts(
        ...
        for name, node in nodes.items():
            if node[u"type"] == NodeType.DUT:
                ...
                logger.debug(f"cpu_ids: {cpu_ids}")
