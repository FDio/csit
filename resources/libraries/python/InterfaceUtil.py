...
class InterfaceUtil(object):
    ...
    def vpp_create_multiple_vxlan_ipv4_tunnels(
        ...
        vxlan_count, dst_ip_list = \
            InterfaceUtil.vpp_create_vxlan_and_vlan_interfaces(
        ...
        InterfaceUtil.vpp_put_vxlan_and_vlan_interfaces_to_bridge_domain(
            node, node_vxlan_if, vxlan_count, op_node, op_node_if, dst_ip_list,
            bd_id_start)
    ...
    def vpp_create_vxlan_and_vlan_interfaces(
        ...
        :type ip_step: int
        :type ip_limit: str
        :returns: Number of created VXLAN interfaces and list of destination IP
        addresses.
        :rtype: tuple
        """
        commands = list()
        ...
        for i in range(0, vxlan_count):
            ...
            #             '{ip}/32'.format(
            #     sw_idx=Topology.get_interface_sw_index(node, node_vxlan_if), ip=src_ip)
            ...
            #                'vni {vni}'.format(
            #                    src_ip=src_ip, dst_ip=dst_ip, vni=vni_start + i)
            ...
            #              '{vlan}'.format(sw_idx=Topology.get_interface_sw_index(node, node_vlan_if), vlan=i + 1)
    ...
    def vpp_put_vxlan_and_vlan_interfaces_to_bridge_domain(
            node, node_vxlan_if, vxlan_count, op_node, op_node_if, dst_ips,
            bd_id_start):
        """
        ...
        :param op_node_if: Opposite VPP node interface key for VXLAN tunnel
        interfaces.
        :param dst_ips: Destination IP addresses.
        :param bd_id_start: Bridge-domain ID start.
        :type node: dict
        :type node_vxlan_if: str
        :type vxlan_count: int
        ...
        for i in range(0, vxlan_count):
            ...
            commands.append(
                ...
                .format(sw_idx=sw_idx_vxlan, ip=dst_ips[i],
                        mac=Topology.get_interface_mac(op_node, op_node_if)))
            ...
            #     node, 'vlan_subif{nr}'.format(nr=subif_id))
            # vlan_to_bd = 'sw_interface_set_l2_bridge ' \
            #              'sw_if_index {sw_idx} bd_id {bd_id} shg 0 ' \
            #              'enable'.format(sw_idx=sw_idx_vlan, bd_id=bd_id)
            # commands.append(vlan_to_bd + '\n')
            commands.append(
                'sw_interface_set_l2_bridge sw_if_index {sw_idx} bd_id {bd_id} '
                'shg 0 enable\n'.format(sw_idx=Topology.get_interface_sw_index(
