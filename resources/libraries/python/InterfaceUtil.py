...
class InterfaceUtil:
    ...
    def init_generic_interface(node, ifc_key, numvfs=0, osi_layer=u"L2", strict=True):
        ...
        for vf_id in range(numvfs):
            ...
            cmd = f"ethtool -n {linuxname} rx-flow-hash udp4"
            exec_cmd(node, cmd, sudo=True)
            cmd = f"ethtool -n {linuxname} rx-flow-hash esp4"
            exec_cmd(node, cmd, sudo=True)
            cmd = f"ethtool -N {linuxname} rx-flow-hash esp4 s d"
            exec_cmd(node, cmd, sudo=True)
            cmd = f"ethtool -N {linuxname} rx-flow-hash udp4 s d f n"
            exec_cmd(node, cmd, sudo=True)
            cmd = f"ethtool -n {linuxname} rx-flow-hash esp4"
            exec_cmd(node, cmd, sudo=True)
            cmd = f"ethtool -n {linuxname} rx-flow-hash udp4"
