...
class InterfaceUtil(object):
    ...
    def set_interface_state(node, interface, state, if_type='key'):
        ...
        if if_type == 'key':
            ...
            raise ValueError('Unknown if_type: {}'.format(if_type))
    ...
    def vpp_node_interfaces_ready_wait(node, timeout=30):
        ...
        while not if_ready:
            ...
            if not not_ready:
                if_ready = True
            if not if_ready and now - start > timeout:
                raise RuntimeError('Timeout, interfaces not up: {0}'.
                                   format(not_ready))
    ...
    def vpp_get_interface_ip_addresses(node, interface, ip_version):
        """Get list of IP addresses from an interface on a VPP node.z
