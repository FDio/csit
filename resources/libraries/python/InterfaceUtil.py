...
class InterfaceUtil:
    ...
    def vpp_create_bond_interface(
        ...
        try:
            ...
            old_cmd = u"bond_create"
            args.pop(u"enable_gso")
            with PapiSocketExecutor(node) as papi_exec:
                sw_if_index = \
                    papi_exec.add(old_cmd, **args).get_sw_if_index(err_msg)
    ...
    def vpp_show_bond_data_on_node(node, verbose=False):
        ...
        members = u"members"
        active_members = u"active_members"
        ...
        try:
            ...
            # Perhaps VPP is an older version
            old_cmd = u"sw_interface_bond_dump"
            members = u"slaves"
            active_members = u"active_slaves"
            with PapiSocketExecutor(node) as papi_exec:
                details = papi_exec.add(old_cmd).get_details(err_msg)
        ...
        for bond in details:
            ...
            data += f"  number of active {members}: {bond[active_members]}\n"
            if verbose:
            ...
            data += f"  number of {members}: {bond[members]}\n"
            if verbose:
                for member in member_data:
                    data += f"    {member[u'interface_name']}\n"
    ...
    def vpp_bond_member_dump(node, interface):
        ...
        try:
            ...
            # Perhaps VPP is an older version
            old_cmd = u"sw_interface_slave_dump"
            member = u"Slave"
            with PapiSocketExecutor(node) as papi_exec:
                details = papi_exec.add(old_cmd, **args).get_details(err_msg)
