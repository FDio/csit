...
import resources.libraries.python.DropRateSearch as DropRateSearch

__all__ = ['TrafficGenerator', 'TGDropRateSearchImpl', 'OptimizedSearch']
...
class TGDropRateSearchImpl(DropRateSearch.DropRateSearch):
...
class TrexRateProvider(DropRateSearch.AbstractRateProvider):
    """Rate provider which uses Trex for rate measurements."""

    def __init__(self, frame_size, traffic_type, warmup_time=0):
    ...
    def measure(self, duration, transmit_rate):
        """Run Trex measurement, parse results, return ReceiveRateMeasurement object."""
        # we need instance of TrafficGenerator instantiated by Robot Framework
        # to be able to use trex_stl-*()
        tg_instance = BuiltIn().get_library_instance(
            'resources.libraries.python.TrafficGenerator')
        if tg_instance.node['subtype'] is None:
            ...
            raise NotImplementedError("TG subtype not supported")
        # TODO: Do we want to upoort other rate types, when Tr=Tx/d implies pps?
        unit_rate = str(transmit_rate) + "pps"
        tg_instance.trex_stl_start_remote_exec(duration, unit_rate, self.frame_size,
                                               self.traffic_type, self.warmup_time)
        tx = tg_instance.get_sent()
        dx = tg_instance.get_loss()
        # TODO: Figure out how to also report latency.
        measuremenmt = DropRateSearch.ReceiveRateMeasurement(duration, transmit_rate, tx, dx)
...
class OptimizedSearch(object):
    ...
    def perform_optimized_ndr_pdr_search(self, frame_size, fail_rate, line_rate, traffic_type, allowed_drop_fraction):
        """Create Trex provider, initialize optimized search, find intervals and return them."""
        rate_provider = TrexRateProvider(int(frame_size), traffic_type)
        algorithm = DropRateSearch.OptimizedSearchAlgorithm(rate_provider)
        result = algorithm.narrow_down_ndr_and_pdr(float(fail_rate), float(line_rate), float(allowed_drop_fraction))
        return result
