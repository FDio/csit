...
class TrafficGenerator(AbstractMeasurer):
    ...
    def _compute_duration(self, duration, multiplier):
        ...
        The final resut is influenced by transaction scale and duration limit.
    ...
    def trex_astf_start_remote_exec(
        ...
        computed_duration = self._compute_duration(duration, multiplier)
    ...
    def trex_stl_start_remote_exec(self, duration, rate, async_call=False):
        ...
        # TODO: Refactor the code so duration is computed only once,
        # and both the initial and the computed durations are logged.
        duration = self._compute_duration(duration=duration, multiplier=rate)
    ...
    def _send_traffic_on_tg_with_ramp_up(
        ...
        if self.ramp_up_rate:
            ...
            if self.ramp_up_start is None:
                ...
                computed_duration = self._compute_duration(
                    duration=duration,
                    multiplier=rate,
                )
                # There are two conditions for inserting ramp-up.
                # If early sessions are expiring already,
                # or of late sessions are to expire before measurement is over.
                ...
                if bigger_delay >= self.state_timeout:
                    ramp_up_neded = True
                    logger.trace(f"delays: start {ramp_up_start_delay} stop {ramp_up_stop_delay}")
            if ramp_up_neded:
                logger.debug(
                ...
                logger.trace(f"Zero loss last started {self.ramp_up_start} ended {self.ramp_up_stop}")
        if ramp_up_only:
            return None
        ...
        # Optimisation: No loss acts as a good ramp-up.
        if result is not None and result.loss_count == 0:
            logger.trace(u"Good trial acts as a ramp-up")
            self.ramp_up_start = trial_start
            self.ramp_up_stop = trial_end
        else:
            logger.trace(u"Loss detected, not usable as ramp-up.")
        return result
...
class OptimizedSearch:
    ...
    def perform_optimized_ndrpdr_search(
        ...
        if transaction_scale:
            ...
            # TODO: What is too much? Depend on transaction_scale?
            timeout = 5400.0
