    ...
    def trex_stl_start_remote_exec(self, duration, rate, framesize,
        ...
        if traffic_type in ["3-node-xconnect", "3-node-bridge"]:
            ...
            (ret, stdout, stderr) = ssh.exec_command(
                ...
                "--p{4}_dst_end_ip 2001:2::27:0F "
                "--p{5}_src_start_ip 2001:2::1 "
                "--p{5}_dst_start_ip 2001:1::0 "
                "--p{5}_dst_end_ip 2001:1::27:0F "
                "{6} --warmup_time={7}'".format(Constants.REMOTE_FW_DIR,
                                                duration, rate, framesize, _p0,
                                                _p1, _async, warmup_time),
                timeout=int(duration)+60)
            ...
            (ret, stdout, stderr) = ssh.exec_command(
                ...
                "--p{4}_src_start_ip 2001:1::1 "
                "--p{4}_dst_start_ip 2001:2::0 "
                "--p{4}_dst_end_ip 2001:2::1:86:9F "
                "--p{5}_src_start_ip 2001:2::1 "
                "--p{5}_dst_start_ip 2001:1::0 "
                "--p{5}_dst_end_ip 2001:1::1:86:9F "
                "{6} --warmup_time={7}'".format(Constants.REMOTE_FW_DIR,
                                                duration, rate, framesize, _p0,
                                                _p1, _async, warmup_time),
                timeout=int(duration)+60)
            ...
            (ret, stdout, stderr) = ssh.exec_command(
                ...
                "--p{4}_src_start_ip 2001:1::1 "
                "--p{4}_dst_start_ip 2001:2::0 "
                "--p{4}_dst_end_ip 2001:2::F:42:3F "
                "--p{5}_src_start_ip 2001:2::1 "
                "--p{5}_dst_start_ip 2001:1::0 "
                "--p{5}_dst_end_ip 2001:1::F:42:3F "
                "{6} --warmup_time={7}'".format(Constants.REMOTE_FW_DIR,
                                                duration, rate, framesize, _p0,
                                                _p1, _async, warmup_time),
                timeout=int(duration)+60)
