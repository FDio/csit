...
class TrafficGenerator(AbstractMeasurer):
    ...
    def trex_astf_start_remote_exec(
        ...
        if computed_duration > 0.0:
            ...
            computed_duration = duration + 0.3
        # Else keep -1.
        if self.duration_limit:
            computed_duration = min(computed_duration, self.duration_limit)
        logger.debug(f"duration_limit {self.duration_limit} transaction scale {self.transaction_scale} multiplier {multiplier} divided {self.transaction_scale / multiplier} computed {computed_duration}")
...
class OptimizedSearch:
    ...
    def perform_optimized_ndrpdr_search(
        ...
        result = algorithm.narrow_down_ndr_and_pdr(
            minimum_transmit_rate=minimum_transmit_rate,
            maximum_transmit_rate=maximum_transmit_rate,
