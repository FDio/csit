...
class TrafficGenerator(AbstractMeasurer):
    ...
    def initialize_traffic_generator(self, osi_layer, loop=True):
        """TG initialization.

        :param osi_layer: 'L2', 'L3' or 'L7' - OSI Layer testing type.
        :para loop: True, if the topology is looped from TG to TG.
        :type osi_layer: str
        :type loop: bool
        ...
        if subtype == NodeSubTypeTG.TREX:
            ...
            if osi_layer == "L2":
                tg_if1_adj_addr = topology["TG_pf2_mac"][0]
                tg_if2_adj_addr = topology["TG_pf1_mac"][0]
                ...
                if loop:
                    ...
                    tg_if1_adj_addr = topology[f"DUT1_{ifl}1_mac"][0]
                    tg_if2_adj_addr = topology[f"DUT{last}_{ifl}2_mac"][0]
            else:
                raise ValueError("Unknown OSI layer!")

            trex_topology = list()
            trex_topology.append(
                dict(interface=topology["TG_pf1"][0], dst_mac=tg_if1_adj_addr)
            )
            trex_topology.append(
                dict(interface=topology["TG_pf2"][0], dst_mac=tg_if2_adj_addr)
            )
            if1_pci = topology["TG_pf1_pci"][0]
            if2_pci = topology["TG_pf2_pci"][0]
