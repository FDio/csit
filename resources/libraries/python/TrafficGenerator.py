...
class TrafficGenerator(AbstractMeasurer):
    ...
    def __init__(self):
        ...
        self._received = None
        # T-rex astf part
        self._c_active_flows = None
        self._c_established_flows = None
        self._s_active_flows = None
        self._s_established_flows = None
        self._c_udp_established_flows = None
        self._c_udp_closed_flows = None
        self._s_udp_accepted_flows = None
        self._s_udp_closed_flows = None
        self._c_tcp_initiated_flows = None
        self._c_tcp_established_flows = None
        self._c_tcp_closed_flows = None
        self._s_tcp_accepted_flows = None
        self._s_tcp_established_flows = None
        self._s_tcp_closed_flows = None
    ...
    def initialize_traffic_generator(
        ...
        if subtype == NodeSubTypeTG.TREX:
            ...
            elif osi_layer == u"L3" or osi_layer == u"L7":
            ...
            if osi_layer in (u"L2", u"L3", u"L7"):
                exec_cmd_no_error(
                    ...
                    f"  port_info:\n"
                    f"      - dest_mac: {if1_adj_addr}\n"
                    f"        src_mac: {if1_addr}\n"
                    f"      - dest_mac: {if2_adj_addr}\n"
                    f"        src_mac: {if2_addr}\n"
                    f"  platform :\n"
        ...
        if subtype == NodeSubTypeTG.TREX:
            for _ in range(0, 3):
                ...
                if osi_layer in (u"L2", u"L3"):
                    cmd += f"trex_stateless_assert.py"
                elif osi_layer == u"L7":
    ...
    def _parse_traffic_results(self, stdout):
        ...
        self._received = self._result.get(u"totalReceived")
        self._sent = self._result.get(u"totalSent")
        self._loss = self._result.get(u"frameLoss")
        self._latency = list()
        self._latency.append(self._result.get(u"latencyStream0(usec)"))
        self._latency.append(self._result.get(u"latencyStream1(usec)"))
        # Additional results from t-rex astf
        self._c_active_flows = self._result.get(u"clientActiveFlows")
        self._c_established_flows = self._result.get(u"clientEstablishedFlows")
        self._s_active_flows = self._result.get(u"serverActiveFlows")
        self._s_established_flows = self._result.get(u"serverEstablishedFlows")
        self._c_udp_established_flows = self._result.get(u"clientUdpConnects")
        self._c_udp_closed_flows = self._result.get(u"clientUdpClosed")
        self._s_udp_accepted_flows = self._result.get(u"serverUdpAccepts")
        self._s_udp_closed_flows = self._result.get(u"serverUdpClosed")
        self._c_tcp_initiated_flows = self._result.get(u"clientTcpConnectInits")
        self._c_tcp_established_flows = self._result.get(u"clientTcpConnects")
        self._c_tcp_closed_flows = self._result.get(u"clientTcpClosed")
        self._s_tcp_accepted_flows = self._result.get(u"serverTcpAccepts")
        self._s_tcp_established_flows = self._result.get(u"serverTcpConnects")
        self._s_tcp_closed_flows = self._result.get(u"serverTcpClosed")
    ...
    def trex_astf_start_remote_exec(
        ...
        """
        reorder = self._ifaces_reordered
    ...
    def trex_stl_start_remote_exec(
        ...
        stdout, _ = exec_cmd_no_error(
            ...
            message=u"T-Rex stateless runtime error"
        )
    ...
    def measure(self, duration, transmit_rate):
        ...
        :param transmit_rate: Target aggregate transmit rate [pps] (T-Rex
            stateless) or multiplier of profile CPS (T-Rex astf).
        :type duration: float
