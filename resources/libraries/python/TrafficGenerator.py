...
class TrafficGenerator(AbstractMeasurer):
    ...
    def trex_stl_start_remote_exec(self, duration, rate, async_call=False, delay=None):
        ...
        command_line.add_if(u"latency", self.use_latency)
        command_line.add_if(u"force", Constants.TREX_SEND_FORCE)
        command_line.add_with_value(u"delay", Constants.PERF_TRIAL_STL_DELAY if delay is None else delay)
    ...
    def _send_traffic_on_tg_internal(
            self, duration, rate, async_call=False, delay=None):
        """Send traffic from all configured interfaces on TG.
    ...
    def _send_traffic_on_tg_with_ramp_up(
            self, duration, rate, async_call=False, ramp_up_only=False, delay=None):
    ...
    def measure(self, duration, transmit_rate, delay=None):
        ...
        result = self._send_traffic_on_tg_with_ramp_up(
            ...
            rate=14763779.5,
            async_call=False,
            delay=1.0 - 2.0 * (transmit_rate / 18750000.0),
        )
        self._rate = transmit_rate
