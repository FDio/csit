...
class TrafficGeneratorDriver(object):
    ...
    def match_node(self, node):
        ...
        return (node is self._node
                or node['host'] == self._node['host']
                and node['port'] == self._node['port'])
    ...
    def setup(self, osi_layer, links):
        ...
        including suite-level network addressing values. The values are picked
...
class TrexDriver(TrafficGeneratorDriver):
    ...
    def teardown(self):
        cmd = "sh -c 'if pid=$(pgrep t-rex) ; then kill ${pid} && sleep 3 ; fi'"
...
# TODO: this would be more useful as part of a suite of classes in topology.
...
# TODO: this could be a subclass of typing.NamedTuple, with hacking in
# DirectedLink.__new__.__defaults__ = (None,) for dest_mac.
