...
class TrafficGenerator(AbstractMeasurer):
    ...
    def trex_stl_start_remote_exec(
            ...
            latency=True, warmup_time=5.0, unidirection=False):
        ...
        _p0, _p1 = (1, 0) if self._ifaces_reordered else (0, 1)
        profile_path = ("{0}/resources/traffic_profiles/trex/{1}.py".format(
            Constants.REMOTE_FW_DIR, traffic_type))
        command = ("sh -c '{0}/resources/tools/trex/trex_stateless_profile.py"
            + " --profile {1} --duration {2} --frame_size {3} --rate {4}"
            + " --warmup_time {5} --port_0 {6} --port_1 {7}").format(
                Constants.REMOTE_FW_DIR, profile_path, duration, framesize,
                rate, warmup_time, _p0, _p1)
        if async_call:
            command += " --async"
        if latency:
            command += " --latency"
