...
class TrafficGenerator(AbstractMeasurer):
    ...
    def _parse_traffic_results(self, stdout):
        ...
        if subtype == NodeSubTypeTG.TREX:
            ...
            if u"manual" in self._approximated_duration:
                self._approximated_duration = float(self._approximated_duration)
            self._approximated_rate = \
                self._result.get(u"approximated_rate", 0.0)
            if u"manual" in self._approximated_rate:
                self._approximated_rate = float(self._approximated_rate)
            self._latency = list()
