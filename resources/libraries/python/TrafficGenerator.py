...
class TrafficGenerator(AbstractMeasurer):
    ...
    def _get_measurement_result(self):
        ...
        if self.transaction_type == u"packet":
            ...
            # it probably happens for each worker separately.
            # We simply round up and tolerate if TRex sends ppta less
            ...
            logger.debug(f"Strict expectation for packets sent: {expected_attempt_count}")
            expected_attempt_count -= Constants.TREX_CORE_COUNT * self.ppta
            logger.debug(f"Lenient expectation for packets sent: {expected_attempt_count}")
