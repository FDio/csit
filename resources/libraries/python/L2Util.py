...
class L2Util(object):
    ...
    def setup_network_namespace(node, namespace_name, vhost_if, ip_address,
                                prefix):
        """Setup namespace on given node and attach vhost interface and IP to
        this namespace.
        ...
        cmd = ('ip link set dev {0} up netns {1}'.format(vhost_if,
                                                         namespace_name))
        exec_cmd_no_error(node, cmd, sudo=True)

        cmd = ('ip netns exec {0} ip addr add {1}/{2} dev {3}'.format(
            namespace_name, ip_address, prefix, vhost_if))
        exec_cmd_no_error(node, cmd, sudo=True)
    ...
    @staticmethod
    def l2_tag_rewrite(node, interface, tag_rewrite_method, tag_id=None):
        ...
        :param tag_id: Optional tag1 ID for vlan
        :type node: dict
        :type interface: str or int
