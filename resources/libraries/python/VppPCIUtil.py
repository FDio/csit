...
class VppPCIUtil(object):
    ...
    def __init__(self, node):
        ...
        for d in self._kernel_devices:
            ...
            for i in d['interface'].split(','):
                ...
                if ret != 0:
                    logger.debug('Could not execute ip addr command on {}'.format(node['host']))
                    logger.debug('stdout: {}'.format(stdout))
                    logger.debug('stderr: {}'.format(stderr))
                    raise RuntimeError('Could not execute DPDK script on node {}'.format(node['host']))
                ...
                addr = re.findall(r'link/ether \w+:\w+:\w+:\w+:\w+:\w+', stdout)[0].split(' ')[1]
                l2addr.append(addr)
            d['l2_addr'] = l2addr
    ...
    def show_vpp_devices(devices):
        ...
        header = "{:15} {:25} {:50}".format("PCI ID", "Kernel Interface(s)", "Description")
    ...
    Rebind the devices being used by vpp.
    '''

    @staticmethod
    def rebind_vpp_devices(node, devices):

        ssh = SSH()
        ssh.connect(node)

        for d in devices:
            unused = d['unused'].split(',')[0]
            cmd = DPDK_SCRIPT + ' -b ' + unused + ' ' + d['id']
            (ret, stdout, stderr) = ssh.exec_command(cmd)
            if ret != 0:
                logger.debug('Could not execute DPDK script on {}'.format(node['host']))
                logger.debug('stdout: {}'.format(stdout))
