...
class LispRemoteMapping(object):
    ...
    def vpp_add_lisp_remote_mapping(node, vni, deid, deid_prefix, seid,
        ...
        if not is_mac:
            eid_type = 0 if ip_address(str(deid)).version == 4 else 1
            eid_packed = ip_address(str(deid)).packed
            seid_packed = ip_address(str(seid)).packed
            eid_len = deid_prefix
        ...
        rlocs = [dict(is_ip4=1 if ip_address(str(rloc)).version == 4 else 0,
...
class LispAdjacency(object):
    ...
    def vpp_add_lisp_adjacency(node, vni, deid, deid_prefix, seid,
        ...
        if not is_mac:
            eid_type = 0 if ip_address(str(deid)).version == 4 else 1
            reid = ip_address(str(deid)).packed
            leid = ip_address(str(seid)).packed
...
class LispMapResolver(object):
    ...
    def vpp_add_map_resolver(node, map_resolver_ip):
        ...
        args = dict(is_add=1,
                    is_ipv6=0 if ip_address(str(map_resolver_ip)).version \
                                 == 4 else 1,
                    ip_address=ip_address(str(map_resolver_ip)).packed)
...
class LispLocalEid(object):
    ...
    def vpp_add_lisp_local_eid(node, locator_set_name, vni, eid,
        ...
        if prefix_len:
            eid_type = 0 if ip_address(str(eid)).version == 4 else 1
            eid_packed = ip_address(str(eid)).packed
        else:
            eid_type = 2
...
class LispSetup(object):
    ...
    def vpp_set_lisp_map_resolver(node, map_resolver):
        ...
        for map_ip in map_resolver:
            lisp_map_res.vpp_add_map_resolver(node,
                                              map_ip.get(u"map resolver"))
    ...
    def vpp_unset_lisp_map_resolver(node, map_resolver):
        ...
        lisp_map_res = LispMapResolver()
        for map_ip in map_resolver:
            lisp_map_res.vpp_del_map_resolver(node,
                                              map_ip.get(u"map resolver"))
