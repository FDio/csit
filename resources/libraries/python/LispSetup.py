...
# used in /csit/tests/vpp/func/ip4_tunnels/lisp/eth2p-ethip4lisp-l2bdbasemaclrn-func.robot
        ...
        cmd = 'lisp_eid_table_vni_dump'
        err_msg = "Failed to get LISP vni dump on host {host}".format(
            host=node['host'])
        with PapiSocketExecutor(node) as papi_exec:
        ...
        args = dict(is_l2=0)
        cmd = 'lisp_eid_table_map_dump'
        err_msg = "Failed to get LISP eid map dump on host {host}".format(
            host=node['host'])
        with PapiSocketExecutor(node) as papi_exec:
