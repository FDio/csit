...
class NodePath:
    ...
    def compute_circular_topology(
        self, nodes, filter_list=None, path_count=1):
        """Return computed circular path.

        :param nodes: Nodes to append to the path.
        :param filter_list: Filter criteria list.
        :param path_count: Number of links used.
        :type nodes: dict
        :type filter_list: list of strings
        :type path_count: int
        ...
        n_dict["duts"] = duts
        n_dict["duts_count"] = len(duts)
        n_dict["layer"] = "pf"

        for idx in range(0, path_count):
        ...
        while True:
            ...
            n_dict[f"{n_pfx}"] = node
            n_dict[f"{n_pfx}_{p_pfx}"] = [interface]
            n_dict[f"{n_pfx}_{p_pfx}_mac"] = \
                [Topology.get_interface_mac(node, interface)]
            # Backward compatibility below
            n_dict[f"{n_pfx}"] = node
            n_dict[f"{n_pfx}_{i_pfx}"] = interface
            n_dict[f"{n_pfx}_{i_pfx}_mac"] = \
                Topology.get_interface_mac(node, interface)
