...
class MlrSearch:
    ...
    def iterate_search(
        ...
        try:
            duration, load = self.generator.send(trial_result)
            return ((duration, load), None)
    ...
    def iperf_result_into_measurement(
        self, sec: float, mbps: float, dur: float, lbps: float
    ) -> MeasurementResult:
        """FIXME"""
        return MeasurementResult(
            intended_duration=dur,
            intended_load=lbps,
            # TODO: Distinguish unsent and lost data.
            offered_count=dur * lbps,
            forwarding_count=sec * mbps,
            offered_duration=sec,
            # intended_count=dur * lbps,
        )
