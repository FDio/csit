...
from .MLRsearch import (
    ...
    MultipleLossRatioSearch, Pep3140Dict, SearchGoal,
)
...
class MlrSearch:
    ...
    def iterate_search(trial_result: MeasurementResult) -> Union[Tuple[Tuple[float, float], None], Tuple[None, Tuple[GoalResult, ...]]]:
        """FIXME"""
        if not self.generator:
            raise RuntimeError("No active search goals.")
        try:
            self.duration, self.load = self.generator(trial_result)
    ...
    def iperf_result_into_measurement(bps, sec):
        """FIXME"""
