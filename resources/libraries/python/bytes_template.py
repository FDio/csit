        ...
        for index in range(length):
            if inequals and first[index] == second[index]:
                ...
                try:
                    ...
                    logger.trace(f"first {first!r}")
                    logger.trace(f"second {second!r}")
                    logger.trace(f"reserve_1 {reserve_1!r}")
                    logger.trace(f"reserve_2 {reserve_2!r}")
                    logger.trace(f"inequals {inequals!r}")
                    logger.trace(f"numbers {numbers!r}")
                    logger.trace(f"offset {offset}")
                    logger.trace(f"text {text!r}")
                    logger.trace(f"template {template!r}")
                    raise
                list_tuple = [
