...
class BytesTemplate:
    ...
    The first (int, int) element (unless optionally disabled)
    is assumed to be context ID, external source for that is needed
    (as it can be shared with other code).

    See from_template docstring for limitations of the current implementation.
    ...
    def __init__(self, template):
        ...
        No real logic here, template is assumed to be a list of correct types.
    ...
    def __str__(self):
        ...
        The bytes string are written without quotes, in hex form.
    ...
    def from_two_messages(cls, first, second):
        """Return new template instance fitting the two bytes string messages.

        The key issue here is distinguishing between static and dynamic content.
        The current implementation compares the two messages byte-by-byte,
        and most of the time it assumes the dynamic field will differ
        either in all bytes, or at least in the last byte
        (the least significant byte of u32 in network order).

        There are two patterns that violate that.

        The first violating pattern is creation of random security keys.
        Currently, there is a workaround in IPsecUtils that makes sure
        the second "random" key differs in every byte from the previous one.

        The second violating pattern is a value incrementing
        by a multiple of 256, typically IPv4 address jumping over /24 ranges.
        This happens to work if the previous field is static.
        There is no easy fix if the previous field is not static.
        But currently no test is failing due to this,
        so no workaround is applied.
        ...
        # Numbers always contain inequal bytes, but sometimes also equal bytes.
