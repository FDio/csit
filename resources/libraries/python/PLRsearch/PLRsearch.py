...
class PLRsearch(object):
    ...
    Two constants are stored as slass fields for speed.

    Method othed than search (and than __init__) are just
    ...
    def __init__(
        ...
        # TODO: Pass the following as an explicit argument,
        #       or re-structure the variables and data fields overall.
        self.packet_loss_per_second_target = None
    ...
    def search(self, min_rate, max_rate):
        ...
        When time is up, the last estimate is returned,
        ...
        upper limit on rate of packets processed), there are still
        too many different shapes of possible loss functions
        that full Bayesian reasoning is not tractable.
        ...
        First measurement is done at max_rate, next few measurements
        ...
        as linear approximation of the fitting function becomes good enough
        (thus reducing impact of the overall shape of fitting function).
        ...
        while 1:
            ...
            self.packet_loss_per_second_target = (
            ...
            if (trial_number - self.trial_number_offset) <= 3:
    ...
    def lfit_stretch(load, mrr, spread):
        ...
        :param spread: The x-scaling parameter (positive). No nice semantics,
            roghly corresponds to size of "tail" for loads below mrr.
        :type load: float
        :type mrr: float
        :type spread: float
...
#                print "found", rate
    ...
    def log_weight(lfit_func, trial_result_list, mrr, spread):
        ...
        to get likelihood of one trial, the overal likelihood is product.
...
#        print "returning log_likelihood", log_likelihood
    ...
    def measure_and_compute(
        ...
        High level steps: Prepare and launch computation threads,
        perform the measurement, stop computation and process results.
        ...
        on temporary worker per fitting function. After the measurement,
        Average and stdev of the critical rate (not log) of each thread
        ...
        TODO: Convert to multiprocessing (as opposed to threading).
        TODO: More threads to the slower fitting function?
...
#        random.seed(0)
        def start_computing(fitting_function, bias_avg, bias_cov):
            ...
            Define function for integrator, create thread and queues,
            start computation, return the queues.
            ...
            :returns: Control queue and result queue.
            :rtype: 2-tuple of Queue.Queue
            """
...
#                print "mrr", mrr, "spread", spread
                ...
                value = math.log(self.find_critical_rate(
                    fitting_function,
                    math.log(self.packet_loss_per_second_target), mrr, spread))
                return value, logweight
            control_queue = Queue.Queue(1)
            result_queue = Queue.Queue(1)
            control_queue.put(
                (dimension, value_logweight_func, bias_avg, bias_cov))
            worker = threading.Thread(target=Integrator.estimate_nd,
                                      args=(control_queue, result_queue))
            worker.setDaemon(True)
            worker.start()
            return control_queue, result_queue
        erf_control_queue, erf_result_queue = start_computing(
            self.lfit_erf, erf_bias_avg, erf_bias_cov)
        stretch_control_queue, stretch_result_queue = start_computing(
            self.lfit_stretch, stretch_bias_avg, stretch_bias_cov)
        measurement = self.measurer.measure(trial_duration, transmit_rate)
        erf_control_queue.put(None)
        stretch_control_queue.put(None)
        stretch_avg, stretch_stdev, stretch_bias_avg, stretch_bias_cov = (
            stretch_result_queue.get(True, 1.0))
        erf_avg, erf_stdev, erf_bias_avg, erf_bias_cov = (
            erf_result_queue.get(True, 1.0))
