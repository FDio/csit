...
class PLRsearch(object):
    ...
    def search(self, min_rate, max_rate):
        ...
        message = "Trial %(number)r duration %(dur)r load %(load)r computed avg"
        message += " %(avg)r stdev %(stdev)r stretch %(a1)r erf %(a2)r"
        message += " difference %(diff)r focus_trackers %(tracker)r"
        transmit_rate = (min_rate + max_rate) / 2.0
        lzlsml = [max_rate, max_rate]  # Last Zero Loss Streak Minimal Load.
        consecutive_zeros = [0, 0]
        while 1:
            trial_number += 1
            ...
            logging.info(message, {
                "number": trial_number, "dur": measurement.duration,
                "load": measurement.target_tr, "avg": average, "stdev": stdev,
                "a1": avg1, "a2": avg2, "diff": avg2 - avg1,
                "tracker": focus_trackers})
            index = trial_number % 2
            if measurement.loss_count == 0:
                lzlsml[index] = min(lzlsml[index], transmit_rate)
                consecutive_zeros[index] += 1
            else:
                lzlsml[index] = max_rate
                consecutive_zeros[index] = 0
            ...
            if (trial_number - self.trial_number_offset) <= 1:
                ...
                if lzlsml[not index] < next_load:
                    diff = next_load - lzlsml[not index]
                    for _ in range(consecutive_zeros[not index]):
                        diff *= 2
                    logging.info("!!! diff %(diff)r", {"diff": diff})
                    next_load += diff
