...
class PLRsearch(object):
    ...
    def search(self, min_rate, max_rate):
        ...
        while 1:
            ...
            if trial_number % 2:
                # Measurement was from stretch estimate.
                stretch_zeros += 1
                if measurement.loss_fraction >= self.packet_loss_ratio_target:
                    for _ in range(2 * stretch_zeros):
                        lossy_loads.append(measurement.target_tr)
                if measurement.loss_count > 0:
                    stretch_zeros = 0
            else:
                # Measurement was from erf estimate.
                erf_zeros += 1
                if measurement.loss_fraction >= self.packet_loss_ratio_target:
                    for _ in range(2 * erf_zeros):
                        lossy_loads.append(measurement.target_tr)
                if measurement.loss_count > 0:
                    erf_zeros = 0
            lossy_loads.sort()
            ...
            if (trial_number - self.trial_number_offset) <= 1:
                ...
                if trial_number % 2:
                    # Next measurement will be from erf estimate.
                    next_load = avg2
                    zeros = erf_zeros
                else:
                    # Next measurement will be from stretch estimate.
                    next_load = avg1
                    zeros = stretch_zeros
                orig = next_load
                ll0 = lossy_loads[0]
                if zeros > 0 and next_load < ll0:
                    diminisher = math.pow(2.0, 1 - zeros)
                    next_load = ll0 + diminisher * next_load
                    next_load /= (1.0 + diminisher)
                    if len(lossy_loads) > 1:
                        lossy_loads = lossy_loads[1:]
                logging.info("Zeros %(z)r orig %(o)r next %(n)r loads %(s)r",
                             {"z": zeros, "o": orig, "n": next_load,
                              "s": lossy_loads})
