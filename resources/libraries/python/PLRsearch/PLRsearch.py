...
#                print "mrr", mrr, "spread", spread
            ...
            worker = multiprocessing.Process(
                target=Integrator.estimate_nd, args=(worker_pipe_end,))
            worker.daemon = True
            worker.start()
        ...
        if not (stretch_pipe.poll(1.0) and erf_pipe.poll(1.0)):
            raise RuntimeError("Some worker did not finish!")
        result = stretch_pipe.recv()
        stretch_avg, stretch_stdev, stretch_bias_avg, stretch_bias_cov = result
        result = erf_pipe.recv()
        erf_avg, erf_stdev, erf_bias_avg, erf_bias_cov = result
