...
class Iperf3:
    ...
    def initialize_iperf_server(
            ...
            namespace=None, cpu_skip_cnt=0, cpu_cnt=1):
        ...
        if u"tap" in interface:
            affinity = f"{self._s_affinity},{self._c_affinity}"
            exec_cmd_no_error(
                node,
                f"grep -h 'vhost' /proc/*/comm | xargs pidof | "
                f"sudo xargs -n 1 taskset -pc {affinity}"
            )
...
class IPerf3Client:
    ...
    def iperf3_cmdline(**kwargs):
        ...
        cmd_options = OptionString(prefix=u"--")
        # Client connecting to an iPerf server running on host.
        cmd_options.add_with_value_from_dict(
