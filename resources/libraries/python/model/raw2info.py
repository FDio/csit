...
def _process_test_name(data):
    ...
    # Set test type. Detect dut_type but not handle it yet.
    test_type, dut_type = _detect_test_and_dut_type(data)
    ...
    # Fall-back for wrong dut_type.
    if data[u"dut_type"] != dut_type:
        if data[u"dut_type"] == u"unknown":
            # Probably the test failed before interacting with DUT.
            # Set dut_type for PAL, but leave dut_version empty.
            data[u"dut_type"] = dut_type
        else:
            raise RuntimeError("Dut type set wrong, should be {dut_type}")


def _detect_test_and_dut_type(data):
    """Return test_type and dut_type, as inferred from robot test tags.

    :param data: Raw data, perhaps some fields converted into info data already.
    :type data: dict
    :returns: The inferred test type and dut type.
    :rtype: Tuple[str, str]
