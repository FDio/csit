...
def _convert_from_raw_to_info_in_memory(data):
    ...
    if u"result_type" not in results_node:
        # Device or something else not supported.
        return data
    result_type = results_node[u"result_type"]

    # More processing depending on result type. TODO: Separate functions?

    # Compute avg and stdev for mrr.
    if result_type == u"mrr":
        values = results_node[u"samples"][u"rate"][u"values"]
        stats = AvgStdevStats.for_runs(values)
...
def _merge_into_suite_info_file(teardown_info_path):
    ...
    setup_data = load_from(setup_info_path)

    suite_data = dict(
        version=setup_data[u"version"],
        start_time=setup_data[u"start_time"],
        end_time=teardown_data[u"end_time"],
        suite_name=setup_data[u"suite_name"],
        suite_id=setup_data[u"suite_id"],
        suite_documentation=setup_data[u"suite_documentation"],
    )
    ...
    if u"sut_type" in setup_data:
        suite_data[u"sut_type"] = setup_data[u"sut_type"]
    if u"sut_version" in setup_data:
        suite_data[u"sut_version"] = setup_data[u"sut_version"]
    suite_data[u"setup_log"] = setup_data[u"log"]
