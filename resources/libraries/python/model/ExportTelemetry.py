...
def add_telemetry_items(telemetry_id, telemetry_type, items, timestamp=None):
    """Add a telemetry record with given items in it.

    User probably needs to interact with telemetry_item class
    to create telemetry items.
    If timestamp is missing, local UTC time is used.

    :param telemetry_id: Identifier for this record according to model.
    :param telemetry_type: Type of this telemetry record according to model.
    :param items: List of data items for this record.
    :param timestamp: Use this time if given.
    :type telemetry_id: str
    :type telemetry_type: str
...
def add_single_telemetry_item(
        telemetry_id, telemetry_type, name, value, labels, timestamp=None
    ):
    ...
    :param telemetry_id: Identifier for this record according to model.
    :param telemetry_type: Type of this telemetry recond according to model.
    ...
    add_telemetry_items(telemetry_id, telemetry_type, [item], timestamp)
...
def export_runtime_counters(host, socket, trial_type, runtime_nz, timestamp):
    ...
    :type timestamp: str
    """
    telemetry_id = f"trial type: {trial_type}"
    telemetry_type = u"vpp_show_runtime"
    telemetry_name = u"show_runtime"
    labels = dict(
        host=host,
        socket=socket,
    )
    ...
    for node_item in runtime_nz:
        ...
        for metric in (u"calls", u"clocks", u"suspends", u"vectors"):
            ...
            for thread_index, value in enumerate(value_list):
                ...
                items.append(telemetry_item(telemetry_name, value, labels))
    add_telemetry_items(telemetry_id, telemetry_type, items, timestamp)
