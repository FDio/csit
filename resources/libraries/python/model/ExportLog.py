...
def export_ssh_command(
    ...
    if data is None:
        return
    ssh_record = {
        u"source-type": u"host,port",
        u"source-id": f"{host},{port}",
        u"msg-type": u"ssh-command",
        u"log-level": u"DEBUG",
        u"timestamp": now() if timestamp is None else str(timestamp),
        u"msg": "",
        u"data": str(command),
...
def export_ssh_result(
        host, port, code, stdout, stderr, timestamp=None
    ):
    ...
    Message is empty, data has fields "rc", "stdout" and "stderr".
