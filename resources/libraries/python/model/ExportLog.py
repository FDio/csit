...
def export_ssh_command(
    ...
    if data is None:
        return
    ssh_record = {
        u"source_type": u"host,port",
        u"source_id": f"{host},{port}",
        u"msg_type": u"ssh_command",
        u"log_level": u"DEBUG",
...
def export_mlrsearch_debug(message, timestamp=None):
    ...
    if data is None:
        return
    mlrsearch_record = {
        u"source_type": u"search_algorithm",
        u"source_id": u"mlrsearch",
        u"msg_type": u"mlrsearch_debug",
        u"log_level": u"DEBUG",
        u"timestamp": now() if timestamp is None else str(timestamp),
        u"msg": str(message),
        u"data": u"",
