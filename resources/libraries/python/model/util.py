...
def descend(parent_node, key, default=None):
    ...
    :param default: What to insert if the key does not exist.
        None means dict(). The other populat option is list().
    ...
    if key not in parent_node:
        parent_node[key] = dict() if default is None else default
...
def get_export_data():
    ...
    If it has not (outside tests, e.g. in suite setup), return None.
    Also return None if Robot is not running.

    :returns: Current library instance's data member.
    :rtype: Optional[dict]
    """
    name = u"resources.libraries.python.model.export_json"
    instance = get_library_instance(name)
    if instance is None:
        return None
