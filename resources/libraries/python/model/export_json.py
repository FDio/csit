...
class export_json():
    ...
    def export_pending_data(self):
        ...
        """
        logger.info(u"Maybe exporting data for {self.raw_file_path}")
        if not Constants.EXPORT_JSON or not self.raw_file_path:
            return
        logger.info(u"Definitely exporting data for {self.raw_file_path}")
        data = _pre_serialize_root(self.raw_data)
