...
class export_json():
    ...
    def start_suite_setup_export(self):
        ...
        self.debug_file_path = os.path.join(
            self.output_dir, suite_path_part, u"setup.debug.json.log"
        )
        self.info_file_path = os.path.join(
            self.output_dir, suite_path_part, u"setup.info.json.log"
        )
        data = dict()
    ...
    def start_test_export(self):
        ...
        self.debug_file_path = os.path.join(
            self.output_dir, suite_path_part, test_name + u".debug.json.log"
        )
        self.info_file_path = os.path.join(
            self.output_dir, suite_path_part, test_name + u".info.json.log"
        )
        data = dict()
        data[u"version"] = Constants.MODEL_VERSION
        # TODO: "ci".
    ...
    def start_suite_teardown_export(self):
        ...
        suite_path_part = os.path.join(*suite_name.split(u"."))
        self.debug_file_path = os.path.join(
            self.output_dir, suite_path_part, u"teardown.debug.json.log"
        )
        self.info_file_path = os.path.join(
            self.output_dir, suite_path_part, u"teardown.info.json.log"
        )
        data = dict()
        data[u"version"] = Constants.MODEL_VERSION
        # TODO: Add example to model document.
    ...
    def finalize_test_export(self):
        ...
        self.debug_data[u"end_time"] = end_time
        self.debug_data[u"status"] = get_variable(u"\\${TEST_STATUS}")
        self.debug_data[u"message"] = get_variable(u"\\${TEST_MESSAGE}")
