...
class export_json():
    ...
    def export_pending_data(self):
        ...
        if u"results" in info_data:
            ...
            if self.info_test_schema_validator is None:
                ...
                #  /_modules/jsonschema/validators/#validate
                # when 4.1.2 was the stable version of jsonschema.
                validator_class = jsonschema.validators.validator_for(schema)
                validator_class.check_schema(schema)
                # I wish I found a better documentation on format checkers.
                # The checkers we want are added upon import:
                # https://github.com/Julian/jsonschema/blob
                #  /af78d65ccf3164f37083eaf023db5704d004bbc6
                #  /jsonschema/_format.py#L326
                # so here we just need to instantiate.
                fmt_checker = jsonschema.FormatChecker()
                # The following is based on a single line in source code:
                # https://github.com/Julian/jsonschema/blob
                #  /af78d65ccf3164f37083eaf023db5704d004bbc6
                #  /jsonschema/validators.py#L171
    ...
    def start_suite_setup_export(self):
        ...
        self.raw_file_path = os.path.join(
            output_dir, u"output_raw", suite_path_part, u"setup.json"
        )
        self.info_file_path = os.path.join(
            output_dir, u"output_info", suite_path_part, u"setup.json"
        )
        data = dict()
    ...
    def start_test_export(self):
        ...
        self.raw_file_path = os.path.join(
            self.output_dir, u"output_raw", suite_path_part, test_name + u".json"
        )
        self.info_file_path = os.path.join(
            self.output_dir, u"output_info", suite_path_part, test_name + u".json"
        )
        data = dict()
        data[u"version"] = Constants.MODEL_VERSION
        # TODO: "ci".
    ...
    def start_suite_teardown_export(self):
        ...
        suite_path_part = os.path.join(*suite_name.split(u"."))
        self.raw_file_path = os.path.join(
            self.output_dir, u"output_raw", suite_path_part, u"teardown.json"
        )
        self.info_file_path = os.path.join(
            self.output_dir, u"output_info", suite_path_part, u"teardown.json"
        )
        data = dict()
        data[u"version"] = Constants.MODEL_VERSION
        data[u"start_time"] = start_time
