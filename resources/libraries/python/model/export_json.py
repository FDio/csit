...
class export_json():
    ...
    def export_pending_data(self):
        ...
        if not Constants.EXPORT_JSON or not self.raw_file_path:
            ...
            self.raw_file_path = None
            return
        new_file_path = write_output(self.raw_file_path, self.raw_data)
        # Data is going to be cleared (as a sign that raw export succeeded),
    ...
    def start_suite_setup_export(self):
        ...
        self.raw_data[u"telemetry"] = list()
    ...
    def process_results(self):
        ...
        """
        if u"result" not in self.raw_data:
            return
        result_node = self.raw_data[u"result"]
        result_type = result_node[u"type"]
        if result_type == u"unknown":
            # Device or something else not supported.
            return

        # More processing depending on result type. TODO: Move to ExportResult?
