...
def export_hoststack_ab_result(result):
    ...
    for key, value in result.items():
        ...
        if key == u"latency_unit":
            ...
            rate_node = result_node.get(u"rate", dict())
...
def export_hoststack_iperf_result(parsed):
    ...
    :param parsed: JSON-deserialized output, as parsed in HoststackUtil.
    :type parsed: dict
    :raises ValueError: If the parse output does not conform to UTI model.
    """
    client_object = parsed.pop(u"intervals")
    server_object = parsed.pop(u"server_output_json").pop(u"intervals")
...
def export_hoststack_vpp_echo_result(parsed_output):
    ...
    parsed_output[u"rx_rate"] = dict(value=parsed_output.pop(u"rx_rate")
    parsed_output[u"tx_rate"] = dict(value=parsed_output.pop(u"tx_rate")
