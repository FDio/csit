...
def export_hoststack_ab_result(result):
    ...
    old_items = list(result.items())
    result[u"_latency"] = dict()
    result[u"_transfer_rate"] = dict()
    result[u"_rate"] = dict()
    for key, value in old_items:
        if key == u"latency_unit":
            result[u"_latency"][u"unit"] = value
        elif key == u"latency_value":
            result[u"_latency"][u"value"] = value
        elif key == u"transfer_unit":
            result[u"_transfer_rate"][u"unit"] = value
            # To be converted when creating .info.
        elif key == u"transfer_rate":
            result[u"_transfer_rate"][u"value"] = value
        elif key == u"quantity":
            result[u"_rate"][u"unit"] = value
            result[u"type"] = f"ab_{value}"
        elif key == u"rate":
            result[u"_rate"][u"value"] = value
        else:
