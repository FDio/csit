...
def export_search_bound(text, value, bandwidth=None):
    ...
    debug_ndrpdr_node[upper_or_lower + u"_gbps"] = bandwidth
...
def export_ndrpdr_latency(text, latency):
    ...
    """
    # TODO: R0914: Too many local variables (20/15) (too-many-locals)
    # Probably by moving the l_* blocks to a single function.
    debug_data, info_data = get_export_data()
    debug_results_node = debug_data[u"results"]
    info_results_node = info_data[u"results"]
    ...
    debug_forward_node = descend(debug_ndrpdr_node, u"latency_forward")
    ...
    if l_min > -1:
        info_forward_node = descend(info_ndrpdr_node, u"latency_forward")
    ...
    if len(latency) < 2:
        return
    debug_reverse_node = descend(debug_ndrpdr_node, u"latency_reverse")
    ...
    debug_percent_node[u"hdrd"] = l_hdrh
    if l_min > -1:
        info_reverse_node = descend(info_ndrpdr_node, u"latency_reverse")
