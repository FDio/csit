...
def append_mrr_value(mrr_value):
    ...
    test_node = data.get_subdata([u"test"])
    test_node[u"test-type"] = u"MRR"
    test_node = data.get_subdata([u"test", u"results", u"test"])
    if u"value" not in test_node:
        test_node[u"value"] = dict()
    value_node = test_node[u"value"]
    if u"receive-rate" not in value_node:
        value_node[u"receive-rate"] = list()
    receive_rate_node = value_node[u"receive-rate"]
    receive_rate_node.append(float(mrr_value))
