...
def export_vpp_version(version):
    """Export the argument as sut-version.

    No-op if export data does not exist (e.g. in suite setup).
    Sets suit-type to "VPP".
    ...
    if data is None:
        return
    data[u"sut-type"] = u"VPP"
    data[u"sut-version"] = str(version)
...
def export_search_bound(text, value, bandwidth=None):
    ...
    If "throughput" (or "critical-rate") node does not exist, it is created.
    If a previous value exists, it is overwritten silently.
    ...
    data[u"test-type"] = test_type
    results_node = data[u"results"]
    rate_name = u"throughput" if test_type == u"NDRPDR" else u"critical-rate"
