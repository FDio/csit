...
def export_dut_type_and_version(dut_type=None, dut_version=None):
    ...
    """
    logger.debug(f"export called with type {dut_type} version {dut_version}")
    if dut_type is None:
        dut_type = BuiltIn().get_variable_value(u"\\${DUT_TYPE}", u"unknown")
        if dut_type == u"unknown":
            raise RuntimeError(u"Dut type not provided.")
    else:
        logger.debug(f"storing type {dut_type}")
        BuiltIn().set_suite_variable(u"\\${DUT_TYPE}", dut_type, children=True)
    if dut_version is None:
        dut_version = BuiltIn().get_variable_value(u"\\${DUT_VERSION}", u"unknown")
        if dut_type == u"unknown":
            raise RuntimeError(u"Dut version not provided.")
    else:
        logger.debug(f"storing version {dut_version}")
        BuiltIn().set_suite_variable(
            u"\\${DUT_VERSION}", dut_version, children=True
        )
