...
def export_dpdk_version(version):
    """Export the argument as sut_version.

    Sets suit_type to "DPDK".

    :param version: DPDK version as seen on DUT1 disk.
    :type version: str
    """
    data = get_export_data()
    data[u"sut_type"] = u"DPDK"
...
def append_mrr_value(mrr_value, unit):
    ...
    if not unit:
        return
    data = get_export_data()
    data[u"results"][u"result_type"] = u"mrr"
    samples_node = descend(data[u"results"], u"samples")
    rate_node = descend(samples_node, u"rate")
...
def export_search_bound(text, value, unit, bandwidth=None):
    ...
    results_node[u"result_type"] = result_type
    ...
    if result_type == u"soak":
        descend(results_node, u"critical_rate")[upper_or_lower] = rate_item
        return
    descend(results_node, ndr_or_pdr)[upper_or_lower] = rate_item


def _add_latency(results_node, percent, whichward, latency_string):
    ...
    :param results_node: UTI tree node to descend from.
    ...
    :type results_node: dict
    :type percent: int
    :type whichward: str
    :latency_string: str
    """
    l_min, l_avg, l_max, l_hdrh = latency_string.split(u"/", 3)
    whichward_node = descend(results_node, f"latency_{whichward}")
...
def export_ndrpdr_latency(text, latency):
    ...
    if u"90" in text:
        ...
        percent = 10
    _add_latency(results_node, percent, u"forward", latency[0])
    # Else TRex does not support latency measurement for this traffic profile.
    if len(latency) < 2:
        return
    _add_latency(results_node, percent, u"reverse", latency[1])
