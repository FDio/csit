...
def export_hoststack_ab_result(result):
    ...
    The edits needed are performed in-place on the argument.
    ...
    for key, value in result.items():
        # Pair units with values.
        # Do not expect any particular order of keys.
        # As there is name overlap, placeholders are needed.
        if key == u"latency_unit":
            latency_node = result.get(u"_latency", dict())
            latency_node[u"unit"] = value
        elif key == u"latency_value":
            latency_node = result.get(u"_latency", dict())
            latency_node[u"value"] = value
        elif key == u"transfer_unit":
            transfer_rate_node = result.get(u"_transfer_rate", dict())
            transfer_rate_node[u"unit"] = value
            # To be converted when creating .info.
        elif key == u"transfer_rate":
            transfer_rate_node = result.get(u"_transfer_rate", dict())
            transfer_rate_node[u"value"] = value
        elif key == u"quantity":
            rate_node = result.get(u"_rate", dict())
            rate_node[u"unit"] = value
            result[u"type"] = f"ab_{value}"
        elif key == u"rate":
            rate_node = result.get(u"_rate", dict())
            rate_node[u"value"] = value
        else:
            # The rest are values, units will be added to .info.
            result[key] = dict(value=value)
    # Move the temporary placeholders.
    result[u"latency"] = result.pop(u"_latency")
    result[u"transfer_rate"] = result.pop(u"_transfer_rate")
    result[u"rate"] = result.pop(u"_rate")
    # Put the edited node.
    get_export_data()[u"result"] = result
...
def export_hoststack_iperf_result(parsed_output):
    ...
    for endpoint_object in (client_object, server_object):
        for interval in endpoint_object[u"intervals"]:
            for stream in interval[u"streams"]:
                _process_iperf_item(stream)
            _process_iperf_item(interval[u"sum"])
    result_node = get_export_data()[u"result"]
    result_node[u"client"] = client_object
    result_node[u"server"] = server_object
    item = client_object[u"intervals"][u"sum"]
