...
def append_mrr_value(mrr_value, unit):
    ...
    if not unit:
        return
    data = get_export_data()
    mrr_node = descend(data[u"results"], u"mrr")
    samples_node = descend(mrr_node, u"samples")
    samples_node[u"unit"] = str(unit)
    values_list = descend(samples_node, u"values", list)
...
def export_search_bound(text, value, unit, bandwidth=None):
    ...
    data = get_export_data()
    type_node = descend(data[u"results"], test_type)
    rate_item = dict(rate=dict(value=value, unit=unit))
    if bandwidth:
        rate_item[u"bandwidth"] = dict(value=float(bandwidth), unit=u"Gbps")
    if test_type == u"soak":
        type_node[upper_or_lower] = rate_item
        return
    descend(type_node, ndr_or_pdr)[upper_or_lower] = rate_item
...
def export_ndrpdr_latency(text, latency):
    ...
    """
    data = get_export_data()
    ndrpdr_node = descend(data[u"results"], u"ndrpdr")
    percent = 0
    if u"90" in text:
        percent = 90
    elif u"50" in text:
        ...
        percent = 10
    _add_latency(ndrpdr_node, percent, u"forward", latency[0])
    # Else TRex does not support latency measurement for this traffic profile.
    if len(latency) < 2:
        return
    _add_latency(ndrpdr_node, percent, u"reverse", latency[1])
