...
class ExportJson():
    ...
    def finalize_test_export_once(self):
        ...
        # Do not override message from validation failure.
        self.process_passed_and_message()
    ...
    def process_passed_and_message(self):
        """Process the test status information as boolean and set message.

        Boolean is used to make post processing more efficient.
        Message is set only on first call,
        as before second cal it is set by vlidate().

        In case the test status is PASS, we truncate the test message.
        In case the test is not passing, we reset test result and telemetry,
        ...
        if status is not None:
            if "passed" in self.data:
                # Second call implies validation failure,
                # even when Robot did not update the status in teardown.
                status = "FAIL"
            ...
            if self.data["passed"]:
                ...
                if "message" not in self.data:
                    self.data["message"] = BuiltIn().get_variable_value(
                        "\\${TEST_MESSAGE}"
                    )
