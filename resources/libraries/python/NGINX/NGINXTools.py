...
class NGINXTools:
    ...
    def cleanup_nginx_framework(node):
        ...
        """
        command = f"rm -rf {Constants.NGINX_INSTALL_DIR}"
    ...
    def cleanup_nginx_framework_on_all_duts(nodes):
        ...
        for node in nodes.values():
            if node['type'] == NodeType.DUT:
                NGINXTools.cleanup_nginx_framework(node)

    @staticmethod
    def install_original_nginx_framework(node, nginx_version):
        """
        ...
        """
        nginx_path = f'{Constants.NGINX_INSTALL_DIR}/nginx-{nginx_version}'
