...
class Constants:
    ...
    EXPORT_JSON = Constants.get_optimistic_bool_from_env("EXPORT_JSON")
    ...
    TREX_INSTALL_DIR = "/opt/trex-core"

    # TRex pcap files directory.
    TREX_PCAP_DIR = f"{TREX_INSTALL_DIR}/scripts/avl"

    # TRex limit memory.
    TREX_LIMIT_MEMORY = Constants.get_int_from_env("TREX_LIMIT_MEMORY", 8192)

    # TRex limit memory in case multiple dual interfaces configurations.
    TREX_LIMIT_MEMORY_MULTI = Constants.get_int_from_env(
        "TREX_LIMIT_MEMORY_MULTI", 16384
    )

    # TRex number of cores.
    TREX_CORE_COUNT = Constants.get_int_from_env("TREX_CORE_COUNT", 16)

    # TRex number of cores in case multiple dual interface configurations.
    TREX_CORE_COUNT_MULTI = Constants.get_int_from_env(
        "TREX_CORE_COUNT_MULTI", 8
    )

    TREX_PORT_MTU = Constants.get_int_from_env("TREX_PORT_MTU", 0)

    # TRex set number of RX/TX descriptors.
    # Set to 0 to use default values.
    TREX_TX_DESCRIPTORS_COUNT = Constants.get_int_from_env(
        "TREX_TX_DESCRIPTORS_COUNT", 0
    )

    TREX_RX_DESCRIPTORS_COUNT = Constants.get_int_from_env(
        "TREX_RX_DESCRIPTORS_COUNT", 0
    )

    # Trex force start regardless ports state.
    TREX_SEND_FORCE = Constants.get_pessimistic_bool_from_env("TREX_SEND_FORCE")

    # TRex extra commandline arguments.
    TREX_EXTRA_CMDLINE = Constants.get_str_from_env(
        "TREX_EXTRA_CMDLINE", "--mbuf-factor 32"
    )

    # TRex port driver default vfio-pci or set to igb_uio.
    TREX_PORT_DRIVER = Constants.get_str_from_env(
        "TREX_PORT_DRIVER", "vfio-pci"
    )

    # Graph node variant value
    GRAPH_NODE_VARIANT = Constants.get_str_from_env("GRAPH_NODE_VARIANT", "")

    # Default memory page size in case multiple configured in system
    DEFAULT_HUGEPAGE_SIZE = Constants.get_str_from_env(
    ...
    PERF_STAT_EVENTS = Constants.get_str_from_env(
        "PERF_STAT_EVENTS",
    ...
    PERF_TRIAL_MULTIPLICITY = Constants.get_int_from_env(
        "PERF_TRIAL_MULTIPLICITY", 10
    )

    # Duration [s] of one trial in MRR test.
    PERF_TRIAL_DURATION = Constants.get_float_from_env(
        "PERF_TRIAL_DURATION", 1.0
    )

    # Whether to use latency streams in main search trials.
    PERF_USE_LATENCY = Constants.get_pessimistic_bool_from_env(
        "PERF_USE_LATENCY"
    )

    # Whether to apply low-load trial to avoid flooding other machines.
    INFRA_WARMUP_DURATION = Constants.get_float_from_env(
        "INFRA_WARMUP_DURATION", 0
    )

    # Load just high enough to show all MAC addresses.
    INFRA_WARMUP_RATE = Constants.get_float_from_env("INFRA_WARMUP_RATE", 300.0)

    # Whether to warm-up the traffic generator with high load.
    TG_WARMUP_DURATION = Constants.get_float_from_env("TG_WARMUP_DURATION", 1.0)

    ## TG warmup load tweaking should not be needed, default max rate is best.
    # TG_WARMUP_RATE = Constants.get_float_from_env("TG_WARMUP_RATE")

    # Duration of one latency-specific trial in NDRPDR test.
    PERF_TRIAL_LATENCY_DURATION = Constants.get_float_from_env(
    ...
    PERF_TRIAL_STL_DELAY = Constants.get_float_from_env(
        "PERF_TRIAL_STL_DELAY", 0.0
    )

    # ASTF usually needs a different value for the delay.
    PERF_TRIAL_ASTF_DELAY = Constants.get_float_from_env(
    ...
    ASTF_N_DATA_FRAMES = Constants.get_int_from_env("ASTF_N_DATA_FRAMES", 5)

    # Extended debug (incl. vpp packet trace, linux perf stat, ...).
    # Full list is available as suite variable (__init__.robot) or is
    # override by test.
    EXTENDED_DEBUG = Constants.get_pessimistic_bool_from_env("EXTENDED_DEBUG")
    ...
    DUT1_UUID = Constants.get_str_from_env("DUT1_UUID", "")

    # Global "kill switch" for CRC checking during runtime.
    FAIL_ON_CRC_MISMATCH = Constants.get_pessimistic_bool_from_env(
