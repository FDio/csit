...
class SetupDocker(object):
    ...
    def create_docker_container_on_dut(node, container_name, cont_active=30000):
        ...
        if SetupDocker.check_container_exist(node, container_name):
            print "Docker with name {0} already exists. \
    ...
    def connect_container_with_namespace(node, docker_id, timeout=20):
        ...
        while True:
            ...
            if rc != 0:
                if 'No such image or container' in stderr:
                    ...
                    raise RuntimeError(
                        'Unknown error :{0}'.format(stderr))
            if int(stdout) == 0:
                current_time = time()
                if (current_time - start_time) > timeout:
                    raise RuntimeError(
                        'Could not set correct pid for container: {0}'.format(
                            stdout)
                    )
            else:
                pid = stdout
        ...
        cmd = 'ln -s /proc/{0}/ns/net /var/run/netns/{1}'.format(
            int(pid), docker_id
        )
        (rc, stdout, stderr) = exec_cmd(node, cmd, timeout=5, sudo=True)
        if rc != 0:
            raise RuntimeError(
                'Could not connect container with namespace, reason:{0}'.format(
                    stderr)
            )
    ...
    def remove_and_clean_all_containers(node):
        ...
        if rc != 0:
            if 'requires a minimum of 1 argument' in stderr:
                ...
                raise RuntimeError(
                    'Could not stop containers, reason:{0}'.format(stderr))
        ...
        if rc != 0:
            if 'requires a minimum of 1 argument' in stderr:
                ...
                raise RuntimeError(
                    'Could not clean containers, reason:{0}'.format(stderr))
