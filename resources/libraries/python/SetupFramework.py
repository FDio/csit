...
def rsync_framework(node):
    ...
    cmd = f"sudo rm -rf {path}; mkdir -p {path}"
    exec_cmd_no_error(
        node, cmd,
        message=f"Failed prepare remote dir {path} at node {node_type} "
        f"host {host}, port {port}.", timeout=30, include_reason=True
    )
    # TODO: Support priv keys.
    ret_code, output = run([
        u"sshpass", u"-p", f"{passwd}", u"rsync", u"-a", u".",
        # The vlaue in path already starts with a slash.
