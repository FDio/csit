...
def setup_node(tarball, remote_tarball, node, results=None):
...
class SetupFramework(object):
    ...
    def setup_framework(nodes):
        ...
        thread_count = len(threads) + 1
        now = datetime.datetime.now()
        while threading.active_count() < thread_count and \
                datetime.datetime.now() - now < datetime.timedelta(seconds=1):
            pass
