...
def copy_tarball_to_node(tarball, node):
    ...
    """
    host = node[u"host"]
    logger.console(f"Copying tarball to {node} ({host}) starts.")
    scp_node(node, tarball, u"/tmp/")
    logger.console(f"Copying tarball to {node} ({host}) done.")
...
def extract_tarball_at_node(tarball, node):
    ...
    host = node[u"host"]
    logger.console(
        f"Extracting tarball to {con.REMOTE_FW_DIR} on {node} ({host})starts."
    )
    cmd = f"sudo rm -rf {con.REMOTE_FW_DIR}; mkdir {con.REMOTE_FW_DIR}; " \
        f"tar -zxf {tarball} -C {con.REMOTE_FW_DIR}; rm -f {tarball}; ll"
    exec_cmd_no_error(
        node, cmd,
        message=f"Failed to extract {tarball} at node {node} ({host})",
        timeout=30, include_reason=True
    )
    logger.console(
        f"Extracting tarball to {con.REMOTE_FW_DIR} on {node} ({host}) done."
    )
...
def create_env_directory_at_node(node):
    ...
    host = node[u"host"]
    logger.console(
        f"Virtualenv setup including requirements.txt "
        f"on {node} ({host}) starts."
    )
    ...
    exec_cmd_no_error(
        ...
        message=f"Failed install at node {node} ({host})"
    )
    logger.console(f"Virtualenv setup on {node} ({host}) done.")
...
def setup_node(node, tarball, remote_tarball, results=None):
    ...
    try:
        ...
        logger.console(f"Node {node} ({host}) setup failed, error: {exc!r}")
        result = False
    else:
        logger.console(f"Setup of node {node} ({host}) done.")
        result = True
...
def delete_framework_dir(node):
    ...
    """
    host = node[u"host"]
    logger.console(f"Deleting framework directory on {node} ({host}) starts.")
    exec_cmd_no_error(
        node, f"sudo rm -rf {con.REMOTE_FW_DIR}",
        message=f"Framework delete failed at node {node} ({host})",
        timeout=100, include_reason=True
    )
    logger.console(f"Deleting framework directory on {node} ({host}) done.")
...
def cleanup_node(node, results=None):
    ...
    try:
        ...
        logger.error(f"Cleanup of node {node} ({host}) failed.")
        result = False
    else:
        logger.console(f"Cleanup of node {node} ({host}) done.")
        result = True
...
class SetupFramework(object):
    ...
    def setup_framework(nodes):
        ...
        if all(results):
            ...
            for node in nodes.values():
                logger.info(
                    f"Setup of node {node} ({node[u'host']}) done."
                )
        else:
