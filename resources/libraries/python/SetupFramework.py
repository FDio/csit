...
def extract_tarball_at_node(tarball, node):
    ...
    cmd = f"sudo rm -rf {con.REMOTE_FW_DIR}; mkdir {con.REMOTE_FW_DIR}; " \
        f"tar -zxf {tarball} -C {con.REMOTE_FW_DIR}; rm -f {tarball}; ls -l"
    out, err = exec_cmd_no_error(
        node, cmd,
...
def create_env_directory_at_node(node):
    ...
    out, err = exec_cmd_no_error(node, f"ls -l {con.REMOTE_FW_DIR}", timeout=100)
    logger.console(
        f"output of ls -l {con.REMOTE_FW_DIR}:\n{out}\n"
        f"stderr: {err}"
    )
    cmd = f"cd {con.REMOTE_FW_DIR}/csit && rm -rf env && virtualenv " \
