...
class QemuUtils(object):
    ...
    def qemu_add_vhost_user_if(self, socket, server=True, mac=None):
        ...
        :param mac: Vhost-user interface MAC address (optional).
        :type socket: str
        :type server: bool
        """
        self._vhost_id += 1
    ...
    def _qemu_qmp_exec(self, cmd):
        ...
        if len(out_list) < 3:
            raise RuntimeError('Invalid QMP output on {1}'.format(
                self._node['host']))
    ...
    def _wait_until_vm_boot(self, timeout=300):
        ...
        while 1:
            if time() - start > timeout:
                raise RuntimeError('timeout, VM not booted')
    ...
    def _update_vm_interfaces(self):
        ...
        mac_name = {}
        if not interfaces:
            raise RuntimeError('Get VM interface list failed')
