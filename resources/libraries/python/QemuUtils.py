...
class QemuUtils(object):
    ...
    def qemu_setup_network_namespace(self,namespace_name,vhost_if,ip_address):
        (ret_code, stdout, stderr) = self._ssh.exec_command_sudo('ip netns add {0}'.format(namespace_name))
        print "Adding namespace"
        print "======================"
        print "Ret code: {0}".format(ret_code)
        print "Stdout: {0}".format(stdout)
        print "Stderr: {0}".format(stderr)
        print "==================================="
        (ret_code, stdout, stderr) = self._ssh.exec_command_sudo('ip link set {0} netns {1}'.format(vhost_if,namespace_name))
        print "Connecting namespace with interface"
        print "======================"
        print "Ret code: {0}".format(ret_code)
        print "Stdout: {0}".format(stdout)
        print "Stderr: {0}".format(stderr)
        print "==================================="
        (ret_code, stdout, stderr) = self._ssh.exec_command_sudo('ip netns exec {0} ip link set {1} up'.format(namespace_name,vhost_if))
        print "Set inf up"
        print "======================"
        print "Ret code: {0}".format(ret_code)
        print "Stdout: {0}".format(stdout)
        print "Stderr: {0}".format(stderr)
        print "==================================="
        (ret_code, stdout, stderr) = self._ssh.exec_command_sudo('ip netns exec {0} ip addr add {1} dev {2}'.format(namespace_name,ip_address,vhost_if))
