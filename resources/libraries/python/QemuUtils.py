...
class QemuOptions(object):
    ...
    def add(self, param, value):
        ...
        """
        self.params.append({str(param): value})

    def get(self, param):
        """Get value of parameter(s) from list.

        :param param: Parameter to get.
        :type param: str
        :returns: List of values or value that matches parameter.
        :rtype: list or str
        """
        match = [d[param] for d in self.params if param in d]
    ...
    def get_values(self):
        ...
        :rtype: list
        """
        return [d.values()[0] for d in self.params]
...
class QemuUtils(object):
    ...
    def __init__(self, node=None, qemu_id=1, smp=1, sockets=1, cores=1,
        ...
        self._opt.add('mem', mem)
        self._opt.add('smp', smp)
        self._opt.add('sockets', sockets)
        self._opt.add('cores', cores)
        self._opt.add('threads', threads)
    ...
    def add_default_params(self):
        ...
        self._params.add('smp', '{smp},sockets={sockets},cores={cores},'
                         'threads={threads}'.
                         format(smp=self._opt.get('smp'),
                                sockets=self._opt.get('sockets'),
                                cores=self._opt.get('cores'),
                                threads=self._opt.get('threads')))
    ...
    def add_nested_params(self):
        ...
        # TODO: Remove try after fully upgrade to Bionic.
        try:
            self._params.add('drive',
                             'file={img},format=raw,cache=none,if=virtio'
                             '{locking}'.
    ...
    def get_qemu_pids(self):
        ...
        :rtype: list
        """
        command = ('sleep 2; ls /proc/$(sudo cat {pid})/task/ | tail -{smp}'.
                   format(pid=self._temp.get('pid'),
    ...
    def qemu_add_vhost_user_if(self, socket, server=True, mac=None,
        ...
        mac = ('52:54:00:00:{qemu:02x}:{vhost:02x}'.
               format(qemu=self._opt.get('qemu_id'),
                      vhost=self._vhost_id)) if mac is None else mac
    ...
    def qemu_clear_socks(self):
        ...
        exec_cmd(self._node, 'fuser -k {info[serial]}/tcp'.
