...
class QemuUtils(object):
    ...
    def qemu_set_affinity(self, *host_cpus):
        ...
        for qemu_cpu, host_cpu in zip(qemu_cpus, host_cpus):
            ret_code, _, _ = self._ssh.exec_command_sudo(
                'taskset -pc {cpu} {thread}'.
                format(cpu=host_cpu, thread=qemu_cpu['thread_id']))
            if int(ret_code) != 0:
    ...
    def qemu_set_scheduler_policy(self):
        ...
        for qemu_cpu in qemu_cpus:
            ret_code, _, _ = self._ssh.exec_command_sudo(
                'chrt -r -p 1 {thread}'.format(thread=qemu_cpu['thread_id']))
            if int(ret_code) != 0:
    ...
    def qemu_add_vhost_user_if(self, socket, server=True, mac=None,
        ...
        netdev = (' -netdev vhost-user,id=vhost{vhost_id},'
                  'chardev=char{vhost_id},queues={queues}'.
                  format(vhost_id=self._vhost_id,
                         queues=self._qemu_opt.get('queue_count')))
        ...
        queue_size = (',rx_queue_size={size},tx_queue_size={size}'.
                      format(size=self._qemu_opt.get('queue_size')))\
    ...
    def _qemu_qmp_exec(self, cmd):
        ...
        ret_code, stdout, _ = self._ssh.exec_command(
            ...
            '{{ \\"execute\\": \\"{cmd}\\" }}" | '
            'sudo -S socat - UNIX-CONNECT:{socket}'.
            format(cmd=cmd, socket=self._qemu_opt.get('qmp_sock')))
        if int(ret_code) != 0:
    ...
    def _qemu_qga_flush(self):
        ...
        ret_code, stdout, _ = self._ssh.exec_command(
            '(printf "\xFF"; sleep 1) | sudo -S socat - UNIX-CONNECT:{socket}'.
            format(socket=self._qemu_opt.get('qga_sock')))
        if int(ret_code) != 0:
    ...
    def _qemu_qga_exec(self, cmd):
        ...
        ret_code, stdout, _ = self._ssh.exec_command(
            ...
            'sudo -S socat - UNIX-CONNECT:{socket}'.
            format(cmd=cmd, socket=self._qemu_opt.get('qga_sock')))
        if int(ret_code) != 0:
    ...
    def qemu_start(self):
        ...
        # TODO: Need to parametrize QEMU per version. looks like 2.10+ is not compatible with 2.5
        # Drive option
        drive = '-drive file={0},format=raw,cache=none,if=virtio,force-share,readonly'.format(
        ...
        # PID file
        pid = '-pidfile {0}'.format(self._qemu_opt.get('pid_file'))
