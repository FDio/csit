...
class QemuOptions(object):
    ...
    where lists can be added multiple times. This emulates the QEMU behavior
    where one command line parameter can be used multiple times (1..N). Example
    can be device or object (so it is not an issue to have one memory block of
    2G and and second memory block of 512M but from other numa).
