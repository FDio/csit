...
class QemuUtils(object):
    ...
    def __init__(self, node, qemu_id=1, smp=1, mem=512, vnf=None,
        ...
        if img == Constants.QEMU_VM_IMAGE:
            ...
            image_stdout = exec_cmd_no_error(
                node,
                'ls -1 {img}*'.format(img=Constants.QEMU_VM_KERNEL),
                message='Qemu Kernel VM image not found!')
            img = image_stdout.strip().splitlines()[-1]  # -1 is the latest
            self._opt['img'] = img
            ...
            initrd_stdout = exec_cmd_no_error(
                node,
                'ls -1 {initrd}*'.format(
                    initrd=Constants.QEMU_VM_KERNEL_INITRD),
                message='Qemu Kernel initrd image not found!')
            initrd = initrd_stdout.strip().splitlines()[-1]  # -1 is the latest
            self._opt['initrd'] = initrd
        else:
