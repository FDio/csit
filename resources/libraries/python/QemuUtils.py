...
class QemuUtils:
    ...
    def _wait_csr_ip4base(self, retries=600):
        """Wait until QEMU with csr_ip4base is booted.

        :param retries: Number of retries.
        :type retries: int
        """
        line = u"protocol on Interface GigabitEthernet1, changed state to up"
        for _ in range(retries):
            command = f"tail -50 {self._temp.get(u'log')}"
            stdout = None
            try:
                stdout, _ = exec_cmd_no_error(self._node, command, sudo=True)
                sleep(1)
    ...
    def _wait_csr_ip4scale2k(self, retries=600):
        """Wait until QEMU with csr_ip4scale2k is booted.

        :param retries: Number of retries.
        :type retries: int
        """
        self._wait_csr_ip4base(retries=retries)

    def _wait_csr_ip4scale20k(self, retries=600):
        """Wait until QEMU with csr_ip4scale20k is booted.
    ...
    def _wait_csr_ip4scale200k(self, retries=600):
        """Wait until QEMU with csr_ip4scale200k is booted.
    ...
    def _wait_csr_ethip4ipsec1tnl(self, retries=600):
        """Wait until QEMU with csr_ethip4ipsec1tnl is booted.
    ...
    def _wait_csr_ethip4ipsec40tnl(self, retries=600):
        """Wait until QEMU with csr_ethip4ipsec40tnl is booted.
