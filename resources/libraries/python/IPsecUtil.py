...
class IPsecUtil(object):
    ...
    @staticmethod
    def _create_container_ipsec_script_files(dut, instances):
        scripts = []
        for cnf in range(0, instances):
            script_filename = '/tmp/ipsec_create_tunnel_cnf_{dut}_'\
                              '{cnf}.config'.format(cnf=cnf + 1, dut=dut)
            scripts.append(open(script_filename, 'w', buffering=0))
        return scripts

    @staticmethod
    def _close_and_copy_container_ipsec_script_files(dut, nodes, instances, scripts):
        for cnf in range(0, instances):
            scripts[cnf].close()
            script_filename = (
                '/tmp/ipsec_create_tunnel_cnf_{dut}_{cnf}.config'.
                format(cnf=cnf + 1, dut=dut))
    ...
    def vpp_ipsec_create_tunnel_interfaces_in_containers(
        ...
        dut1_scripts = IPsecUtil._create_container_ipsec_script_files('DUT1', n_instances)
        dut2_scripts = IPsecUtil._create_container_ipsec_script_files('DUT2', n_instances)
        ...
        for cnf in range(0, n_instances):
            ...
            dut2_scripts[cnf].write(
                'ip route add {raddr}/{mask} via {laddr} memif1/{cnf}\n\n'
                .format(
                    cnf=cnf + 1,
                    raddr=if1_ip_addr,
                    mask='8',
                    laddr=ip_address(unicode(if2_ip_addr)) + cnf + 100))
        ...
        for tnl in range(0, n_tunnels):
            ...
            dut1_scripts[cnf].write(
                'set interface ip address {uifc} {laddr}/32\n'
                'create ipsec tunnel '
                'local-ip {laddr} '
                'local-spi {local_spi} '
                'remote-ip {raddr} '
        ...
        IPsecUtil._close_and_copy_container_ipsec_script_files('DUT1', nodes, n_instances, dut1_scripts)
        IPsecUtil._close_and_copy_container_ipsec_script_files('DUT2', nodes, n_instances, dut2_scripts)
