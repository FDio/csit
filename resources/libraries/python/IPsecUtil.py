...
from resources.libraries.python.InterfaceUtil import (
    InterfaceUtil, InterfaceStatusFlags
)
...
class IPsecUtil:
    ...
    def gen_key(cls, length):
        ...
        as long as they type bytes and correct length.
        Fast scale data generator needs each byte to be different
        from corresponding byte of the previous key,
        so we flip the last bit.

        To ensure all calls to this method still create only letters,
        we remove 'A', 'Z', 'a' and 'z' from list of initial letters.
        ...
        for index, key_byte in enumerate(key):
            if index == len(cls.LAST_KEY_BITS):
                cls.LAST_KEY_BITS.append(bool(key_byte & 1))
            elif cls.LAST_KEY_BITS[index]:
                key[index] &= 0xfe
                cls.LAST_KEY_BITS[index] = False
            else:
                key[index] |= 1
                cls.LAST_KEY_BITS[index] = True
        return bytes(key)
    ...
    def vpp_ipsec_add_sad_entry(
            ...
            integ_key=b"", tunnel_src=None, tunnel_dst=None):
        """Create Security Association Database entry on the VPP node.

        The keys are assumed to be bytes.
        If a key is str, utf-8 encode is attempted.
        This is useful if keys are not generated, but written in Robot suite.
        ...
        :type spi: int
        :type crypto_alg: CryptoAlg
        :type crypto_key: bytes or str
        :type integ_alg: IntegAlg
        :type integ_key: bytes or str
        :type tunnel_src: str
        :type tunnel_dst: str
        """
    ...
    def vpp_ipsec_add_sad_entries(
            ...
            integ_alg=None, integ_key=b"", tunnel_src=None, tunnel_dst=None):
        ...
        fast = 0 if n_entries < 3 else n_entries
    ...
    def vpp_ipsec_set_ip_route(
        ...
        fast = 0 if n_tunnels < 3 else n_tunnels
        with PapiSocketExecutor(node, do_async=True) as papi_exec:
    ...
    class CreTunIntParam:
        """Data class for holding common parameters.

        This is a way to get around pylint's too-many-locals violation.
        Pylint dislikes more than 10 fields, so other params
        are passed as separate (not included here) arguments.
        """

        def __init__(
                self, nodes, tun_ips, if1_key, if2_key, crypto_alg, integ_alg,
                raddr_ip1, raddr_ip2, addr_incr, spi_d):
            """Set the fields to the values.

            All fields here are considered immutable.

            TODO: Convert to dataclass once we are on Python 3.7 or later.
            ...
            """
            self.nodes = nodes
            self.tun_ips = tun_ips
            self.if1_key = if1_key
            self.if2_key = if2_key
            self.crypto_alg = crypto_alg
            self.integ_alg = integ_alg
            self.raddr_ip1 = raddr_ip1
            self.raddr_ip2 = raddr_ip2
    ...
    def _ipsec_create_tunnel_interfaces_dut1(
        ...
        with PapiSocketExecutor(node, do_async=True) as papi_exec:
            ...
            fast = 0 if new_tunnels < 3 else new_tunnels
            iter_max = existing_tunnels + 2 if fast else n_tunnels
            for i in range(existing_tunnels, iter_max):
                args[u"prefix"] = IPUtil.create_prefix_object(
                    param.tun_ips[u"ip1"] + i * param.addr_incr,
                    128 if param.tun_ips[u"ip1"].version == 6 else 32
                )
                papi_exec.add(cmd, **args)
            err_msg = u"Failed to add address."
            papi_exec.get_replies(
                err_msg=err_msg, fast_send=fast, fast_receive=True
            )
            ...
            args = dict(
                ...
                local_spi=0,
                remote_spi=0,
                crypto_alg=param.crypto_alg.alg_int_repr,
                local_crypto_key_len=0,
                local_crypto_key=None,
                remote_crypto_key_len=0,
                remote_crypto_key=None,
                integ_alg=(
                    param.integ_alg.alg_int_repr if param.integ_alg else 0
                ),
            ...
            for i in range(existing_tunnels, iter_max):
                ckeys.append(IPsecUtil.gen_key(
                    IPsecUtil.get_crypto_alg_key_len(param.crypto_alg)
                ))
                if param.integ_alg:
                    ikeys.append(IPsecUtil.gen_key(
                        IPsecUtil.get_integ_alg_key_len(param.integ_alg)
                    ))
                args[u"local_spi"] = param.spi_d[u"spi_1"] + i
                args[u"remote_spi"] = param.spi_d[u"spi_2"] + i
            ...
            for i in range(existing_tunnels, iter_max):
                args[u"route"] = IPUtil.compose_vpp_route_structure(
                    node, (param.raddr_ip2 + i).compressed,
    ...
    def _ipsec_create_tunnel_interfaces_dut2(
        ...
        """Create multiple IPsec tunnel interfaces on DUT2 node.
        ...
        with PapiSocketExecutor(node, do_async=True) as papi_exec:
            if not existing_tunnels:
                ...
                args = dict(
                    ...
                    prefix=IPUtil.create_prefix_object(
                        param.tun_ips[u"ip2"],
                        96 if param.tun_ips[u"ip2"].version == 6 else 24
                    )
                )
                err_msg = f"Failed to set IP address on interface" \
                    f" {param.if2_key} on host {node[u'host']}"
                papi_exec.add(cmd, **args).get_reply(err_msg)
            # Configure IPsec tunnel interfaces
            cmd = u"ipsec_tunnel_if_add_del"
            args = dict(
            ...
            iter_max = existing_tunnels + 2 if fast else n_tunnels
            for i in range(existing_tunnels, iter_max):
                args[u"local_spi"] = param.spi_d[u"spi_2"] + i
                args[u"remote_spi"] = param.spi_d[u"spi_1"] + i
                args[u"local_ip"] = IPAddress.create_ip_address_object(
                    param.tun_ips[u"ip2"]
                )
                args[u"remote_ip"] = IPAddress.create_ip_address_object(
            ...
            if not existing_tunnels:
                ...
                args = dict(
                    ...
                    route=IPUtil.compose_vpp_route_structure(
                        node, param.tun_ips[u"ip1"].compressed,
                        prefix_len=(
                            32 if param.tun_ips[u"ip1"].version == 6 else 8
                        ),
                        interface=param.if2_key,
                        gateway=(param.tun_ips[u"ip2"] - 1).compressed
                    )
                )
            ...
            for i in range(existing_tunnels, iter_max):
                args[u"route"] = IPUtil.compose_vpp_route_structure(
                    param.nodes[u"DUT1"], (param.raddr_ip1 + i).compressed,
    ...
    def vpp_ipsec_create_tunnel_interfaces(
        ...
        param = IPsecUtil.CreTunIntParam(
        ...
        if u"DUT2" not in nodes.keys():
            return (
                ckeys[0], ikeys[0], spi_d[u"spi_1"], spi_d[u"spi_2"]
            )
