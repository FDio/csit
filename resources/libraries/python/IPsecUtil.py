...
class IPsecUtil:
    ...
    def vpp_ipsec_create_tunnel_interfaces(
            ...
            existing_tunnels=0, spi_1 = 100000, spi_2 = 200000):
        ...
        :param spi_1: VPP node 1 SPI. Default 100000.
        :param spi_2: VPP node 2 SPI. Default 200000.
        :type nodes: dict
        ...
        spi_1 = int(spi_1)
        spi_2 = int(spi_2)
        tun_if1_ip = ip_address(tun_if1_ip_addr)
        tun_if2_ip = ip_address(tun_if2_ip_addr)
        raddr_ip1 = ip_address(raddr_ip1)
        raddr_ip2 = ip_address(raddr_ip2)
        addr_incr = 1 << (128 - raddr_range) if tun_if1_ip.version == 6 \
            else 1 << (32 - raddr_range)
        ...
        if n_tunnels - existing_tunnels > 10:
            ...
            mask = 96 if tun_if2_ip.version == 6 else 24
            mask2 = 128 if tun_if2_ip.version == 6 else 32
            ...
            with open(tmp_fn1, 'w') as tmp_f1:
                ...
                if not existing_tunnels:
                    tmp_f1.write(
                        ...
                        f"{if1_n} {tun_if2_ip - 1}/{mask}\n"
                        f"exec set ip neighbor {if1_n} {tun_if2_ip}/{mask2} "
                        f"{rmac} static\n"
                    )
                for i in range(existing_tunnels, n_tunnels):
                    ckeys.append(
                    ...
                    tmp_f1.write(
                        ...
                        f"{integ} "
                        f"local_ip {tun_if1_ip + i * addr_incr} "
                        f"remote_ip {tun_if2_ip} "
                        f"instance {i}\n"
                    )
            vat.execute_script(
            ...
            with open(tmp_fn2, 'w') as tmp_f2:
                ...
                for i in range(existing_tunnels, n_tunnels):
                    ...
                    tmp_f2.write(
                        ...
                        f"remote_crypto_key {ckeys[i].hex()} "
                        f"{integ} "
                        f"local_ip {tun_if2_ip} "
                        f"remote_ip {tun_if1_ip + i * addr_incr} "
                        f"instance {i}\n"
                    )
            vat.execute_script(
            ...
            with open(tmp_fn2, 'w') as tmp_f2:
                if not existing_tunnels:
                    tmp_f2.write(
                        f"exec ip route add {tun_if1_ip}/8 via {tun_if2_ip - 1}"
                        f" {if2_n}\n"
                    )
                for i in range(existing_tunnels, n_tunnels):
                    tmp_f2.write(
            ...
            with PapiSocketExecutor(nodes[u"DUT1"]) as papi_exec:
                ...
                args1 = dict(
                    ...
                    prefix=IPUtil.create_prefix_object(
                        tun_if2_ip - 1, 96 if tun_if2_ip.version == 6 else 24
                    )
                )
                ...
                args4 = dict(
                    ...
                    neighbor=dict(
                        ...
                        ip_address=str(ip_address(tun_if2_ip_addr))
                    )
                )
        ...
        with PapiSocketExecutor(nodes[u"DUT1"]) as papi_exec:
            ...
            for i in range(existing_tunnels, n_tunnels):
                ...
                args1[u"prefix"] = IPUtil.create_prefix_object(
                    tun_if1_ip + i * addr_incr,
                    128 if tun_if1_ip.version == 6 else 32
                )
        ...
        with PapiSocketExecutor(nodes[u"DUT2"]) as papi_exec:
            if not existing_tunnels:
                ...
                args1 = dict(
                    ...
                    del_all=False,
                    prefix=IPUtil.create_prefix_object(
                        tun_if2_ip, 96 if tun_if2_ip.version == 6 else 24
                    )
                )
            ...
            args2 = dict(
                is_add=True,
                local_ip=IPAddress.create_ip_address_object(tun_if2_ip),
                remote_ip=None,
                local_spi=0,
                remote_spi=0,
                crypto_alg=crypto_alg.alg_int_repr,
            ...
            if not existing_tunnels:
                ...
                route = IPUtil.compose_vpp_route_structure(
                    nodes[u"DUT2"], tun_if1_ip.compressed,
                    prefix_len=32 if tun_if1_ip.version == 6 else 8,
                    interface=if2_key,
                    gateway=(tun_if2_ip - 1).compressed
                )
                args1 = dict(
                    is_add=1,
                    is_multipath=0,
    ...
    @staticmethod
    def vpp_ipsec_create_tunnel_interfaces_on_2node(
            nodes, if1_ip_addr, if2_ip_addr, if1_key, if2_key, n_tunnels,
            crypto_alg, integ_alg, raddr_ip1, raddr_ip2, raddr_range,
            existing_tunnels=0):
        """Create multiple IPsec tunnel interfaces DUT1 VPP node.

        :param nodes: VPP nodes to create tunnel interfaces.
        :param if1_ip_addr: VPP node 1 interface 1 IPv4/IPv6 address.
        :param if2_ip_addr: VPP node 1 interface 2 IPv4/IPv6 address.
        :param if1_key: VPP node 1 interface key 1 from topology file.
        :param if2_key: VPP node 1 interface key 2 from topology file.
        :param n_tunnels: Number of tunnel interfaces to be there at the end.
        :param crypto_alg: The encryption algorithm name.
        :param integ_alg: The integrity algorithm name.
        :param raddr_ip1: Policy selector remote IPv4/IPv6 start address for the
            first tunnel in direction node1->tg.
        :param raddr_ip2: Policy selector remote IPv4/IPv6 start address for the
            first tunnel in direction tg->node1.
        :param raddr_range: Mask specifying range of Policy selector Remote
            IPv4/IPv6 addresses. Valid values are from 1 to 32 in case of IPv4
            and to 128 in case of IPv6.
        :param existing_tunnels: Number of tunnel interfaces before creation.
    ...
    @staticmethod
    def vpp_ipsec_create_tunnel_interfaces_on_2node_1(
            nodes, if1_ip_addr, if2_ip_addr, if1_key, if2_key, n_tunnels,
            crypto_alg, integ_alg, raddr_ip1, raddr_ip2, raddr_range,
            existing_tunnels=0):
        """Create multiple IPsec tunnel interfaces DUT1 VPP node.
        ...
        with PapiSocketExecutor(nodes[u"DUT1"]) as papi_exec:
            ...
            if integ_alg:
                ikeys.append(
                    gen_key(IPsecUtil.get_integ_alg_key_len(integ_alg)))
            history = bool(not 1 < 0 < n_tunnels - 2)
            args2 = dict(
                is_add=1,
                ...
                remote_spi=1001,
                crypto_alg=crypto_alg.alg_int_repr,
                local_crypto_key_len=len(ckeys[0]),
                local_crypto_key=ckeys[0],
                remote_crypto_key_len=len(ckeys[0]),
                remote_crypto_key=ckeys[0],
                #local_crypto_key_len=16,
                #local_crypto_key=bytes("wdwRWPHpQXQWGXiK", 'utf-8'),
                #remote_crypto_key_len=16,
                #remote_crypto_key=bytes("wdwRWPHpQXQWGXiK", 'utf-8'),
                integ_alg=integ_alg.alg_int_repr if integ_alg else 0,
                local_integ_key_len=len(ikeys[0]),
                local_integ_key=ikeys[0],
                remote_integ_key_len=len(ikeys[0]),
                remote_integ_key=ikeys[0],
                tx_table_id=0
            )
            err_msg = f"Failed to add IPsec tunnel interfaces " \
                f"on host {nodes[u'DUT1'][u'host']}"
            ...
            if not existing_tunnels:
                ...
                cmd1 = u"ip_route_add_del"
                route = IPUtil.compose_vpp_route_structure(
                    nodes[u"DUT1"], raddr_ip2.compressed,
                    prefix_len=32 if raddr_ip2.version == 6 else 8,
                    interface=if1_key,
                    gateway=(if2_ip).compressed
                )
                args1 = dict(
                    is_add=1,
                    is_multipath=0,
            ...
            err_msg = f"Failed to add IP routes " \
                f"on host {nodes[u'DUT1'][u'host']}"
            args1[u"unnumbered_sw_if_index"] = ipsec_tunnels[0]
            args2[u"sw_if_index"] = ipsec_tunnels[0]
            args3[u"route"] = IPUtil.compose_vpp_route_structure(
                nodes[u"DUT1"], (raddr_ip2 + 0).compressed,
                prefix_len=128 if raddr_ip2.version == 6 else 32,
                interface=ipsec_tunnels[0]
                )
            history = bool(not 1 < 0 < n_tunnels - 2)
            papi_exec.add(cmd1, history=history, **args1).\
                add(cmd2, history=history, **args2).\
            ...
            papi_exec.get_replies(err_msg)
    @staticmethod
    def vpp_ipsec_create_tunnel_interfaces_on_2node_2v1(
            nodes, if1_ip_addr, if2_ip_addr, if1_key, if2_key, n_tunnels,
            crypto_alg, integ_alg, raddr_ip1, raddr_ip2, raddr_range,
            existing_tunnels=0):
        """Create multiple IPsec tunnel interfaces DUT1 VPP node.
