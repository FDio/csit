        ...
        if int(n_entries) > 10:
            ...
            with open(tmp_filename, 'w') as tmp_file:
                for i in range(n_entries):
                    ...
                    sa = f' sa {sa_id.vat_str()}' if sa_id is not None else ''
                    ...
                    spd_cfg = f"exec ipsec policy add spd {spd_id} " \
                        f"priority {priority.vat_str()} {direction}" \
                        f"{protocol} action {action}{sa} " \
                        f"local-ip-range {laddr_range.vat_str()} " \
                        f"remote-ip-range {raddr_range.vat_str()}" \
