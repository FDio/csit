        ...
        with open(tmp_fn1, u"w") as tmp_f1:
            ...
            for i in range(existing_tunnels, n_tunnels):
                ...
                tmp_f1.write(
                    ...
                    f"{integ} "
                    f"tunnel-src {tun_ips[u'ip1'] + i * addr_incr} "
                    f"tunnel-dst {tun_ips[u'ip2']}\n"
                    ...
                    f"crypto-key {ckeys[i].hex()} "
                    f"{integ} "
                    f"tunnel-src {tun_ips[u'ip2']} "
                    f"tunnel-dst {tun_ips[u'ip1'] + i * addr_incr} "
                    f"inbound\n"
                    
                    f"exec ipsec tunnel protect "
                    f"sa-out {i} "
                    f"sa-in {100000 + i}\n"
                )
        vat.execute_script(
