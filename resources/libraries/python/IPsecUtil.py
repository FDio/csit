...
class IPsecUtil:
    ...
    def vpp_ipsec_create_tunnel_interfaces(
        ...
        logger.debug(f"n_tunnels {n_tunnels} existing_tunnels {existing_tunnels}")
        spi_1 = 100000
        spi_2 = 200000
        ...
        if n_tunnels - existing_tunnels > 10:
            logger.debug(u"VAT part starts.")
            ...
            os.remove(tmp_fn1)
            os.remove(tmp_fn2)
            logger.debug(u"VAT part ended.")
            return

        if not existing_tunnels:
            with PapiSocketExecutor(nodes[u"DUT1"]) as papi_exec:
                logger.debug(u"PAPI, DUT1, before tunnels 1.")
            ...
            logger.debug(u"PAPI, DUT1, existing tunnels.")
        ...
        with PapiSocketExecutor(nodes[u"DUT1"]) as papi_exec:
            ...
            for i in range(existing_tunnels, n_tunnels):
                logger.debug(f"PAPI, DUT1, loop1: index {i}")
                ckeys.append(
            ...
            for reply in replies:
                ...
                if u"sw_if_index" in reply:
                    ...
                    logger.debug(f"appended: {ipsec_tunnels[-1]!r}")
                    logger.debug(f"ipsec_tunnels size: {len(ipsec_tunnels)}")
            # Configure IP routes
            ...
            err_msg = f"Failed to add IP routes " \
                f"on host {nodes[u'DUT1'][u'host']}"
            for i in range(existing_tunnels, n_tunnels):
                logger.debug(f"PAPI, DUT1, loop2: index {i}")
        ...
        with PapiSocketExecutor(nodes[u"DUT2"]) as papi_exec:
            if not existing_tunnels:
                logger.debug(u"PAPI, DUT2, before tunnels 1.")
            ...
            ipsec_tunnels = [None] * existing_tunnels
            for i in range(existing_tunnels, n_tunnels):
                logger.debug(f"PAPI, DUT2, loop1: index {i}")
            ...
            for reply in replies:
                ...
                if u"sw_if_index" in reply:
                    ...
                    logger.debug(f"ipsec_tunnels size: {len(ipsec_tunnels)}")
            if not existing_tunnels:
                logger.debug(u"PAPI, DUT2, before tunnels 2.")
                # Configure IP routes
            ...
            err_msg = f"Failed to add IP routes " \
                f"on host {nodes[u'DUT2'][u'host']}"
            for i in range(existing_tunnels, n_tunnels):
                logger.debug(f"PAPI, DUT2, loop2: index {i}")
                args1[u"unnumbered_sw_if_index"] = ipsec_tunnels[i]
                args2[u"sw_if_index"] = ipsec_tunnels[i]
                args3[u"route"] = IPUtil.compose_vpp_route_structure(
