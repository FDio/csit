...
class IPsecUtil(object):
    ...
    def vpp_ipsec_create_tunnel_interfaces(node1, node2, if1_ip_addr,
            ...
            for i in range(0, n_tunnels):
                ...
                dut1_tunnel = (
                    ...
                    .format(
                        ...
                        integ=integ,
                        local_ip=ip_address(unicode(if1_ip_addr)) + i,
                        remote_ip=ip_address(unicode(if2_ip_addr)) + i))
                dut2_tunnel = (
                    'ipsec_tunnel_if_add_del '
                    'local_spi {local_spi} '
                    'remote_spi {remote_spi} '
                    ...
                    .format(
                        ...
                        remote_crypto_key=ckey,
                        integ=integ,
                        local_ip=ip_address(unicode(if2_ip_addr)) + i,
                        remote_ip=ip_address(unicode(if1_ip_addr)) + i))
        ...
        with open(tmp_fn1, 'w') as tmp_f1, open(tmp_fn2, 'w') as tmp_f2:
            for i in range(0, n_tunnels):
                dut1 = (
                    'exec ip route add {raddr}/32 via {addr} ipsec{i}\n'
                    'exec set interface unnumbered ipsec{i} use {uifc}\n'
                    'exec set interface state ipsec{i} up\n'
                    .format(
                        raddr=ip_address(unicode(raddr_ip2)) + addr_incr * i,
                        addr=ip_address(unicode(if2_ip_addr)) + i,
                        i=i,
                ...
                dut2 = (
                    ...
                    'exec set interface state ipsec{i} up\n'\
                    .format(
                        raddr=ip_address(unicode(raddr_ip1)) + addr_incr * i,
                        addr=ip_address(unicode(if1_ip_addr)) + i,
                        i=i,
