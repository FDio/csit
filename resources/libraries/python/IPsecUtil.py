...
class IPsecUtil:
    ...
    # We are generating random keys. But if the last byte
    # happens to be same in first two messages, fast papi generator
    # could "overflow" to a preceding field.
    # In order to prevent that, we make sure the last bit always flips.
    LAST_KEY_BIT = False
    ...
    def gen_key(cls, length):
        ...
        if cls.LAST_KEY_BIT:
            last_byte &= 0xfe
            cls.LAST_KEY_BIT = False
        else:
            last_byte |= 1
            cls.LAST_KEY_BIT = True
        key = key[:-1] + bytes([last_byte])
        return key
