...
class IPsecUtil:
    ...
    def vpp_ipsec_create_spds_match_nth_entry(
            ...
            inbound=False, bidirectional=True, other_dir_traffic=False):
        ...
        :param other_dir_traffic: Add an entry for all IPv4/IPv6 traffic in the
            other traffic direction. Other selectors are the same.
        :type node: dict
        ...
        if no_match_entry_amount > 0:
            ...
            no_match_laddr_range = NetworkIncrement(
                ip_network(local_addr_range), 1
            )
            next(no_match_laddr_range)

            no_match_raddr_range = NetworkIncrement(
                ip_network(remote_addr_range), 1
            )
            next(no_match_raddr_range)
            ...
            IPsecUtil.vpp_ipsec_add_spd_entries(
                ...
                inbound=inbound, laddr_range=no_match_laddr_range,
                raddr_range=no_match_raddr_range
            )
