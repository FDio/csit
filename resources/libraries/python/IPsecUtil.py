...
class IPsecUtil:
    ...
    def vpp_ipsec_set_ip_route(
        ...
        with PapiSocketExecutor(node) as papi_exec:
            for i in range(n_tunnels):
                args1[u"prefix"] = AddressWithPrefix.of(
    ...
    def vpp_ipsec_add_spd_entry(
        ...
        spd_entry = dict(
            ...
            remote_address_start=Address.of(remote_net.network_address),
            remote_address_stop=Address.of(remote_net.broadcast_address),
            local_address_start=Address.of(local_net.network_address),
            local_address_stop=Address.of(local_net.broadcast_address),
    ...
    def _ipsec_create_tunnel_interfaces_dut1_papi(
        ...
        with PapiSocketExecutor(nodes[u"DUT1"]) as papi_exec:
            ...
            for i in range(existing_tunnels, n_tunnels):
                args[u"prefix"] = AddressWithPrefix.of(
            ...
            for i in range(existing_tunnels, n_tunnels):
                args[u"tunnel"][u"src"] = Address.of(
                    tun_ips[u"ip1"] + i * addr_incr
                )
                args[u"tunnel"][u"dst"] = Address.of(tun_ips[u"ip2"])
                papi_exec.add(
                    cmd, history=bool(not 1 < i < n_tunnels - 2), **args
                )
    ...
    def _ipsec_create_tunnel_interfaces_dut2_papi(
        ...
        with PapiSocketExecutor(nodes[u"DUT2"]) as papi_exec:
            ...
            for i in range(existing_tunnels, n_tunnels):
                ...
                args[u"tunnel"][u"dst"] = Address.of(
                    tun_ips[u"ip1"] + i * addr_incr
                )
                papi_exec.add(
                    cmd, history=bool(not 1 < i < n_tunnels - 2), **args
