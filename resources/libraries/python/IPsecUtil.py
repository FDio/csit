    ...
    def _ipsec_create_tunnel_interfaces_dut1_papi(
        ...
        with PapiSocketExecutor(nodes[u"DUT1"]) as papi_exec:
            ...
            for i in range(existing_tunnels, n_tunnels):
                ...
                if integ_alg:
                    args[u"entry"][u"integ_key"][u"length"] = len(ikeys[i])
                    args[u"entry"][u"integ_key"][u"data"] = ikeys[i]
                args[u"entry"][u"tunnel_src"] = \
                    IPAddress.create_ip_address_object(
                        tun_ips[u"ip1"] + i * addr_incr
                    )
            ...
            cmd = u"ipsec_tunnel_protec_update"
            n_hop = dict(
                address=0,
