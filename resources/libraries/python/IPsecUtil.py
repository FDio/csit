        ...
        if n_tunnels > 10:
            ...
            with open(tmp_fn1, 'w') as tmp_f1, open(tmp_fn2, 'w') as tmp_f2:
                ...
                tmp_f1.write(
                    ...
                    f"{if1_n} {if2_ip - 1}/{mask2}\n"
                    f"exec set ip arp {if1_n} {if2_ip}/{mask2} {rmac1} static\n"
                )
                tmp_f2.write(
                    f"sw_interface_set_table {if2_n} vrf 0\n"
                    f"exec set interface ip address {if2_n} {if2_ip}/{mask2}\n"
                    f"exec set ip arp "
                    f"{if2_n} {if2_ip - 1}/{mask2} {rmac2} static\n"
                    f"exec ip route add {ip_network(if1_ip_addr + '/8', False)}"
                    f" via {if2_n} {if2_ip - 1}\n"
                )
                for i in range(n_tunnels):
                    ckey = gen_key(
