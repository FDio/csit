...
class IPsecUtil:
    ...
    def vpp_ipsec_spd_add_entries(
        ...
        spd_entry = dict(
            ...
            is_outbound=not inbound,
            sa_id=int(sa_id) if sa_id else 0,
            policy=getattr(PolicyAction,u"PROTECT").policy_int_repr,
            protocol=0,
    ...
    def _ipsec_create_tunnel_interfaces_dut1_papi(
        ...
        with PapiSocketExecutor(nodes[u"DUT1"]) as papi_exec:
            ...
            # for reply in papi_exec.get_replies(
            #         f"Failed to add IPsec tunnel interfaces on host "
            #         f"{nodes[u'DUT1'][u'host']}"):
            #     if u"sw_if_index" in reply:
        ...
        for tnl in range(0, n_tunnels):
            ...
            if integ_alg:
                ...
                integ = (
                    ...
                    f"local-integ-key {ikey.hex()} "
                    f"remote-integ-key {ikey.hex()} "
                )
            ...
            dut1_scripts[cnf].write(
                ...
                f"crypto-alg {crypto_alg.alg_name} "
                f"local-crypto-key {ckey.hex()} "
                f"remote-crypto-key {ckey.hex()} "
                f"instance {i} "
                f"salt 0x0 "
                f"{integ} \n"
