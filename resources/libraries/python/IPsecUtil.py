...
from resources.libraries.python.CommonUtil import ObjIncrement, \
    NetworkIncrement
...
class IPsecUtil:
    ...
    @staticmethod
    def vpp_ipsec_create_spds_with_rules_nth_match(
            node, dir1_interface, dir2_interface, rule_amount,
            src_addr_range, dst_addr_range):
        """Create one matching SPD rule for outbound traffic on a DUT for
        each traffic direction and also create rule_amount - 1 non-matching
        SPD rules. Create a Security Policy Database on each outbound
        interface where these rules will be configured.
        The matching SPD rule will have the lowest priority, action BYPASS and
        will be configured to match the IP flow. The non-matching rules will
        be the same, except with higher priority and non-matching IP flows.

        :param node: VPP node to configured the SPDs and their rules.
        :param dir1_interface: The interface in direction 1 where the rules
            will be checked.
        :param dir2_interface: The interface in direction 2 where the rules
            will be checked.
        :param rule_amount: The number of SPD rules to configure. If
            rule_amount == 1, no non-matching rules will be configured.
        ...
        # matching rule direction 1
        IPsecUtil.vpp_ipsec_add_spd_entry(
            node, spd_id_dir1, matching_priority, PolicyAction.BYPASS,
            inbound=False, laddr_range=src_addr_range,
            raddr_range=dst_addr_range
        )
        # matching rule direction 2, the address ranges are switched
        IPsecUtil.vpp_ipsec_add_spd_entry(
            node, spd_id_dir2, matching_priority, PolicyAction.BYPASS,
            inbound=False, laddr_range=dst_addr_range,
            raddr_range=src_addr_range
        )

        # non-matching rules
        no_match_rule_amount = rule_amount - 1
        if no_match_rule_amount > 0:
            # create a NetworkIncrement representation of the network,
            # then skip the matching network
            no_match_src_addr_range = next(NetworkIncrement(
                src_addr_range, 1
            ))

            no_match_dst_addr_range = next(NetworkIncrement(
                dst_addr_range, 1
            ))

            # non-matching rules direction 1
            IPsecUtil.vpp_ipsec_add_spd_entries(
                node, no_match_rule_amount, spd_id_dir1,
                ObjIncrement(matching_priority + 1, 1), PolicyAction.BYPASS,
                inbound=False,
                laddr_range=NetworkIncrement(no_match_src_addr_range, 1),
                raddr_range=NetworkIncrement(no_match_dst_addr_range, 1)
            )

            # non-matching rules direction 2, the address ranges are switched
            IPsecUtil.vpp_ipsec_add_spd_entries(
                node, no_match_rule_amount, spd_id_dir2,
                ObjIncrement(matching_priority + 1, 1), PolicyAction.BYPASS,
                inbound=False,
                laddr_range=NetworkIncrement(no_match_dst_addr_range, 1),
                raddr_range=NetworkIncrement(no_match_src_addr_range, 1)
            )
    ...
    def vpp_ipsec_add_spd_entries(
        ...
        :type n_entries: int
        :type spd_id: int
        :type priority: IPsecUtil.Increment
        :type action: IPsecUtil.PolicyAction
        :type inbound: bool
        :type sa_id: IPsecUtil.Increment
        :type proto: int
        ...
        if laddr_range is None:
            laddr_range = u"::/0" if is_ipv6 else u"0.0.0.0/0"
            laddr_range = NetworkIncrement(laddr_range, 0)

        if raddr_range is None:
            raddr_range = u"::/0" if is_ipv6 else u"0.0.0.0/0"
            raddr_range = NetworkIncrement(raddr_range, 0)
        ...
        if int(n_entries) > 10:
            ...
            with open(tmp_filename, 'w') as tmp_file:
                for i in range(n_entries):
                    ...
                    sa = f' sa {sa_id}' if sa_id is not None else ''
                    ...
                    spd_cfg = f"exec ipsec policy add spd {spd_id} " \
                        ...
                        f"local-ip-range {laddr_range} " \
                        f"remote-ip-range {raddr_range}" \
                        f"{local_port_range}{remote_port_range}\n"

                    if i < 3:
                        logger.trace(spd_cfg)
        ...
        for i in range(n_entries):
            IPsecUtil.vpp_ipsec_add_spd_entry(
                node, spd_id, next(priority), action, inbound, next(sa_id),
    ...
    def vpp_ipsec_add_multiple_tunnels(
        ...
        IPsecUtil.vpp_ipsec_add_spd_entries(
            ...
            raddr_range=NetworkIncrement(raddr_ip2, 1)
        )
        ...
        IPsecUtil.vpp_ipsec_add_spd_entries(
            ...
            raddr_range=NetworkIncrement(raddr_ip1, 1)
        )
        ...
        if u"DUT2" in nodes.keys():
            ...
            IPsecUtil.vpp_ipsec_add_spd_entry(
                ...
                laddr_ip_start=u"100.0.0.0", laddr_ip_stop=u"100.255.255.255",
                raddr_ip_start=u"100.0.0.0", raddr_ip_stop=u"100.255.255.255"
            )
            IPsecUtil.vpp_ipsec_add_spd_entry(
                nodes[u"DUT2"], spd_id, p_hi, PolicyAction.BYPASS,
            ...
            IPsecUtil.vpp_ipsec_add_spd_entries(
                ...
                action=PolicyAction.PROTECT, inbound=True,
                sa_id=ObjIncrement(sa_id_1, 1),
                raddr_ip_start=ObjIncrement(ip_address(raddr_ip2), 1)
            )
            ...
            IPsecUtil.vpp_ipsec_add_spd_entries(
                ...
                action=PolicyAction.PROTECT, inbound=False,
                sa_id=ObjIncrement(sa_id_2, 1),
                raddr_ip_start=ObjIncrement(ip_address(raddr_ip1), 1)
            )
