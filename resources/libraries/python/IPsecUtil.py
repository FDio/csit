...
class IPsecUtil(object):
    ...
    def vpp_ipsec_create_tunnel_interfaces_in_containers(
        ...
        for cnf in range(0, n_instances):
            cnf_script_file = '/tmp/ipsec_create_tunnel_cnf_dut2_{cnf}-1.config'.format(cnf=cnf)
            cnf_scripts.append(open(cnf_script_file, 'w'))
            cnf_scripts[cnf].write(
                'exec set interface ip address {uifc} {iaddr}/24\n'
                .format(
                    iaddr=ip_address(unicode(if2_ip_addr)) + cnf,
                    uifc=Topology.get_interface_name(nodes['DUT2'], if2_key)))
            cnf_script2_file = '/tmp/ipsec_create_tunnel_cnf_dut2_{cnf}-2.config'.format(cnf=cnf)
            cnf_scripts2.append(open(cnf_script2_file, 'w'))
            cnf_scripts2[cnf].write(
                'exec ip route add {raddr} via {uifc}\n'
                .format(
                    raddr=ip_network(unicode(if1_ip_addr+'/8'), False),
                    uifc='memif1/{cnf}'.format(cnf=cnf+1)))
        cnf = 0
        i = 0
        with open(dut1_script_file, 'w') as dut1_script:
            for tnl in range(0, n_tunnels):
                ckey = gen_key(IPsecUtil.get_crypto_alg_key_len(crypto_alg)).encode('hex')
                integ = ''
                if integ_alg:
                    ikey = gen_key(IPsecUtil.get_integ_alg_key_len(integ_alg)).encode('hex')
                    integ = (
                        'integ_alg {integ_alg} '
                        'local_integ_key {local_integ_key} '
                        'remote_integ_key {remote_integ_key} '
                ...
                dut1_script.write(
                    ...
                    .format(
                        ...
                        remote_crypto_key=ckey,
                        integ=integ,
                        laddr=ip_address(unicode(if1_ip_addr)) + (tnl * addr_incr),
                ...
                cnf_scripts[cnf].write(
                    ...
                    .format(
                        ...
                        remote_crypto_key=ckey,
                        integ=integ,
                        raddr=ip_address(unicode(if1_ip_addr)) + (tnl * addr_incr),
                ...
                if cnf +1 < n_instances:
                    cnf += 1
                else:
                    cnf = 0
                    i += 1

        vat.execute_script(dut1_script_file, nodes['DUT1'], timeout=3600, json_out=False,
                           copy_on_execute=True)
        for cnf in range(0, n_instances):
            cnf_scripts[cnf].close()
            cnf_script_file = '/tmp/ipsec_create_tunnel_cnf_dut2_{cnf}-1.config'.format(cnf=cnf)
            vat.execute_script_in_container(cnf_script_file, nodes['DUT2'], container_id='DUT2_CNF{cnfid}'.format(cnfid=cnf+1),
                                            timeout=3600, json_out=False)
        time.sleep(20)
        for cnf in range(0, n_instances):
            cnf_scripts2[cnf].close()
            cnf_script_file2 = '/tmp/ipsec_create_tunnel_cnf_dut2_{cnf}-2.config'.format(cnf=cnf)
            vat.execute_script_in_container(cnf_script_file2, nodes['DUT2'], container_id='DUT2_CNF{cnfid}'.format(cnfid=cnf+1),
                                            timeout=3600, json_out=False)
