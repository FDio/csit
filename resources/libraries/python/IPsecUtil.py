...
class IPsecUtil(object):
    ...
    def vpp_ipsec_set_ip_route(
        ...
        address_args = dict(
            ...
            is_add=1,
            is_ipv6=1 if tunnel_src.version == 6 else 0,
            del_all=0,
        ...
        with PapiSocketExecutor(node) as papi_exec:
            for param in params:
                address_args['address'] = IPUtil.packed(param['tunnel_src'])
...
def _dut1_vpp_ipsec_create_tunnels(
    ...
    with PapiSocketExecutor(nodes['DUT1']) as papi_exec:
        ...
        args1 = dict(
            ...
            is_add=1,
            is_ipv6=1 if if2_ip_addr.version == 6 else 0,
            del_all=0,
            address_length=96 if if2_ip_addr.version == 6 else 24,
            address=IPUtil.packed(if2_ip_addr - 1))
        ...
        args1 = dict(
            sw_if_index=loop_sw_if_idx,
            is_add=1,
            is_ipv6=1 if if1_ip_addr.version == 6 else 0,
            del_all=0,
            address_length=128 if if1_ip_addr.version == 6 else 32,
            address='')
