...
class IPsecUtil:
    ...
    def vpp_ipsec_add_sad_entry(
        ...
        sad_entry = dict(
            ...
            anti_replay_window_size=64,  # default value in api
        )
    ...
    def _ipsec_create_tunnel_interfaces_dut1_papi(
            ...
            raddr_ip2, addr_incr, spi_d, existing_tunnels=0, udp_encap=False):
        ...
        with PapiSocketExecutor(nodes[u"DUT1"], is_async=True) as papi_exec:
            ...
            flags = IPsecSadFlags.IPSEC_API_SAD_FLAG_NONE
            if udp_encap:
                flags |= IPsecSadFlags.IPSEC_API_SAD_FLAG_UDP_ENCAP
            sad_entry = dict(
                sad_id=None,
                spi=None,
                protocol=int(IPsecProto.IPSEC_API_PROTO_ESP),
            ...
            for i in range(existing_tunnels, n_tunnels):
                ...
                if integ_alg:
                    args[u"entry"][u"integrity_key"][u"length"] = len(ikeys[i])
                    args[u"entry"][u"integrity_key"][u"data"] = ikeys[i]
                flags = IPsecSadFlags.IPSEC_API_SAD_FLAG_IS_INBOUND
                if udp_encap:
                    flags |= IPsecSadFlags.IPSEC_API_SAD_FLAG_UDP_ENCAP
                args[u"entry"][u"flags"] = flags
                papi_exec.add(
                    cmd, history=bool(not 1 < i < n_tunnels - 2), **args
                )
    ...
    def _ipsec_create_tunnel_interfaces_dut2_papi(
        ...
        ikeys, raddr_ip1, addr_incr, spi_d, existing_tunnels=0, udp_encap=False
    ):
    ...
    def vpp_ipsec_create_tunnel_interfaces(
            ...
            existing_tunnels=0, return_keys=False, udp_encap=False):
