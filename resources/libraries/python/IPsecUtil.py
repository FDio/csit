...
class IPsecUtil:
    ...
    @staticmethod
    def vpp_ipsec_create_spd_bidir_match_nth_entry_outbound_bypass(
            node, dir1_interface, dir2_interface, entry_amount,
            src_addr_range, dst_addr_range):
        """Create one matching SPD entry for outbound traffic on a DUT for
        each traffic direction and also create entry_amount - 1 non-matching
        SPD entries. Create a Security Policy Database on each outbound
        interface where these entries will be configured.
        The matching SPD entry will have the lowest priority, action BYPASS and
        ...
        if no_match_entry_amount > 0:
            ...
            # non-matching entries direction 2, the address ranges are switched
            IPsecUtil.vpp_ipsec_add_spd_entries(
