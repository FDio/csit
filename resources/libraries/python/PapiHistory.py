...
def setup_papi_history(nodes):
    """Setup PAPI history for DUT nodes.

    :param nodes: DUT nodes to setup PAPI history for.
    :type nodes: dict
    :returns: Dictionary to store the PAPI history for all nodes.
    :rtype: dict
    """
    duts_papi_history = dict()
    try:
        PapiHistory.reset_papi_history_on_all_duts(nodes)
        return duts_papi_history
    except AttributeError:
        # Necessary for the generation of source code documentation.
        pass


DICT__DUTS_PAPI_HISTORY = setup_papi_history(DICT__nodes)
...
class PapiHistory(object):
    ...
    def add_to_papi_history(node, cmd, **kwargs):
        ...
        for key, val in kwargs.iteritems():
            args += "{key}={val}, ".format(key=key, val=val)
        item = "{cmd}({args})".format(cmd=cmd, args=args[:-2])
