...
def install_dmm_test(node):
    ...
    (ret_code, _, stderr) = ssh.exec_command(
            'cd {0}/{1} && ./install_prereq.sh 2>&1 | tee log_install_prereq.txt'
            .format(con.REMOTE_FW_DIR, con.DMM_SCRIPTS), timeout=600)

    if ret_code != 0:
        logger.error('Install the prereq error: {0}'.format(stderr))
        raise RuntimeError('Install the prereq failed')
    else:
        logger.console('Install the prereq on {0} success!'.format(node['host']))
