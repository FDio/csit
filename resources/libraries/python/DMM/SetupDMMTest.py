...
def pack_framework_dir():
    ...
    """
    # Remove the DMM and DPDK dir existed.
    os.system("rm -rf dmm")
    os.system("rm -rf dpdk")
    os.system("rm -f dpdk-16.04.tar.xz")

    # Get the latest DMM and dpdk code.
    os.system("git clone https://gerrit.fd.io/r/dmm")
    os.system("wget http://fast.dpdk.org/rel/dpdk-16.04.tar.xz")
    os.system("cd ../../../../tests/dmm/dmm_scripts/ && "
              "chmod +x build_dpdk_dmm.sh && ./build_dpdk_dmm.sh")
    os.system("cd -")
    ...
    proc = Popen(
        split("tar --exclude-vcs --exclude=./tmp --exclude=dmm_depends.tar.gz -zcf "
              "{0} .".format(file_name)), stdout=PIPE, stderr=PIPE)
...
def extract_tarball_at_node(tarball, node):
    ...
    """
    tarball2 = "/tmp/dmm_depends.tar.gz"
    ...
    cmd = 'sudo rm -rf {1}; mkdir {1} ; tar -zxf {0} -C {1}; ' \
          'rm -f {0}; tar -zxf {2} -C {1}; sudo rm -f {2}'.format(
          tarball, con.REMOTE_FW_DIR, tarball2)
...
def install_dmm_test(node):
    ...
    logger.console('Install the DMM on {0} ({1})'.format(node['host'],
                                                          arch))
