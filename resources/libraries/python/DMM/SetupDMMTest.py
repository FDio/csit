...
def pack_framework_dir():
    ...
    proc = Popen(
        split("tar --exclude-vcs --exclude=./tmp --exclude=./dmm "
              "--exclude=dmm_depends.tar.gz --exclude=dpdk-16.04.tar.xz -zcf "
...
def setup_node(args):
    ...
    try:
        if node['type'] == NodeType.DUT:
            ...
            # extract dmm_depends.tar.gz which is already copied
