...
class SingleCliSer(object):
    ...
    def exec_the_base_lwip_test(dut1_node, dut2_node):
        ...
        cmd = 'cd {0}/{1} && ./run_dmm_with_lwip.sh {2} {3} {4} ' \
            .format(con.REMOTE_FW_DIR, con.DMM_SCRIPTS, dut1_ip, dut2_ip, 0)
    ...
    def echo_running_log (dut1_node, dut2_node):
        ...
        (ret_code, stdout, _) = ssh.exec_command(cmd, timeout=100)
        if ret_code != 0:
            raise RuntimeError('Failed to get runningLog at node {0}'
                               .format(dut1_node['host']))
        else:
    ...
    def echo_dpdk_log (dut1_node, dut2_node):
        ...
        (ret_code, stdout, _) = ssh.exec_command(cmd, timeout=100)
        if ret_code != 0:
            raise RuntimeError('Failed to get nstack_dpdkLog at node {0}'
                               .format(dut1_node['host']))
        else:
    ...
    @staticmethod
    def echo_lpci_and_ifconfig (dut1_node, dut2_node):
        """
        :param dut1_node:
        :param dut2_node:
        :return:
        ...
        ssh = SSH()
        ssh.connect(dut1_node)
        cmd = 'ifconfig; lspci; ifconfig -a ;  lspci -nn; lsmod | grep uio'

        (ret_code, stdout, _) = ssh.exec_command(cmd, timeout=100)
        if ret_code != 0:
            raise RuntimeError('Failed to get ifconfig && lspci at node {0}'
                               .format(dut1_node['host']))
        else:
            logger.console('....ifconfig && lspci on Server node {1}.... {0}'
                           .format(stdout, dut1_node['host']))

        ssh = SSH()
        ssh.connect(dut2_node)
        cmd = 'ifconfig; lspci; ifconfig -a ;  lspci -nn'
        ...
        if ret_code != 0:
            ...
            logger.console('....ifconfig && lspci on Client node {1}.... {0}'
                           .format(stdout, dut2_node['host']))

    @staticmethod
    def echo_lsmod (dut1_node, dut2_node):
        """
        :param dut1_node:
        :param dut2_node:
        :return:
    ...
    @staticmethod
    def check_dpdk_installation(dut1_node, dut2_node):
        """
        :param dut1_node:
        :param dut2_node:
        :return:
        ...
        ssh = SSH()
        ssh.connect(dut1_node)
        cmd = 'find /usr -name dpdk'

        (ret_code, stdout, _) = ssh.exec_command(cmd, timeout=100)
        if ret_code != 0:
            raise RuntimeError('Failed to get dpdk info at node {0}'
                               .format(dut1_node['host']))
        else:
            logger.console('....Server dpdk info on node {1}.... {0}'
                           .format(stdout, dut1_node['host']))
        ...
        if ret_code != 0:
            ...
            logger.console('....Client dpdk info on node {1}.... {0}'
                           .format(stdout, dut2_node['host']))
