...
class IPFIXSetup(object):
    ...
    def setup_ipfix_exporter(node, collector, source, fib=None, mtu=None,
        ...
        fib = "vrf_id "+str(fib) if fib is not None else ''
        mtu = "path_mtu "+str(mtu) if mtu is not None else ''
        interval = "template_interval "+str(interval)\
    ...
    def assign_interface_to_flow_table(node, interface, table_id,
        ...
        if isinstance(interface, basestring):
            ...
            raise Exception("what is this interface")

        table = ip_version+"-table "+str(table_id)
    ...
    def set_ipfix_stream(node, domain=None, src_port=None):
        ...
        domain = "domain "+str(domain) if domain is not None else ''
        src_port = "src_port "+str(src_port) if src_port is not None else ''
