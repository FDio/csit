...
class VppHugePageUtil(object):
    ...
    def get_actual_huge_pages(self):
        ...
        :returns the total, free and hugepage size
        :rtype: tuple
        """
    ...
    def show_huge_pages(self):
        ...
        node = self._node
        hugepages = node['hugepages']
        print "  {:15}: {}".format("Actual Huge Page Total",
                                   hugepages['actual_total'])
        print "  {:15}: {}".format("Configured Huge Page Total",
                                   hugepages['total'])
        print "  {:15}: {}".format("Huge Pages Free", hugepages['free'])
        print "  {:15}: {}".format("Huge Page Size", hugepages['size'])
