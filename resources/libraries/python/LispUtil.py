...
class LispUtil(object):
    ...
    def vpp_show_lisp_state(node):
        ...
        data[u"feature_status"] = u"enabled" if reply[u"feature_status"] else \
            u"disabled"
        data[u"gpe_status"] = u"enabled" if reply[u"gpe_status"] else u"disabled"
        return data
    ...
    def vpp_show_lisp_eid_table(node):
        ...
        for eid_details in details:
            ...
            if eid_details[u"eid_type"] == 0:
                ...
                eid = str(IPv4Address(eid_details[u"eid"][0:4])) + u"/" + prefix
    ...
    def vpp_show_lisp_map_register(node):
        ...
        err_msg = f"Failed to get LISP map register state on host {node[u'host']}"
    ...
    def vpp_show_lisp_map_request_mode(node):
        ...
        data = dict()
        data[u"map_request_mode"] = u"src-dst" if reply[u"mode"] else u"dst-only"
        logger.info(data)
        return data
