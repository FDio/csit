...
class LispUtil(object):
    ...
    def vpp_show_lisp_state(node):
        ...
        with PapiExecutor(node) as papi_exec:
            rpl = papi_exec.add(cmd).get_replies(err_msg).\
                verify_reply(err_msg=err_msg)

        data["feature_status"] = "enabled" if rpl["feature_status"] else \
            "disabled"
        data["gpe_status"] = "enabled" if rpl["gpe_status"] else "disabled"
        return data
    ...
    def vpp_show_lisp_locator_set(node, items_filter):
        ...
        args = dict( items_filter=items_filter )
