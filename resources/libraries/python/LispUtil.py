...
class LispUtil(object):
    ...
    def vpp_show_lisp_locator_set(node, items_filter):
        ...
        try:
            with PapiExecutor(node) as papi_exec:
                data = papi_exec.add(cmd, **args).get_dump(err_msg)
# TODO: remove
                logger.info(data)
                logger.info(data.reply)
                logger.info(data.reply[0]["api_reply"])
                logger.info(data.reply[0]["api_reply"][0]["lisp_locator_set_details"])
                logger.info(data.reply[0]["api_reply"][0]["lisp_locator_set_details"]["ls_name"])
