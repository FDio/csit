...
class LispUtil(object):
    ...
    def vpp_show_lisp_locator_set(node, items_filter):
        ...
        try:
            ...
            for details in dump.reply[0]["api_reply"]:
                data.append({"ls_name": details["lisp_locator_set_details"]\
                    ["ls_name"].rstrip('\x00'),
                             "ls_index": details["lisp_locator_set_details"]\
                                 ["ls_index"]})
            return data
    ...
    def vpp_show_lisp_eid_table(node):
        ...
        for details in dump.reply[0]["api_reply"]:
            ...
            if details["lisp_eid_table_details"]["eid_type"] == 0:
                prefix = str(details["lisp_eid_table_details"]\
                                 ["eid_prefix_len"])
                eid = str(IPv4Address(details["lisp_eid_table_details"]\
                                          ["eid"][0:4])) + "/" + prefix
            elif details["lisp_eid_table_details"]["eid_type"] == 1:
                prefix = str(details["lisp_eid_table_details"]\
                                 ["eid_prefix_len"])
                eid = str(IPv6Address(details["lisp_eid_table_details"]\
                                          ["eid"])) + "/" + prefix
            elif details["lisp_eid_table_details"]["eid_type"] == 2:
                eid = str(L2Util.bin_to_mac(details["lisp_eid_table_details"]\
                                                ["eid"][0:6]))
            data.append({"action": details["lisp_eid_table_details"]["action"],
                         "is_local": details["lisp_eid_table_details"]\
                             ["is_local"],
                         "eid": eid,
                         "vni": details["lisp_eid_table_details"]["vni"],
                         "ttl": details["lisp_eid_table_details"]["ttl"],
                         "authoritative": details["lisp_eid_table_details"]\
                             ["authoritative"]})
        return data
    ...
    def vpp_show_lisp_map_resolver(node):
        ...
        data = []
        for details in dump.reply[0]["api_reply"]:
            addr = 'Bad is_ipv6 flag'
            if details["lisp_map_resolver_details"]["is_ipv6"] == 0:
                addr = str(IPv4Address(details["lisp_map_resolver_details"]\
                                           ["ip_address"][0:4]))
            elif details["lisp_map_resolver_details"]["is_ipv6"] == 1:
                addr = str(IPv6Address(details["lisp_map_resolver_details"]\
                                           ["ip_address"]))
            data.append({"map resolver": addr})
        return data
    ...
    def vpp_show_lisp_map_server(node):
        ...
        for details in dump.reply[0]["api_reply"]:
            ...
            if details["lisp_map_server_details"]["is_ipv6"] == 0:
                addr = str(IPv4Address(details["lisp_map_server_details"]\
                                           ["ip_address"][0:4]))
            elif details["lisp_map_server_details"]["is_ipv6"] == 1:
                addr = str(IPv6Address(details["lisp_map_server_details"]\
                                           ["ip_address"]))
            data.append({"map-server": addr})
        logger.info(data)
        return data
...
# Note: VAT is returning ipv6 address instead of ipv4
        ...
        addr = 'Bad is_ip4 flag'
        if reply["is_ip4"] == 0:
            addr = str(IPv6Address(reply["address"]))
        elif reply["is_ip4"] == 1:
            addr = str(IPv4Address(reply["address"][0:4]))
        data["address"] = addr
        logger.info(data)
        return data
