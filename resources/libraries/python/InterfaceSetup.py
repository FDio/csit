...
class InterfaceSetup(object):
    ...
    def create_vlan_subinterface(node, interface, vlan):
        ...
        :type interface: str
        :type vlan: int
        :return: index of created subinterface
        :rtype: str
        """
        ...
        #     vat.vat_terminal_exec_cmd('exec create sub {interface} {vlan} dot1q {vlan} exact-match'.format(interface=interface, vlan=vlan))
        #     vat.vat_terminal_exec_cmd('exec set interface state {interface}.{vlan} up'.format(interface=interface, vlan=vlan))

        output = VatExecutor.cmd_from_template(node, "add_vlan_subif.vat", sw_if_index=sw_if_index, vlan=vlan)
        output = output[0]
        ...
        VatExecutor.cmd_from_template(node, 'set_if_state.vat', sw_if_index=sw_subif_index, state='admin-up')

        with VatTerminal(node, False) as vat:
            vat.vat_terminal_exec_cmd('exec show interfaces')

        # return '{}.{}'.format(interface, vlan), 9
