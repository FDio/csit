...
class ContainerManager(object):
    ...
    def start_all_containers_with_vpp(self):
...
class ContainerEngine(object):
    ...
    def restart_vpp(self):
        ...
        self.execute('service vpp stop && sleep 3')
        self.execute('service vpp start && sleep 3')

    def create_vpp_startup_config(self):
        """Create base startup configuration of VPP on container on DUT node."""
        ...
        if cpuset_cpus:
            worker_thread = ','.join(str(cpu) for cpu in cpuset_cpus)
            vpp_config.add_cpu_corelist_workers(worker_thread)
        vpp_config.add_plugin_disable('dpdk_plugin.so')

        self.execute('echo "{0}" | sudo tee {1}'\
...
class Docker(ContainerEngine):
    ...
    def is_container_running(self):
        ...
        :rtype: bool
        """
        cmd = 'docker ps -q --filter name={c.name}'.format(c=self.container)
