...
class ContainerManager(object):
    ...
    def configure_vpp_in_all_containers(self, vat_template_file, topo=None):
        """Configure VPP in all containers.

        :param vat_template_file: Template file name of a VAT script.
        :param topo: Topology can be chain or cross-pipe.
        ...
        else if topo == 'cross-pipe':
            for i, container in enumerate(self.containers):
                mid1 = i % mod + 1
                sid1 = i % mod * 2 + 1
                self.engine.container = self.containers[container]
                self.engine.create_vpp_startup_config('dummy')
        else:
            raise RuntimeError('Container topology {topo} not implemented'.
                               format(topo=topo))
