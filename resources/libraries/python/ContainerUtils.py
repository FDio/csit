...
class ContainerManager(object):
    ...
    def configure_vpp_in_all_containers(self, chain_topology, dut1_if=None,
                                        dut2_if=None, memif_rx_mode='polling'):
        ...
        :param memif_rx_mode: Memif interface rx-mode (optional, default is
            polling).
        :type chain_topology: str
        :type dut1_if: str
        :type dut2_if: str
        :type memif_rx_mode: str
        """
        ...
        if chain_topology == 'chain':
            for i, container in enumerate(self.containers):
                ...
                sid1 = i % mod * 2 + 1
                sid2 = i % mod * 2 + 2
                if memif_rx_mode == 'interrupt':
                    memif1_rx_mode = 'set interface rx-mode memif{sid}/{mid} ' \
                                     'interrupt'.format(mid=mid1, sid=sid1)
                    memif2_rx_mode = 'set interface rx-mode memif{sid}/{mid} ' \
                                     'interrupt'.format(mid=mid2, sid=sid2)
                else:
                    memif1_rx_mode = ''
                    memif2_rx_mode = ''
                self.engine.container = self.containers[container]
                if self.engine.container.cpuset_cpus:
                ...
                self.engine.create_vpp_exec_config(container_vat_template, \
                    ...
                    guest_dir=guest_dir, socket1='memif-{c.name}-{sid}'. \
                    format(c=self.engine.container, sid=sid1), \
                    socket2='memif-{c.name}-{sid}'. \
                    format(c=self.engine.container, sid=sid2), \
                    memif1_rx_mode=memif1_rx_mode, \
                    memif2_rx_mode=memif2_rx_mode)
...
class ContainerEngine(object):
    ...
    def install_vpp(self):
        ...
        self.execute('deb=$(mktemp) && wget -O "${deb}" '
                     ...
                     'dpkg -i "${deb}" && '
                     'rm -f "${deb}"')
        # index = 1 if self.container.cpuset_cpus else 0
        self.execute(
            'dpkg -i --force-all '
