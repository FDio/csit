...
class ContainerEngine(object):
    ...
    def install_vpp(self, install_dkms=False):
        ...
        if install_dkms:
            ...
            self.execute('for i in $(ls -I \"*dkms*\" {0}/install_dir/); do '
                         'dpkg -i --force-all /mnt/host/install_dir/$i; done')
...
class LXC(ContainerEngine):
    ...
    def execute(self, command):
        ...
        cmd = "lxc-attach {env} --name {c.name} -- /bin/sh -c '{command}'; exit $?"\
...
class Docker(ContainerEngine):
    ...
    def execute(self, command):
        ...
        """
        cmd = "docker exec --interactive {c.name} /bin/sh -c '{command}'; exit $?"\
