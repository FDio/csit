...
SUPPORTED_PARAMS = ['command', 'cpuset_mems', 'env', 'host_dir', 'image',
                    'name', 'publish']
...
class ContainerManager(object):
    ...
    def construct_container(self, **kwargs):
        """Construct container object on node with specified name.
        ...
        if kwargs['cpu_shared']:
            ...
            skip_cnt = kwargs['skip_cnt'] + kwargs['i'] * kwargs['cpu_cnt']
    ...
    def construct_containers(self, **kwargs):
        ...
        :param args: Key-value pairs used to construct container.
        :param name: str
        :param args: dict
        """
        ...
        for i in range(kwargs['count']):
            ...
            name = ''.join([kwargs.pop('name'), str(i)])
            # Create container
            self.construct_container(name=name, i=i, **kwargs)

    def acquire_all_containers(self):
        """Acquire all stored containers."""

        for container in self.containers:
            self.engine.container = self.containers[container]
            self.engine.create()

    def run_all_containers(self):
        """Run all stored containers."""
    ...
    def install_vpp_in_all_containers(self):
        """Install VPP into all stored containers."""
    ...
    def configure_vpp_in_all_containers(self, vat_template_file):
        """Configure VPP in all stored containers."""
...
class LXC(ContainerEngine):
    ...
    def create(self, force_create=True):
        ...
        cmd = 'lxc-create -t download --name {c.name} -- {c.image} '\
...
class Docker(ContainerEngine):
    ...
    def run(self):
        ...
        cpuset_cpus = '--cpuset-cpus={0}'.format(
            ','.join('%s' % var for cpu in self.container.cpuset_cpus))

        cpuset_mems = '--cpuset-mems={0}'.format(self.container.cpuset_mems)\
            if self.container.cpuset_mems else ''

        env = '{0}'.format(
            ' '.join('--env %s' % var for var in self.container.env))
        ...
        cmd = 'docker run '\
            ...
            '--cgroup-parent "" {cpuset_cpus} {cpuset_mems} {publish} '\
...
class Container(object):
    ...
    def __init__(self, node):
        # Store DUT node and connect to DUT Node
        self._node = node
        self._ssh = SSH()
    ...
    def command(self):
        ...
        :returns: Container startup command value.
        :rtype: str
        """
    ...
    def publish(self):
        ...
        :returns: Publish application ports to host.
        :rtype: str
        """
