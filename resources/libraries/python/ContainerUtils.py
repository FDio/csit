...
class ContainerManager(object):
    ...
    def construct_container(self, **kwargs):
        ...
        if kwargs['cpu_shared']:
            ...
            skip_cnt = kwargs['skip_cnt'] + (kwargs['i']-1) * kwargs['cpu_cnt']
    ...
    def construct_containers(self, **kwargs):
        ...
        for i in range(kwargs['count']):
            ...
            name = ''.join([kwargs.pop('name'), str(i+1)])
            # Create container
            self.construct_container(name=name, i=i+1, **kwargs)
...
class LXC(ContainerEngine):
    ...
    def acquire(self, force=True):
        """Acquire a privileged system object where is stored the configuration
