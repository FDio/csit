...
class ContainerManager(object):
    ...
    def _configure_vpp_chain_vswitch(self, **kwargs):
        ...
        cpuset_cpus = CpuUtils.get_affinity_nf(nodes, 'DUT2', nf_chains=1,
                          nf_nodes=1, nf_chain=1, nf_node=1, vs_dtc=0,
                          nf_dtc=1, nf_mtcr=8, nf_dtcr=1)
        self.engine.create_vpp_startup_config_vswitch(cpuset_cpus, rxq, rxd,
                        txd, buffers, heapsize, if1_pci, if2_pci)
        script = []
        script.append('create memif socket id 1 filename {socket1}\n'\
        ...
        for i in range(1, n_instances +1):
            script.append('create interface memif id {mid1} socket-id {sid1} master\n'\
    ...
    def _configure_vpp_chain_ipsec(self, **kwargs):
        ...
        nodes = kwargs['nodes'];
        cpuset_cpus = CpuUtils.get_affinity_nf(nodes, 'DUT2', nf_chains=1,
                          nf_nodes=2, nf_chain=1, nf_node=nf_instance,
                          vs_dtc=10, nf_dtc=1, nf_mtcr=2, nf_dtcr=1)
        ...
        script.append('create memif socket id 1 filename {socket1}\n'\
                      ...
                      .format(socket1='{guest_dir}/memif-vswitch-1'.format(**kwargs),
                              ...
                              local_ip='{base}.{instance}'.format(base=kwargs['dut2_if1_ip4'].rsplit('.', 1)[0], instance=nf_instance),
                              remote_ip='{base}.{instance}'.format(base=kwargs['dut2_if2_ip4'].rsplit('.', 1)[0], instance=nf_instance)))
        self.engine.execute('echo "{script_content}" > /tmp/running.exec'\
                            .format(script_content=''.join(script)))
...
class ContainerEngine(object):
    ...
    def create_vpp_startup_config_vswitch(self, cpuset_cpus, rxq, rxd, txd, buffers, heapsize, *devices):
        ...
        vpp_config.add_dpdk_dev_default_rxd(rxd)
        vpp_config.add_dpdk_dev_default_txd(txd)
