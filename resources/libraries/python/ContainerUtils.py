...
SUPPORTED_PARAMS = ['command', 'cpuset_mems', 'env', 'host_dir', 'image', 'name'
                    'publish']
...
class ContainerManager(object):
    ...
    def construct_container(self, **kwargs):
        """Create container object on node with specified name.
        ...
        if kwargs['cpu_shared']:
            ...
            skip = kwargs['skip_cnt'] + kwargs['i'] * kwargs['cpu_cnt']
            self.engine.container.cpuset_cpus = \
                CpuUtils.cpu_slice_of_list_per_node(
                    node=kwargs['node'], cpu_node=kwargs['cpu_node'],
    ...
    def construct_containers(self, **kwargs):
        ...
        :param args: Key-value pairs used to create container.
        :param name: str
        :param args: dict
        """
    ...
    def start_all_containers(self, **kwargs):
        """Start all stored containers."""

        for container in self.containers:
            self.engine.container = self.containers[container]
            self.engine.start(**kwargs)
...
class Docker(ContainerEngine):
    ...
    def start(self, **kwargs):
        ...
        cmd = 'docker run '\
            '--detach --interactive --tty --name {c.name} --rm {e} '\
            '--cgroup-parent "" '\
            '--cpuset-mems={c.cpuset_mems} '\
            '--cpuset-cpus={p} '\
            '--volume {c.host_dir}:{c.guest_dir} '\
            '--privileged {c.image}'\
            .format(c=self.container, e=env_var, p=cpuset_cpus)
