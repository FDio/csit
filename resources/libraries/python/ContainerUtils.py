...
class ContainerManager(object):
        ...
        for i in range(kwargs['count']):
            ...
            if kwargs['cpu_shared']:
                self.engine.container.cpuset_cpus = \
                    CpuUtils.cpu_list_per_node_str(node=kwargs['node'],
    ...
    def start_all_containers_with_vpp(self):
        """Start all stored containers and install vpp."""
        for container in self.containers:
            self.engine.container = self.containers[container]
            self.engine.start()
...
class LXC(ContainerEngine):
    ...
    def restart_vpp(self):
        ...
        self.execute('service vpp restart')

    def create_vpp_running_config(self, vat_template_file, **args):
