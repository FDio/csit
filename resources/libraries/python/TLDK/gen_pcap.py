...
from resources.libraries.python.TLDK import TLDKConstants as con
...
def gen_ipv4_checksum_pcap():
    ...
    writer = PcapWriter(con.TLDK_TESTCONFIG + "/test_ipv4_checksum_rx.pcap", append=False)
...
def gen_ipv6_checksum_pcap():
    ...
    writer = PcapWriter(con.TLDK_TESTCONFIG + "/test_ipv6_checksum_rx.pcap", append=False)
...
def gen_ipv4_fragment_pcap():
    ...
    writer = PcapWriter(con.TLDK_TESTCONFIG + "/test_ipv4_fragment_rx.pcap", append=False)
    for i in range(1, 1474):
        packets = create_packet(i, False)
        for packet in packets:
            writer.write(packet)
...
def gen_ipv6_fragment_pcap():
    ...
    writer = PcapWriter(con.TLDK_TESTCONFIG + "/test_ipv6_fragment_rx.pcap", append=False)
    for i in range(1, 1454):
        packets = create_packet(i, True)
        for packet in packets:
            writer.write(packet)
...
def gen_ipv4_assemble_pcap():
    ...
    writer = PcapWriter(con.TLDK_TESTCONFIG + "/test_ipv4_assemble_rx.pcap", append=False)
...
def gen_all_pcap():
    #Genearte all test cases input pcap file.
