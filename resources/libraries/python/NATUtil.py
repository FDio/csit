...
class NATUtil(object):
    ...
    def show_nat(node):
        ...
        with PapiExecutor(node) as papi_exec:
            papi_exec.add("nat44_interface_addr_dump").get_dump()

        with PapiExecutor(node) as papi_exec:
            papi_exec.add("nat44_address_dump").get_dump()

        with PapiExecutor(node) as papi_exec:
            papi_exec.add("nat44_static_mapping_dump").get_dump()

        with PapiExecutor(node) as papi_exec:
            papi_exec.add("nat_worker_dump").get_dump()

        with PapiExecutor(node) as papi_exec:
            papi_exec.add("nat44_user_dump").get_dump()

        with PapiExecutor(node) as papi_exec:
            papi_exec.add("nat44_interface_dump").get_dump()

        with PapiExecutor(node) as papi_exec:
            papi_exec.add("nat44_user_session_dump").get_dump()
