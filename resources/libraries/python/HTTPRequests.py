...
class HTTPRequestError(Exception):
    """Exception raised by HTTPRequests objects."""
...
class HTTPRequests(object):
    ...
    def get(node, path, headers=None, timeout=10, enable_logging=True):
        ...
        :param path: HTTP path without IP address and port, e.g. /index.html
        ...
        """
        url = HTTPRequests.create_full_url(node['host'],
                                           node['honeycomb_port'],
                                           path)
        try:
            auth = HTTPBasicAuth(node['honeycomb_user'],
                                 node['honeycomb_passwd'])
            ...
            if enable_logging:
                raise HTTPRequestError("Not possible to connect to {}\n".
                                       format(url) + repr(err))
            else:
                raise HTTPRequestError("Not possible to connect to {}\n".
                                       format(url) + repr(err),
            ...
            raise HTTPRequestError("Invalid HTTP response from {}\n".
                                   format(url) + repr(err))
    ...
    def put(node, path, headers=None, payload=None, timeout=10):
        ...
        :param node: honeycomb node
        :param path: HTTP path without IP address, e.g. /index.html
        :param headers: Dictionary of HTTP Headers to send with the Request.
