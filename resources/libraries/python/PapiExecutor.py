...
class PapiSocketExecutor:
    ...
    def _execute_async(
        ...
        logging.debug(f"max in flight {max_inflight}")
...
#                logger.trace(f"send template {send_template}")
                ...
                if non is not None:
                    raise RuntimeError(f"Got non-None {non!r}")
                # Debug: We expect the next get to fail.
                non = transport.q.get(block=True, timeout=read_timeout)
                if non is not None:
                    raise RuntimeError(f"Got second non-None {non!r}")
                continue
