...
# VPP class is inserted here if needed.
imports = dict()


def get_vpp_class():
    """Return VPP class with bindings to all API calls.
    ...
    """
    imported = imports.get("VPP", None)
    if imported is not None:
        return imported
    download_dir = os.getenv("DOWNLOAD_DIR", None)
    if download_dir is None:
        raise RuntimeError("DOWNLOAD_DIR is needed to extract *.api.json")
        ...
        for root, dirs, files in os.walk(tmp_dir)
            for name in files:
                if name == "vpp_papi.py":
                    ...
                    break
        if modules_path:
            # TODO: Contribute a more programmatic way to locate .api.json files.
...
class PapiExecutor(object):
    ...
    def get_replies(self, err_msg="Failed to get replies.",
        ...
        :type timeout: int
        :returns: Papi response including: papi_reply, stdout, stderr and
            return code.
        :rtype: PapiResponse
        """
    ...
    def _execute(self, method='request', process_reply=True,
        ...
        # Log processed papi_reply to be able to check API replies changes
...
class PapiSocketExecutor(object):
    ...
    def get_replies(self, err_msg="Failed to get replies."):
        ...
        :param err_msg: The message used if the PAPI command(s) execution fails.
        :type err_msg: str
        :returns: Papi response with only papi_reply being non-trivial.
        :rtype: PapiResponse
        """
        return self._execute(method='request', err_msg=err_msg)
    ...
    def get_dump(self, err_msg="Failed to get dump."):
        ...
        :rtype: PapiResponse
        """
        return self._execute(method='dump', err_msg=err_msg)

    def _execute(self, method='request', err_msg=""):
        """Execute commands from internal list, return PAPI response.

        # TODO: Should we remove "method" argument until stats are supported?
        ...
        # Clear first as execution may fail.
        self._api_command_list = list()
        pkey = (RSAKey.from_private_key(StringIO.StringIO(node['priv_key']))
                if 'priv_key' in node else None)
        with SSHTunnelForwarder(
            (node['host'], port=node['port']), ssh_username=node['username'],
            ssh_password=node.get('password'), ssh_pkey=pkey,
            remote_bind_address=self._remote_socket_path,
            local_bind_address=self._local_socket_path
        ) as _:
            VPP = get_vpp_class()
            vpp = VPP(
                use_socket=True, server_address=self._local_socket_path)
            vpp.connect("csit_socket")
            try:
                papi_reply = list()
                for item in local_list:
                    api_name = item["api_name"]
                    papi_fn = getattr(vpp.api, api_name)
                    api_reply = papi_fn(**item["api_args"])
                    papi_reply.append(dict(
                        api_name=api_name, api_reply=api_reply))
            finally:
                vpp.disconnect()
        return PapiResponse(
            papi_reply=papi_reply, stdout=None, stderr=None,
            requests=[rqst["api_name"] for rqst in local_list])
