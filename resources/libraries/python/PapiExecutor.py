...
__all__ = ['PapiExecutor', 'PapiInitError', 'PapiJsonFileError',
...
class PapiCommandError(PapiError):
    """This exception is raised when Python API failed."""
    pass


class PapiCommandInputError(PapiCommandError):
    """This exception is raised when incorrect input of Python API used."""
    pass
...
class PapiExecutor(object):
    """Contains methods for executing PAPI commands on DUTs."""
    ...
    def _revert_api_reply(self, api_r):
        """Process API reply / a part of API reply.

        Apply binascii.unhexlify() method for string values.
        ...
        for reply_key, reply_v in api_r.iteritems():
            for k, v in reply_v.iteritems():
                value = binascii.unhexlify(v) if isinstance(v, str) else v
                reply_value[k] = value
    ...
    def execute_papi(self, api_data, timeout=120):
        ...
        json_data = json.dumps(self._process_api_data())
