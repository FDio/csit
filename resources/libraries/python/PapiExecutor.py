...
from resources.libraries.python.ssh import raise_from, SSH, SSHTimeout
...
class PapiExecutor(object):
    ...
    def get_reply(self, err_msg="Failed to get reply.", timeout=120):
        ...
        replies = self._execute(
            method='request', err_msg=err_msg, timeout=timeout)
        if len(replies) != 1:
            raise RuntimeError("Expected single reply, got {replies!r}".format(
                replies=replies))
        return replies[0]

    def get_details(self, err_msg="Failed to get dump.", timeout=120):
    ...
    def _execute(self, method='request', err_msg="", timeout=120):
        ...
        try:
            ...
            raise
        logger.trace("Whole jsoned object {obj}".format(obj=json_data))
        for data in json_data:
            if method == "request":
                api_reply = self._process_reply(data["api_reply"])
                logger.trace("Api reply {obj}".format(obj=api_reply))
                # api_reply contains single key, *_reply.
                obj = api_reply.values()[0]
                logger.trace("Method 'request' object {obj}".format(obj=obj))
                retval = obj["retval"]
                if retval != 0:
                    # TODO: What exactly to log and raise here?
                    msg = err_msg if err_msg else "Got retval {rv}".format(
                        rv=retval)
                    raise RuntimeError(msg)
                replies.append(obj)
            elif method == "dump":
#                # Avoid double processing for details.
#                api_reply = data["api_reply"]
                api_reply = self._process_reply(data["api_reply"])
                logger.trace("Api reply {obj}".format(obj=api_reply))
                # api_reply is a list where item contas single key, *_details.
                for item in data["api_reply"]:
                    obj = item.values()[0]
                    logger.trace("Method 'dump' object {obj}".format(obj=obj))
                    replies.append(obj)
            else:
