...
class PapiSocketExecutor(object):
    ...
    def run_cli_cmd(node, cmd, log=True):
        ...
        with PapiSocketExecutor(node) as papi_exec:
            data = papi_exec.add(cli, **args).get_replies(err_msg)["reply"]

        if log:
            logger.info("{cmd}:\n{data}".format(cmd=cmd, data=data))
...
class PapiExecutor(object):
    ...
    Note: Use only with "with" statement, e.g.:
        ...
        with PapiExecutor(node) as papi_exec:
            papi_resp = papi_exec.add('show_version').get_replies()
    ...
    a. One request with no arguments:
        ...
        with PapiExecutor(node) as papi_exec:
            datum = papi_exec.add('show_version').get_reply()

    b. Three requests with arguments, the second and the third ones are the same
       but with different arguments.

        with PapiExecutor(node) as papi_exec:
            reply_list = papi_exec.add(cmd1, **args1).add(cmd2, **args2).\
    ...
    2. Dump functions
        ...
        cmd = 'sw_interface_rx_placement_dump'
        with PapiExecutor(node) as papi_exec:
            details_list = papi_exec.add(cmd, sw_if_index=ifc['vpp_sw_index']).\
                get_details(err_msg)
