    ...
    def _execute_async(self, local_list, err_msg=u"Undefined error message"):
        ...
        for count, command in enumerate(local_list[:max_inflight]):
            ...
            if count == 0:
                ...
                logger.trace(f"Template list initial: {template!r}")
#                iterator = template_instance.generator(3, vpp_instance.get_context).__iter__()
                iterator = template_instance.generator(3).__iter__()
            else:
                generated = iterator.__next__()  # Fails after 3.
                logger.trace(f"Generator says: {generated.hex()}")
                logger.trace(f"Template list after: {template}")
