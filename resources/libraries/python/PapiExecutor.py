...
class PapiSocketExecutor:
    ...
    def _read(vpp_instance, tries=3):
        """Blockingly read within timeout, raise on early None.

        Most of the time, early None means VPP crashed (see VPP-2033),
        call _read_internal is that should be tolerated.
        ...
        :rtype: namedtuple
        :raises RuntimeError: If no reply within timeout.
        """
        reply = PapiSocketExecutor._read_internal(vpp_instance)
        if not reply:
            raise RuntimeError(f"Empty {reply=}")
        return reply
