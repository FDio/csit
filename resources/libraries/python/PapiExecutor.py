...
def get_vpp_class():
    ...
    """
    imported = IMPORTS.get("vpp_class", None)
        ...
        for root, _, files in os.walk(tmp_dir):
            for name in files:
                if name == "vpp_papi.py" and not "python3" in root:
                    module_path = os.path.split(root)[0]
                    logger.debug(module_path)
                    stdout = subprocess.check_output(["ls", "-l", module_path])
                    logger.debug(stdout)
                    stdout = subprocess.check_output(["ls", "-l", root])
                    logger.debug(stdout)
#                    module_paths.extend([root, module_path])
                    module_paths.extend([root])
                    break
            else:
                continue
            break
        if module_paths:
            ...
            logger.debug(repr(sys.path))
            sys.path.extend(module_paths)
            logger.debug(repr(sys.path))
            from vpp_papi import VPP as vpp_class
        else:
        ...
        if sys.path[-1] == module_paths[0]:
            sys.path[-1:] = []
    IMPORTS["vpp_class"] = vpp_class
    return vpp_class
...
class PapiSocketExecutor(object):
    ...
    def _execute(self):
        ...
        subprocess.check_call([
            "sshpass", node.get('password'), "ssh", "-f", "-L",
            self._local_socket_path, "-R", self._remote_socket_path, "-p",
            node['port'], node['username'] + "@" + node['host'],
            "sleep", "1"])
