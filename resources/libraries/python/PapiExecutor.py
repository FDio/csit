...
#        logger.trace(u"Phase two.")
        send_avg, recv_avg, send_rec, recv_rec = 0.0, 0.0, 0.0, 0.0
...
#            logger.trace(f"send {send_index}")
            send_index += 1
            receive_index += 1
        logger.trace(f"send record {send_rec/1e9} receive_record {recv_rec/1e9}")
        logger.trace(f"send average {send_avg/1e9} receive_average {recv_avg/1e9}")
