...
class PapiSocketExecutor:
    ...
    def key_for_node_and_socket(cls, node, remote_socket):
        ...
        return (
            node[u"host"],
            node[u"port"],
            # It is possible for different users to see different paths.
            node[u"username"],
            remote_socket,
        )
    ...
    def set_connected_client(self, client):
        """Add (or update) a connected client instance in/to cache.

        This hides details of what the node key is.

        :param client: VPP client instance in connected state.
        :type client: vpp_papi.
        """
        self.__class__.conn_cache[self.key_for_self()] = client
    ...
    def get_connected_client(self, check_connected=True):
        ...
        if check_connected and ret is None:
            raise RuntimeError(f"Client not cached as connected, key: {key}")
        return ret
    ...
    @classmethod
    def disconnect(cls, node, remote_socket):
        """Disconnect a connected client instance, noop it not connected.
