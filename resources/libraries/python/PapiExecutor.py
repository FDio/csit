...
def dictize(obj):
    ...
    If the object is namedtuple-like, its _asdict() form is used,
    to provide enhanced __getitem__ function, which accepts also string keys
    and returns dictized elements.
    ...
    get_by_str = obj._asdict().__getitem__
    new_get = lambda self, key: dictize(
        (old_get if isinstance(key, int) else get_by_str)(self, key))
    obj.__getitem__ = new_get
    return obj
    ...
    def vpp_instance(self):
        ...
        try:
            ...
            self.__class__._vpp_instance = vpp_class(
                use_socket=True, server_address="TBD",
                async_thread=False)
            self.__class__._last_node = self._node
        finally:
