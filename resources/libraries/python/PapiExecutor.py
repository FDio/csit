...
class PapiSocketExecutor(object):
    ...
    def _execute_async(self, local_list, err_msg="Undefined error message"):
        """Send command messages at once, process and return replies.
        ...
        and context numbers increase one by one (and matching for replies.
        ...
        """
        lll = len(local_list)
        via = self.vpp_instance.api
        ret_list = list()
        # Phase one: only sending commands.
        logger.trace("Sending {lll} commands.".format(lll=lll))
        for command in local_list:
            papi_fn = getattr(via, command["api_name"])
            context = papi_fn(**command["api_args"])
            # Context is type "long" here, reply has "int", but dict is smart.
        # Phase two: only receiving replies.
        logger.trace("Processing replies.".format(lll=lll))
        for _ in xrange(lll):
            ...
            # No CRC checks, we want to read fast.
