...
class PapiSocketExecutor(object):
    ...
    def initialize_vpp_instance(self):
        ...
        try:
            ...
            # The following is the official way to set log level. Sigh.
            ser_log = logging.getLogger("vpp_serializer")
            logger.trace("ser log {sl!r}".format(sl=ser_log))
            logger.trace("level before {l!r}".format(l=ser_log.getEffectiveLevel()))
            ser_log.setLevel(logging.INFO)
            logger.trace("level after {l!r}".format(l=ser_log.getEffectiveLevel()))
    ...
    def _execute_async(self, local_list, err_msg="Undefined error message"):
        ...
        ser_log = logging.getLogger("vpp_serializer")
        logger.trace("ser log {sl!r}".format(sl=ser_log))
        logger.trace("level {l!r}".format(l=ser_log.getEffectiveLevel()))
        ret_list = list()
