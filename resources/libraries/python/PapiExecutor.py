...
class PapiSocketExecutor(object):
    ...
    def add(self, csit_papi_command, history=True, **kwargs):
        ...
        Unless disabled, new entru to papi history is also added at this point.
    ...
    def _execute(self, err_msg="Undefined error message"):
        ...
        self._api_command_list = list()
        # Debug control ping.
        getattr(vpp_instance.api, "control_ping")()
        replies = list()
        for command in local_list:
            ...
            try:
                ...
                logger.info("Reconnect after error: {err!r}".format(err=err))
                self.vpp_instance.disconnect()
                # Testing shower immediate reconnect fails.
                time.sleep(1)
