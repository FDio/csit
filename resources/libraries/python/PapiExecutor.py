...
class PapiSocketExecutor(object):
    ...
    def _execute_async(self, local_list, err_msg="Undefined error message"):
        ...
        lll = len(local_list)
        logger.trace("Executing {lll} commands".format(lll=lll))
        # Phase one: only sending commands.
        logger.trace("Phase one starts.")
        ...
        logger.trace("Phase one stops, phase two starts.")
        recv_index = 0
        while 1:
            ...
            # No CRC checking here, we want to read fast.
        ...
        logger.trace("Phase two stops, phase three starts.")
        for _ in local_list[-max_inflight:]:
            ...
            dict_item = dictize_and_check_retval(response)
            ret_list.append(dict_item)
        logger.trace("Phase three stops, done.")
        return ret_list
