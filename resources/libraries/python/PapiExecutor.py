...
class PapiSocketExecutor:
    ...
    def get_replies(
        ...
        replies are only checked to be equal to the content predicted
        by (autodetected) receive template. No other processing is done,
        no list of replies is returned. This speeds up interaction even more,
...
#            logger.trace(f"recv {send_index - max_inflight}")
            ...
            if fast_receive:
                ...
                if received != predicted:
                    err = AssertionError(
                        f"Received {received.hex()} predicted {predicted.hex()}"
                    )
            else:
                ...
                if response is None:
                    ...
                    cmd = local_list[recv_index]
                    err = AssertionError(
                        f"Timeout index {recv_index} cmd {cmd!r}"
                    )
...
#            logger.trace(f"count {count}")
            ...
            if err:
                raise_from(RuntimeError(err_msg), err, f"INFO")
        return None if fast_receive else ret_list
