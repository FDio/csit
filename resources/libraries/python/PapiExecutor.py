...
class PapiSocketExecutor:
    ...
    def _execute_async(
        ...
        # We do not use for, as the cyce lenght will be updated.
        # For the continue statement to work correctly,
        # it is safer to track two counters.
        count_next = 0
        while 1:
            count = count_next
            count_next += 1
            ...
            if not fast_send or count <= 1:
                ...
                if count == 0:
                    ...
                    logger.trace(f"length {len(first_sent)} in flight {max_inflight}")
                    len_one = min(max_inflight, lll)
                    continue
