...
class PapiExecutor(object):
    ...
    def execute_should_pass(self, err_msg="", timeout=120):
        ...
        response = self.execute(timeout)

        if response.ret_code is None:
            raise RuntimeError("First execute the PAPI command(s)!")
        if response.ret_code != 0:
            raise AssertionError(err_msg)
        return response

    def execute_should_fail(self, err_msg="", timeout=120):
        """Execute the PAPI commands and check the return code.
