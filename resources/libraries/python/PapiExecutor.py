...
class PapiSocketExecutor(object):
    ...
    def vpp_instance(self):
        ...
        try:
            ...
            inner_cmd = " ".join([
                "tar", "cvJf", "/tmp/papi.txz", "--sort=name", "--exclude=*.pyc",
    ...
    def _execute(self):
        ...
        for item in local_list:
            ...
            try:
                ...
                logger.error("Reconnecting after error: {err!r}".format(err=err))
                self.vpp_instance.disconnect()
    ...
    def dump_and_log(node, cmds):
        ...
        with PapiSocketExecutor(node) as papi_exec:
            for cmd in cmds:
                dump = papi_exec.add(cmd).get_details().verify_details()
                logger.debug("{cmd}:\n{data}".format(
    ...
    def run_cli_cmd(node, cmd, log=True):
        ...
        with PapiSocketExecutor(node) as papi_exec:
            data = papi_exec.add(cli, **args).get_replies().verify_reply(
                err_msg)
