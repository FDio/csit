...
class PapiSocketExecutor(object):
    ...
    def vpp_instance(self):
        ...
        # Package path has to be one level above vpp_papi directory.
        ...
        try:
            ...
            # Contrary to subprocess, ssh expands wildcards.
            exec_cmd_no_error(node, [
                "tar", "cvJf", "/tmp/papi.txz", "--exclude=*.pyc",
                "/usr/lib/python*/dist-packages/vpp_papi",
                "/usr/share/vpp/api"])
            ...
            package_path = glob.glob(
                tmp_dir + "/usr/lib/python*/dist-packages")[0]
    ...
    def __enter__(self):
        ...
        if node.get('password'):
            ssh_cmd = ["sshpass", "-p", node.get('password')] + ssh_cmd
