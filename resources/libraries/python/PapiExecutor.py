...
class PapiSocketExecutor:
    ...
    The currently use implementation caches the connected client instances,
    ...
    As a downside, clients should be explicitly told to disconnect
    before VPP restart.
    There is some amount of retries and disconnects on teardown,
    ...
    This class variable holds temporary directory once created. When python exist,
    the directory is deleted, so no downloaded file leaks.
    ...
    reusable_vpp_instance_list = list()
    ...
    conn_cache = dict()
    """Mapping from node (and remote socket) to open connection."""
    ...
    def get_connected_client(self, check_connected=True):
        ...
        """
        ret = self.__class__.conn_cache.get(
            (self._node, self._remote_vpp_socket), None
        )
        if check_connected and ret is None:
            raise RuntimeError(
                f"Client not cached as connected: "
                f"host {self._node[u'host']} socket {self._remote_vpp_socket}"
            )
        return ret
    ...
    def __enter__(self):
        ...
        vpp_instance.csit_local_vpp_socket = temp_path + u"/vpp-api.sock"
        ...
        run([u"rm", u"-rvf", vpp_instance.csit_local_vpp_socket])
        ...
        ssh_cmd = [
            ...
            u"-L", vpp_instance.csit_local_vpp_socket + u":" + self._remote_vpp_socket,
        ...
        while time.time() < time_stop:
            ...
            ret_code, _ = run(
                [u"ls", u"-l", vpp_instance.csit_local_vpp_socket], check=False
            )
            if not ret_code:
                break
            time.sleep(0.1)
        ...
        vpp_instance.transport.server_address = vpp_instance.csit_local_vpp_socket
        ...
        # Client instance is connected and ready, store into cache.
        self.__class__.conn_cache[
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        # pass  # But pylint says it is unnecessary.

    @classmethod
    def disconnect(node, remote_socket):
        ...
        """
        node_and_socket_pair = (node, remote_socket)
        client_instance = cls.conn_cache.get(
            node_and_socket_pair, None
        )
        if client_instance is None:
            return
        logger.debug(f"Disconnecting {node_and_socket_pair}")
        # Invalidate cache first.
        del cls.conn_cache[node_and_socket_pair]
        ...
        try:
            client_instance.csit_temp_dir.clenup()
        ...
        cls.reusable_vpp_instance_list.append(client_instance)
    ...
    def disconnect_all_papi_connections():
        ...
        Call this method just before killing/restarting VPP instances.
        """
        ...
        for node, socket in keys_copy:
            cls.disconnect(node, socket)
