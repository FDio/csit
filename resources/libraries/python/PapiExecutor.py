...
class PapiSocketExecutor:
    ...
    def get_replies(
        ...
        other replies are only checked to be equal to the content predicted
        by (autodetected) receive template. No other processing is done,
        only two replies are returned. This speeds up interaction even more,
        but some tests need to extract values from the replies.
        ...
        with PapiSocketExecutor(node, do_async=True) as papi_exec:
            ...
            papi_exec.get_replies(err_msg, fast, True)
    ...
    def _execute_async(
        ...
        if not fast_send:
            # TODO: Does it make sense to suppor fast receive without fast send?
            fast_receive = False
        ...
        # TODO: Perform "autodetection", meaning the code sends one message
        # and then computes the limit based on its length.
        max_inflight = 100  # Ipsec uses quite long messages.
        vpp_instance = self.vpp_instance
        ...
        ret_list = list()
        lll = max(fast_send, len(local_list))
        # Phase one: only sending commands.
        logger.trace(f"Phase one. Command: {local_list[0][u'api_name']}")
        len_one = min(max_inflight, lll)
        for count in range(len_one):
#            logger.trace(f"count {count}")
            if not fast_send or count <= 1:
                ...
                if count == 0:
                    ...
                    logger.trace(f"first sent {first_sent.hex()}")
                    continue
                # fast_send and count == 1.
                ...
                logger.trace(f"send template {send_template}")
                send_iterator = send_template.generator(
                    fast_send - 2, vpp_instance.get_context).__iter__()
                if not fast_receive:
                    continue
                # We need to wait for replies here, as we are not sure
                # whether first real reads are in phase two or three.
                logger.trace(u"receiving first")
                first_received = vpp_instance.transport.q.get(
                    timeout=vpp_instance.read_timeout
                )
                logger.trace(f"received {first_received.hex()}")
                logger.trace(u"receiving second")
                second_received = vpp_instance.transport.q.get(
                    timeout=vpp_instance.read_timeout
                )
                logger.trace(f"received {second_received.hex()}")
                receive_template = BytesTemplate.from_two_messages(
                    first_received, second_received
                )
                logger.trace(f"receive template {receive_template}")
                receive_iterator = receive_template.generator(
                    fast_send - 2
                ).__iter__()
                # Check possible retval. No peek for multiprocessing.Queue
                # https://stackoverflow.com/a/43088825
                vpp_instance.transport.q.put(
                    first_received, timeout=vpp_instance.read_timeout
                )
                ...
                vpp_instance.transport.q.put(
                    second_received, timeout=vpp_instance.read_timeout
                )
                response = vpp_instance.read_blocking()
                if response is None:
...
#            logger.trace(f"recv {send_index - max_inflight}")
            ...
            if fast_receive:
                ...
                if receive_index >= 2:
                    predicted = receive_iterator.__next__()
                    received = vpp_instance.transport.q.get(
                        timeout=vpp_instance.read_timeout
                    )
                    if received != predicted:
                        err = AssertionError(
                            f"Index {receive_index}"
                            f" received {received.hex()}"
                            f" predicted {predicted.hex()}"
                        )
            else:
                ...
                if response is None:
                    ...
                    err = AssertionError(
                        f"Timeout index {receive_index} cmd {cmd!r}"
                    )
