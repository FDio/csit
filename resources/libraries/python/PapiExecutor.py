...
class PapiSocketExecutor(object):
    ...
    def _execute_async(self, local_list, err_msg="Undefined error message"):
        ...
        :raises RuntimeError: If the replies are not all correct.
        """
        max_inflight = 1500000  # TODO: Customize.
        max_block = 2000000
        vpp_instance = self.vpp_instance
        ret_list = list()
        block_list = list()
        while local_list:
            block_list.append(local_list[:max_block])
            local_list = local_list[max_block:]
        # TODO: Put repeated blocks into functions?
        for block in block_list:
            block_len = len(block)
            logger.trace("Executing block len {bl}".format(bl=block_len))
            ctx_order = list()  # For reply ordering.
            # Phase one: only sending commands.
            for command in block[:max_inflight]:
                api_name = command["api_name"]
                papi_fn = getattr(vpp_instance.api, api_name)
            ...
            while 1:
                ...
                if response is None:
                    err = AssertionError("Timeout index {i} cmd {c!r}".format(
                        i=recv_index, c=block[recv_index]))
                    raise_from(AssertionError(err_msg), err, "INFO")
                if response.context != context:
                    err = AssertionError("Reply {r!r} not for context {c}".format(
                        r=response, c=context))
                    raise_from(AssertionError(err_msg), err, "INFO")
