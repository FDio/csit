...
class PapiSocketExecutor(object):
    ...
    def __enter__(self):
        ...
        exec_cmd_no_error(
            node, "chmod o+rwx " + self._remote_socket_path, sudo=True)
        ...
        if priv_key:
            ...
            key_file.close()
        # Intentionally go through exceptional path, exit is not reliable by itself.
        ret_code, _ = run(["ls", ssh_socket])
        if ret_code != 2:
            run(["ssh", "-S", ssh_socket, "-O", "exit", "0.0.0.0"])
            # TODO: Is any sleep necessary? How to prove if not?
