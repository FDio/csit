...
class PapiSocketExecutor(object):
    ...
    def initialize_vpp_instance(self):
        ...
        try:
            ...
            # Async thread is used for notifications only, so do not bother.
            ...
            logger.trace("loggerDict {ld!r}".format(ld=logging.root.manager.loggerDict))
            ser1_log = logging.getLogger("vpp_serializer")
            ser2_log = logging.getLogger("vpp_papi.vpp_serializer")
            logger.trace("ser1 log {sl!r}".format(sl=ser1_log))
            logger.trace("ser2 log {sl!r}".format(sl=ser2_log))
            logger.trace("level1 before {l!r}".format(l=ser1_log.getEffectiveLevel()))
            logger.trace("level2 before {l!r}".format(l=ser2_log.getEffectiveLevel()))
            ser1_log.setLevel(logging.INFO)
            ser2_log.setLevel(logging.WARN)
            logger.trace("level1 after {l!r}".format(l=ser1_log.getEffectiveLevel()))
            logger.trace("level2 after {l!r}".format(l=ser2_log.getEffectiveLevel()))
    ...
    def _execute_async(self, local_list, err_msg="Undefined error message"):
        ...
        The implementation support multiple replies anyway,
        it just does not wait after first reply is seen.
        ...
        :raises RuntimeError: If the replies are not all correct.
        """
        max_inflight = 24  # TODO: Customize.
        vpp_instance = self.vpp_instance
        ret_list = list()
        ctx_order = list()  # For reply ordering.
        ctx_to_rep = collections.defaultdict(list)  # For reply storing.
        # TODO: Put repeader blocks into functions.
        # Phase one: only sending commands.
        for index, command in list(enumerate(local_list))[:max_inflight]:
            api_name = command["api_name"]
            papi_fn = getattr(vpp_instance.api, api_name)
            ...
            # Context is type "long" here, reply has "int", but dict is smart.
            ctx_order.append(context)
            logger.trace("sent {i}".format(i=index))
        # Phase two: receive one, send one.
        lll = len(local_list)
        recv_index = 0
        while 1:
            send_index = recv_index + max_inflight
            if send_index >= lll:
                break
            # Receive one.
            context = ctx_order[recv_index]
            replies = ctx_to_rep[context]
            if not replies:
                # Read loop. r_context can be different than context.
                while 1:
                    # Blocks up to timeout.
                    # TODO: In future we will need to insert no_type_conversion.
                    response = vpp_instance.read_blocking()
                    if response is None:
                        # Nothing else to read from socket.
                        logger.trace("Read timeout, should have avoided this.")
                        break
                    r_context = response.context
                    # No handling here, we want to read fast.
                    ctx_to_rep[r_context].append(response)
                    if r_context == context:
                        # This is the reply we were waiting for,
                        # no reason to read more and risk stalling.
                        logger.trace("Got the reply needed.")
                        break
                # Refresh replies, we might have read a response.
                replies = ctx_to_rep[context]
            if not replies:
                logger.trace("ctx_to_rep {ctr!r}".format(ctr=ctx_to_rep))
                err = AssertionError("Timeout index {i} cmd {c!r}".format(
                    i=recv_index, c=local_list[recv_index]))
                raise_from(AssertionError(err_msg), err, "INFO")
            logger.trace("received {i}".format(i=recv_index))
            for item in replies:
                # Request CRC has been checked in add, here check reply CRC.
                self.crc_checker.check_api_name(item.__class__.__name__)
                dict_item = dictize_and_check_retval(item)
            ...
            # Context is type "long" here, reply has "int", but dict is smart.
            ctx_order.append(context)
            logger.trace("sent {i}".format(i=send_index))
            recv_index += 1
        # Phase three: only receiving.
        for index, context in list(enumerate(ctx_order))[-max_inflight:]:
            replies = ctx_to_rep[context]
            if not replies:
                # Read loop. r_context can be different than context.
                while 1:
            ...
            if not replies:
                ...
                raise_from(AssertionError(err_msg), err, "INFO")
            logger.trace("received {i}".format(i=index))
            for item in replies:
                # Request CRC has been checked in add, here check reply CRC.
                self.crc_checker.check_api_name(item.__class__.__name__)
                dict_item = dictize_and_check_retval(item)
