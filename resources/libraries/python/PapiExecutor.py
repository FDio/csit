...
class PapiSocketExecutor:
    ...
    def set_connected_client(self, client):
        ...
        """
        logger.debug(f"Caching a new client.")
        logger.debug(f"Previous entry key {self.key_for_self()} value {self.__class__.conn_cache.get(self.key_for_self(), None)}")
        self.__class__.conn_cache[self.key_for_self()] = client
        logger.debug(f"Client {client!r} cached.")
    ...
    def get_connected_client(self, check_connected=True):
        ...
        logger.debug(f"Retreived cached client {ret} for key {key}")
        return ret
