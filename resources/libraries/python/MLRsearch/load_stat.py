...
class LoadStat(DiscreteLoad):
    ...
    loss_ratios: List[float] = None  # Cannot be non-default due to parent.
    """FIXME"""
    good_counts: List[int] = None
    """FIXME"""
    bad_counts: List[int] = None
    """FIXME"""
    intended_duration: float = None
    """FIXME"""
    ...
    def add(self, measurement: MeasurementResult) -> None:
        ...
        if measurement.intended_duration != self.intended_duration:
            raise RuntimeError(
                f"Attempting to add duration {measurement.intended_duration}"
                f" to result set for {self.intended_duration}"
            )
        ...
        self.all_count += 1
        self.all_duration += measurement.duration_with_overheads
        self.duration_per_trial = self.all_duration / self.all_count

    def satisfies(self, criterion, duration):
        """FIXME"""
    ...
    def determined(self, criterion, duration):
        """FIXME"""
        if not self.all_count:
            return False
        optimistic, pessimistic = self.satisfies(criterion, duration)
        return optimistic == pessimistic
    ...
    def new_empty(
        ...
        return LoadStat(
            ...
            indended_duration=intended_duration,
        )
