...
class LoadStat(DiscreteLoad):
    ...
    def add(self, measurement: MeasurementResult) -> None:
        ...
        if measurement.intended_duration != self.subtrial_duration:
            raise RuntimeError(
                f"Attempting to add duration {measurement.intended_duration}"
                f" to result set for {self.subtrial_duration}"
            )
