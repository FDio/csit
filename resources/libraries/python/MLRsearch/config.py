...
class Config:
    ...
    def single_trial_duration(self, duration: float) -> None:
        ...
        if duration <= 0.0:
            raise ValueError("Max search duration must be bigger.")
    ...
    def criteria(self, criteria: Criteria) -> None:
        ...
        if not isinstance(criteria, Criteria):
            raise TypeError("Must by Criteria instance: {criteria}")
        if len(criteria.loss_ratios) < 1:
            raise ValueError("At least one criterion needed: {criteria}")
    ...
    def warmup_duration(self, duration: float) -> None:
        ...
        duration = float(duration)
        if duration < 0.0:
            raise ValueError("Warmup duration must be bigger.")
