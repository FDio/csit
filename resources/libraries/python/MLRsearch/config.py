...
class Config:
    ...
    Properties is defined to enforce the requirements on allowed values.
    ...
    _final_relative_width: Union[float, List[float]] = 0.005
    ...
    def final_relative_width(self):
        ...
        """
        if isinstance(self._final_relative_width, float):
            widths = [self._final_relative_width]
            widths *= len(self._target_loss_ratios)
            return widths
        if len(self._final_relative_width) != len(self._target_loss_ratios):
            raise RuntimeError(u"Width length does not match ratios.")
        return list(self._final_relative_width)

    @final_relative_width.setter
    def final_relative_width(self, widths):
        """Check type and store the value.

        When the value is a list, this should be called *after* setting ratios.
        ...
        if isinstance(widths, float):
            self._final_relative_width = widths
            return
        if isinstance(widths, list):
            self._final_relative_width = map(float, widths)
            return
        raise RuntimeError(f"Unexpected widths: {widths}")
