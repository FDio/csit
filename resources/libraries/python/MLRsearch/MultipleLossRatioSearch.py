...
class MultipleLossRatioSearch:
            ...
            for index, ratio in enumerate(state.packet_loss_ratios):
                ...
                if new_tr is None:
                    ...
                    if index > 0:
                        ...
                        continue
                    lower_bound, _, _ = database.get_values_for_ratio(ratio)
    ...
    def _select_for_ratio(self, ratio):
        ...
        state = self.state
        data = state.database
        lower_bound, upper_bound, hint = data.get_values_for_ratio(ratio)
        if hint is not None:
            hint_tr = hint.target_tr
            self.debug(f"Re-measuring a bound for {ratio}, hint: {hint_tr}")
            return hint_tr
        # No hint from previous duration, we have to search.
        if lower_bound is None:
    ...
    def _extend_down(self, ratio, upper_bound):
        ...
        if new_tr >= old_tr:
            self.debug(f"Extend down hits max rate for {ratio}")
            return None
        state.remember_width(old_tr, new_tr)
        self.debug(f"Extending down for {ratio}")
        self.debug(
            f"Extending down for {ratio}: old {old_tr} new {new_tr}"
        )
        return new_tr
    ...
    def _extend_up(self, ratio, lower_bound):
        ...
        if new_tr <= old_tr:
            self.debug(f"Extend up hits max rate for {ratio}")
            return None
        state.remember_width(old_tr, new_tr)
        self.debug(
            f"Extending up for {ratio}: old {old_tr} new {new_tr}"
        )
        return new_tr
    ...
    def _bisect(self, ratio, lower_bound, upper_bound):
        ...
        if width <= state.width_goal:
            self.debug(f"No more bisects needed for {ratio}")
            return None
        new_tr = half_step_up(width, lower_bound.target_tr)
        self.debug(
            f"Bisecting for {ratio}: lower {lower_bound.target_tr},"
            f" upper {upper_bound.target_tr}, new {new_tr}"
        )
        return new_tr
