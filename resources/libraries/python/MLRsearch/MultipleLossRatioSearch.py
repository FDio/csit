...
class MultipleLossRatioSearch:
            ...
            for index, ratio in enumerate(state.packet_loss_ratios):
                ...
                if new_tr is None:
                    ...
                    if index > 0:
                        ...
                        continue
                    lower_bound, _, _, _, _, _, _, _ = database.get_bounds(ratio)
    ...
    def _select_for_ratio(self, ratio):
        ...
        if cur_lo1 is None and cur_hi1 is None:
            ...
            return new_tr
        if cur_lo1 is None:
            # Upper bound exists (cur_hi1), so we want to extend down.
#            # Do we have a hint for next best upper bound?
#            next_bound = cur_hi2
#            # Do not bother with pre_lo, we re-measured and got cur_hi.
#            if pre_hi1 is not None:
#                if next_bound is None or pre_hi1.target_tr < next_bound.target_tr:
#                    if pre_hi1.target_tr > cur_hi1.target_tr:
#                        next_bound = pre_hi1
            ...
            return new_tr
        if cur_hi1 is None:
            # Lower bound exists (cur_lo1), so we want to extend up.
#            # Do we have a hint for next best lower bound?
#            next_bound = cur_lo2
#            # Do not bother with pre_hi, we re-measured and got cur_lo.
#            if pre_lo1 is not None:
#                if next_bound is None or pre_lo1.target_tr > next_bound.target_tr:
#                    if pre_lo1.target_tr < cur_lo1.target_tr:
#                        next_bound = pre_lo1
        ...
        # or for previous ratio (we are extending down).
