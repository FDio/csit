...
class MultipleLossRatioSearch:
    ...
    def narrow_down_intervals(self, min_rate, max_rate, packet_loss_ratios):
        ...
        measurements = list()
        logging.debug(u"First measurement at max rate.")
        ...
        logging.debug(u"Second measurement at mrr.")
        ...
        if min_rate < mrr2 < max_rate:
            ...
            logging.debug(u"Third measurement at mrr2.")
            mrr_measurement = self.measurer.measure(
                duration=self.initial_trial_duration,
                transmit_rate=mrr2,
            )
    ...
    def ndrpdr(self, start_time):
        ...
        logging.info(
            u"Starting phase with iterations with %ss duration"
            u" and %s relative width goal.",
        ...
        if lower_bound is None:
            if upper_bound is None:
                ...
                if short_lower_bound is None:
                    ...
                    logging.debug(u"Re-measuring upper bound for %s", ratio)
                    return short_upper_bound.target_tr
                # Short lower bound is not None; re-measure it.
                logging.debug(u"Re-measuring lower bound for %s", ratio)
    ...
    def _extend_down(self, ratio, upper_bound):
        ...
        if new_tr >= old_tr:
            logging.debug(u"Extend down hits max rate for %s", ratio)
            return None
    ...
    def _extend_up(self, ratio, lower_bound):
        ...
        if new_tr <= old_tr:
            logging.debug(u"Extend up hits max rate for %s", ratio)
            return None
        state.remember_width(old_tr, new_tr)
        return new_tr
    ...
    def _bisect(self, ratio, lower_bound, upper_bound):
        ...
        if width <= state.width_goal:
            logging.debug(u"No more bisects needed for %s", ratio)
            return None
        logging.debug(u"Bisecting for %s", ratio)
        new_tr = half_step_up(width, lower_bound.relative_width)
        return new_tr
