...
class MultipleLossRatioSearch:
    ...
    def ndrpdr(self, start_time):
        ...
        self.logger.debug(
            ...
            u" and {state.width_goal} relative width goal."
        )
        failing_fast = False
        ...
        if lower_bound is None:
            if upper_bound is None:
                ...
                if short_lower_bound is None:
                    ...
                    self.logger.debug(u"Re-measuring upper bound for {ratio}")
                    return short_upper_bound.target_tr
                # Short lower bound is not None; re-measure it.
                self.logger.debug(u"Re-measuring lower bound for {ratio}")
    ...
    def _extend_down(self, ratio, upper_bound):
        ...
        if new_tr >= old_tr:
            self.logger.debug(u"Extend down hits max rate for {ratio}")
            return None
    ...
    def _extend_up(self, ratio, lower_bound):
        ...
        if new_tr <= old_tr:
            self.logger.debug(u"Extend up hits max rate for {ratio}")
            return None
        state.remember_width(old_tr, new_tr)
        return new_tr
    ...
    def _bisect(self, ratio, lower_bound, upper_bound):
        ...
        if width <= state.width_goal:
            self.logger.debug(u"No more bisects needed for {ratio}")
            return None
        self.logger.debug(u"Bisecting for {ratio}")
