...
class CandidateSelector:
    ...
    def candidate_after(
        self, measured: Optional[MeasurementResult]
    ) -> LoadCandidate:
        """Update state after newest measurement, nominate new candidate.

        This is the main entry point.
        The measured result is expected to have discrete_load attribute.

        :param measured: The trial result measured after last call (if any).
        :type measured: Optional[MeasurementResult]
        ...
        if handle:
            load = self.handler.handle(
                load=load,
                width_goal=self.target.discrete_width,
                clo=self.bounds.clo,
                chi=self.bounds.chi,
            )
    ...
    def _halve(self) -> Optional[DiscreteLoad]:
        ...
        if self.bounds.clo or self.bounds.chi:
            return None
        if not (load := self.prev_load) or not plo <= load <= phi:
            return None
    ...
    def _extend_down(self) -> Optional[DiscreteLoad]:
        ...
        self.prev_width = (
            self.prev_width
            if self.prev_width
            else self.target.discrete_width
        )
