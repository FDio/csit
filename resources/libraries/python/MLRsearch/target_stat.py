...
class TargetStat:
    ...
    bad_long: float = 0.0
    """Sum of durations of long enough trials not satisfying target loss ratio."""
    ...
    def add(self, measurement: MeasurementResult) -> None:
        ...
        :param measurement: The trial result to add to the stats.
        :type measurement: MeasurementResult
        """
        intended = measurement.intended_duration
        overall = measurement.duration_with_overheads
        if intended >= self.target.trial_duration:
            if measurement.loss_ratio > self.target.loss_ratio:
    ...
    def estimates(self):
        ...
        pessimistic = (
            effective_dursum - self.good_long
        ) <= limit_dursum
