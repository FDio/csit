...
class LoadStats(DiscreteLoad):
    ...
    def conditional_throughput(self) -> Optional[DiscreteLoad]:
        """When trimmed, compute conditional throughput from the load.
        ...
        if len(self.target_to_stat) != 1:
            raise RuntimeError(f"Which target conditional throughput? {self}")
        ...
        stat = list(self.target_to_stats.values())[0]
