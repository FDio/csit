...
class IntervalExpander:
    ...
    def __post_init__(self):
        ...
        if self.initial_lower_load and self.initial_upper_load:
            new_width = self.initial_upper_load - self.initial_lower_load
            self.next_width = max(self.next_width, new_width)
        self.expand()

    def expand(self):
        """FIXME."""
        self.next_width *= self.target.expansion_coefficient
        if self.next_width < self.target.discrete_width:
            self.next_width = self.target.discrete_width

    def limit(self, last_width):
        """FIXME."""
        self.next_width = last_width
        self.expand()
