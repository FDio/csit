...
class StrategyBase(ABC):
    ...
    the relevant bound of the preceding target,
    ...
    def done(self, bounds: RelevantBounds, load: DiscreteLoad) -> bool:
        """Return False if the selector should keep measuring the previous load.

        This should be called only for winner candidates.

        False implies an intended resolution happened
        (the previous load has become a new current bound),
        even if the new bound is infavorable.
        It signals selector should start asking strategies for nominations again.

        Notably, an unexpected change of context
        (e.g. preceding target bound becoming invalidated)
        is NOT a reason to stop measuring the current load,
        as a relevant bound for the current target is still needed
        and the current load is still the most useful one to measure.

        All strategies should use the same logic for the return value
        (the one implemented in this base class),
        but they may want to update some additional internal state
        before returning that value.
        For that reason, subclasses should call this,
        and debug logging logic here looks at class name of self.
