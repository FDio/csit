    ...
    def nomination_iterator(self) -> Iterator[LoadCandidate]:
        ...
        while 1:
            ...
            if not self.bounds.clo and not self.bounds.chi:
                if not self.current_target.preceding:
                    raise RuntimeError("Lost bounds at initial target.")
            ...
            if not self.target_stack:
                ...
                continue
            self.current_target = self.target_stack.pop()
            self.initial_lower_load = self.bounds.clo
            self.initial_upper_load = self.bounds.chi
            self._update_bounds()
            self._recreate_strategies()
