...
class BisectStrategy(StrategyBase):
    ...
    def _enter_lo(
        ...
        """FIXME."""
        if not self.initial_lower_load or load >= self.initial_lower_load:
            return None
    ...
    def _enter_hi(
        ...
        """FIXME."""
        if not self.initial_upper_load or load >= self.initial_upper_load:
            return None
    ...
    def keep_going(self, bounds: RelevantBounds, load: DiscreteLoad) -> bool:
        ...
        if bounds.clo == self.old_clo and bounds.chi == self.old_chi:
            self.debug("No change, keep doing the bounded thing.")
            return True
