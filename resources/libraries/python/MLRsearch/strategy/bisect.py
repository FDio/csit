...
class BisectStrategy(StrategyBase):
    ...
    def _enter_lo(
        ...
        """FIXME."""
        if not self.initial_lower_load or bounds.clo >= self.initial_lower_load:
            self.debug("No enter lo: Clo not below initial (if any).")
            return None
        self.debug(
        ...
        if not new_load or new_load <= load:
            self.debug("No enter lo: Rounded load not high enough")
            return None
        return new_load
    ...
    def _enter_hi(
        ...
        """FIXME."""
        if not self.initial_upper_load or bounds.chi <= self.initial_upper_load:
            self.debug("No enter hi: Load below initial (if any).")
            return None
        self.debug(
        ...
        if not new_load or new_load >= load:
            self.debug("No enter lo: Rounded load not low enough")
            return None
        return new_load
