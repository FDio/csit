...
class RefineLoStrategy(StrategyBase):
    ...
    def nominate(self, bounds: RelevantBounds) -> Optional[DiscreteLoad]:
        ...
        FIXME.
        """
        if not (load := self.initial_lower_load) or not self.initial_upper_load:
            return None
        if bounds.plo != load or (bounds.clo and bounds.clo >= load):
            return None
        thi = bounds.phi if bounds.phi else bounds.chi
        if not thi or not load < thi <= self.initial_upper_load:
            return None
    ...
    def keep_going(self, bounds: RelevantBounds, load: DiscreteLoad) -> bool:
        ...
        if bounds.clo and bounds.clo == load:
            self.debug("Lower bound confirmed, refinement done successfully.")
            return False
        if bounds.chi and bounds.chi == load:
            self.debug("Lower bound invalidated, stopping refinement.")
            return False
        if bounds.clo and bounds.clo >= load:
            self.debug("Lower bound appeared high, canceling refinement.")
            return False
        if bounds.chi and bounds.chi <= load:
            self.debug("Upper bound appeared low, canceling refinement.")
            return False
        # If load becomes the new phi, we still want to keep going
        # as we have not confirmed the load as a valid clo or chi yet.
        self.debug(f"Lowerbound refinement continues at {load}")
        return True
