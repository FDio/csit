    ...
    def keep_going(self, bounds: RelevantBounds, load: DiscreteLoad) -> bool:
        ...
        if bounds.clo:
            if load == bounds.clo:
                ...
                return False
            self.debug("Unexpected clo appeared, canceling extend down.")
            return False
        if bounds.chi:
            if load == bounds.chi:
                self.debug(
                    "Extend found another invalid lowerbound, canceling."
                )
            ...
            if bounds.chi == self.old_chi:
                ...
                return True
            self.debug("Unexpected chi change, canceling extend down.")
            return False
