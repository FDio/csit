...
class ExtendHiStrategy(StrategyBase):
    ...
    def enter(self, bounds: RelevantBounds) -> Optional[DiscreteLoad]:
        ...
        if not load > bounds.clo:
            return None
        self.old_clo = bounds.clo
        self.debug(f"No chi, extending down: {load}")
        return load
    ...
    def keep_going(self, bounds: RelevantBounds, load: DiscreteLoad) -> bool:
        ...
        if bounds.chi:
            if load == bounds.chi:
                ...
                return False
            self.debug("Unexpected chi appeared, aborting extend down.")
            return False
        if bounds.clo:
            if load == bounds.clo:
                self.debug("Extend found another invalid upperbound, aborting.")
                self.interval_expander.expand()
                return False
            if bounds.clo == self.old_clo:
                self.debug("No change, continue examining extend up.")
                return True
            self.debug("Unexpected clo change, aborting extend up.")
            return False
