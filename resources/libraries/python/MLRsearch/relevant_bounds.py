...
class RelevantBounds:
    ...
    def from_database(
        ...
        bounds = database.get_valid_bounds(
            criterion, current_duration, trial_duration
        )
        logger.debug(f"fd main bounds {bounds}")
        plo1, phi1, clo1, chi1, clo2, chi2 = None, None, *bounds
        if previous_duration is not None:
            bounds = database.get_valid_bounds(
                criterion, previous_duration, trial_duration
            )
            logger.debug(f"fd previous bounds {bounds}")
