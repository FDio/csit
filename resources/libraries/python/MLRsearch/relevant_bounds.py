...
class RelevantBounds:
    ...
    # FIXME: The two below are never used?
    current: TargetSpec
    """Reference to the current target."""
    preceding: Optional[TargetSpec]
    """Reference to the preceding target (if any)."""
    ...
    def from_database(
        ...
        ret = RelevantBounds(
            clo=clo, chi=chi, plo=plo, phi=phi,
            current=target, preceding=preceding,
        )
        return ret
