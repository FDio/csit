...
class Selector:
    ...
    and rotating targets towards the final target.

    While the public method have the same names and meaning as the ones
    in strategy classes, their signature is diferent.
    Selector adds the current target trial duration to the output of nominate,
    and adds the current bound to the input of done.

    The nominate method does not return a complete Candidate instance,
    as we need to avoid circular dependencies (candidate will refer to selector).
    """
    ...
    """Reference to strategy used for last nomination."""
    # Cache.
    ...
    def nominate(
        ...
        Current target is rotated when (now preceding) target is reached.
        As each strategy never becomes done before at least one
        bound relevant to the current target becomes available,
        it is never needed to revert to the preceding target after rotation.

        As the initial trials had inputs relevant to all initial targets,
        the only way for this to not nominate a load
        ...
        while 1:
            ...
            if not self.bounds.clo and not self.bounds.chi:
                raise RuntimeError("No clo nor chi, internal error.")
    ...
    def done(self, load: DiscreteLoad) -> bool:
        """Return whether current strategy wants another trial at the same load.

        Only call when previously nominated candidate became winner.
        ...
        """
        self._update_bounds()
        return self.current_strategy.done(bounds=self.bounds, load=load)
