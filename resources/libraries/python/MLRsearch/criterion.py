...
class Criterion:
    ...
    """FIXME."""
    trials_duration: float = field(compare=False, default=60.0)
    """FIXME."""
    ...
    def validate(self):
        ...
        if self.expansion_coefficient < 1:
            raise ValueError("Expansion coefficient is too small: {self}")
        too_small = False
        if self.intermediate_phases < 0:
            ...
            if 1 != self.trials_duration:
                too_small = True
        if too_small:
            raise ValueError("Number of intermediate phases too small: {self}")
