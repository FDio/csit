...
class SearchGoal:
    """
    This is the part of overall inputs that can be repeated
    ...
    Most fields (called attributes) of this compound object
    ...
    (in units of the tightest upper bound tps)."""
    initial_trial_duration: float = field(default=1.0)
    """Shortest trial duration allowed when searching for this goal."""
    final_trial_duration: float = field(default=5.0)
    """Longest trial duration allowed when searching for this goal."""
    min_duration_sum: float = field(default=61.0)
    """Minimal sum of durations of relevant trials needed to declare a load
    to be upper or lower bound for this goal."""
    preceding_targets: int = field(default=2)
    ...
    def __post_init__(self):
        ...
        super().__setattr__("min_duration_sum", float(self.min_duration_sum))
        super().__setattr__(
            "preceding_targets", int(self.preceding_targets)
        )
        super().__setattr__(
    ...
    def validate(self):
        ...
        if self.min_duration_sum < self.final_trial_duration:
            raise ValueError(
                f"Min duration sum must be at least final trial duration: {self}"
            )
        ...
        if self.preceding_targets < 0:
            ...
            if self.initial_trial_duration < self.min_duration_sum:
                too_small = True
        if too_small:
            raise ValueError(f"Number of preceding targets is too small: {self}")
