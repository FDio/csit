...
class MeasurementDatabase:
    ...
    def _normalize(self):
        ...
        Also set effective loss fractions.
        ...
        fraction_previous = None
        for measurement in measurements_new:
            if fraction_previous is None:
                fraction_previous = measurement.loss_fraction
    ...
    def get_valid_lower_bound(self, ratio, duration=None):
        """Return None or a valid lower bound measurement.

        Two modes are supported. If duration is None, any measurement is used.
        If duration is not None, only long enough measurements are used.

        :param ratio: Target ratio, valid has to be lower or equal.
        :param duration: Target duration [s], valid has to be higher or equal.
        :type ratio: float
        :type duration: float
        ...
        for measurement in self.measurements:
            if measurement.effective_loss_fraction > ratio:
                ...
                break
            if duration is None or measurement.duration >= duration:
                bound = measurement
                # Continuing to find a tighter bound.
        return bound

    def get_valid_upper_bound(self, ratio, duration=None):
    ...
    def select_tr_for_lower_bound(self, ratio, min_rate, max_rate):
        ...
        loss fraction upon re-measuring at higher durations,
        this method attemps to find such a measurement and return its target_tr.
        The measurement has to have low enough effective loss fraction.
        ...
        for measurement in reversed(self.measurements):
            ...
            if measured_tr < min_rate:
                break
            measured_fraction = measurement.effective_loss_fraction
            if measured_fraction > ratio:
                continue
            return measured_tr
        return min_rate
    ...
    def select_tr_for_upper_bound(self, ratio, min_rate, max_rate):
        ...
        loss fraction upon re-measuring at higher durations,
        this method attemps to find such a measurement and return its target_tr.
        The measurement has to have high enough effective loss fraction.
        MLRsearch gives min_rate and max_rate as additional conditions,
        due to window width reasons. If no measurement fits in the interval
    ...
    def get_results(self, duration, ratio_list):
        ...
        The current implementation honors duration for valid bounds,
        but not for invalid bounds.

        :param duration: Ignore measurements shorter than this [s].
        ...
        for ratio in ratio_list:
            lower_bound = self.get_valid_lower_bound(ratio, duration)
            if lower_bound is None:
                lower_bound = self.measurements[0]
            upper_bound = self.get_valid_upper_bound(ratio, duration)
