...
class MeasurementDatabase:
    ...
    def __init__(self, database):
        ...
        for duration, data in database.items():
            if duration not in self.durations:
                self._insert_duration(duration)
    ...
    def _insert_duration(self, duration):
        """Add new empty per duration database for this duration.

        :param duration: New duration for per duration database, in seconds.
        :type duration: float
        :raises RuntimeError: If the duration is present, even partially.
        """
        if duration in self.durations:
            raise RuntimeError(f"Duration {duration} already present!")
        if duration in self.data_for_duration.keys():
