...
class TargetTracker:
    ...
    def candidate_after(
        ...
        self.debug(f"winner {self.winner} bounds {self.bounds}")
        self._update_flags()
    ...
    def _update_flags(self) -> None:
        ...
        if self.prev_action == ActionEnum.HALVE:
            if self.bounds.clo and self.prev_candidate <= self.bounds.clo:
                self.halved = True
                self.refined_lo = True
            elif self.bounds.chi and self.prev_candidate >= self.bounds.chi:
                self.halved = True
                self.refined_hi = True
            return
        self.halved = True
    ...
    def select_load(self) -> Tuple[DiscreteLoad, ActionEnum, bool]:
        ...
        if not self.bounds.clo:
            if self.prev_action == ActionEnum.EXT_LO:
                if (load := self.prev_candidate) < self.bounds.chi:
        ...
        if self.prev_action == ActionEnum.EXT_HI and not self.bounds.chi:
            if (load := self.prev_candidate) > self.bounds.clo:
    ...
    def _halve(self) -> Optional[DiscreteLoad]:
        ...
        if wig > 1.0:
            ...
            return load
        if self.bounds.clo or self.bounds.chi:
            return None
