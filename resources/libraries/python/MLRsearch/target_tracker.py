...
class TargetTracker:
    ...
    """FIXME"""
    prev_action: ActionEnum = ActionEnum.NONE
    """FIXME"""
    prev_width: Optional[DiscreteWidth] = None
    """FIXME"""
    ...
    def candidate_after(
        ...
        self.debug(f"bounds {self.bounds}")
        self._update_flags()
    ...
    def _update_flags(self) -> None:
        ...
        if self.prev_action == ActionEnum.HALVE:
            if self.prev_candidate == self.bounds.clo1:
                self.halved = True
                self.refined_lo = True
            elif self.prev_candidate == self.bounds.chi1:
                self.halved = True
                self.refined_hi = True
            return
        self.halved = True
    ...
    def select_load(self) -> Tuple[DiscreteLoad, ActionEnum, bool]:
        ...
        if not self.bounds.clo1:
            if self.prev_action == ActionEnum.EXT_LO:
                if self.prev_candidate < self.bounds.chi1:
                    self.debug("Continue investigating extended lo candi.")
                    return self.prev_candidate, ActionEnum.EXT_LO, False
            ...
            return None, ActionEnum.DONE, False
        if self.prev_action == ActionEnum.EXT_HI:
            if self.prev_candidate not in (self.bounds.clo1, self.bounds.chi1):
                self.debug("Continue investigating extended hi candi.")
                return self.prev_candidate, ActionEnum.EXT_HI, False
