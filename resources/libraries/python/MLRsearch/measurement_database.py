...
class MeasurementDatabase:
    ...
    def get_relevant_bounds(
    ...
    ) -> Tuple[Optional[TrimmedStat], Optional[TrimmedStat]]:
        ...
        :rtype: Tuple[Optional[TrimmedStat], Optional[TrimmedStat]]
        """
    ...
    def get_interval(self, target: TargetSpec) -> StatInterval:
        """Return stat interval for given target spec.

        Attempt to construct interval of relevant bounds.
        If a valid bound is missing, use smallest/biggest intended load
        for lower/upper bound.
        This can result in degenerate interval,
        ...
        :rtype: StatInterval
        """
        lower_bound, upper_bound = self.get_relevant_bounds(target=target)
