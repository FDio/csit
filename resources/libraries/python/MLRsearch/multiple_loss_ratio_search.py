...
class MultipleLossRatioSearch:
    ...
    def _halving_load(
        ...
        if wig > 1.0:
            ...
            return load
        relo = not bounds.clo1 or (bounts.plo1 and bounts.plo1 > bounds.clo1)
        rehi = not bounds.chi1 or (bounts.phi1 and bounts.phi1 < bounds.chi1)
        if relo and rehi:
    ...
    def _lo_remea_load(
        ...
        if bounds.phi1 and (not thi or thi > bounds.phi1):
            thi = bounds.phi1
        if not bounds.plo1 or not thi:
            return None
        if bounds.clo1:
            interval = DiscreteInterval(bounds.clo1, thi)
            if interval.width_in_goals(width_goal) <= 1.0:
                self.debug("Not re-measuring low when narrow alrady.")
                return None
        interval = DiscreteInterval(bounds.plo1, thi)
        if interval.width_in_goals(width_goal) > 1.0:
