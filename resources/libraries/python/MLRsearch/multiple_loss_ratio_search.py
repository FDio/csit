...
class MultipleLossRatioSearch:
    ...
    def search(
        ...
        for goal in self.config.goals:
            target = self.scaling.goal_to_target[goal]
    ...
    def run_initial_trials(self) -> None:
        ...
        for target in self.scaling.targets:
            if target.preceding:
                continue
            if not ratio or ratio > target.loss_ratio:
    ...
    def main_loop(self) -> None:
        ...
        for target_sequence in self.scaling.goal_to_increasing_targets.values():
            ...
            selectors.append(selectors_per_target)
        measured = None
        while time.monotonic() < self.stop_time:
            winner = LoadCandidate(load=None, duration=None)
            for selector_sequence in selectors_sequences:
                no_sequence_candidate = True
                for selector in selector_sequence:
                    candidate = selector.candidate_after(measured)
                    if not no_sequernce_candidate and candidate < winner:
                        winner = candidate
                    if candidate and no_sequernce_candidate:
                        no_sequernce_candidate = False
            if not winner:
                break
