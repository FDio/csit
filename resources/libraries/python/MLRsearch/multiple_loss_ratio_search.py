...
class MultipleLossRatioSearch:
    ...
    def measure(
        ...
        if not isinstance(load_stat, LoadStat):
            raise RuntimeError(f"Load has to be discrete: {load_stat!r}")
        if not load_stat.is_round:
            raise RuntimeError(f"Told to measure unrounded: {load_stat!r}")
        ret_list = list()
        while load_stat.all_duration < duration:
            result = self.measurer.measure(
                intended_duration=self.config.single_trial_duration,
    ...
    def do_initial_measurements(self):
        ...
        if self.config.warmup_duration:
            # Not interfering with database.
            load = LoadStat.new_empty(max_load, self.config.criteria)
            self.measure(self.config.warmup_duration, load, criterion)
        self.debug(f"First measurement at max rate: {max_load}")
        load = self.database.stat_for(max_load)
        results = self.measure(duration, load, criterion)
        self.database.update(load)
        ...
        load = self.database.stat_for(mrr)
        results = self.measure(duration, load, criterion)
        self.database.update(load)
        ...
        load = self.database.stat_for(mrr2)
        self.measure(duration, load, criterion)
        self.database.update(load)
    ...
    def ndrpdr_iteration(self, criterion: Criterion, phase: int) -> None:
        ...
        while time.monotonic() < self.stop_time:
            ...
            load = self.database.stat_for(load)
