...
class MultipleLossRatioSearch:
    ...
    def ndrpdr_iteration(self, ratio, phase):
        ...
        while time.monotonic() < self.stop_time:
            ...
            self.debug(f"load after limit handling: {load}")
            if load is None:
                break
    ...
    def _handle_load_with_excludes(
        ...
        if width <= width_goal:
            ...
            return None
        # Due to rounding strategy and width scaling,
        # it is not easy to avoid re-measurement loads being moved here.
        # Try to not touch loads close to the middle.
        lazy_min = step_up(minimum, width_goal * 0.6)
        lazy_max = step_down(maximum, width_goal * 0.6)
        if lazy_min < load < lazy_max:
            self.debug(u"Not moving when not too close to limits.")
            return load
