...
class MultipleLossRatioSearch:
    ...
    def ndrpdr_iteration(self, ratio: float, phase: int) -> None:
        ...
        self.debug(
            ...
            f" and relative width goal {width_goal} (integer units)."
        )
    ...
    def select_load(
        ...
        if selection.remeasure:
            if load := self._lo_remea_load(bounds, width_goal):
                if selection.halve:
                    # In case this can legitimately happen,
                    # we can allow one more re-measurement (and clear halve).
                    raise RuntimeError(u"Should not happen.")
                return SelectionInfo(load=load)
        ...
        if (extend_load := self._extend_down(bounds, width_goal)) > bisect_load:
            raise RuntimeError(u"Prove this cannot happen.")
    ...
    def _lo_remea_load(
        ...
        if bounds.clo1:
            ...
            if interval.width_in_goals(width_goal) <= 1.0:
                raise RuntimeError(u"Still possible?")
    ...
    def _extend_up(
        ...
        :raises RuntimeError: If algorithm inconsistency is detected.
        """
        if bounds.clo1.discrete_load >= self.config.max_load:
    ...
    def _bisect(
        ...
        """
        interval = MeasurementInterval(bounds.clo1, bounds.chi1)
        if (goals := interval.width_in_goals) <= 1.0:
