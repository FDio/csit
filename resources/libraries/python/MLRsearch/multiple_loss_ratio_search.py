...
from .expander import CurrentWidth
...
class MultipleLossRatioSearch:
    ...
    focused only on one goal (but the focus can switch between goals frequently).
    ...
    FIXME revisit this when draft text is stable enough.
    """
    ...
    def main_loop(self, load0: DiscreteLoad, load1: DiscreteLoad) -> None:
        ...
        In case of a tie, previous winner is preferred, if it is not done.
        If it is done, selectors for earlier goals are preferred.
        ...
        current_width = CurrentWidth.from_loads(load0, load1)
        selectors = []
        ...
        while time.monotonic() < self.stop_time:
            for selector in selectors:
                if selector == winner.selector:
                    continue
                # Order of arguments is important
                # when two targets nominate the same candidate.
                winner = min(Candidate.nominated_by(selector), winner)
            if not winner:
                break
            if winner.width:
                # Winner is known and wants to update current width.
                current_width.width = winner.width
