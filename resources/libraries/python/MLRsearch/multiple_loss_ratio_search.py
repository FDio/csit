...
class MultipleLossRatioSearch:
    ...
    def search(
        ...
        two_trials = self.run_initial_trials()
        self.main_loop(two_trials)
    ...
    def run_initial_trials(self) -> Tuple[DiscreteResult, DiscreteResult]:
        ...
        result1 = self.measure(duration, max_load)
        rfr = result1.relative_forwarding_rate
        ...
        result2 = self.measure(duration, mrr)
        # Attempt to get narrower width.
        result_ratio = result2.loss_ratio
        if result_ratio > ratio:
            rfr2 = result2.relative_forwarding_rate
        ...
        result3 = self.measure(duration, mrr2)
    ...
    def main_loop(
        self, result_pair: Tuple[DiscreteResult, DiscreteResult]
    ) -> None:
        ...
        :param result_pair: Two measurements returned from run_initial_trials.
        :type result_pair: Tuple[DiscreteResult, DiscreteResult]
        :raises RuntimeError: If the search takes too long.
        """
        prev_width = DiscreteInterval(*result_pair).discrete_width
        selector_sequences = []
