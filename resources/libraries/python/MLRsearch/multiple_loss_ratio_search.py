...
class MultipleLossRatioSearch:
    ...
    def ndrpdr_iteration(self, ratio, phase):
        ...
        while time.monotonic() < self.stop_time:
            ...
            self.debug(f"bounds for load selection: {bounds}")
            load, handle, remeasure = self.select_load(
                bounds, width_goal, remeasure
            )
            if handle:
                self.debug(f"selected load before limit handling: {load}")
                load = self.handle_load_limits(
                    load, width_goal, bounds.clo1, bounds.chi1
                )
            self.debug(f"selected load: {load}")
            if load is None:
                break
    ...
    def select_load(self, bounds, width_goal, remeasure):
        """Return None or new target_tr to measure at, and some flags.
        ...
        The two flags control whether the load should be checked against limits,
        and whether the next selection should attempt a re-measurement.
        (Due to rounding and limit handling, it is not easy to detect
        whether the previous selection has attempted a re-measurement already.)

        :param bounds: Relevant bounds obtained from database.
        :param width_goal: Relative width goal, considered narrow enough.
        :param remeasure: Whether this selection can attempt re-measurement.
        :type bounds: RelevantBounds
        :type width_goal: float
        :returns: The next load rate to measure at, None to end phase;
            whether the load should be limit handled, whether to remeasure next.
        :rtype: Tuple[Optional[float], bool, bool]
        ...
        if bounds.chi2 is None:
            ...
            return bisect_load, False, False
        ...
        if extend_load is not None and extend_load > bisect_load:
            ...
            return extend_load, False, False
