...
class MultipleLossRatioSearch:
    ...
    ) -> Pep3140Dict[Criterion, DiscreteInterval]:
    ...
    def do_initial_measurements(self):
        ...
        for spec in self.scaling.specs:
            if spec.coarser:
                continue
            if not ratio or ratio > spec.loss_ratio:
                ratio = spec.loss_ratio
            if not duration or duration < spec.single_duration_whole:
                duration = spec.single_duration_whole
            if not width or width < spec.discrete_width:
