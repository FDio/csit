...
class MultipleLossRatioSearch:
    ...
    def measure(
        ...
        while 1:
            ...
            if opt == pes or as_upper_only and opt:
                break
    ...
    def select_load(
        ...
        if extend_load := self._extend_up(bounds, width_goal, criterion):
            if (load := bounds.phi1) and extend_load >= load:
        ...
        if load and bisect_load > load:
            self.debug(f"Phi1 < bisect < extend, doing phi1 first: {load}")
            return SelectionInfo(load=load, as_upper_only=True)
