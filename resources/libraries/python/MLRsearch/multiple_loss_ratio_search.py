...
class MultipleLossRatioSearch:
    ...
    def search(
        ...
        self.ndrpdr_root()
        return self.database.get_intervals(self.config.subtrial_duration)
    ...
    def ndrpdr_iteration(self, criterion: Criterion, phase: int) -> None:
        ...
        while time.monotonic() < self.stop_time:
            bounds = RelevantBounds.from_database(
                ...
                subtrial_duration=self.config.subtrial_duration,
            )
    ...
    def select_load(
        ...
        bad_load = self._upgrade_bad(bounds)
        if extend_load := self._extend_up(bounds, width_goal, criterion):
            if bad_load:
                if extend_load >= bad_load:
                    self.debug("Bad load is smaller so doing that.")
                    return SelectionInfo(load=bad_load)
                self.debug("Extend load is smaller so doing that.")
    ...
    def _upgrade_bad(self, bounds: RelevantBounds) -> Optional[DiscreteLoad]:
        """FIXME"""
        load = bounds.phi1
        if load and (not bounds.chi1 or load < bounds.chi1):
            self.debug(f"Previous bad load needs to be upgraded: {load}")
            return load
        return None
