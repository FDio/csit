...
class Sfdp:
    ...
    def enable_sfdp_external_nat_interface(
        node: dict, sw_if_index: int, tenant_id: int
    ) -> None:
        """FIXME"""
    ...
    @staticmethod
    def create_sfdp_resetter(node: dict, max_tenant: int = 1) -> Callable[[], None]:
        ...
        def kill_all_sfdp_sessions() -> None:
            ...
            with PapiSocketExecutor(node) as papi_exec:
                papi_exec.add(cmd, **args).get_reply(err_msg)
            for tenant_id in range(1, max_tenant + 1):
                Sfdp.wait_for_no_remaining_sfdp_sessions(node, tenant_id)
