...
class Sfdp:
    ...
    def set_sfdp_services_ip4(node, tenant_id=1):
        ...
        args = dict(
            tenant_id=tenant_id,
            services=[
                dict(data="sfdp-l4-lifecycle"),
                #dict(data="sfdp-tcp-check"),
                dict(data="ip4-lookup"),
            ],
            n_services=2,
        )
        #with PapiSocketExecutor(node) as papi_exec:
        ...
        #with PapiSocketExecutor(node) as papi_exec:
        #    papi_exec.add(cmd, **args).get_reply(err_msg)
        PapiSocketExecutor.run_cli_cmd(node, "set sfdp service tenant 1 sfdp-l4-lifecycle ip4-lookup forward")
        PapiSocketExecutor.run_cli_cmd(node, "set sfdp service tenant 1 sfdp-l4-lifecycle ip4-lookup reverse"
