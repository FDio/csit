...
class Sfdp:
    ...
    # Not a keyword, just a helper function.
    def get_remaining_sfdp_tenant_sessions(node, tenant_id=1, log=True):
        """FIXME"""
        detail = None
        try:
            ...
            raise RuntimeError(f"{detail=} {err=}")
    ...
    @staticmethod
    def wait_for_no_remaining_sfdp_sessions(node, tenant_id=1, delay=0.1, timeout=10):
        """FIXME"""
        time_stop = monotonic() + timeout
        while monotonic < time_stop:
            remaining, _ = Sfdp.get_remaining_sfdp_tenant_sessions(node, tenant_id, log=False)
            if not remaining:
                break
        else:
