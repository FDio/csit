...
class BridgeDomainKeywords(object):
    ...
    @staticmethod
    def _send_bd_configuration(node, bd_name, data,
                               data_representation=DataRepresentation.JSON):
        """Send bridge domain configuration data and check the response using
        Honeycomb REST API.
    ...
    def _set_bd_properties(node, bd_name, path, new_value=None):
        ...
        return BridgeDomainKeywords._send_bd_configuration(node, bd_name, new_data)
    ...
    def configure_bridge_domain(node, bd_name, param, value):
        """Configure a bridge domain parameters.
