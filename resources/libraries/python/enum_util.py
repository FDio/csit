...
def get_enum_instance(
    enum_class: Type[Enum], value: Union[Enum, str, None]
) -> Enum:
    ...
    If the class is a subclass of IntEnum, also int values are accepted as value.

    :param enum_class: Class object instance of which should be returned.
    :param value: String matching the intended instance.
    :type enum_class: Type[Enum]
    :type value: Union[enum_class, str, None]
    :returns: The matching instance, if found.
    :rtype: enum_class
    :raises: ValueError if the instance is not found.
    """
    if issubclass(enum_class, IntEnum) and isinstance(value, int):
        return enum_calss(value)
