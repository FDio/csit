...
class WireGuardUtil:
    ...
    def _wireguard_create_tunnel_interface_on_dut(
            ...
            keepalive_time, raddr_ip, raddr_range, dut_private_key,
            peer_pubkey):
            ...
            :param raddr_ip: Policy selector remote IPv4/IPv6 start address for the
                first tunnel in direction node2->node1(in dut1)/node1->node2(in dut2).
            ...
            :type peer_pubkey: bytes
            """
            #Set VPP node as DUT interface to up state
            InterfaceUtil.set_interface_state(node, if1_key, state=u'up')
            #Set IP address on VPP node interface
            IPUtil.vpp_interface_set_ip_address(
                node, if1_key, src_ip, prefix_length=raddr_range
            )
            ...
            InterfaceUtil.set_interface_state(
                node, dut_wg_sw_index, state=u'up')
            ...
            args = dict(
                ...
                prefix=IPUtil.create_prefix_object(
                    ip_address(dut_wg_ip), raddr_range
                )
            )
            ...
            IPUtil.vpp_route_add(
                node, raddr_ip, raddr_range,
                gateway=(raddr_ip+1).compressed,
    ...
    def vpp_wireguard_create_tunnel_interface_on_duts(
            ...
            n_allowed_ips, port, keepalive_time,
            ...
            :param if2_ip_addr: VPP node 2 / TG node (in case of 2-node topology)
                interface IPv4/IPv6 address.
            ...
            dut1_allowed_ips = \
                [IPUtil.create_prefix_object(raddr_ip2,raddr_range),]
            dut2_allowed_ips = \
                [IPUtil.create_prefix_object(raddr_ip1,raddr_range),]
            #Configure WireGuard interface on DUT1
            WireGuardUtil._wireguard_create_tunnel_interface_on_dut(
                nodes[u'DUT1'], if1_key, if2_mac_addr, if1_ip_addr,
                if2_ip_addr, dut1_allowed_ips, n_allowed_ips,
                wg_if1_ip_addr, port, keepalive_time, raddr_ip2,
                raddr_range, dut1_privatekey, dut2_pubkey
            )
            #Configure WireGuard interface on DUT2
            WireGuardUtil._wireguard_create_tunnel_interface_on_dut(
                nodes[u'DUT2'], if2_key, if1_mac_addr, if2_ip_addr,
                if1_ip_addr, dut2_allowed_ips, n_allowed_ips,
                wg_if2_ip_addr, port, keepalive_time, raddr_ip1,
                raddr_range, dut2_privatekey, dut1_pubkey
            )
