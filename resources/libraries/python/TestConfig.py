        ...
        with PapiSocketExecutor(node) as papi_exec:
            for i in range(0, vxlan_count):
                ...
                args1[u"prefix"] = AddressWithPrefix(
                    src_ip, src_ip_start.max_prefixlen
                )
    ...
    def vpp_put_vxlan_and_vlan_interfaces_to_bridge_domain(
        ...
        if vxlan_count > 1:
            ...
            for i in range(0, vxlan_count):
                ...
                commands.append(
                    f"ip_route_add_del "
                    f"{dst_ip}/{dst_ip.max_prefixlen} count 1 "
        ...
        route = IPUtil.compose_vpp_route_structure(
            node, str(dst_ip_start), dst_ip_start.max_prefixlen, **kwargs
        )
        args2 = dict(
            is_add=1,
            is_multipath=0,
