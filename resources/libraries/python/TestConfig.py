...
class TestConfig(object):
    ...
    def vpp_put_vxlan_and_vlan_interfaces_to_bridge_domain(
        ...
        with PapiExecutor(node) as papi_exec:
            logger.debug('ip_step: {}'.format(ip_step))
            for i in xrange(0, vxlan_count):
                dst_ip = dst_ip_addr_start + i * ip_step
                logger.debug('dst_ip: {}'.format(dst_ip))
                args1['neighbor']['ip_address'] = str(dst_ip)
                logger.debug("args1['neighbor']['ip_address']: {}".format(
                    args1['neighbor']['ip_address']))
                logger.debug("args1['neighbor']['mac_address']: {}".format(
                    args1['neighbor']['mac_address']))
                ...
                history = False if 1 < i < vxlan_count else True
                logger.debug("args1: {}".format(args1))
                logger.debug("args2: {}".format(args2))
                logger.debug("args3: {}".format(args3))
                logger.debug("args4: {}".format(args4))
                papi_exec.add(cmd1, history=history, **args1). \
                    add(cmd2, history=history, **args2). \
                    add(cmd3, history=history, **args3). \
