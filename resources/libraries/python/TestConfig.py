    ...
    def vpp_create_vxlan_and_vlan_interfaces(
        ...
        with PapiSocketExecutor(node) as papi_exec:
            for i in range(0, vxlan_count):
                ...
                args2[u"src_address"] = Address.from_ip_address(src_ip)
                args2[u"dst_address"] = Address.from_ip_address(dst_ip)
        ...
        with PapiSocketExecutor(node) as papi_exec:
            for i in range(0, vxlan_count):
                ...
                args2[u"route"][u"prefix"][u"address"][u"un"] = \
                    AddressUnion.from_ip_addredd(dst_ip_start + i * ip_step)
                args2[u"route"][u"paths"][0][u"nh"][u"address"] = \
                    AddressUnion.from_ip_address(dst_ip_start + i * ip_step)
