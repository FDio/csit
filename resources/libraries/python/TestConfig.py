...
class TestConfig(object):
    ...
    def vpp_create_vxlan_and_vlan_interfaces(
        ...
        :rtype: int
        """
        src_ip_addr_start = ip_address(src_ip_start)
        dst_ip_addr_start = ip_address(dst_ip_start)
        ...
        with PapiSocketExecutor(node) as papi_exec:
            for i in range(0, vxlan_count):
                ...
                args3[u"vlan_id"] = i + 1
                history = False if 1 < i < vxlan_count - 1 else True
    ...
    def vpp_put_vxlan_and_vlan_interfaces_up(node, vxlan_count, node_vlan_if):
        ...
        if vxlan_count > 10:
            tmp_fn = u"/tmp/put_subinterfaces_up.config"
            commands = list()
            for i in range(0, vxlan_count):
                ...
                vlan_subif_name = f"{vlan_if_name}.{i + 1}"
                vlan_found = False
                vlan_idx = None
                for data in if_data:
    ...
    def vpp_put_vxlan_and_vlan_interfaces_to_bridge_domain(
        ...
        if vxlan_count > 1:
            ...
            tmp_fn = u"/tmp/configure_routes_and_bridge_domains.config"
            commands = list()
            for i in range(0, vxlan_count):
                dst_ip = dst_ip_addr_start + i * ip_step
                commands.append(
        ...
        with PapiSocketExecutor(node) as papi_exec:
            for i in range(0, vxlan_count):
                dst_ip = dst_ip_addr_start + i * ip_step
                args1[u"neighbor"][u"ip_address"] = str(dst_ip)
