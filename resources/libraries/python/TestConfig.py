...
class TestConfig(object):
    ...
    def vpp_create_vxlan_and_vlan_interfaces(
        ...
        args1 = dict(
            ...
            is_add=1,
            is_ipv6=1 if src_ip_start.version == 6 else 0,
            del_all=0,
            address_length=128 if src_ip_start.version == 6 else 32,
            address=None)
    ...
    def vpp_put_vxlan_and_vlan_interfaces_to_bridge_domain(
        ...
        neighbor = dict(
            ...
            flags=0,
            mac_address=str(Topology.get_interface_mac(op_node, op_node_if)),
            ip_address=None)
        args1 = dict(
            is_add=1,
            neighbor=neighbor)
        ...
        route = IPUtil.compose_vpp_route_structure(
            node, str(dst_ip_start), 128 if dst_ip_start.version == 6 else 32,
            **kwargs)
        args2 = dict(
            is_add=1,
            is_multipath=0,
        ...
        with PapiExecutor(node) as papi_exec:
            for i in xrange(0, vxlan_count):
                ...
                args3['rx_sw_if_index'] = Topology.get_interface_sw_index(
                    node, 'vxlan_tunnel{nr}'.format(nr=i+1))
                args3['rx_sw_if_index'] = int(bd_id_start+i)
                args4['rx_sw_if_index'] = Topology.get_interface_sw_index(
                    node, 'vlan_subif{nr}'.format(nr=i+1))
                args4['rx_sw_if_index'] = int(bd_id_start+i)
                history = False if 1 < i < vxlan_count else True
                papi_exec.add(cmd1, history=history, **args1). \
                    add(cmd2, history=history, **args2). \
                    add(cmd3, history=history, **args3). \
