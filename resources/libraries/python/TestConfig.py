...
class TestConfig:
    ...
    def vpp_create_vxlan_and_vlan_interfaces(
        ...
        fast = 0 if vxlan_count < 100 else vxlan_count
        with PapiSocketExecutor(node, do_async=True) as papi_exec:
            for count in range(2 if fast else vxlan_count):
                try:
                ...
                history = bool(not 1 < count < vxlan_count - 2)
            ...
            for count in range(2 if fast else vxlan_count):
                ...
                args2[u"vni"] = int(vni_start) + count
                history = bool(not 1 < count < vxlan_count - 2)
    ...
    def vpp_put_vxlan_and_vlan_interfaces_up(node, vxlan_count, node_vlan_if):
        ...
        with PapiSocketExecutor(node, do_async=True) as papi_exec:
            ...
            for count in range(2 if fast else vxlan_count):
                ...
                history = bool(not 1 < count < vxlan_count - 1)
    ...
    def vpp_put_vxlan_and_vlan_interfaces_to_bridge_domain(
        ...
        with PapiSocketExecutor(node, do_async=True) as papi_exec:
            ...
            for count in range(2 if fast else vxlan_count):
                ...
                args3[u"bd_id"] = int(bd_id_start + count)
                history = bool(not 1 < count < vxlan_count - 2)
                papi_exec.add(cmd3, history=history, **args3)
            papi_exec.get_replies(
                err_msg=err_msg, fast_send=fast, fast_receive=True
            ...
            for count in range(2 if fast else vxlan_count):
                ...
                args4[u"bd_id"] = int(bd_id_start + count)
                history = bool(not 1 < count < vxlan_count - 2)
