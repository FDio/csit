...
class CsrCounters:
    ...
    @staticmethod
    def show_csr_system_all(node):
        """Show CSR system all on single VM.

        :param node: Topology node [NodeType.VM].
        :type node: dict
        """
        cmd = "show platform software system all"
        exec_cmd(node, cmd, disconnect=True)
    ...
    @staticmethod
    def show_csr_ip_nat_translations(node):
        """Show CSR NAT translations on single VM.
    ...
    @staticmethod
    def show_csr_ip_nat_statistics(node):
        """Show CSR NAT statistics on single VM.
    ...
    @staticmethod
    def show_csr_nat_datapath_drop(node):
        """Show CSR NAT datapath drop on single VM.

        :param node: Topology node [NodeType.VM].
        :type node: dict
        """
        cmd = "show platform hardware qfp active feature nat datapath drop"
        exec_cmd(node, cmd, disconnect=True)

    @staticmethod
    def show_csr_nat_datapath_drstatsop(node):
        """Show CSR NAT datapath drstatop on single VM.

        :param node: Topology node [NodeType.VM].
        :type node: dict
        """
        cmd = "show platform hardware qfp active feature nat datapath drstatsop"
        exec_cmd(node, cmd, disconnect=True)
    ...
    @staticmethod
    def show_csr_nat_statistics(node):
        """Show CSR NAT statistics on single VM.

        :param node: Topology node [NodeType.VM].
        :type node: dict
        """
        CsrCounters.show_csr_ip_nat_translations(node)
        CsrCounters.show_csr_ip_nat_statistics(node)
        CsrCounters.show_csr_nat_datapath_drop(node)
        CsrCounters.show_csr_nat_datapath_drstatsop(node)

    @staticmethod
    def show_csr_nat_statistics_on_all_duts(nodes):
        """Show CSR NAT statistics on all VMs.
        ...
        for node in nodes.values():
            if node[u"type"] == NodeType.VM:
                CsrCounters.show_csr_nat_statistics(node)
    ...
    def show_csr_statistics(node):
        ...
        :type node: dict
        """
        CsrCounters.show_csr_system_all(node)
    ...
    @staticmethod
    def clear_csr_statistics(node):
        """SClearhow CSR statistics on single VM.
    ...
    def clear_csr_statistics_on_all_duts(nodes):
        ...
        for node in nodes.values():
            if node[u"type"] == NodeType.VM:
                CsrCounters.clear_csr_statistics(node)
