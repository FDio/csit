...
function clean_environment () {
    ...
    docker volume rm "${DCR_VOLUMES[dut1_vol]}" || {
...
function read_env_variables () {
    ...
    DCR_VOLUMES+=([dut1_vol]="${CSIT_DUT1_VOL}")
...
function set_env_variables () {
    ...
    CSIT_DUT1_VOL="${DCR_VOLUMES[dut1_vol]}"
...
function start_topology_containers () {
    ...
    DUT1_VOL_UUID=$(uuidgen)
    # Create DUT1 /tmp volume to be able to install VPP in "nested" container.
    dut1_vol_param="--volume DUT1_VOL_${DUT1_VOL_UUID}:/tmp "
    DCR_VOLUMES+=([dut1_vol]="DUT1_VOL_${DUT1_VOL_UUID}")
    docker volume create --name "${DCR_VOLUMES[dut1_vol]}" || {
    ...
    params=(${dcr_stc_params} ${dut1_vol_param} --name csit-dut1-$(uuidgen) ${dcr_image})
