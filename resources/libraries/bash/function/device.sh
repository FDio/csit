...
function read_env_variables () {
    ...
    TG_NETMACS=("${CSIT_TG_INTERFACES_PORT_MAC}")
...
function reread_env_variables () {
    ...
    IFS='@'
    read -a TG_NETMACS <<< "${CSIT_TG_INTERFACES_PORT_MAC}"
    read -a TG_PCIDEVS <<< "${CSIT_TG_INTERFACES_PORT_PCI}"
    read -a TG_DRIVERS <<< "${CSIT_TG_INTERFACES_PORT_DRV}"
    read -a TG_VLANS <<< "${CSIT_TG_INTERFACES_PORT_VLAN}"
    read -a TG_MODELS <<< "${CSIT_TG_INTERFACES_PORT_MODEL}"
    read -a DUT1_NETMACS <<< "${CSIT_DUT1_INTERFACES_PORT_MAC}"
    read -a DUT1_PCIDEVS <<< "${CSIT_DUT1_INTERFACES_PORT_PCI}"
    read -a DUT1_DRIVERS <<< "${CSIT_DUT1_INTERFACES_PORT_DRV}"
    read -a DUT1_VLANS <<< "${CSIT_DUT1_INTERFACES_PORT_VLAN}"
    read -a DUT1_MODELS <<< "${CSIT_DUT1_INTERFACES_PORT_MODEL}"
    IFS=$' \t\n'
