...
function dpdk_compile () {
    ...
    sed_build_fix='s/#include <rte_ethdev_core.h>/#include "rte_ethdev_core.h"/g'
    sed_file="./lib/librte_ethdev/rte_ethdev.h"

    sed -i "${sed_build_fix}" "${sed_file}" || die "DPDK build patch failed"
