...
function dpdk_app_check () {
    ...
    # When done, both links should be reported as up.
    # This function checks for done each second up to 30 times,
    # and once for the links after done.
    #
    # L3fwd can only be started with its own link checking
    # (waits up to 9 seconds) and without LSC (Link State Change) interrupts.
    # So it is either fully ready when it prints "done",
    ...
    # If severe symptoms of CSIT-1848 ever return, it would be better
    # to keep tests failing so infra fix gets priority.
    #
    # Arguments:
