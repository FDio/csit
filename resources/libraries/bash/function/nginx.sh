...
function export_vsap_vcl_conf () {
    ...
    #
    # Variables read:
    # - VSAP_CONFIG_DIR - Nginx Vsap Conf File Path
    # - VCL_CONF_PATH - Vcl Conf File Path
    # Functions called:
    # - die - Print to stderr and exit.+
    cp ${VSAP_CONFIG_DIR}/nginx-vsap.conf ${VSAP_CONFIG_DIR}/nginx-tmp.conf \
    || die "copy vsap conf file  failed."
    VCL_CONF_PATH=/tmp/openvpp-testing/resources/templates/vcl/vcl_iperf3.conf
...
function set_ngignx_conf() {
    ...
    RPS_CPS=${2}
    TLS_TCP=${3}
    NGINX_CONF=$(readlink -e "/usr/local/nginx") || {
        die "Readlink failed."
    }
    VSAP_CONFIG_DIR=${NGINX_CONF}/conf
