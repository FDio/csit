...
function set_ngignx_conf() {
    ...
    cp ${VSAP_NGINX_CONF} ${VSAP_CONFIG_DIR}/nginx-tmp.conf \
    || die "copy vsap conf file  failed."
    cp ${VSAP_NGINX_CONF} ${VSAP_CONFIG_DIR}/nginx-vsap.conf
    ...
    sed -i "s|keepalive_timeout ${current_keepalive_time_out}}|keepalive_timeout ${keepalive_time_out}s;|" \
          ${VSAP_CONFIG_DIR}/nginx-tmp.conf || die 'Sed failed.'

    sed -i "s|worker_processes ${current_worker_processes}|worker_processes ${nginx_cores};|" \
          ${VSAP_CONFIG_DIR}/nginx-tmp.conf || die "Sed failed."

    sed -i "s|listen ${current_listen_port}|listen ${listen_port};|" \
          ${VSAP_CONFIG_DIR}/nginx-tmp.conf || die "Sed failed."
    cat ${VSAP_CONFIG_DIR}/nginx-tmp.conf
