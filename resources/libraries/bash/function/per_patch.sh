...
function main_bisect_loop () {
    ...
    # It examines git.log and breaks out of the loop if the bisect is finished.
    ...
    # and git bisect with the adjective is executed. The new iteration starts.
...
function parse_results () {
    ...
    # causing bisection to focus on the cause (or the fix) of the failures.
    #
...
function parse_results_mrr () {
    ...
    pattern='Maximum Receive Rate trial results in packets' || die
...
function parse_results_ndrpdr () {
    ...
    # - ${2} - Path to (overwritten if exists) output file. Required.
    # Variables read:
    # - FDIO_CSIT_PARSE_NDR - If defined and positive, parse for NDR, not PDR.
    # Files read:
    # - output.xml - The input file from argument location.
    # Files updated:
    # - results.txt - (Re)created, in argument location.
    ...
    in_file="${1}" || die "Two arguments needed."
    out_file="${2}" || die "Two arguments needed."
    if [[ "${FDIO_CSIT_PARSE_NDR:-no}" == "yes" ]]; then
...
function parse_results_passrate () {
    ...
    #
    # This function alway passes (or dies).
    #
    ...
    # A medium sized value is chosen for passed run.
