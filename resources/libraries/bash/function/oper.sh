...
# This library defines functions related to operational branches.
# Keep functions ordered alphabetically, please.

# TODO: Add a link to bash style guide.


function checkout_csit_oper_for_vpp_branch () {
    ...
    # Functions called:
    # - die - Print to stderr and exit, defined on "common" library.
    ...
    pushd "${CSIT_DIR}" || die
    csit_branches="$(git branch -r | grep -E "${branch_id}-[0-9]+")" || die
    csit_branch="$(echo "${csit_branches}" | tail -n 1)" || die
