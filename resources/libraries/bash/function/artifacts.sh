...
function download_ubuntu_artifacts () {
    ...
    apt_cache_opts="-o Dir::Etc::SourceList=$(basename ${apt_fdio_repo_file}) "
    apt_cache_opts+="-o Dir::Etc::SourceParts=$(dirname ${apt_fdio_repo_file})"

    packages=$(apt-cache ${apt_cache_opts} dumpavail \
    ...
    if [ -z "${VPP_VERSION-}" ]; then
        ...
        VPP_VERSION=$(apt-cache ${apt_cache_opts} --no-all-versions \
