...
# TODO: Separate gather_* functions into their own file?
...
function help () {
    ...
    die "Usage: `basename $0` csit-[dpdk|vpp|ligato]-[2n-skx|3n-skx|3n-hsw]"
}
    ...
    case "${TEST_CODE}" in
        ...
        csit-*)
            ...
            then
                ...
                install_args="--vpp ${vpp_stable_ver} --dkms ${dpdk_stable_ver}"
                bash "${install_script}" --skip-install ${install_args} || {
                    die "Failed to get VPP packages!"
                }
            fi
            ;;
