...
function compose_pybot_arguments () {
    ...
    for tag in "${TAGS[@]}"; do
        if [[ ${tag} == "!"* ]]; then
            ...
            EXPANDED_TAGS+=("${ROBOT_SELECTION}" "${TOPOLOGIES_TAGS}AND${tag}")
        fi
    done
}
...
function select_tags () {
    ...
    # - ROBOT_SELECTION - Selection criteria [test, suite, include, exclude].
    ...
    awk_nics_sub_cmd+='print "*"$7"-"$11"."$3"-"$1"-"$11"-"$5'
