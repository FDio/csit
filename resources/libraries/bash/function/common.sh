...
function check_vpp_api_signature () {
    ...
    ls -l "${origin_dir}"
    ls -l "${origin_dir}/core"
    ls -l "${origin_dir}/plugins"
    ...
    rm -rf "${tmp_dir}"/* || die
    dpkg -x "${DOWNLOAD_DIR}/vpp-plugin-core_"*".deb" "${tmp_dir}" || die
    mv "${tmp_dir}/usr/share/vpp/api"/* "${curr_dir}" || die
    rm -rf "${tmp_dir}" || die
    ls -l "${origin_dir}"
    ls -l "${origin_dir}/core"
    ls -l "${origin_dir}/plugins"
    if diff -dur "${origin_dir}" "${curr_dir}"; then
        echo "Current build is matching API signature at VPP origin."
        return
    elif [[ -d "${gerrit_dir}" ]]; then
        if diff -dur "${gerrit_dir} "${curr_dir}""; then
...
function common_dirs () {
    ...
    ls -l "${CSIT_DIR}/resources/api/vpp"
    ls -l "${CSIT_DIR}/resources/api/vpp/core"
    ls -l "${CSIT_DIR}/resources/api/vpp/plugins"
}
