...
function cancel_all () {
    # Arguments:
    # - ${1} - Path to file defining reserved topology. Required.
    # Variables read:
    ...
    # TODO: Is is better to NOT do the cleanup after test (only before)?
...
function common_dirs () {
    ...
    BASH_FUNCTION_DIR=$(dirname $(readlink -e "${BASH_SOURCE[0]}")) || {
        die 1 "Some error during localizing this source directory."
    }
    CSIT_DIR=$(readlink -e "${BASH_FUNCTION_DIR}/../../../..") || {
    ...
    mkdir -p "${ARCHIVE_DIR}" || die 1 "Mkdir failed."
    DOWNLOAD_DIR=$(readlink -f "${CSIT_DIR}/download_dir") || {
        die 1 "Readlink failed."
    }
    mkdir -p "${DOWNLOAD_DIR}" || die 1 "Mkdir failed."
}
...
function copy_archives () {
    ...
    if [[ -n "${WORKSPACE-}" ]]; then
        mkdir -p "${WORKSPACE}/archives/" || die 1 "Archives dir create failed."
...
function die () {
    # FIXME: Follow style-recommended documentation structure.
...
function reserve_testbed () {
    ...
    while true; do
        ...
        sleep_time=$[ ( $RANDOM % 20 ) + 180 ]s || {
            die 1 "Sleep time calculation failed."
        }
...
function run_pybot () {
    ...
    local -i tries || die 1 "Setting type of variable failed."
    tries="${1:-1}" || die 1 "Argument evaluation failed."
    ...
    while true; do
        ...
        pybot --outputdir "${ARCHIVE_DIR}"\
            ${PYBOT_ARGS} ${EXPANDED_TAGS[@]} "${CSIT_DIR}/tests/"
