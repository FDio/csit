...
function activate_docker_topology () {
    ...
    params=("${NODENESS}")
    params+=("${FLAVOR}")
    params+=("$(< ${CSIT_DIR}/VPP_DEVICE_RELEASE)")
    ...
    case "${case_text}" in
        "1n_skx")
            ...
            cmd="ssh root@$(hostname -i) -p 6022 bash -s"
            env=$(${cmd} < ${input} ${params[@]}) || {
                die "Running reservation via shim-dcr failed!"
            }
            set -a
            source <(echo "$env" | grep CSIT_) || die "Source failed!"
            set +a
            ;;
        "1n_vbox")
            ...
            source ${input} ${params[@]} || {
                die "Running reservation locally failed!"
            }
            ;;
        *)
    ...
    # Replace all varibales in template with those in environment.
    source <(echo 'cat <<EOF >topology.yaml'; cat ${template}; echo EOF;) || {
        die "Topology file create failed!"
    }

    mv topology.yaml /tmp/ || {
...
function deactivate_docker_topology () {
    # Destroy virtual vpp-device topology.
    #
    # Variables read:
    # - BASH_FUNCTION_DIR - Path to existing directory this file is located in.
    # - CSIT_DIR - Path to existing root of local CSIT git repository.
    ...
    input="${BASH_FUNCTION_DIR}/destroy.sh"
    params=("${CSIT_TG_UUID}")
    params+=("${CSIT_DUT1_UUID}")
    params+=("${CSIT_TG_INTERFACES_PORT1_PCI}")
    params+=("${CSIT_TG_INTERFACES_PORT1_DRV}")
    params+=("${CSIT_TG_INTERFACES_PORT2_PCI}")
    params+=("${CSIT_TG_INTERFACES_PORT2_DRV}")
    params+=("${CSIT_DUT1_INTERFACES_PORT1_PCI}")
    params+=("${CSIT_DUT1_INTERFACES_PORT1_DRV}")
    params+=("${CSIT_DUT1_INTERFACES_PORT2_PCI}")
    params+=("${CSIT_DUT1_INTERFACES_PORT2_DRV}")

    case_text="${NODENESS}_${FLAVOR}"
    case "${case_text}" in
        "1n_skx")
            echo "Destroying over shim"
            #cmd="ssh root@$(hostname -i) -p 6022 bash -s"
            #${cmd} < ${input} ${params[@]} || {
            #    die "Running cleanup via shim-dcr failed!"
            #}
            ;;
        "1n_vbox")
            echo "Destroying locally"
            #source ${input} ${params[@]} || {
            #    die "Running cleanup locally failed!"
            #}
            ;;
        *)
            die "Unknown specification: ${case_text}!"
