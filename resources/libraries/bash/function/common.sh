...
function activate_docker_topology () {
    ...
    case "${case_text}" in
        "1n_skx")
            ...
            run=("enter_mutex || die;")
            run+=("get_available_interfaces ${NODENESS} ${FLAVOR} || die;")
            run+=("start_topology_containers ${device_image} || die;")
            run+=("bind_interfaces_to_containers || die;")
            run+=("set_env_variables || die;")
            run+=("get_env_variables || die;")
            run+=("exit_mutex || die")
            env_vars=$(${ssh} "$(typeset -f); ${run[@]}") || {
        ...
        "1n_vbox")
            ...
            enter_mutex || die
            get_available_interfaces "${NODENESS}" "${FLAVOR}" || die
            start_topology_containers "${device_image}" || die
            bind_interfaces_to_containers || die
            set_env_variables || die
            get_env_variables || die
            exit_mutex || die
            ;;
        *)
...
function deactivate_docker_topology () {
    ...
    case "${case_text}" in
        "1n_skx")
            ...
            run=("enter_mutex || die;")
            run+=("read_env_variables ${env_vars} || die;")
            run+=("clean_environment || die;")
            run+=("exit_mutex || die")
            ${ssh} "$(typeset -f); ${run[@]}" || {
