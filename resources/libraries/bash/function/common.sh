...
function get_test_tag_string () {
    ...
    if [[ "${GERRIT_EVENT_TYPE-}" == "comment-added" ]]; then
        ...
        # Removing them and trigger word, see https://unix.stackexchange.com/a/13472
        # (except relaying on \s whitespace and \S non-whitespace).
        # The last string is concatenated, only the middle part is expanded.
        cmd=("grep" "-oP" '\S*'"${trigger}"'\S*\s\K\S+$')
