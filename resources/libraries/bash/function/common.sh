...
function die () {
    ...
    set -x
    set +eu
    warn "${1:-Run-time error occurred!}"
    exit "${2-1}"
}
        ...
        for topology in "${TOPOLOGIES[@]}"; do
            ...
            if [[ "${result}" == "0" ]]; then
                ...
                trap "cancel_all '${WORKING_TOPOLOGY}'" EXIT || {
                    die "SIGNAL TRAP FAILED. UNRESERVE TESTBED MANUALLY." 2
                }
                break
            fi
        done
...
function select_topology () {
    ...
    case "${NODENESS}_${FLAVOR}" in
        ...
        *)
            ...
            die "Unknown specification: ${1}_${2}"
    esac
...
function unreserve_testbed () {
    ...
    wt="${WORKING_TOPOLOGY}"  # Just to avoid long lines.
    code="0"
    # TODO: Is is better to NOT do the cleanup after test (only before)?
    python "${PYTHON_SCRIPTS_DIR}/topo_cleanup.py" -t "${wt}" || code="1"
    python "${PYTHON_SCRIPTS_DIR}/topo_reservation.py" -c -t "${wt}" || {
        die "UNRESERVE FAILED. REPAIR MANUALLY." 2
    }
    set -eu
    if [[ "${code}" == "1" ]]; then
        die "Unreserve passed, but cleanup failed before that."
    fi
}
