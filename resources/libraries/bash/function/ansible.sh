...
function ansible_hosts () {
    ...
    # so the caller can ignore it. That is important in "teardown" functions,
    # which need to proceed with subsequent actions.
    # Typical example: If one of testbed machines becomes unresponsive,
    # we want to proceed with unreservation, so the failure repeats again
    # (instead of keeping the testbed reserved, without any human noticing).
    #
    # Variable read:
