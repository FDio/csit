...
function select_verify_tags () {
    ...
    # NIC SELECTION
    # All topologies NICs
    available_nic=($(grep -hoPR "model: \K.*" topologies/available/* | sort -u))
    # Selected topology NICs
    reserved_nic=($(grep -hoPR "model: \K.*" ${WORKING_TOPOLOGY} | sort -u))
    # All topologies NICs - Selected topology NICs
    available_in="<(printf '%s\n' \"${available_nic[@]}\")"
    reserved_in="<(printf '%s\n' \"${reserved_nic[@]}\")"
    exclude_nic=($(comm -13 ${reserved_in} ${available_in}))
    ...
    case "${TEST_CODE}" in
        ...
        *ndrpdr-weekly* )
            ...
            ;;
        *ndrpdr-timed* )
            # FIXME: Test tag array is needed!
            ;;
        *mrr-daily* )
    ...
    test_tag_array+=("${exclude_nic[@]/#/!NIC_}")
