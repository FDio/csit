...
#if test "$(id -u)" -ne 0
#then
#    echo "Please use root or sudo to be able to init Kubernetes"
#    exit 1
#fi
...
if ! sudo /usr/local/bin/kubectl version | grep ${K8S_VERSION}
then
...
sudo kubeadm init --token-ttl 0 --pod-network-cidr=192.168.0.0/16 || \
