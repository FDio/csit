...
# between the current HEAD and a commit specified in the first argument
...
#
# If any test failure happens, up to 2 retries are attempted.
# If all retries fail, an artificial result is used to distinguish
...
function run_and_parse () {
    ...
    # Retry up to 2 times if there was a failure in the test.
