...
git checkout -b old
# We want to remove 46023762d1da674ff73954f0d1523d450bfcf03c (22671, breaks ipsec)
...
git checkout -b new
# The following (26540) is the upper bound before altering the history.
git reset --hard ea9bc28592473e24c6dff85735e7e101a9dde4d4
# The offline editing magic.
# See https://stackoverflow.com/a/12395024
GIT_SEQUENCE_EDITOR='sed -i -e "1d"' git rebase -i old
# Another editing round, reinit "old" with content of "new".
git branch -C old
git checkout old
# We want to skip both the following breakage and its fix:
# https://gerrit.fd.io/r/c/vpp/+/22982
# https://gerrit.fd.io/r/c/vpp/+/23288
# The bad new is that the previous rebase
# does not create deterministic commit hashes.
# Searching based in Change-Id is needed.
# Adapted from https://stackoverflow.com/a/46996714
hash_22982=$(git --no-pager log --format=format:%H -1 --grep "Change-Id: I1712729b3f5d4ab659f51a2d3bf798db16b50820")
git reset --hard "${hash_22982}"
git checkout new
hash_23288=$(git --no-pager log --format=format:%H -1 --grep "Change-Id: I6c0058928e5991d61b3c5fcba706f35e6886b0f2")
# Double editing magic.
# String truncation from https://www.unix.com/shell-programming-and-scripting/136031-truncate-string-variable.html
GIT_SEQUENCE_EDITOR='sed -i -e "1d" "$1" && sed -i "/'"${hash_23288:0:9}"'/d" "$1"' git rebase -i old
# Head of this new "new" branch is the upper bound.
# The following puts old to an earlier lower bound.
git checkout old
git reset --hard 66a332cf199fc97cae61aac913d98b9b51dc3a22
git checkout new
...
# 2019-08-20:
# v20.01-rc0-68   053204ab039d34a990ff0e14c32ce3b294fcce0e refs/changes/61/18361/22
...
#  - Memif API change.
# 2019-10-09:
# v20.01-rc0-364  0938547eaaf5440de28877fa4f873c21bcb8cfbf refs/changes/25/21925/9
#  - Read_blocking introduced. Fast PAPI uses that.
# 2019-10-11:
# v20.01-rc0-395  11b40e7ead069eecac51eeed0b7effbf4d53ecf4 refs/changes/77/22677/3
#  - Last good build before the next breakage.
# 2019-10-15:
# v20.01-rc0-396  6df2c7954126a316f86908526c3bb4d649f06597 refs/changes/72/22672/3
#  - .sendall broke socket papi https://gerrit.fd.io/r/c/vpp/+/22672
# 2019-10-16:
# v20.01-rc0-403  8921dc675458b238fc03c5aed53d3462bcdbdb3c refs/changes/99/22699/3
...
# 2019-12-11:
# v20.01-rc0-831 5d4c99f27ee89a823d993853fe6b52e12c9e6c58 refs/changes/77/22277/7
#  - vhost API change
...
set_aside_parent_build_artifacts old || die
