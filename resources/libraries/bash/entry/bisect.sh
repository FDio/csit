...
# Currently only MRR and PDR test outputs is supported.
#
...
# bisect interval focuses on biggest relative change (with respect to max).
#
...
# Frequently, cherry-picking the bisect Change to an earlier parent
...
# Frequently, bisection will be cherry-picked to earlied CSIT commits.
# To reduce the number of workarounds, two ansible functions are supported.
