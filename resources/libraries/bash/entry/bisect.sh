...
# Apply git bisect right now to avoid unneeded work.
git bisect old | tee "git.log" || die "Invalid bisect interval?"
git status || die
git describe || die
if head -n 1 "git.log" | cut -b -11 | fgrep -q "Bisecting:"; then
    echo "Testing initial bounds."
else
    echo "Single commit, no testing needed."
    exit 0
fi
...
# Sometimes, bisection will be cherry-picked to old CSIT commits.
