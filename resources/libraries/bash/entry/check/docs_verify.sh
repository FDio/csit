...
cd "${DOC_GEN_DIR}" || die
GERRIT_BRANCH="$(git rev-parse --abbrev-ref HEAD)" || die
...
piped_command="tee ${work_dir}/docs_verify.log | wc -l"
lines=$(source ./run_doc.sh ${GERRIT_BRANCH} 2> >( ${piped_command} )) || die
if [ "${lines}" != "0" ]; then
    warn
    die "Docs verify checker: FAIL"
fi
