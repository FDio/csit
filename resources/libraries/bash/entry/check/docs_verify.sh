...
GERRIT_BRANCH="$(git symbolic-ref --short HEAD)" || die
pushd "${DOC_GEN_DIR}" || die
source ./run_doc.sh ${GERRIT_BRANCH} 2> ${work_dir}/docs_verify.log || die
popd

if [[ -f "docs_verify.log" && -s "docs_verify.log" ]]; then
    # Output file is non empty.
    warn
    warn "Docs verify checker: FAIL"
    exit 1
fi
