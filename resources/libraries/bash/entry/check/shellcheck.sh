...
#TODO: remove once shellcheck is installed on jenkins executors
if ! type shellcheck ; then
    apt install -y shellcheck || die "Installing shellcheck failed"
fi
shellcheck_out="shellcheck.xml.log"
shellcheck_args=("--shell=bash" "--format=checkstyle")
find_args=("." "-name" "*.sh" "!" "-path" "*/.*" "-print0")
if find "${find_args[@]}" | \
        xargs -0r shellcheck "${shellcheck_args[@]}" > "${shellcheck_out}"; then
    warn
