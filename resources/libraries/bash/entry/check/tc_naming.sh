...
r_testc_rules=(
    ...
    '(eth|dot1q|dot1ad)'
    # TODO: Packet encapsulation (here majority of TC are failing).
    #'(ip4|ip6|ip6ip6)'
    #'(ipsec[[:digit:]]+tnlhw|ipsec[[:digit:]]+tnlsw|icmpv4|icmpv6|'
...
for idx in "${!testcases_names[@]}"; do
    for pass in "${!r_testc_rules[@]}"; do
        ...
        if [[ ! "${testcases_names[idx]}" =~ ${r_rule} ]]; then
            ...
            msg+="is not matching naming rule!"
            echo "${msg}" | tee -a "tc_naming.log"
