...
ssd="${CSIT_DIR}/resources/tools/search_simulators"
pushd "${ssd}"
time python resoaker.py &> "txt.log" || echo "failed?"
mv -vf "${ssd}/txt.log" "${ARCHIVE_DIR}/"
popd
pushd ${CSIT_DIR}
pybot --outputdir "${ARCHIVE_DIR}" foo.robot
popd
