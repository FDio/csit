...
# This script calls a Python script to validate json schema(s)
# with respect to their metaschemas.
# This is useful for finding editing mistakes locally,
# without the need to run real (e.g. device) tests.
...
if ! grep -q '^docs/model/schema/' "${impl_log}"; then
    ...
    warn
    warn "CSIT model schema checker: PASS"
    exit 0
fi

pushd "${CSIT_DIR}/docs/model/schema"
if ! python3 "check_json_schema.py"; then
    warn
    warn "CSIT model schema checker: FAIL"
    popd
    exit 1
fi
popd

warn "Model schema edited, schema valid according to metaschema."
warn
warn "CSIT model schema checker: PASS"
exit 0
