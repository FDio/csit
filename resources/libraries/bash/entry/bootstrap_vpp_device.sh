...
function start_topology_containers () {
    ...
    dcr_image="$(< ${CSIT_DIR}/VPP_DEVICE_RELEASE)"
...
function clean_vpp_device () {
    # Cleanup environment by removing tpoology containers and binding
    ...
    docker rm --force "${DCR_UUIDS[@]}" || die "Cleanup DCR failed."

    # TODO: FIXME
    wget -q https://git.dpdk.org/dpdk/plain/usertools/dpdk-devbind.py || {
        die "Download DPDK util failed."
    }
    python dpdk-devbind.py -b i40evf "${TG_PCIDEVS[@]}" "${DUT1_PCIDEVS[@]}" || {
        die "Unbind failed."á
    }
}
...
function make_topology_file () {
    ...
    cat ${CSIT_DIR}/topologies/available/topology.yaml | grep -v password || {
        die "Topology read failed."
    }

    WORKING_TOPOLOGY=${CSIT_DIR}/topologies/available/topology.yaml
}
