...
function start_topology_containers () {
    ...
    trap 'docker rm  --force "${DCR_UUIDS[@]}" || die "Cleanup DCR failed."' \
        EXIT ERR SIGHUP SIGINT SIGKILL SIGQUIT SIGTERM || {
...
function make_topology_file () {
    ...
    # Variables read:
    # - PCI_ADDR - PCI address of network device.
    # - DCR_PORTS - Docker Container UUIDs.

    set -exuo pipefail

    template="${CSIT_DIR}/topologies/available/vpp_device.template"

    export CSIT_TG_HOST="$(hostname --ip-address)"
    export CSIT_TG_PORT="${DCR_PORTS[tg]#*:}"
    export CSIT_DUT1_HOST="$(hostname --ip-address)"
    export CSIT_DUT1_PORT="${DCR_PORTS[dut1]#*:}"
    export CSIT_TG_INTERFACES_PORT1_MAC="${TG_NETMACS[0]}"
    export CSIT_TG_INTERFACES_PORT1_PCI="${TG_PCIDEVS[0]}"
    export CSIT_TG_INTERFACES_PORT2_MAC="${TG_NETMACS[1]}"
    export CSIT_TG_INTERFACES_PORT2_PCI="${TG_PCIDEVS[1]}"
    export CSIT_DUT1_INTERFACES_PORT1_MAC="${DUT1_NETMACS[0]}"
    export CSIT_DUT1_INTERFACES_PORT1_PCI="${DUT1_PCIDEVS[0]}"
    export CSIT_DUT1_INTERFACES_PORT2_MAC="${DUT1_NETMACS[1]}"
    export CSIT_DUT1_INTERFACES_PORT2_PCI="${DUT1_PCIDEVS[1]}"

    source /dev/stdin \
        <<<"$(echo 'cat <<EOF >topology.yml'; cat ${template}; echo EOF;)"
