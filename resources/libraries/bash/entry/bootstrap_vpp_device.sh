...
function reserve_sriov_vf () {
    ...
    # Variables set:
    # - MUTEX_TIMEOUT - Timeout value for acquiring mutex.
    # - RESERVATION_DIR - Mutex directory that acts as mutex.

    set -exuo pipefail

    # Test if we have any VFs available
    ...
    if [[ -z ${east_netdevs[@]::2} || -z ${west_netdevs[@]::2} ]]; then
    ...
    ip link set ${east_netdevs[0]} netns ${DCR_UUIDS[tg]} || {
        die "Moving interface to ${DCR_UUIDS[tg]} namepsace failed."
    }
    ip link set ${east_netdevs[1]} netns ${DCR_UUIDS[tg]} || {
        die "Moving interface to ${DCR_UUIDS[tg]} namepsace failed."
    }
    ip link set ${west_netdevs[0]} netns ${DCR_UUIDS[sut1]} || {
        die "Moving interface to ${DCR_UUIDS[sut1]} namepsace failed."
    }
...
echo Port TG: ${dcr_ports[tg]#*:}
echo Port DUT1: ${dcr_ports[sut1]#*:}
echo CPID TG: ${dcr_cpids[tg]}
echo CPID DUT1: ${dcr_cpids[sut1]}
