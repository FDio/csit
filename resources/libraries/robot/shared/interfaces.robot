...
| | ${dut_str}= | Convert To Lowercase | ${dut}
| | :FOR | ${id} | IN RANGE | 1 | ${count} + 1
| | | ${if1_vlan}= | Get Interface Vlan | ${nodes['${dut}']} | ${${dut_str}_if1}
| | | ${if2_vlan}= | Get Interface Vlan | ${nodes['${dut}']} | ${${dut_str}_if2}
| | | ${if1_vlan}= | Run Keyword If | ${if1_vlan}
| | | ... | Set Variable | ${if1_vlan}
| | | ... | ELSE
| | | ... | Evaluate | ${100} + ${id} - ${1}
| | | ${if2_vlan}= | Run Keyword If | ${if2_vlan}
| | | ... | Set Variable | ${if2_vlan}
| | | ... | ELSE
| | | ... | Evaluate | ${200} + ${id} - ${1}
