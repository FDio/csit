...
| | ... | - tg - TG node
| | ... | - tg_if{m} - Mth TG interface towards DUT.
| | ... | - tg_if{m}_mac - Mth TG interface MAC address.
| | ... | - dut{n} - DUTn node
| | ... | - dut{n}_if{m} - Mth DUTn interface.
| | ... | - dut{n}_if{m}_mac - Mth DUTn interface MAC address.
| |
| | ... | *Arguments:*
...
| | &{n_dict}= | Compute Circular Topology
| | ... | ${nodes} | filter_list=${nic_model_list} | path_count=${nic_pfs}
| | ${keys}= | Get Dictionary Keys | ${n_dict}
| | FOR | ${key} | IN | @{keys}
| | | ${val}= | Get From Dictionary | ${n_dict} | ${key}
| | | Set Suite Variable | ${${key}} | ${val}
| | END
...
| |
| | FOR | ${dut} | IN | @{duts}
| | | Set Suite Variable | ${${dut}_vf1}[0] | ${${dut}_pf1}[0]
| | | Set Suite Variable | ${${dut}_vf2}[0] | ${${dut}_pf2}[0]
| | END
...
| | | ... | ${${dut}_${layer}1}[0] | ${${dut}_${layer}2}[0]
| | END

| Additional Suite Setup Action For performance_dut
| | [Documentation]
...
| | [Arguments] | ${dut}
| |
| | FOR | ${i} | IN RANGE | 1 | ${nic_pfs} + 1
| | | ${_vif}=
| | | ... | Init ${nic_driver} interface
| | | ... | ${nodes['${dut}']} | ${${dut}_pf${i}}[0] | numvfs=${nic_vfs}
| | | ... | osi_layer=${osi_layer}
| | | ... | Set Suite Variable | ${${dut}_vf${i}} | ${_vif}
| | | ... | Set Suite Variable | ${layer} | vf
| | END
...
| | | ... | Additional Suite Setup Action For performance_dut | ${dut}
| | END
| | Initialize traffic generator
| | ... | ${tg} | ${tg_if1} | ${tg_if2}
| | ... | ${dut1} | ${dut1_${layer}1}[0]
| | ... | ${dut${duts_count}} | ${dut${duts_count}_${layer}2}[0]
| | ... | ${osi_layer}
