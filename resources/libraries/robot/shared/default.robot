...
| | ... | ELSE | Set Variable | ${queue_count}
| | FOR | ${dut} | IN | @{duts}
# | | | ${if_list} | Get node interfaces | ${nodes['${dut}']}
| | | ${if_list} | Get Dictionary Keys | ${nodes['${dut}']['interfaces']}
| | | ${if_numa_node}= | Get interface numa node | ${nodes['${dut}']}
| | | ... | ${if_list}[0]
| | | &{dma_resource_info}= | Get DMA Device Info on DUT | ${nodes['${dut}']}
| | | ... | ${if_numa_node}
| | | ${dma_list}= | Get Dictionary Keys | ${dma_resource_info}
| | | ${len} | Get Length | ${dma_list}
