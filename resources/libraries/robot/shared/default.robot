...
| | ...
| | [Arguments] | ${force_init}=${False} | ${numvfs}=${1}
| | ...
| | ${duts}= | Get Matches | ${nodes} | DUT*
| | :FOR | ${dut} | IN | @{duts}
| | | ${if1_vf0_arr}= | Init AVF interface | ${nodes['${dut}']} | ${${dut}_if1} | numvfs=${numvfs}
| | | ${if2_vf0_arr}= | Init AVF interface | ${nodes['${dut}']} | ${${dut}_if2} | numvfs=${numvfs}
| | | Set Suite Variable | ${${dut}_if1_vf0} | ${if1_vf0_arr[0]}
| | | Set Suite Variable | ${${dut}_if2_vf0} | ${if2_vf0_arr[0]}
