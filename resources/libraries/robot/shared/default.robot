...
| | ...
# TODO: Move to python.
# TODO: Logic: Get PF PCI from TOPO, Init VF, return LIST of PCIs, Include back into TOPO and replace PF interfaces variables
| | ${duts}= | Get Matches | ${nodes} | DUT*
| | :FOR | ${dut} | IN | @{duts}
| | | ${if1_pci}= | Get Interface PCI Addr | ${nodes['${dut}']} | ${${dut}_if1}
| | | ${if2_pci}= | Get Interface PCI Addr | ${nodes['${dut}']} | ${${dut}_if2}
| | | Init AVF interface | ${nodes['${dut}']} | ${if1_pci} | numvfs=${numvfs}
| | | Init AVF interface | ${nodes['${dut}']} | ${if2_pci} | numvfs=${numvfs}
