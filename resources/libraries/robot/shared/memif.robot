...
| | ... | - memif_1_varname - Name of the first Memif interface (Optional).
| | ... | Type: string, default value: memif_if1
| | ... | - memif_2_varname - Name of the second Memif interface (Optional).
...
| | ... | - \${\${memif_1_varname}} - 1st Memif interface.
| | ... | - \${\${memif_2_varname}} - 2nd Memif interface.
| |
| | ... | *Example:*
| |
| | ... | \| Set up memif interfaces on DUT node \
...
| | ... | ${memif_1_varname}=memif_if1 | ${memif_2_varname}=memif_if2 | ${rxq}=${1}
| | ... | ${txq}=${1} | ${role}=MASTER
| |
...
| | Set Test Variable | ${${memif_1_varname}} | ${memif_1_swindex}
| | Set Test Variable | ${${memif_2_varname}} | ${memif_2_swindex}
...
| | ... | - memif_varname - Name of the Memif interface (Optional).
| | ... | Type: string
...
| | ... | ${memif_varname}=memif_if1 | ${rxq}=${1} | ${txq}=${1} | ${role}=MASTER
| |
...
| | Set Test Variable | ${${memif_varname}} | ${memif}
