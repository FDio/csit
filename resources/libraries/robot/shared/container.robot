...
| | | ${tmp_volume}= | Get Environment Variable | CSIT_${dut}_TMP
| | | ... | /tmp:/mnt/host
| | | ${mnt}= | Create List | ${tmp_volume}:/tmp | /dev/vfio:/dev/vfio
...
| | | ${uuid_str}= | Run Keyword If | '${tmp_volume}' == '/tmp:/mnt/host'
| | | ... | Set Variable | ${EMPTY}
| | | ... | ELSE | Replace String | ${tmp_volume} | tmp | ${EMPTY} | count=1
