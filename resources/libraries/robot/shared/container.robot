...
| | Append To List | ${container_groups} | ${group_name}
...
| | | ... | ${dut1_if1} | ${dut1_if2}
| | | Run Keyword | ${group_name}.Construct containers
| | | ... | name=${dut}_${group_name} | node=${nodes['${dut}']} | image=${image}
| | | ... | cpu_cnt=${cpu_cnt} | skip_cnt=${skip_cpus} | cpu_node=${cpu_node}
| | | ... | smt_used=${False} | cpu_shared=${False} | count=${count}

| Create ETCD container on all DUTs
| | [Documentation] | Create Docker ETCD container on all DUTs.
| | ...
| | ${group_name}= | Set Variable | ETCD
| | Append To List | ${container_groups} | ${group_name}
| | Import Library | resources.libraries.python.ContainerUtils.ContainerManager
| | ... | engine=Docker | WITH NAME | ${group_name}
| | ${duts}= | Get Matches | ${nodes} | DUT*
| | :FOR | ${dut} | IN | @{duts}
| | | ${cpu_node}= | Get interfaces numa node | ${nodes['${dut}']}
| | | ... | ${dut1_if1} | ${dut1_if2}
| | | Run Keyword | ${group_name}.Construct container
| | | ... | name=${dut}_${group_name} | node=${nodes['${dut}']}
| | | ... | image=quay.io/coreos/etcd:v3.2.5 | cpu_cnt=${1} | skip_cnt=${0}
| | | ... | cpu_node=${cpu_node} | smt_used=${False} | cpu_shared=${True}
| | | ... | count=${1}

| Create Kafka container on all DUTs
| | [Documentation] | Create Docker Kafka container on all DUTs.
| | ...
| | ${group_name}= | Set Variable | Kafka
| | Append To List | ${container_groups} | ${group_name}
| | Import Library | resources.libraries.python.ContainerUtils.ContainerManager
| | ... | engine=Docker | WITH NAME | ${group_name}
| | ${duts}= | Get Matches | ${nodes} | DUT*
...
| | | Run Keyword | ${group_name}.Construct container
| | | ... | name=${dut}_${group_name} | node=${nodes['${dut}']}
| | | ... | image=spotify/kafka | cpu_cnt=${1} | skip_cnt=${0}
| | | ... | cpu_node=${cpu_node} | smt_used=${False} | cpu_shared=${True}
| | | ... | count=${1}
...
| | [Documentation] | Configure VPP on all DUT nodes in specific container
| | ... | group.
| | ...
