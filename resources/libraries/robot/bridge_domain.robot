...
| | ${qemu_set_node}= | Run Keyword Unless | "${qemu_name}" == "vm_node"
| | ... | Replace Variables | ${qemu_name}.Qemu Set Node
| | ... | ELSE | Set Variable | Qemu Set Node
| | Run keyword | ${qemu_set_node} | ${dut_node}
| | ${qemu_add_vhost}= | Run Keyword Unless | "${qemu_name}" == "vm_node"
| | ... | Replace Variables | ${qemu_name}.Qemu Add Vhost User If
| | ... | ELSE | Set Variable | Qemu Add Vhost User If
| | Run keyword | ${qemu_add_vhost} | ${sock1}
| | Run keyword | ${qemu_add_vhost} | ${sock2}
| | ${qemu_start}= | Run Keyword Unless | "${qemu_name}" == "vm_node"
| | ... | Replace Variables | ${qemu_name}.Qemu Start
| | ... | ELSE | Set Variable | Qemu Start
