...
| | ${tgf_num} = | Set Variable If | "${dut}" == "DUT1" | ${1} | ${2}
| | ${other_num} = | Set Variable If | "${dut}" == "DUT1" | ${2} | ${1}
| | ${tgf_port_name} = | Set Variable | ${${dut}_${int}${tgf_num}}[0]
| | ${other_port_name} = | Set Variable | ${${dut}_${int}${other_num}}[0]
| | ${tgf_if_name} = | Get Interface Name | ${node} | ${tgf_port_name}
| | ${other_if_name} = | Get Interface Name | ${node} | ${other_port_name}
| | ${sock} = | Set Variable | memif-${dut}_CNF
| | ${memif_1_varname} = | Set Variable | ${dut}-memif-1-if1
| | ${memif_2_varname} = | Set Variable | ${dut}-memif-1-if2
| | Set up memif interfaces on DUT node | ${node}
| | ... | ${sock} | ${sock} | ${1} | ${memif_1_varname}
| | ... | ${memif_2_varname} | ${rxq_count_int} | ${rxq_count_int}
| | ${memif_1_swindex} = | Set Variable | ${${memif_1_varname}}
| | VPP Setup Bidirectional L2 patch | ${node}
| | ... | ${tgf_port_name} | ${memif_1_swindex}
| | ... | ${other_port_name} | ${memif_2_swindex}
| | ... | ${tgf_if_name} | ${other_if_name}
