...
|
| Documentation | PVTI keywords.

*** Keywords ***
| Initialize IP4 forwarding with PVTI tunnel in 3-node circular topology
| | [Documentation]
| | ... | Set UP state on VPP interfaces in path on nodes in 3-node circular
| | ... | topology. Create PVTI tunnel on both DUT nodes, setup IPv4 adresses
...
| |
| | ${dut1_tunnel_if_index}= | Create PVTI Tunnel Interface | ${dut1}
| | ... | source_ip=1.1.1.2 | destination_ip=1.1.1.1
| | ${dut2_tunnel_if_index}= | Create PVTI Tunnel Interface | ${dut2}
| | ... | source_ip=1.1.1.1 | destination_ip=1.1.1.2
| |
