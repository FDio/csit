...
| | [Documentation] |  Start the l2fwd with M worker threads with HTT and rxqueues N
...
| | [Documentation] |  Start the l3fwd with M worker threads with HTT and rxqueues N
| | ...             |  and B(yes or no) jumbo frames in all DUTs
| | ${nb-cores}= | Catenate | ${m}
...
| | ... | *Example:*
| | ...
| | ... | \| Start l3fwd to all DUTs \| 0x402 \| 2 \
| | ... | \| 1 \| no
| | [Arguments] | ${cpu} | ${nb-cores} | ${rxqueues} | ${jumbo_frames}
| | ${duts}= | Get Matches | ${nodes} | DUT*
| | :FOR | ${dut} | IN | @{duts}
