...
*** Settings ***
| Library | resources.libraries.python.QemuUtils | WITH NAME | qemu_node1
| Library | resources.libraries.python.QemuUtils | WITH NAME | qemu_node2
*** Keywords ***
...
| | ... | \| /tmp/sock3 \| /tmp/sock4
| | ...
| | [Arguments] | ${sock1} | ${sock2} | ${sock3} | ${sock4} | ${ip1} | ${ip2}
| | ... | ${ip3} | ${ip4} | ${prefix_length}
| | qemu_node1.Qemu Add Vhost User If | ${sock1} | mac=52:54:00:00:04:01
| | qemu_node1.Qemu Add Vhost User If | ${sock2} | mac=52:54:00:00:04:02
| | qemu_node1.Qemu Add Vhost User If | ${sock3} | mac=52:54:00:00:04:03
| | qemu_node1.Qemu Add Vhost User If | ${sock4} | mac=52:54:00:00:04:04
| | qemu_node1.Qemu Set Node | ${dut1_node}
| | ${vm}= | qemu_node1.Qemu Start
...
| | Set Test Variable | ${vm_dut1} | ${vm}

| Setup QEMU Vhost Dut2 and Run
| | [Documentation] | Setup Qemu vhosts and namespaces.
| | ...
| | ... | *Arguments:*
| | ... | - sock1 - Socket path for first Vhost-User interface. Type: string
...
| | [Arguments] | ${sock1} | ${sock2} | ${sock3} | ${sock4} | ${ip1} | ${ip2}
| | ... | ${ip3} | ${ip4} | ${prefix_length}
| | qemu_node2.Qemu Add Vhost User If | ${sock1} | mac=52:54:00:00:06:01
| | qemu_node2.Qemu Add Vhost User If | ${sock2} | mac=52:54:00:00:06:02
| | qemu_node2.Qemu Add Vhost User If | ${sock3} | mac=52:54:00:00:06:03
| | qemu_node2.Qemu Add Vhost User If | ${sock4} | mac=52:54:00:00:06:04
| | qemu_node2.Qemu Set Node | ${dut2_node}
| | ${vm}= | qemu_node2.Qemu Start
| | ${vhost1}= | Get Vhost User If Name By Sock | ${vm} | ${sock1}
| | ${vhost2}= | Get Vhost User If Name By Sock | ${vm} | ${sock2}
| | ${vhost3}= | Get Vhost User If Name By Sock | ${vm} | ${sock3}
| | ${vhost4}= | Get Vhost User If Name By Sock | ${vm} | ${sock4}
...
| | [Arguments] | ${dut} | ${vm} | ${which_qemu_object}
| | ${set_node}= | Replace Variables | ${which_qemu_object}.Qemu Set Node
| | ${sys_status}= | Replace Variables | ${which_qemu_object}.Qemu System Status
| | ${kill}= | Replace Variables | ${which_qemu_object}.Qemu Kill
| | ${sys_pd}= | Replace Variables | ${which_qemu_object}.Qemu System Powerdown
| | ${quit}= | Replace Variables | ${which_qemu_object}.Qemu Quit
| | ${clear_socks}= | Replace Variables | ${which_qemu_object}.Qemu Clear Socks
