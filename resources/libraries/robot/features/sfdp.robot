...
| | [Documentation]
| | ... | Configure cache and table sizes for SFDP sessions in startup.
| |
| | ... | *Arguments:*
| | ... | - sessions - Number of bidirectional sessions ised by the test.
| |
| | ... | *Example:*
| |
| | ... | \| Set SFDP Capacity For Sessions \| \${4128768} \|
| |
| | [Arguments] | ${sessions}
| |
| | ${log2} = | Evaluate | int(math.ceil(math.log(${sessions})/math.log(2)))
| | ... | modules=math
...
| | [Documentation]
| | ... | Set up IP forwarding, activate SFDP services, create resetter.
| |
| | ... | *Arguments:*
| | ... | - services - List of SFDP service names to activate.
| | ... | - remote_host1_ip - Base IP address to forward towards TG PF 1.
| | ... | - remote_host2_ip - Base IP address to forward towards TG PF 2.
| | ... | - remote_host_mask - Common IP mask (prefix length).
| |
| | ... | *Example:*
| |
| | ... | \| \${services} = \| Create List \| sfdp-l4-lifecycle \| ip4-lookup \|
| | ... | \| Initialize SFDP Services \| \${services} \| remote_host_mask=20 \|
| |
| | [Arguments] | ${services} | ${remote_host1_ip}=${NONE}
| | ... | ${remote_host2_ip}=${NONE} | ${remote_host_mask}=${22}
| |
...
| |
| | Add Sfdp Tenant | ${dut} | ${1}
| | Add Sfdp Tenant | ${dut} | ${2}
| |
| | ${nat_services} = | Copy List | ${services}
| | Append To List | ${nat_services} | sfdp-nat-output
| | Set Sfdp Services | ${dut} | ${nat_services} | ${1} | dir=${0}
| | Set Sfdp Services | ${dut} | ${services} | ${1} | dir=${1}
| | Set Sfdp Services | ${dut} | ${services} | ${2}
| |
| | Enable Sfdp Interface Input | ${dut} | ${ix1} | ${1}
| | Enable Sfdp External Nat Interface | ${dut} | ${ix2} | ${2}
| |
| | Sfdp Nat Snat | ${dut}
| |
| | Vpp Route Add
...
| | ... | interface=${in2}
| |
| | ${resetter} = | Create SFDP Resetter | ${dut} | ${2}
| | ${ramp_up_rate} = | Get Ramp Up Rate
| | Return From Keyword If | ${ramp_up_rate}
| | Set Test Variable | \${resetter}
