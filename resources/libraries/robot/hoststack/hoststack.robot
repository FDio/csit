...
| | ... | FIXME.
| |
| | [Arguments] | ${duration} | ${bps_load}
| |
| | Set To Dictionary | ${iperf3_client_attr} | bandwidth | ${bps_load}
...
| | ... | min_load=${min_rate_soft * ${frame_size} * 8}
| | ... | max_load=${max_rate * ${frame_size} * 8}
...
| | | ${duration} | ${bps_load} = | Set Variable | ${inputs}
| | | Update Iperf3 Client Attributes | ${duration} | ${bps_load}
| | | ${server_pid} = | Run hoststack test program on DUT
| | | ... | ${dut2} | ${dut2_if1} | ${dut2_if1_ip4_addr} | ${dut2_if1_ip4_prefix}
| | | ... | ${iperf3_server_attr}[namespace] | ${core_list}
| | | ... | ${iperf3_server_attr}[cfg_vpp_feature] | ${iperf3_server}
...
| | | ${sum_received} = | Evaluate | json.loads('''${client_output}''') | modules=json
| | | ${mbps} = | Convert To Number | ${sum_received["bits_per_second"]}
| | | ${sec} = | Convert To Number | ${sum_received["seconds"]}
| | | ${measurement} = | Iperf Result Into Measurement
| | | ... | ${sec} | ${mbps} | ${duration} | ${bps_load}
| | END
