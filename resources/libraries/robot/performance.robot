...
| | ... | via 100.100.100.2 sw_if_index ${dut1_vif1_idx} multipath
...
| | ... | via 200.200.200.2 sw_if_index ${dut1_vif2_idx} multipath
...
| | ... | via 100.100.100.1 sw_if_index ${dut2_vif1_idx} multipath
| | Add fib table | ${dut2} | 10.10.10.0 | 24 | ${fib_table_2}
| | ... | via 200.200.200.2 sw_if_index ${dut2_vif2_idx} multipath
...
| | ...                | eal_socket_mem=1024
| | ...                | pmd_fwd_mode=mac
| | ...                | pmd_eth_peer_1=1,XX:XX:XX:XX:XX:XX
| | ...                | pmd_eth_peer_2=2,XX:XX:XX:XX:XX:XX
| | ...                | pmd_disable_hw_vlan=${True}
| | Return From Keyword | ${vm}
...
| | ... | *Example:*
| | ...
| | ... | \| Guest VM with Linux Bridge for Vhost L2BD forwarding is setup \
| | ... | \| ${nodes['DUT1']} \| /tmp/sock1 \| /tmp/sock2 \| DUT1_VM \|
| | [Arguments] | ${dut_node} | ${sock1} | ${sock2} | ${vm_name}
| | Import Library | resources.libraries.python.QemuUtils
| | ...            | WITH NAME | ${vm_name}
