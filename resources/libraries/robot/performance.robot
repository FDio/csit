...
| |      | Assign Interface To Fib Table | ${dut1} | ${dut1_vif1}
| |      | ... | ${fib_table_1}
| |      | Assign Interface To Fib Table | ${dut1} | ${dut1_vif2}
| |      | ... | ${fib_table_2}
| |      | Assign Interface To Fib Table | ${dut2} | ${dut2_vif1}
| |      | ... | ${fib_table_1}
| |      | Assign Interface To Fib Table | ${dut2} | ${dut2_vif2}
| |      | ... | ${fib_table_2}
| |      | IP addresses are set on interfaces
| |      | ... | ${dut1} | ${dut1_vif1} | ${ip_net_vif1}.1 | 30
| |      | ... | ${dut1} | ${dut1_vif2} | ${ip_net_vif2}.1 | 30
| |      | ... | ${dut2} | ${dut2_vif1} | ${ip_net_vif1}.1 | 30
| |      | ... | ${dut2} | ${dut2_vif2} | ${ip_net_vif2}.1 | 30
...
| |      | ... | ${number}
| |      | Add arp on dut | ${dut1} | ${dut1_vif1} | ${ip_net_vif1}.2
| |      | ... | 52:54:00:00:${qemu_id}:01
| |      | Add arp on dut | ${dut1} | ${dut1_vif2} | ${ip_net_vif2}.2
| |      | ... | 52:54:00:00:${qemu_id}:02
| |      | Add arp on dut | ${dut2} | ${dut2_vif1} | ${ip_net_vif1}.2
| |      | ... | 52:54:00:00:${qemu_id}:01
| |      | Add arp on dut | ${dut2} | ${dut2_vif2} | ${ip_net_vif2}.2
| |      | ... | 52:54:00:00:${qemu_id}:02
| |      | Vpp Route Add | ${dut1} | 20.20.20.0 | 24 | ${ip_net_vif1}.2
| |      | ... | ${dut1_vif1} | vrf=${fib_table_1}
| |      | Vpp Route Add | ${dut1} | 10.10.10.0 | 24 | ${ip_net_vif2}.2
| |      | ... | ${dut1_vif2} | vrf=${fib_table_2}
| |      | Vpp Route Add | ${dut2} | 20.20.20.0 | 24 | ${ip_net_vif1}.2
| |      | ... | ${dut2_vif1} | vrf=${fib_table_1}
| |      | Vpp Route Add | ${dut2} | 10.10.10.0 | 24 | ${ip_net_vif2}.2
| |      | ... | ${dut2_vif2} | vrf=${fib_table_2}
...
| | ... | *Arguments:*
| | ... | - skip - Number of cpus which will be skipped. Type: int
| | ... | - count - Number of cpus which will be allocated per qemu. Type: int
| | ...
| | ... | *Example:*
| | ...
| | ... | \| '2' Guest VM with dpdk-testpmd connected via vhost-user is setup \
| | ... | in a 3-node circular topology \|
| | ... | \| '3' Guest VM with dpdk-testpmd connected via vhost-user is setup \
| | ... | in a 3-node circular topology \| skip=${6} \| count=${3} \|
| | ...
| | [Arguments] | ${skip}=${6} | ${count}=${5}
| | ...
| | :FOR | ${number} | IN RANGE | 1 | ${nr}+1
| |      | ${sock1}= | Set Variable | /tmp/sock-${number}-1
| |      | ${sock2}= | Set Variable | /tmp/sock-${number}-2
...
| |      | ...     | skip=${skip} | count=${count} | qemu_id=${number}
...
| | ... | \| '3' Guest VMs with dpdk-testpmd-mac connected via vhost-user is \
| | ... | setup in a 3-node circular topology \| skip=${6} \| count=${3} \|
| | ...
| | [Arguments] | ${skip}=${6} | ${count}=${5}
| | ...
| | :FOR | ${number} | IN RANGE | 1 | ${nr}+1
| |      | ${sock1}= | Set Variable | /tmp/sock-${number}-1
| |      | ${sock2}= | Set Variable | /tmp/sock-${number}-2
| |      | ${dut1_vif1_mac}= | Replace Variables | ${dut1-vhost-${number}-if1_mac}
| |      | ${dut1_vif2_mac}= | Replace Variables | ${dut1-vhost-${number}-if2_mac}
| |      | ${dut2_vif1_mac}= | Replace Variables | ${dut2-vhost-${number}-if1_mac}
| |      | ${dut2_vif2_mac}= | Replace Variables | ${dut2-vhost-${number}-if2_mac}
| |      | ${vm1}= | Guest VM with dpdk-testpmd-mac connected via vhost-user is setup
| |      | ...     | ${dut1} | ${sock1} | ${sock2} | DUT1_VM${number}
| |      | ...     | ${dut1_vif1_mac} | ${dut1_vif2_mac} | skip=${skip}
| |      | ...     | count=${count} | qemu_id=${number}
| |      | Set To Dictionary | ${dut1_vm_refs} | DUT1_VM${number} | ${vm1}
| |      | ${vm2}= | Guest VM with dpdk-testpmd-mac connected via vhost-user is setup
| |      | ...     | ${dut2} | ${sock1} | ${sock2} | DUT2_VM${number}
| |      | ...     | ${dut2_vif1_mac} | ${dut2_vif2_mac} | skip=${skip}
| |      | ...     | count=${count} | qemu_id=${number}
| |      | Set To Dictionary | ${dut2_vm_refs} | DUT2_VM${number} | ${vm2}
...
| | Show Packet Trace on All DUTs | ${nodes} # remove after test
| | Show statistics on all DUTs
| | Run Keyword If Test Failed
| | ... | Traffic should pass with no loss | ${perf_trial_duration} | ${rate}
| | ... | ${framesize} | ${topology_type} | fail_on_loss=${False}
