...
| | ... | Split String | ${src_ip_start} | .
| | ${dst_ip0} | ${dst_ip1} | ${dst_ip2} | ${dst_ip3}=
| | ... | Split String | ${src_ip_start} | .
| | :FOR | ${i} | IN RANGE | 0 | 4
| |      | ${src_ip${i}}= | Convert To Integer | ${src_ip${i}}
| |      | ${dst_ip${i}}= | Convert To Integer | ${dst_ip${i}}
...
| |      | ${src_ip3}= | Evaluate
| |      | ... | ($src_ip3+$ip3_step)%256+($src_ip2+$ip2_step)/256
| |      | ${src_ip2}= | Evaluate
| |      | ... | ($src_ip2+$ip2_step)%256+($src_ip1+$ip1_step)/256
| |      | ${src_ip1}= | Evaluate
| |      | ... | ($src_ip1+$ip1_step)%256+($src_ip0+$ip0_step)/256
| |      | ${src_ip0}= | Evaluate | ($src_ip0+$ip0_step)%256
| |      | ${dst_ip3}= | Evaluate
| |      | ... | ($dst_ip3+$ip3_step)%256+($dst_ip2+$ip2_step)/256
| |      | ${dst_ip2}= | Evaluate
| |      | ... | ($dst_ip2+$ip2_step)%256+($dst_ip1+$ip1_step)/256
| |      | ${dst_ip1}= | Evaluate
| |      | ... | ($dst_ip1+$ip1_step)%256+($dst_ip0+$ip0_step)/256
| |      | ${dst_ip0}= | Evaluate | ($dst_ip0+$ip0_step)%256
| |      | ${sport}= | Evaluate | $sport+$port_step
| |      | ${dport}= | Evaluate | $dport+$port_step
| |      | ${ipv4_limit_reached}= | Set Variable If
| |      | ... | ${src_ip3}>255 or ${dst_ip3}>255 | ${True}
