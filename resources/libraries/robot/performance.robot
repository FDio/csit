...
| L2 bridge domain with '${acl_entries}'+2 entries in one '${type}' ACL initialized in a 3-node circular topology
| | [Documentation]
...
| | ${acl}= | Catenate | ${acl}
| | ...     | ipv4 permit src 10.10.10.0/24, src 20.20.20.0/24
| | Add Replace Acl Multi Entries | ${dut1} | ${rules}=${acl}
| | @{in_acl_list}= | Run Keyword If | 'ingress' in $type | Create List | ${0}
| | ...             | ELSE | Set Variable | ${None}
| | @{out_acl_list}= | Run Keyword If | 'egress' in $type | Create List | ${0}
| | ...              | ELSE | Set Variable | ${None}
| | Set Acl List For Interface | ${dut1} | ${dut1_if1}
| | ... | ${input_acl_idx}=${in_acl_list} | ${output_acl_idx}=${out_acl_list}
| | Set Acl List For Interface | ${dut1} | ${dut1_if2}
| | ... | ${input_acl_idx}=${in_acl_list} | ${output_acl_idx}=${out_acl_list}
