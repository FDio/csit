...
| L2 bridge domain with '${acls_nr}' permit + 2 '${action}' entries in one '${type}' ACL initialized in a 3-node circular topology
| | [Documentation]
...
| | ${acl}= | Catenate | ${acl}
| | ...     | ipv4 ${action} src 10.10.10.0/24, src 20.20.20.0/24
| | Add Replace Acl Multi Entries | ${dut1} | rules=${acl}
| | @{acl_list}= | Create List | ${0}
| | Run Keyword If | 'ingress' in $type | Set Acl List For Interface | ${dut1}
| | ... | ${dut1_if1} | input | ${acl_list}
| | Run Keyword If | 'ingress' in $type | Set Acl List For Interface | ${dut1}
| | ... | ${dut1_if2} | input | ${acl_list}
| | Run Keyword If | 'egress' in $type | Set Acl List For Interface | ${dut1}
