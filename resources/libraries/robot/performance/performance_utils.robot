...
| | ... | \| 1 \| 1 \| 2 \| 1 \|
| | ...
| | [Arguments] | ${dut} | ${dut_numa} | ${chain_id}=${1} | ${node_id}=${1}
| | ... | ${mtcr}=${2} | ${dtcr}=${1} | auto_scale=${False}
| | ...
| | ${sut_sc}= | Set Variable | ${1}
| | ${dut_dc}= | Set Variable | ${1}
| | ${dut_mc}= | Set Variable | ${cpu_count_int}
| | ${skip}= | Evaluate | ${sut_sc} + ${dut_mc} + ${dut_dc}
| | ${dtc}= | Set Variable If | ${auto_scale} | ${cpu_count_int} | ${1}
| | ${nf_cpus}= | Cpu slice of list for NF | ${nodes['${dut}']}
| | ... | ${dut_numa} | chain_id=${chain_id} | node_id=${node_id}
| | ... | mtcr=${mtcr} | dtcr=${dtcr} | dtc=${dtc} | skip=${skip}
