...
| | ... | - directions - Bi- (2) or uni- (1) directional traffic. Type: int
| | ...
| | ... | *Example:*
| | ...
...
| | ... | doublings=${doublings} | directions=${directions}
...
| | ... | ${result.ndr_interval.measured_low.target_tr} / float(${directions})
...
| | ... | ${fail_on_loss}=${True} | ${directions}=${2}
| | ...
| | Send traffic at specified rate | ${duration} | ${rate} | ${frame_size}
| | ... | ${traffic_profile} | directions=${directions}
...
| | ... | ${traffic_profile} | ${subsamples}=${1} | ${directions}=${2}
...
| | | ... | ${traffic_profile} | warmup_time=${0} | directions=${directions}
...
| | ... | directions=${directions} | tx_port=${tx_port} | rx_port=${rx_port}
| | Run Keyword If | ${dut_stats}==${True}
...
| | ... | \| \${1} \|
| | ...
| | [Arguments] | ${rate} | ${directions}=${2} | ${tx_port}=${0}
| | ... | ${rx_port}=${1}
| | ...
| | # Duration of -1 means we will stop traffic manually.
| | Send traffic on tg | ${-1} | ${rate} | ${frame_size} | ${traffic_profile}
