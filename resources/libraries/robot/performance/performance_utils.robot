...
| | ... | - target_tr - Target Tr per stream [pps]. Type: float
| | ... | - framesize - L2 Frame Size [B]. Type: integer
...
| | ... | \| \${64} \| \${2} \| show_latency=\${EMPTY}
| | ...
| | [Arguments] | ${text} | ${target_tr} | ${framesize} | ${nr_streams}
| | ... | ${latency}=${EMPTY}
| | ...
| | ${rate_total}= | Evaluate | ${target_tr}*${nr_streams}
| | ${bandwidth_total}= | Evaluate | ${rate_total}*(${framesize}+20)*8/(10**9)
| | Set Test Message | ${\n}${text} | append=yes
| | Set Test Message | ${\n}RATE: ${rate_total} pps | append=yes
| | Set Test Message | (${nr_streams}x ${target_tr} pps) | append=yes
...
| | ...
| | [Arguments] | ${result} | ${framesize} | ${nr_streams}
| | ...
| | ${framesize}= | Get Frame Size | ${framesize}
...
| | Display single bound | NDR lower bound | ${ndr_lo_tr} | ${framesize} | ${nr_streams} | ${ndr_lo_lat}
| | Display single bound | NDR upper bound | ${ndr_hi_tr} | ${framesize} | ${nr_streams}
| | Display single bound | PDR lower bound | ${pdr_lo_tr} | ${framesize} | ${nr_streams} | ${pdr_lo_lat}
