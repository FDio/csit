...
| | | # Both secondary rates have downsides, report the smaller one.
| | | ${arr}= | Set Variable | ${result.approximated_receive_rate}
| | | ${rrr}= | Set Variable | ${result.relative_receive_rate}
| | | ${rate}= | Evaluate | min(${arr}, ${rrr})
| | | Append To List | ${results} | ${rate}
| | END
