...
| | | # Partial receive rate gives reasonable results
| | | # even at big duration stretching, when profile driver stops
| | | # the measurement soon enough. The value is still measured at
| | | # lower offered load, but that is not a big deal for MRR.
| | | ${receive_rate} = | Set Variable  | ${result.partial_receive_rate}
| | | # One downside of the partial receive rate is the fact it is computed
| | | # from the input rate, which is TPS and unidirectional.
| | | # Packet related tests reen to convert.
| | | ${converted rate} = | Evaluate | ${receive_rate} * ${ppta}
| | | ${receive_rate} = | Set Variable If | "_cps" in "${transaction_type}"
| | | ... | ${receive_rate} | ${converted rate}
