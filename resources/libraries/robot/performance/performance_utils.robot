...
| | ... | ${latency}
| | Run Keyword And Return If | """${transaction_is}""" == """upd_cps"""
| | ... | Display single udp_cps bound | ${text} | ${rate_total} | ${latency}
| | Fail | Unknown transaction type: ${transaction_is}


| Display single udp_cps bound
| | [Documentation]
| | ... | Display one bound of NDR+PDR search for UDP_CPS.
...
| | ... | *Example:*
| |
| | ... | \| Display single udp_cps bound \| NDR lower bound \| \${12345.67} \
| | ... | \| latency=\${EMPTY} \|
| |
...
| | | # TODO: What is the best result formula if duration stretching happens?
| | | ${res}= | Get Measurement Result
| | | Append To List | ${results} | ${res.approximated_receive_rate}
| | END
