...
| | | Exec Cmd | ${node} | cd /tmp/cores && gcore ${pid}
#| | | Exec Cmd | ${node} | cd /tmp/cores && gdb -p ${pid} --eval-command="set confirm off" --eval-command="thread apply all bt" --eval-command="quit"
| | | Exec Cmd | ${node} | cd /tmp/cores && time xz -0 *
