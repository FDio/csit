...
| |
| | ${lower_bound} = | Evaluate | float(${interval.low_end})
...
| | ... | ${result[0].low_end} | ${result[0].low_end.original_result.latency}
| | Display single bound | NDR_UPPER
| | ... | ${result[0].high_end}
| | Display single bound | PDR_LOWER
| | ... | ${result[1].low_end} | ${result[1].low_end.original_result.latency}
