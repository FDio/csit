...
| Initialize IPv4 forwarding with vhost for '${nr}' VMs in 2-node circular topology
| | [Documentation]
| | ... | Create pairs of Vhost-User interfaces for defined number of VMs on
| | ... | VPP node. Set UP state of all VPP interfaces in path. Create ${nr}+1
| | ... | FIB tables on DUT with multipath routing. Assign each Virtual
| | ... | interface to FIB table with Physical interface or Virtual interface on
| | ... | both nodes. Setup IPv4 addresses with /30 prefix on DUT-TG links.
| | ... | Set routing on DUT nodes in all FIB tables with prefix /24 and next
| | ... | hop of neighbour IPv4 address. Setup ARP on all VPP interfaces.
| | ...
| | ... | *Arguments:*
| | ... | _None_
| | ...
...
| | ... | \| qemu_id=${2} \|
| | ...
| | [Arguments] | ${dut} | ${sock1} | ${sock2} | ${vm_name} | ${skip}=${6}
...
| | ${vm_thr_cpus}= | Run keyword if | ${smt_used}
| | ... | Evaluate | int(${vm_phy_cpus}*2) | ELSE | int(${vm_phy_cpus})
| | ${skip_cnt}= | Evaluate | ${skip} + (${qemu_id} - 1) * ${vm_phy_cpus}
| | ${qemu_cpus}= | Cpu slice of list per node | ${nodes['${dut}']}
| | ... | ${dut_numa} | skip_cnt=${skip_cnt} | cpu_cnt=${vm_phy_cpus}
| | ... | smt_used=${smt_used}
...
| | ... | skip_cnt=${1} | cpu_cnt=${thr_count_int}
...
| | | ${sock1}= | Set Variable | /tmp/sock-${number}-1
| | | ${sock2}= | Set Variable | /tmp/sock-${number}-2
| | | ${system_cpus}= | Set Variable | ${1}
| | | ${dut_cpus}= | Evaluate | ${cpu_count_int} + ${1}
| | | ${skip_cpus}= | Evaluate | ${dut_cpus}+${system_cpus}
| | | ${vm}=
| | | ... | Configure guest VM with dpdk-testpmd connected via vhost-user
| | | ... | ${dut} | ${sock1} | ${sock2} | ${dut}_VM${number}
| | | ... | skip=${skip_cpus} | qemu_id=${number} | jumbo=${jumbo}
...
| | ... | \| 00:00:00:00:00:01 \| 00:00:00:00:00:02 \| ${6} \|
| | ...
| | [Arguments] | ${dut} | ${sock1} | ${sock2} | ${vm_name}
| | ... | ${eth0_mac} | ${eth1_mac} | ${skip}=${6} | ${qemu_id}=${1}
...
| | ${testpmd_cpus}= | Cpu list per node str | ${nodes['${dut}']} | ${0}
| | ... | skip_cnt=${1} | cpu_cnt=${testpmd_cpus}
| | Dpdk Testpmd Start | ${vm} | eal_corelist=${testpmd_cpus}
...
| | | ... | ${${dut}-vhost-${number}-if2_mac} | skip=${skip_cpus}
