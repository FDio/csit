...
| | ... | gateway=${ip_net_if1}.1 | interface=${${dut}_if1}
| | ...
...
| | ... | gateway=${ip_net_if2}.2 | interface=${${dut}_if2}
| | ...
...
| | | ... | ${${dut}-memif-${number}-if1} | ${fib_table_1}
| | | Assign Interface To Fib Table | ${nodes['${dut}']}
| | | ... | ${${dut}-memif-${number}-if2} | ${fib_table_2}
...
| | | Configure IP addresses on interfaces
| | | ... | ${dut1} | ${${dut}-memif-${number}-if1} | ${ip_net_memif1}.1 | 30
| | | ... | ${dut1} | ${${dut}-memif-${number}-if2} | ${ip_net_memif2}.1 | 30
| | | Vpp Route Add | ${dut} | ${tg_if2_net} | 24 | vrf=${fib_table_1}
| | | ... | gateway=${ip_net_memif1}.1 | interface=${${dut}-memif-${number}-if1}
| | | ... | multipath=${TRUE}
| | | Vpp Route Add | ${dut} | ${tg_if1_net} | 24 | vrf=${fib_table_2}
| | | ... | gateway=${ip_net_memif2}.1 | interface=${${dut}-memif-${number}-if2}
| | | ... | multipath=${TRUE}
...
| | | ... | ${${dut}-memif-${number}-if2}
| | | ${memif_if2_mac}= | Get interface mac | ${nodes['${dut}']}
| | | ... | ${memif_if2_key}
| | | Add arp on dut | ${dut} | ${${dut}-memif-${number}-if1}
| | | ... | ${ip_net_memif2}.1 | ${memif_if2_mac}
| | | Add arp on dut | ${dut} | ${${dut}-memif-${number}-if2}
| | | ... | ${ip_net_memif1}.1 | ${memif_if1_mac}
