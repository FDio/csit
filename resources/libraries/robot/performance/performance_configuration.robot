...
| | | ${sock1}= | Set Variable | /tmp/sock-1-${number}-1
| | | ${sock2}= | Set Variable | /tmp/sock-1-${number}-2
...
| | | ${sock1}= | Set Variable | /tmp/sock-${nf_chain}-${nf_node}-1
| | | ${sock2}= | Set Variable | /tmp/sock-${nf_chain}-${nf_node}-2
| | | Configure vhost interfaces for L2BD forwarding | ${nodes['${dut}']}
| | | ... | ${sock1} | ${sock2}
| | | ... | ${dut}-vhost-${nf_chain}-${nf_node}-if1
| | | ... | ${dut}-vhost-${nf_chain}-${nf_node}-if2
| | | ${bd_id2}= | Evaluate | ${nf_node}+1
| | | Add interface to bridge domain | ${nodes['${dut}']}
| | | ... | ${${dut}-vhost-${nf_chain}-${nf_node}-if1} | ${nf_node}
| | | Add interface to bridge domain | ${nodes['${dut}']}
| | | ... | ${${dut}-vhost-${nf_chain}-${nf_node}-if2} | ${bd_id2}
...
| | ...
| | ... | \| Configure guest VM with dpdk-testpmd connected via vhost-user \
| | ... | \| ${nodes['DUT1']} \| /tmp/sock-1-2-1 \| /tmp/sock-1-2-2 \
| | ... | \| DUT1_VM2 \| qemu_id=${2} \|
| | ...
...
| | ... | vhost-user on node \| DUT1 \| 1 \| 1 \| False \| 256 \|
| | ...
...
| | | ${sock1}= | Set Variable | /tmp/sock-${nf_chain}-${nf_node}-1
| | | ${sock2}= | Set Variable | /tmp/sock-${nf_chain}-${nf_node}-2
| | | ${nf_name}= | Set Variable | ${dut}_VM${nf_chain}${nf_node}
| | | ${vif1_mac}= | Set Variable If | ${nf_node} == ${1}
| | | ... | "3c:fd:fe:a8:ab:98"
| | | ... | "52:54:00:00:01:02"
| | | ${vif2_mac}= | Set Variable If | ${nf_node} == ${1}
| | | ... | "52:54:00:00:02:01"
| | | ... | "3c:fd:fe:a8:ab:9b"
