...
| | ... | link bonding (EthernetBond) interface on both VPP nodes. Enslave one
| | ... | physical interface towards next DUT by EthernetBond interface. Setup
| | ... | VLAN on EthernetBond interfaces between DUTs. Add one Vhost-User
