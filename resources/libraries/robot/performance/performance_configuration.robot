...
| | [Documentation]
| | ... | Setup L2 patch topology by cross connecting two AVF interfaces on
| | ... | each DUT. Interfaces are brought up.
| | ...
| | ${duts}= | Get Matches | ${nodes} | DUT*
| | :FOR | ${dut} | IN | @{duts}
...
| | | ... | ${if2_pci}
| | | Configure L2patch | ${nodes['${dut}']} | ${dut_eth_vf_if1}
| | | ... | ${dut_eth_vf_if2}
| | | Set Interface State | ${nodes['${dut}']} | ${dut_eth_vf_if1} | up
| | | Set Interface State | ${nodes['${dut}']} | ${dut_eth_vf_if2} | up
| | All Vpp Interfaces Ready Wait | ${nodes}
