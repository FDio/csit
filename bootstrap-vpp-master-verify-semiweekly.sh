...
MIIEpQIBAAKCAQEA+IHXq87GcqMR1C47rzx6Cbip5Ghq8pKURL="https://nexus.fd.io/service/local/artifact/maven/content"
VER="RELEASE"
REPO="fd.io.master.ubuntu.trusty.main"
GROUP="io.fd.vpp"
ARTIFACTS="vpp vpp-dbg vpp-dev vpp-dpdk-dev vpp-dpdk-dkms vpp-lib"
PACKAGE="deb deb.md5"rbqKrP5Nf41HcYrT6
...
# 3rd step: Reserve and prepare HW system
...
# 4th step: Prepare virtual environment and run the test
...
do
    echo
    echo Test loop: ${test_set}
    echo

    # Functional tests on VIRL system
    echo Functional test on VIRL system
    pybot -L TRACE \
    ...
    pybot ${PYBOT_ARGS} \
        ...
        --output log_perf_test_set${test_set} \
        tests/
    PARTIAL_RC=$(echo $?)
    if [ ${PARTIAL_RC} -eq 250 ]; then
        MORE_FAILS=1
...
if [ ${RC} -eq 0 ]; then
    ...
    echo ${RC}" critical tests have failed."
    echo "=============================================================================="
    echo
else
    echo
...
# 6th step: Post-processing test data

# Getting JSON perf data output
rebot --output output_perf.xml \
    ./log_perf_test_set1.xml ./log_perf_test_set2.xml ./log_perf_test_set3.xml

python ${CUR_DIR}/resources/tools/robot_output_parser.py \
       -i ${CUR_DIR}/output_perf.xml \
       -o ${CUR_DIR}/output_perf_data.json \
       -v ${VPP_STABLE_VER}
if [ ! $? -eq 0 ]; then
    echo "Parsing ${CUR_DIR}/output_perf.xml failed"
fi
