...
function upgrade_ssh () {
    ...
    pushd "/usr/src"
    rm -f "${tgz}"
    wget "http://mirror.jmu.edu/pub/OpenBSD/OpenSSH/portable/${tgz}"
    sudo tar -xvzf "${tgz}"
    popd
    sudo yum install rpm-build gcc make wget openssl-devel krb5-devel pam-devel libX11-devel xmkmf libXt-devel
