...
SSH_OPTIONS="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o BatchMode=yes -o LogLevel=error"

function ssh_do() {
    echo
    echo "### "  ssh $@
    ssh -i ${VIRL_PKEY} ${SSH_OPTIONS} $@
}
...
do
    ...
    virl_server_status=$(ssh -i ${VIRL_PKEY} ${SSH_OPTIONS} ${VIRL_USERNAME}@${virl_server_candidate} cat $VIRL_SERVER_STATUS_FILE 2>&1)
...
scp -i ${VIRL_PKEY} ${SSH_OPTIONS} *.deb \
...
function stop_virl_simulation {
    ssh -i ${VIRL_PKEY} ${SSH_OPTIONS} ${VIRL_USERNAME}@${VIRL_SERVER}\
        "stop-testcase ${VIRL_SID}"
}

VIRL_SID=$(ssh -i ${VIRL_PKEY} ${SSH_OPTIONS} \
