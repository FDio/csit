...
for index in "${!VIRL_SERVER[@]}"; do
    ...
    scp ${SSH_OPTIONS} \
        ${VIRL_USERNAME}@${VIRL_SERVER[${index}]}:\
        /scratch/${VIRL_SID[${index}]}/topology.yaml \
    ...
    # Rename the topology file
    mv -f topologies/enabled/topology.yaml topologies/enabled/topology${index}.yaml
done
