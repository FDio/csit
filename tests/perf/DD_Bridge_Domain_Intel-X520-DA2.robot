...
| | ... | [Cfg] DUT runs L2BD switching config with ${wt} thread,\
| | ... | ${wt} phy core, ${rxq} receive queue per NIC port.
| | ... | [Ver] Find NDR for ${framesize} Byte frames using binary search\
| | ... | start at 10GE linerate, step ${threshold}pps
| | Add '${wt}' worker threads and rxqueues '${rxq}' without HTT to all DUTs
...
| | ... | [Ver] Find NDR for ${framesize} Byte frames using binary search\
| | ... | start at 10GE linerate, step ${threshold}pps,\
| | ... | LT=${glob_loss_acceptance}${glob_loss_acceptance_type}.
| | Add '${wt}' worker threads and rxqueues '${rxq}' without HTT to all DUTs
| | Add PCI devices to DUTs from 3-node single link topology
| | Run Keyword If | '${framesize}' < ${1522} | Add No Multi Seg to all DUTs
| | Apply startup configuration on all VPP DUTs
