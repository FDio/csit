...
| | ${entries}= | Set Variable | ${4167}
| | Run base operational read performance trial
| | ... | ${node} | ${cores} | ${cycles} | ${threads} | ${entries}

| TC06: Base operational read with 36 TG threads
| | [Documentation]
| | ... | [Ver] Measure response time using single trial throughput test
| | ... | with 100K requests generated in thirty-six threads.
| | ${cycles}= | Set Variable | ${1}
| | ${threads}= | Set Variable | ${36}
| | ${entries}= | Set Variable | ${2778}
| | Run base operational read performance trial
| | ... | ${node} | ${cores} | ${cycles} | ${threads} | ${entries}
