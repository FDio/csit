...
| ...           | AND | Show Bridge Domain Data On All DUTs
...
| ... | vhost-user interfaces using 5 cores pinned to cpus 5-9 and 10-14 and
...
| | When  L2 bridge domains with Vhost-User for '2' VMs initialized \
| |       in a 3-node circular topology
| | And   '2' Guest VMs with dpdk-testpmd connected via vhost-user is setup \
| |       in a 3-node circular topology
