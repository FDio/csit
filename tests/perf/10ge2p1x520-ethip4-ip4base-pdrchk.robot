...
| ... | NIC_Intel-X520-DA2 | ETH | IP4FWD | BASE | IP4BASE | THIS
| ...
...
| Check PDR for IPv4 routing with jumbo frames
| | [Documentation]
| | ... | [Cfg] DUT runs IPv4 routing config with ${wt} thread(s), ${wt} \
| | ... | phy core(s), ${rxq} receive queue(s) per NIC port.
| | ... | [Ver] Verify ref-PDR for 9000B frames using single trial \
| | ... | throughput test at 2x ${rate}.
| | ...
| | [Arguments] | ${framesize} | ${rate} | {$wt} | ${rxq}
| | # Test Variables required for test teardown
...
| | ...
| | [Template] | Check PDR for IPv4 routing with jumbo frames
