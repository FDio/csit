...
function func_export_lib_path() {
    grep -rn "export LD_LIBRARY_PATH=/usr/local/ssl/lib" /etc/profile
    if [ $? -ne 0 ]; then
        echo "export LD_LIBRARY_PATH=/usr/local/ssl/lib" >> /etc/profile
        echo "export VSAP_OK" >> /etc/profile
    fi
}

# Setting restore.
function func_clean_export() {
    grep -rn "export VSAP_OK" /etc/profile
    if [ $? -ne 0 ]; then
        sed -i 's|export LD_LIBRARY_PATH=/usr/local/ssl/lib||' /etc/profile
        sed -i 's|export VSAP_OK||' /etc/profile
    fi
}
