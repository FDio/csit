...
| ... | NIC_Virtual | ETH | IP4FWD | FEATURE | ACL_STATEFUL | IACL_LIST
| ... | DRV_VFIO_PCI
| ...
...
| ... | interface from TG.
| ... | *[Ver] TG verification:* Test IPv4 packets are sent in  both direction \
...
#ACL variables
| ${acl_type}= | permit+refelct

*** Keywords ***
| Local Template
| | [Documentation]
| | ... | [Ver] Make TG send IPv4 on its interfaces to DUT1; \
...
| | @{permit_list} = | Create List | 10.0.0.1/32 | 10.0.0.2/32 | 10.10.10.2/32 | 20.20.20.2/32
| | Run Keyword If | '${acl_type}' != '${EMPTY}'
| | ... | Configure ACLs on a single interface | ${dut1} | ${dut1_if1} | input
...
| | Then Send packet and verify headers
| | ... | ${tg} | 20.20.20.2 | 10.10.10.2
| | ... | ${tg_if2} | ${tg_if2_mac} | ${dut1_if2_mac}
| | ... | ${tg_if1} | ${dut1_if1_mac} | ${tg_if1_mac}

*** Test Cases ***
| tc01-64B-ethip4-ip4base-iacllistbase-permit-reflect-dev
| | [Tags] | 64B
