...
| ... | NIC_Intel-E810CQ | ETH | FLOW | DRV_VFIO_PCI
...
|
| Documentation | *IP4-IPSEC flow test cases*
| ...
| ... | *[Top] Network Topologies:* TG-DUT1 2-node topology with one link\
| ... | between nodes.
| ... | *[Enc] Packet Encapsulations:* Eth-IP4-ESP or Eth-IP4-AH.
| ... | *[Cfg] DUT configuration:* DUT is configured with IP4-IPSEC flow.
...
*** Test Cases ***
| vf-flow-ip4-ipsec-esp-action-redirect-to-queue
| | Initialize Flow Test Configuration
| | ${flow_index} = | Vpp Create IP4 Ipsec Flow | ${dut1}
| | ... | proto=ESP | spi=${spi}
...
| vf-flow-ip4-ipsec-esp-action-drop
| | Initialize Flow Test Configuration
| | ${flow_index} = | Vpp Create IP4 Ipsec Flow | ${dut1}
| | ... | proto=ESP | spi=${spi}
| | ... | action=drop
...
| vf-flow-ip4-ipsec-esp-action-mark
| | Initialize Flow Test Configuration
| | ${flow_index} = | Vpp Create IP4 Ipsec Flow | ${dut1}
| | ... | proto=ESP | spi=${spi}
| | ... | action=mark | value=${7}
...
| vf-flow-ip4-ipsec-ah-action-redirect-to-queue
| | Initialize Flow Test Configuration
| | ${flow_index} = | Vpp Create IP4 Ipsec Flow | ${dut1}
| | ... | proto=AH | spi=${spi}
| | ... | action=redirect-to-queue | value=${7}
...
| vf-flow-ip4-ipsec-ah-action-drop
| | Initialize Flow Test Configuration
| | ${flow_index} = | Vpp Create IP4 Ipsec Flow | ${dut1}
| | ... | proto=AH | spi=${spi}
| | ... | action=drop
...
| vf-flow-ip4-ipsec-ah-action-mark
| | Initialize Flow Test Configuration
| | ${flow_index} = | Vpp Create IP4 Ipsec Flow | ${dut1}
| | ... | proto=AH | spi=${spi}
| | ... | action=mark | value=${7}
