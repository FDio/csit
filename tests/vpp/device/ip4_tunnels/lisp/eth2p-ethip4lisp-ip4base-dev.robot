...
| ... | *[Enc] Packet Encapsulations:* Eth-IPv4-LISP-IPv4-ICMPv4 on DUT1-TG,\
...
| ... | both directions by TG on links to DUT1; on receive\
...
| | ... | ${src_rloc4} | ${dst_rloc4}
| | When Disable Lisp | ${dut1}
| | Then Packet transmission from port to port should fail
| | ... | ${tg} | ${tg1_ip4} | ${NONE}
| | ... | ${tg_if1} | ${tg_if1_mac} | ${tg_lo_mac}
| | When Enable Lisp | ${dut1}
| | And Vpp Add Lisp Adjacency | ${dut1}
| | ... | ${dut1_to_tg_ip4_static_adjacency['vni']}
| | ... | ${dut1_to_tg_ip4_static_adjacency['deid']}
| | ... | ${dut1_to_tg_ip4_static_adjacency['prefix']}
| | ... | ${dut1_to_tg_ip4_static_adjacency['seid']}
| | ... | ${dut1_to_tg_ip4_static_adjacency['prefix']}
| | Then Wait Until Keyword Succeeds | 2x | 5s | Send packet and verify headers
| | ... | ${tg} | ${tg1_ip4} | ${tg_lo_ip4}
| | ... | ${tg_if1} | ${tg_if1_mac} | ${dut1_lo_mac}

*** Test Cases ***
| tc01-110B-ethip4lisp-ip4base-dev
| | [Tags] | 110B
