# Copyright (c) 2018 Cisco and/or its affiliates.
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at:
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

*** Settings ***
| Resource | resources/libraries/robot/shared/default.robot
| Resource | resources/libraries/robot/l2/l2_xconnect.robot
| Resource | resources/libraries/robot/l2/l2_traffic.robot
| Resource | resources/libraries/robot/l2/tagging.robot
| Resource | resources/libraries/robot/shared/testing_path.robot
| ...
| Force Tags | 2_NODE_SINGLE_LINK_TOPO | DEVICETEST | HW_ENV | DCR_ENV
| ... | FUNCTEST | L2XCFWD | BASE | DOT1Q
| ...
| Test Setup | Set up VPP device test
| ...
| Test Teardown | Tear down VPP device test
| ...
| Documentation | *L2 cross-connect with IEEE 802.1Q test cases*
| ...
| ... | *[Top] Network Topologies:* TG-DUT1-TG 2-node circular topology \
| ... | with single links between nodes.
| ... | *[Enc] Packet Encapsulations:* Eth-IPv4-ICMPv4 for L2 switching of \
| ... | IPv4; Eth-IPv6-ICMPv6 for L2 switching of IPv6 use. Both apply to all \
| ... | links.
| ... | *[Cfg] DUT configuration:* DUT1 is configured with L2 cross-connect \
| ... | switching.
| ... | *[Ver] TG verification:* Test ICMPv4 (or ICMPv6) Echo Request packets \
| ... | are sent in both directions by TG on links to DUT1; on receive TG \
| ... | verifies packets for correctness and their IPv4 (IPv6) src-addr, \
| ... | dst-addr and MAC addresses.
| ... | *[Ref] Applicable standard specifications:* RFC792

*** Variables ***
# VLAN
| ${vlan_id1}= | 110
| ${vlan_id2}= | 120
# IPv6
| ${src_ip6}= | 3ffe:63::1
| ${dst_ip6}= | 3ffe:63::2

*** Test Cases ***
| tc01-eth2p-dot1q-l2xcbase-device
| | [Documentation]
| | ... | [Top] TG-DUT1-TG. [Enc] Eth-IPv4-ICMPv4.
| | ... | [Cfg] Configure L2 cross-connects (L2XC) between both interfaces \
| | ... | towards TG on DUT1.
| | ... | [Ver] Make TG send ICMPv4 Echo Req in both directions between two of \
| | ... | its interfaces to be switched by DUT1; verify all packets are \
| | ... | received.
| | ...
| | Given Configure path in 2-node circular topology
| | ... | ${nodes['TG']} | ${nodes['DUT1']} | ${nodes['TG']}
| | And Configure interfaces in path up
#| | ${vlan1_name} | ${vlan1_index}= | And Create vlan sub-interface
#| | ... | ${dut_node} | ${dut_to_tg_if1} | ${vlan_id1}
#| | ${vlan2_name} | ${vlan2_index}= | And Create vlan sub-interface
#| | ... | ${dut_node} | ${dut_to_tg_if2} | ${vlan_id2}
| | ${vlan1_name} | ${vlan1_index}= | And Create tagged sub-interface
| | ... | ${dut_node} | ${dut_to_tg_if1} | ${vlan_id1}
| | ... | outer_vlan_id=${vlan_id1}
| | ... | type_subif=one_tag exact_match
| | ${vlan2_name} | ${vlan2_index}= | And Create tagged sub-interface
| | ... | ${dut_node} | ${dut_to_tg_if2} | ${vlan_id2}
| | ... | outer_vlan_id=${vlan_id2}
| | ... | type_subif=one_tag exact_match
| | And Configure L2 tag rewrite method on interface | ${dut_node}
| | ... | ${vlan1_index} | translate-1-1 | tag1_id=${vlan_id2}
| | And Configure L2 tag rewrite method on interface | ${dut_node}
| | ... | ${vlan2_index} | translate-1-1 | tag1_id=${vlan_id1}
| | And Configure L2XC | ${dut_node} | ${vlan1_index} | ${vlan2_index}
| | When All Vpp Interfaces Ready Wait | ${nodes}
| | Then Send ICMP packet and verify received packet
| | ... | ${tg_node} | ${tg_to_dut_if1} | ${tg_to_dut_if2} | encaps=Dot1q
| | ... | vlan1=${vlan_id1} | encaps_rx=Dot1q | vlan1_rx=${vlan_id2}

| tc02-eth2p-dot1q-l2xcbase-device
| | [Documentation]
| | ... | [Top] TG-DUT1-TG. [Enc] Eth-IPv6-ICMPv6.
| | ... | [Cfg] Configure L2 cross-connects (L2XC) between both interfaces \
| | ... | towards TG on DUT1.
| | ... | [Ver] Make TG send ICMPv6 Echo Req in both directions between two of \
| | ... | its interfaces to be switched by DUT1; verify all packets are \
| | ... | received.
| | ...
| | Given Configure path in 2-node circular topology
| | ... | ${nodes['TG']} | ${nodes['DUT1']} | ${nodes['TG']}
| | And Configure interfaces in path up
#| | ${vlan1_name} | ${vlan1_index}= | And Create vlan sub-interface
#| | ... | ${dut_node} | ${dut_to_tg_if1} | ${vlan_id1}
#| | ${vlan2_name} | ${vlan2_index}= | And Create vlan sub-interface
#| | ... | ${dut_node} | ${dut_to_tg_if2} | ${vlan_id2}
| | ${vlan1_name} | ${vlan1_index}= | And Create tagged sub-interface
| | ... | ${dut_node} | ${dut_to_tg_if1} | ${vlan_id1}
| | ... | outer_vlan_id=${vlan_id1}
| | ... | type_subif=one_tag exact_match
| | ${vlan2_name} | ${vlan2_index}= | And Create tagged sub-interface
| | ... | ${dut_node} | ${dut_to_tg_if2} | ${vlan_id2}
| | ... | outer_vlan_id=${vlan_id2}
| | ... | type_subif=one_tag exact_match
| | And Configure L2 tag rewrite method on interface | ${dut_node}
| | ... | ${vlan1_index} | translate-1-1 | tag1_id=${vlan_id2}
| | And Configure L2 tag rewrite method on interface | ${dut_node}
| | ... | ${vlan2_index} | translate-1-1 | tag1_id=${vlan_id1}
| | And Configure L2XC | ${dut_node} | ${vlan1_index} | ${vlan2_index}
| | When All Vpp Interfaces Ready Wait | ${nodes}
| | Then Send ICMP packet and verify received packet
| | ... | ${tg_node} | ${tg_to_dut_if1} | ${tg_to_dut_if2} | src_ip=${src_ip6}
| | ... | dst_ip=${dst_ip6} | encaps=Dot1q | vlan1=${vlan_id1} | encaps_rx=Dot1q
| | ... | vlan1_rx=${vlan_id2}
