...
| Documentation | **TPUT on lightweight TCP transactions with SFDP IPv4 routing**
| ... |
...
| ... | routing and two static IPv4 /18 route entries, L4 lifecycle service enabled. \
...
*** Variables ***
| @{plugins_to_enable}= | dpdk_plugin.so | sfdp_services_plugin.so | perfmon_plugin.so
| ${crypto_type}= | ${None}
| ${nic_name}= | Intel-X710
| ${nic_driver}= | vfio-pci
| ${nic_rxq_size}= | 0
...
| | And Initialize SFDP Services | ${sfdp_services}
