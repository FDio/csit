...
| | @{plugins_to_disable}= | Create List | gtpu_plugin.so | ila_plugin.so
| | ... | ixge_plugin.so | kubeproxy_plugin.so | l2e_plugin.so | sixrd_plugin.so
| | Set Global Variable | @{plugins_to_disable}
| | Remove Values From List | ${plugins_to_disable} | kubeproxy_plugin.so
