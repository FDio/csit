...
| ... | VPERF | 1CLIENT | 1STREAM | 1460B
| ... | eth-ip4tcpbase-vperf
|
...
|
| Documentation | **VPerf client -> VPerf server goodput.**
| ... |
...
| ... | vperf_client on DUT1 using the VPP app socket API (VCL).
| ... |
...
| | Set Vperf Server Attributes | protocol=tcp
| | ... | vcl_config=vcl_vperf.conf
| | Set Vperf Client Attributes | protocol=tcp
| | ... | vcl_config=vcl_vperf.conf | bytes=${614400000000}
