...
| ... | RXQ_SIZE_0 | TXQ_SIZE_0 | UDP | QUIC | VCLTEST
| ... | HOSTSTACK | 10CLIENT | 10STREAM | 1280B
| ... | eth-ip4udpquicscale10cl10s-native-vcltest
|
...
|
| Documentation | **VCL Test client -> VCL Test server QUIC goodput (10 clients, 10 streams, native crypto).**
| ... |
...
| ... | vcl_test_client on DUT1 using the VPP app socket API (VCL) with QUIC. \
| ... | 10 independent QUIC connections each carrying 10 streams. \
...
| | Set VCL Test Server Attributes | cfg_vpp_feature=quic | protocol=quic
| | Set VCL Test Client Attributes | cfg_vpp_feature=quic | protocol=quic
| | ... | nclients=${clients} | quic_streams=${streams} | bytes=${bytes}
...
*** Test Cases ***
| 1280B-1c-eth-ip4udpquicscale10cl10s-native-vcltest-bps
| | [Tags] | 1C
| | phy_cores=${1}
