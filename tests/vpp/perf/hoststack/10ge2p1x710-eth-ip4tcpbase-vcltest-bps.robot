...
| ... | RXQ_SIZE_0 | TXQ_SIZE_0 | TCP | VCLTEST
| ... | 1CLIENT | 1STREAM | HOSTSTACK | 1460B | eth-ip4tcpbase-vcltest
|
...
|
| Documentation | **TCP Unidirectional VCL Test Client -> VCL Test Server goodput.**
| ... |
...
*** Variables ***
| @{plugins_to_enable}= | dpdk_plugin.so | perfmon_plugin.so | quic_plugin.so | af_packet_plugin.so | hs_apps_plugin.so | ping_plugin.so
| ${nic_name}= | Intel-X710
| ${nic_driver}= | vfio-pci
| ${nic_rxq_size}= | 0
| ${nic_txq_size}= | 0
...
| | Set VCL Test Server Attributes | cfg_vpp_feature= | protocol=tcp
| | Set VCL Test Client Attributes | cfg_vpp_feature= | protocol=tcp
