...
from resources.libraries.python.autogen.Copyright import COPYRIGHT
from resources.libraries.python.autogen.DefaultTestcase import DefaultTestcase

for filename_prolog in glob.glob("*.prolog"):
    with open(filename_prolog, "r") as file_in:
        text_prolog = file_in.read()
    filename_no_ext = filename_prolog.replace(".prolog", "")
    suite_id = filename_no_ext.split("-", 1)[1]
    print "Processing suite_id", suite_id
    testcase = DefaultTestcase(suite_id)
    filename_robot = filename_no_ext + ".robot"
    with open(filename_robot, "w") as file_out:
        file_out.write(COPYRIGHT + '\n')
        file_out.write(text_prolog[:-1])
        # TODO: testcase.write()?
