...
| Honeycomb adds bridge domain to sub-interface
| | [Documentation] | Check if Honeycomb adds a bridge domain to sub-interface.
| | ...
| | [Tags] | honeycomb_sanity
| | ...
...
| | ... | ${node} | ${sub_interface_name} | ${sub_interface_base_settings}
| | ... | ${sub_interface_settings}
| | When Honeycomb adds bridge domain to sub-interface
...
| Honeycomb sets vlan tag rewrite on bridge domain on sub-interface
| | [Documentation] | Check if Honeycomb adds vlan tag rewrite on bridge domain\
| | ... | on sub-interface.
| | ...
| | [Tags] | honeycomb_sanity
| | ...
...
| Honeycomb edits vlan tag rewrite on bridge domain on sub-interface
| | [Documentation] | Check if Honeycomb updates vlan tag rewrite on bridge\
| | ... | domain on sub-interface.
| | ...
| | [Tags] | honeycomb_sanity
| | ...
| | Given sub-interface configuration from Honeycomb should be
...
| Honeycomb removes vlan tag rewrite from bridge domain on sub-interface
| | [Documentation] | Check if Honeycomb removes vlan tag rewrite on bridge\
| | ... | domain on sub-interface.
| | ...
| | [Tags] | honeycomb_sanity
| | ...
...
| Honeycomb sets again vlan tag rewrite on bridge domain on sub-interface
| | [Documentation] | Check if Honeycomb adds vlan tag rewrite on bridge domain\
| | ... | on sub-interface if it was disabled before.
| | ...
| | [Tags] | honeycomb_sanity
| | ...
| | Given sub-interface configuration from Honeycomb should be
...
| | ... | ${node} | ${sub_interface_name} | ${sub_interface_base_settings}
| | ... | ${sub_interface_settings}
| | When Honeycomb tries to remove sub-interface
| | ... | ${node} | ${sub_interface_name}
| | Then sub-interface configuration from Honeycomb should be
| | ... | ${node} | ${sub_interface_name} | ${sub_interface_base_settings}
| | ... | ${sub_interface_settings}
...
| | ... | "rewrite-operation" parameter in "vlan-tag-rewrite".
| | ...
| | [Tags] | honeycomb_sanity
| | ...
...
| | And rewrite tag from Honeycomb should be
| | ... | ${node} | ${sub_interface_name} | ${rw_params_disabled}
| | When Honeycomb tries to set wrong rewrite tag
...
| | [Documentation] | Negative test: Honeycomb tries to set a wrong value of\
| | ... | "first-pushed" parameter in "vlan-tag-rewrite".
| | ...
| | [Tags] | honeycomb_sanity
| | ...
| | Given sub-interface configuration with bd and rw from Honeycomb should be
...
| Honeycomb tries to set sub-interface down while its super-interface is up
| | [Documentation] | Honeycomb tries to set the sub-interface down while its\
| | ... | super-interface is up. It must not be possible.
| | ...
| | [Tags] | honeycomb_sanity
| | ...
| | Given interface state from Honeycomb should be
...
| | And Interface state from VAT should be
| | ... | ${node} | ${sub_interface_name} | up
| | When Honeycomb tries to set sub-interface down
| | ... | ${node} | ${sub_interface_name}
| | Then interface state from Honeycomb should be
| | ... | ${node} | ${sub_interface_name} | up
| | And Interface state from VAT should be
...
| Honeycomb tries to set sub-interface up while its super-interface is down
...
| | And interface state from VAT should be
| | ... | ${node} | ${interface} | down
| | When Honeycomb tries to set sub-interface up
| | ... | ${node} | ${sub_interface_name}
| | Then interface state from Honeycomb should be
| | ... | ${node} | ${interface} | down
| | And interface state from VAT should be
