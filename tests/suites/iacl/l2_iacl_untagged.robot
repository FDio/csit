...
| ...
| ... | *[Top] TG - DUT1 - DUT2 - TG with one link between the nodes.
| ... | *[Cfg] DUT2 is configured with L2 Cross connect.
| ... |        DUT1 is configured with iACL classification on link to TG,
| ... | *[Ver] Test ICMPv4 Echo Request packets are sent in one direction
| ... |        by TG on link to DUT1 and received on TG link to DUT2.
| ... |        On receive TG verifies if packets are dropped.
...
| | [Documentation]
| | ... | [Top] TG-DUT1-DUT2-TG
| | ... | [Cfg] On DUT1 add EtherType IPv4 to classify table with 'deny'.\
| | ... | [Ver] Make TG verify matching packets are dropped.
| | Given Path for 3-node testing is set
| | ... | ${nodes['TG']} | ${nodes['DUT1']} | ${nodes['DUT2']} | ${nodes['TG']}
| | And Interfaces in 3-node path are up
