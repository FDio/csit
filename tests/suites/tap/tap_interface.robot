...
| | Attach Interface To Namespace | ${node} | tap_int1 | container1
| | Attach Interface To Namespace | ${node} | tap_int2 | container2
...
| | Set Int IP For Namespace | ${node} | container1 | tap_int1 | ${tap1_NM} | ${prefix}
| | Set Int IP For Namespace | ${node} | container2 | tap_int2 | ${tap2_NM} | ${prefix}
| | Set Int State For Namespace | ${node} | container1 | tap_int1 | up
| | Set Int State For Namespace | ${node} | container2 | tap_int2 | up

| Setup Bridge Domain
| | [Arguments] | ${node}
| | Bridge domain on DUT node is created | ${node} | ${bid} | learn=${TRUE}
| | Interface is added to bridge domain | ${node} | 9 | ${bid} | 0
| | Interface is added to bridge domain | ${node} | ${dut_to_tg_if1} | ${bid} | 0
