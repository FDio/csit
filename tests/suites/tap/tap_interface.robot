...
| ... | bridge-domain (L2BD) and Docker containers;
| ... | MAC learning enabled and Split Horizon Groups (SHG) are set
| ... | depending on test case; Docker Containers with namespaces (NM) are set
| ... | on DUT1.
...
| | ... | ${container1_name} | ${bid_NM} | ${tap_int1} | ${tap_int2}
...
| | ... | (Top) TG-DUT1-TG. (Enc) Eth-IPv4-ICMPv4.
| | ... | (Cfg) On DUT1
| | ... | configure two if's with IPv4 address ( dut_to_tg_if and TAP )
| | ... | and one in docker container.
