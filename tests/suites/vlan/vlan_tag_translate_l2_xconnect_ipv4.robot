...
| | ... | that packet is received. [Ref] 802.1q
...
| | And L2 Tag Rewrite Method Set On Interface | ${dut1_node} | ${vlan1_index}
| | ... | translate-1-1 | tag1_id=${outer_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut2_node} | ${vlan2_index}
| | ... | pop-1
...
| | ... | method. [Ver] Make TG send ICMPv4 Echo Req tagegd with one Dot1q tag
| | ... | from one of its interfaces to another one via DUT1 and DUT2; verify
| | ... | that packet is not received. [Ref] 802.1q
| | Given Path for 3-node testing is set
| | ... | ${nodes['TG']} | ${nodes['DUT1']} | ${nodes['DUT2']} | ${nodes['TG']}
| | And Interfaces in 3-node path are up
| | ${vlan1_name} | ${vlan1_index}= | When Vlan Subinterface Created
...
| | ... | ${dut2_node} | ${dut2_to_dut1} | ${outer_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut1_node} | ${vlan1_index}
| | ... | translate-1-1 | tag1_id=${outer_vlan_wrong}
| | And L2 Tag Rewrite Method Set On Interface | ${dut2_node} | ${vlan2_index}
| | ... | pop-1
| | And Interfaces and VLAN sub-interfaces inter-connected using L2-xconnect
| | ... | ${dut1_node} | ${dut1_to_dut2} | ${vlan1_index}
...
| | And L2 Tag Rewrite Method Set On Interface | ${dut1_node} | ${vlan1_index}
| | ... | translate-1-2 | push_dot1q=Disabled | tag1_id=${outer_vlan_id2}
| | ... | tag2_id=${inner_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut2_node} | ${vlan2_index}
| | ... | pop-2
| | And Interfaces and VLAN sub-interfaces inter-connected using L2-xconnect
| | ... | ${dut1_node} | ${dut1_to_dut2} | ${vlan1_index}
...
| | ... | type_subif=two_tags dot1ad
| | And L2 Tag Rewrite Method Set On Interface | ${dut1_node} | ${vlan1_index}
| | ... | translate-1-2 | push_dot1q=Disabled | tag1_id=${outer_vlan_wrong}
| | ... | tag2_id=${inner_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut2_node} | ${vlan2_index}
| | ... | pop-2
| | And Interfaces and VLAN sub-interfaces inter-connected using L2-xconnect
...
| | ... | set on Dot1ad sub-interface of DUT2; on DUT2 configure L2 cross-connect
| | ... | with one interface to TG and one Dot1ad sub-interface towards DUT1 with
| | ... | VLAN tag rewrite pop-2 method. [Ver] Make TG send ICMPv4 Echo Req
| | ... | tagegd with one Dot1q tag from one of its interfaces to another one
| | ... | via DUT1 and DUT2; verify that packet is not received.
| | ... | [Ref] 802.1q, 802.1ad
...
| | ... | ${dut2_node} | ${dut2_to_dut1} | ${outer_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut1_node} | ${vlan1_index}
| | ... | translate-2-1 | tag1_id=${outer_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut2_node} | ${vlan2_index}
| | ... | pop-1
| | And Interfaces and VLAN sub-interfaces inter-connected using L2-xconnect
| | ... | ${dut1_node} | ${dut1_to_dut2} | ${vlan1_index}
...
| | ... | ${dut2_node} | ${dut2_to_dut1} | ${outer_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut1_node} | ${vlan1_index}
| | ... | translate-2-1 | tag1_id=${outer_vlan_wrong}
| | And L2 Tag Rewrite Method Set On Interface | ${dut2_node} | ${vlan2_index}
| | ... | pop-1
| | And Interfaces and VLAN sub-interfaces inter-connected using L2-xconnect
| | ... | ${dut1_node} | ${dut1_to_dut2} | ${vlan1_index}
...
| | ... | that packet is received. [Ref] 802.1ad
| | Given Path for 3-node testing is set
| | ... | ${nodes['TG']} | ${nodes['DUT1']} | ${nodes['DUT2']} | ${nodes['TG']}
| | And Interfaces in 3-node path are up
| | ${vlan1_name} | ${vlan1_index}= | When Tagged Subinterface Created
...
| | ... | type_subif=two_tags dot1ad
| | And L2 Tag Rewrite Method Set On Interface | ${dut1_node} | ${vlan1_index}
| | ... | translate-2-2 | push_dot1q=Disabled | tag1_id=${outer_vlan_id2}
| | ... | tag2_id=${inner_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut2_node} | ${vlan2_index}
| | ... | pop-2
| | And Interfaces and VLAN sub-interfaces inter-connected using L2-xconnect
...
| | ... | method. [Ver] Make TG send ICMPv4 Echo Req via DUT1 and DUT2; verify
| | ... | that packet is not received. [Ref] 802.1ad
| | Given Path for 3-node testing is set
| | ... | ${nodes['TG']} | ${nodes['DUT1']} | ${nodes['DUT2']} | ${nodes['TG']}
| | And Interfaces in 3-node path are up
| | ${vlan1_name} | ${vlan1_index}= | When Tagged Subinterface Created
...
| | ... | type_subif=two_tags dot1ad
| | And L2 Tag Rewrite Method Set On Interface | ${dut1_node} | ${vlan1_index}
| | ... | translate-2-2 | push_dot1q=Disabled | tag1_id=${outer_vlan_wrong}
| | ... | tag2_id=${inner_vlan_id2}
| | And L2 Tag Rewrite Method Set On Interface | ${dut2_node} | ${vlan2_index}
| | ... | pop-2
| | And Interfaces and VLAN sub-interfaces inter-connected using L2-xconnect
