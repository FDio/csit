...
| Documentation | *Source RPF check on IPv6 test cases*
| ...
| ... | Reverse Path Forwarding (RPF) tests use 3-node topology
| ... | TG - DUT1 - DUT2 - TG with one link between the nodes.
| ... | DUT2 are configured with L2 Cross connect.
| ... | DUT1 is configured with IP source check on link to TG,
| ... | Test ICMPv6 Echo Request packets are sent in one direction
| ... | by TG on link to DUT1 and received on TG link to DUT2. On receive TG
| ... | verifies if packets which source address is not in routes are dropped.
