...
| ... | domain and MAC learning enabled. Qemu Guest is attached to VPP via VHOST
| ... | interfaces. Guest is running DPDK testpmd interconnecting vhost
| ... | interfaces using 3 cores pinned to cpu mask 0xE0 and 2048M. Testpmd is
| ... | using socket-mem=1024M (512x2M hugepages), 3 cores (1 main core and 2
| ... | cores dedicated for io), forwarding mode is set to io, rxq/txq=2048,
