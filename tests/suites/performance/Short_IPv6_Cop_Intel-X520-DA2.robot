...
| ... | all packets containing Ethernet header, IPv6 header and random payload.
...
| ... | links TG - DUT1 and DUT2 - TG, is tested with two static IPv6 /64
| ... | route entries and additional IPv6 FIB table with index 1 with whitelist
| ... | /32 filter entry. DUT1 and DUT2 tested with 2p10GE NIC X520 Niantic
| ... | by Intel.
...
| | ... | test. DUT runs IPv6 routing and whitelist filters config with
...
| | ... | Verify ref-NDR for 78 Byte frames using single trial throughput
| | ... | test. DUT runs IPv6 routing and whitelist filters config with
| | ... | 2 thread, 2 phy core, 1 receive queue per NIC port.
