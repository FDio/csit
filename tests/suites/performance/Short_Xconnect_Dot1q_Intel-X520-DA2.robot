...
| ... | is tested with with two cross connect interfaces and applied dot1q
| ... | tagging (4 Byte) between DUT1 and DUT2. DUT1 and DUT2 tested with
...
| | ... | test. DUT runs dot1q config with 1 thread, 1 phy core, 1
...
| | [Documentation]
| | ... | Verify ref-NDR for 64 Byte frames using single trial throughput
| | ... | test. DUT runs dot1q config with 2 thread, 2 phy core, 1
| | ... | receive queue per NIC port.
...
| | [Documentation]
| | ... | Verify ref-NDR for 64 Byte frames using single trial throughput
| | ... | test. DUT runs dot1q config with 4 thread, 4 phy core, 2
| | ... | receive queue per NIC port.
