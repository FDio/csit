...
| ... | configured with IPv4 routing and IPv4 security iAcl whitelist ingress
| ... | filtering applied on links TG - DUT1 and DUT2 - TG, is tested with two
| ... | static IPv4 /24 route entries and additional L3 Classify table with
| ... | whitelist for single entry and IPv4.dst masking. DUT1 and DUT2 tested
| ... | with 2p10GE NIC X520 Niantic by Intel.

*** Test Cases ***
| TC01: Verify 64B ref-NDR at 2x 3.5Mpps - DUT IPv4 iAcl whitelist - 1thread 1core 1rxq
| | [Documentation]
...
| | ... | Verify ref-NDR for 64 Byte frames using single trial throughput
| | ... | test. DUT runs IPv4 routing and whitelist filters config with
| | ... | 4 thread, 4 phy core, 2 receive queue per NIC port.
