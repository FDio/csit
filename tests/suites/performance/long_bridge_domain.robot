...
| | ...             | bridge domain in 3-node topology
| | [Tags] | 1_THREAD_NOHTT_RSS_1 | SINGLE_THREAD
| | ${framesize}= | Set Variable | 64
| | ${start_rate}= | Set Variable | 4500000
...
| Find NDR with 2 cores and rss 1 by using binary search and 64B frames through bridge domain in 3-node topology
| | [Documentation] | Find NDR with 2 cores and rss 1 by using binary search
| | ...             | and 64B frames through bridge domain in 3-node topology
| | [Tags] | 2_THREAD_NOHTT_RSS_1 | MULTI_THREAD
| | ${framesize}= | Set Variable | 64
| | ${start_rate}= | Set Variable | 8500000
| | ${step_rate}= | Set Variable | 100000
| | ${min_rate}= | Set Variable | 100000
| | ${max_rate}= | Set Variable | 14880952
...
| Find NDR with 4 cores and rss 2 by using linear search and 64B frames through bridge domain in 3-node topology
| | [Documentation] | Find NDR with 4 cores and rss 2 by using linear search
| | ...             | and 64B frames through bridge domain in 3-node topology
| | [Tags] | 4_THREAD_NOHTT_RSS_2 | MULTI_THREAD
| | ${framesize}= | Set Variable | 64
| | ${start_rate}= | Set Variable | 98000000
| | ${step_rate}= | Set Variable | 100000
| | ${min_rate}= | Set Variable | 100000
| | ${max_rate}= | Set Variable | 14880952
