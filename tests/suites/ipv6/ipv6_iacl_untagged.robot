...
| TC20:DUT with iACL IPv6 MAC src-addr and UDP src-ports and dst-ports drops matching pkts
| | [Documentation] |
| | ... | On DUT1 configure two classify tables, add MAC src-addr to one,
| | ... | add UDP source and destination ports to the other table with
| | ... | 'deny'. Make TG verify that matching packets are dropped.
| | Given Path for 3-node testing is set
| | ... | ${nodes['TG']} | ${nodes['DUT1']} | ${nodes['DUT2']} | ${nodes['TG']}
| | And Interfaces in 3-node path are up
| | And L2 setup xconnect on DUT
