...
if [ $proc_name -eq "0"  ]; then
  man_ip=$man_ip_1
  dut1_to_dut2_if_name=$4
  echo dut1_to_dut2_if_name: $dut1_to_dut2_if_name
  ifconfig $dut1_to_dut2_if_name
else
  man_ip=$man_ip_2
  dut2_to_dut1_if_name=$4
  echo dut2_to_dut1_if_name: $dut2_to_dut1_if_name
  ifconfig $dut2_to_dut1_if_name
fi
...
if [ "$OS_ID" == "centos" ]; then
    ...
    ifaddress1=$ifaddress
    echo $ifaddress1
    ifaddresscut=${ifaddress%.*}
    echo $ifaddresscut
...
ls -l ${LIB_PATH}
sudo LD_LIBRARY_PATH=${LIB_PATH} ./start_nstack.sh
cat /proc/meminfo | grep Huge
...
sudo NSTACK_LOG_ON=DBG LD_LIBRARY_PATH=${LIB_PATH} ./vc_common -p 20000 -d ${man_ip_1} -a 10000 -s ${man_ip_2} -l 200 -t 50 -i 0 -f 1 -r 20000 -n 1 -w 10 -u 10000 -e 10 -x 1
fi
...
echo "running log"
cat /var/log/nStack/running.log

echo "dpdk log"
cat /var/log/nstack-dpdk/nstack_dpdk.log
