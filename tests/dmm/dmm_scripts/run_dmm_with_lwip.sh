...
if [ "$OS_ID" == "centos" ]; then
    ...
    echo $ifaddress1
    ifaddresscut=$(ifconfig $INTERFACE | grep 'inet' | head -n 1 | awk -F " " '{print $2}' | awk -F "." '{print $1"."$2"."$3}')
    echo $ifaddresscut
    ifmac=$(ifconfig $INTERFACE | grep 'ether' | awk -F " " '{print $2}')
    echo $ifmac
elif [ "$OS_ID" == "ubuntu" ]; then
    ifaddress1=$(ifconfig $INTERFACE | grep 'inet' | head -n 1 | cut -d: -f2 | awk '{print $1}')
    echo $ifaddress1
    ifaddresscut=$(ifconfig $INTERFACE | grep 'inet' | head -n 1 | cut -d: -f2 | awk '{print $1}' | awk -F "." '{print $1"."$2"."$3}')
    echo $ifaddresscut
    ifmac=$(ifconfig $INTERFACE | grep 'HWaddr' | awk -F " " '{print $5}')
    echo $ifmac
fi
...
if [ "$OS_ID" == "centos" ]; then
    sed -i 's!eth7!$INTERFACE!1' ip_data.json
elif [ "$OS_ID" == "ubuntu" ]; then
    sed -i 's!eth7!$INTERFACE!1' ip_data.json
fi
...
if [ "$OS_ID" == "centos" ]; then
    sed -i 's!eth7!$INTERFACE!1' network_data_tonStack.json
elif [ "$OS_ID" == "ubuntu" ]; then
    sed -i 's!eth7!$INTERFACE!1' network_data_tonStack.json
fi
sed -i 's!eth7!$INTERFACE!1' network_data_tonStack.json
