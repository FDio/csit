...
SCRIPT_DIR=$ROOTDIR/dmm/scripts

#proc_name => 0 = server, 1= client
proc_name=$1
ifname=$2

dut1_if1=192.168.100.1
dut2_if1=192.168.200.1
...
sed -i 's!.*check_hugepage.sh!#skip hugepage check!1' $SCRIPT_DIR/prep_app_test.sh
sed -i 's!enp0s8!'$ifname'!1' $ROOTDIR/dmm/scripts/prep_app_test.sh
bash -x $ROOTDIR/dmm/scripts/prep_app_test.sh
