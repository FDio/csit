...
| ... | Show Vpp Settings | ${dut_node} | AND
...
| | [Tags] | tmp
| | [Documentation]
| | ... | Test uses VRF to route IPv4 traffic through 2 vhost user interfaces. Both have IP addresses from same network. On VM is set bridge to pass packet from a one vhost user interface to another one.
| |
...
| | ${vhost2_mac}= | And Get Vhost User Mac By SW Index | ${dut_node} | ${vhost2}
...
| | Add IP Neighbor | ${dut_node} | ${dut_to_tg_if2} | ${net3_ip2} | ${tg_to_dut_if2_mac} | vrf=${fib_table_2}

| | When VM for Vhost L2BD forwarding is setup | ${dut1_node} | ${sock1} | ${sock2}
