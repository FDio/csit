...
| Force Tags | 3_NODE_SINGLE_LINK_TOPO | VM_ENV | EXPECTED_FAILING | lisp1
| ...
...
| | ... | Case: ip6-lispgpe-ipsec-ip6 - main fib, phys2lisp \
| | ... | [Ref] RFC6830, RFC4303.
| | ...
...
| | ... | both DUTs and LISP GPE tunnel between them; verify IPv6 headers on
| | ... | received packets are correct.
| | ... | Case: ip6-ipsec-lispgpe-ip6 - main fib, phys2lisp
| | ... | [Ref] RFC6830, RFC4303.
| | ...
| | ${encr_alg}= | Crypto Alg AES CBC 128
| | ${auth_alg}= | Integ Alg SHA1 96
...
| | [Documentation]
| | ... | [Top] TG-DUT1-DUT2-TG.
| | ... | [Enc] Eth-IPv6-LISPGPE-IPSec-IPv6-ICMP on DUT1-DUT2, Eth-IPv6-ICMP \
| | ... | on TG-DUTn.
| | ... | [Cfg] Configure IPv6 LISP static adjacencies on DUT1 and DUT2 with \
...
| | Vpp Set If IPv6 Addr | ${dut1_node} | ${vhost1} | 2001:cdba:1::9
| | ... | ${prefix6}
| | Set Interface State | ${dut1_node} | ${vhost1} | up | link_up=${TRUE}
| | Set Interface State | ${dut1_node} | ${vhost2} | up | link_up=${TRUE}
