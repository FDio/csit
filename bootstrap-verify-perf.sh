...
# Trap function
function cancel_all() {															# TOOD: Add TB cleanup
    python ${SCRIPT_DIR}/resources/tools/scripts/topo_installation.py -d ${INSTALLATION_DIR} -t $1
...
# JOB SETTING																	#TODO: parametrize script
case ${JOB_NAME} in
    *2n-skx*)
...
pip install -r requirements.txt || {
    die 1 "Failed to install requirements in virtual env."
}
...
# On script exit we cancel the reservation and installation and clean testbed.
trap "cancel_all ${WORKING_TOPOLOGY}" EXIT

																				# TODO: Move under PYBOT
