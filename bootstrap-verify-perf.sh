...
if [ ! -z $NUM_BMRR_TRIALS ]; then
    PYBOT_ARGS="$PYBOT_ARGS --variable trial_multiplicity:$NUM_BMRR_TRIALS"
fi
if [ ! -z $TRIAL_DURATION ]; then
    PYBOT_ARGS="$PYBOT_ARGS --variable perf_trial_duration:$TRIAL_DURATION"
fi
