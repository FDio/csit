...
case "$TEST_TAG" in
    ...
    VERIFY-PERF-MRR )
        pybot ${PYBOT_ARGS} \
              -v TOPOLOGY_PATH:${WORKING_TOPOLOGY} \
              -s "tests.vpp.perf" \
              --include mrrAND1t1cANDip4base \
              tests/
        RETURN_STATUS=$(echo $?)
        mkdir -p $JENKINS_HOSTNAME/$JOB_NAME/$BUILD_NUMBER
        mkdir -p $JENKINS_HOSTNAME/$JOB_NAME/$BUILD_NUMBER/archives
        for i in ${LF_ARCHIVE_ARTIFACTS[@]}; do
            cp $( readlink -f ${i} | tr '\n' ' ' ) $JENKINS_HOSTNAME/$JOB_NAME/$BUILD_NUMBER/archives/
        done
        ;;
    PERFTEST_MRR_DAILY )
        pybot ${PYBOT_ARGS} \
