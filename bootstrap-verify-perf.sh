...
case ${JOB_NAME} in																#TODO: Parametrize script
    ...
    *dpdk*)
        ...
        wget --no-check-certificate ${DPDK_REPO}${DPDK_STABLE_VER} || {
            die 1 "Failed to get DPDK package!"
        }
        ;;
    *)
...
python ${SCRIPT_DIR}/resources/tools/scripts/topo_cleanup.py \
    -t ${WORKING_TOPOLOGY}

# There are 2 options. 1) Let CSIT to copy itself (pack_framework_dir) so
# anything downloaded into csit dir will be transferred (mauy not be alsways
# wanted?); 2) Write special script for copy (more complex).
# Per comments lets try to manage it by CSIT framework (1).
#python ${SCRIPT_DIR}/resources/tools/scripts/topo_transfer.py \
#    -t ${WORKING_TOPOLOGY} -d ${INSTALLATION_DIR} || {
#    die 1 "Failed to copy items to hosts from: ${WORKING_TOPOLOGY}!"
#}

# CSIT EXECUTION
PYBOT_ARGS="--variable TOPOLOGY_PATH:${WORKING_TOPOLOGY} --suite tests.${DUT}.perf"
