...
cases = {
    ...
    "isecasync": "IMIX-2c-ethip4ipsec8tnlswasync-scheduler-ip4base-int-aes256gcm-ndrpdr",
...
with open("comparison.csv", "wt") as fout:
    fout.write("refspec")
    for col in cols_in:
        fout.write(f",{col}_siz,{col}_avg,{col}_std")
    fout.write("\n")
    for ver_key in stats:
        ...
        for col in cols_in:
            if col in stat_dict:
                stat = stat_dict[col]
                fout.write(f",{stat.size},{stat.avg},{stat.stdev}")
            else:
                fout.write(f",NA,NA")
        fout.write(f"\n")
