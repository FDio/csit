...
JOB_NAME="csit-vpp-perf-verify-master-3n-icx"
...
for run in `grep -o '"[0-9]*/index\.html"' "${JOB_NAME}" | cut -d '"' -f 2 | cut -d '/' -f 1 | sort -un`; do
    ...
    if [[ ! -f "timing.txt" ]]; then
        ...
        if [[ -f "console-timestamp.log" ]]; then
            grep 'Checking out Revision\|IMIX-2c-\|DR_LOWER: \|packets\|established\|Minimal load is an upper bound' console-timestamp.log > "timing.txt"
        fi
    fi
    if [[ -f "console-timestamp.log" ]]; then
