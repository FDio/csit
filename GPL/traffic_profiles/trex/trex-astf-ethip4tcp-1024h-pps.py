...
class TrafficProfile(TrafficProfileBaseClass):
    ...
    def __init__(self):
        ...
        self.headers_size = 58  # 4B l1 crc + 14B l2 + 20B ipv4 + 20B tcp
        # Framesize is not known at init time.
    ...
    def define_profile(self):
        ...
        :rtype: tuple
        """
        # Framesize is known now.
        self.mss = self.framesize - self.headers_size
        self.data_size = 10 * self.mss
        print(f"fs {self.framesize} hs {self.headers_size} mss {self.mss} ds {self.data_size}")
        ...
        prog_s = ASTFProgram()
        prog_s.accept()
        prog_s.recv(self.data_size)
        prog_s.send(u"1" * self.data_size)
