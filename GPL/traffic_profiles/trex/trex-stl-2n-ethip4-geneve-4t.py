...
Stream profile:
 ...
 - Direction 0 --> 1:
   - Packet: ETH / IP
   - Source IP address range:            10.32.1.0 - 10.32.4.255
   - Destination IP address range:       10.16.1.0 - 10.16.4.255
 - Direction 1 --> 0:
   - Packet: ETH / IP / UDP / GENEVE / ETH / IP
   - Outer Source IP address range:      1.1.1.1
   - Outer Destination IP address range: 1.1.1.2
   - Inner Source IP address range:      10.16.1.0 - 10.16.4.255
   - Inner Destination IP address range: 10.32.1.0 - 10.32.4.255
...
class TrafficStreams(TrafficStreamsBaseClass):
    ...
    def __init__(self):
        ...
        self.p1_src_start_ip = IPv4Address(u"10.32.1.0")
        # self.p1_src_end_ip = self.p1_src_start_ip + self.n_tunnels * 256 - 1
        self.p1_dst_start_ip = IPv4Address(u"10.16.1.0")
        # self.p1_dst_end_ip = \
        #     f"{IPv4Address(self.p1_dst_start_ip) + self.n_tunnels * 256 - 1}"

        # IPs used in packet headers of Direction 1 --> 0.
        self.p2_outer_src_ip = u"1.1.1.1"
        self.p2_outer_dst_ip = u"1.1.1.2"

        self.p2_inner_src_start_ip = IPv4Address(u"10.16.1.0")
        # self.p2_inner_src_end_ip = \
        #     f"{IPv4Address(self.p2_inner_src_start_ip) + self.n_tunnels * 256 - 1}"
        self.p2_inner_dst_start_ip = IPv4Address(u"10.32.1.0")
    ...
    def define_packets(self):
        ...
        base_pkt_a = (
            Ether() /
            IP(
                src=str(self.p1_src_start_ip),
                dst=str(self.p1_dst_start_ip),
                proto=61
            )
        )
        # Direction 1 --> 0
        base_pkt_b = (
            Ether() /
            IP(
                src=str(self.p2_outer_src_ip),
                dst=str(self.p2_outer_dst_ip),
                proto=17
            ) /
            UDP(
            ...
            IP(
                src=str(self.p2_inner_src_start_ip),
                dst=str(self.p2_inner_dst_start_ip),
                proto=61
            )
        )
        ...
        vm1 = STLScVmRaw(
            [
                STLVmFlowVar(
                    name=u"ip_src",
                    min_value=str(self.p1_src_start_ip),
                    # max_value=self.p1_src_end_ip,
                    max_value=str(
                        self.p1_src_start_ip + self.n_tunnels * 256 - 1
                    ),
                    size=4,
                    op=u"inc"
                ),
                ...
                STLVmFlowVar(
                    name=u"ip_dst",
                    min_value=str(self.p1_dst_start_ip),
                    # max_value=self.p1_dst_end_ip,
                    max_value=str(
                        self.p1_dst_start_ip + self.n_tunnels * 256 - 1
                    ),
                    size=4,
                    op=u"inc"
                ),
        ...
        vm2 = STLScVmRaw(
            [
                ...
                STLVmWrMaskFlowVar(
                    ...
                    pkt_cast_size=4,
                    mask=0xffffffff,
                    add_value=int(self.p2_inner_src_start_ip),
                ...
                STLVmWrMaskFlowVar(
                    ...
                    pkt_cast_size=4,
                    mask=0xffffffff,
                    add_value=int(self.p2_inner_dst_start_ip),
