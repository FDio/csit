...
class TrafficProfile(TrafficProfileBaseClass):
    ...
    def __init__(self, **kwargs):
        ...
        self.delay = 8000000  # delay 2s (2,000,000 usec)
    ...
    def define_profile(self):
        ...
        :rtype: tuple
        """
        self.tcp_req += self._gen_padding(self.headers_size + len(self.tcp_req))
        self.tcp_res += self._gen_padding(
            self.headers_size + len(self.tcp_res), 1518)
        ...
        prog_c.send(self.tcp_req)
        prog_c.recv(len(self.tcp_res))
        ...
        prog_s.recv(len(self.tcp_req))
        prog_s.send(self.tcp_res)
        ...
        temp_c = ASTFTCPClientTemplate(
            program=prog_c,
            ip_gen=ip_gen,
            limit=10000,  # TODO: set via input parameter
            port=8080
        )
