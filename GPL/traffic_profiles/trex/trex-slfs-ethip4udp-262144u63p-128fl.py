...
Stream profile:
 ...
 - Direction 0 --> 1:
   ...
   - UDP sport range:              1024 - 1086
   - UDP dport range:              1024
 - Direction 1 --> 0:
   - Source IP address range:      20.0.0.0
   - Destination IP address range: 68.142.68.0 - 68.142.68.255
   - UDP sport range:              1024
   - UDP dport range:              1024 - 65535
"""
import ipaddress
...
class TrafficStreams(TrafficStreamsBaseClass):
    ...
    def __init__(self):
        ...
        self.n_flows = 16515072
        self.n_ports_per_ip = 63
        self.n_streams = 128
        #self.full_cycle = 0.125
        #self.flows_per_second = self.cps_rate
        self.duty_cycle = 0.5
    ...
    def define_packets(self):
        ...
        self.n_src_ips_per_stream = self.n_flows / self.n_streams / \
            self.n_ports_per_ip
        self.n_usable_ports = (self.p2_dst_end_udp_port - \
            self.p2_dst_start_udp_port + 1)
        ...
        for stream in range(self.n_streams):
            ...
            base_pkt_b_item = (
                Ether() /
                IP(
                    src=str(ipaddress.ip_address(self.p2_src_start_ip) + \
                        ( int(self.p2_dst_start_udp_port + (stream * \
                        self.n_flows_per_stream) + self.n_flows_per_stream \
                        - 1) // 65536)),
    ...
    def create_streams(self):
        ...
        if isinstance(self.framesize, int):
            ...
            if self.cps_rate:
                ...
                self.flows_per_second = 8064
            self.full_cycle = self.n_flows / self.flows_per_second
