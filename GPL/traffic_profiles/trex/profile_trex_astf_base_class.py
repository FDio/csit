...
class TrafficProfileBaseClass:
    ...
    def create_profile(self):
        ...
        """
        prog_c, prog_s, client_msg, server_msg, ip_gen_c, ip_gen_s, \
        ip_gen_glob = self.define_profile()

        # In most cases you will not have to change the code below:

        # client commands
        prog_c.send_msg(client_msg, size=self.framesize)
        prog_c.recv_msg(len(server_msg))

        # server commands
        prog_s.recv_msg(len(client_msg))
        prog_s.send_msg(server_msg, size=self.framesize)
