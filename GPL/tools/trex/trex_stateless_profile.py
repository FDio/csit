    ...
    try:
        ...
        if isinstance(framesize, int):
            if latency:
                ...
                last_stream_a = int(len(streams) / 2)
            client.add_streams(streams[0:last_stream_a], ports=[port_0])
            if traffic_directions > 1:
                last_stream_b = (last_stream_a * 2)
                client.add_streams(streams[last_stream_a:last_stream_b], ports=[port_1])
        ...
        if latency:
            try:
                if isinstance(framesize, int):
                    ...
                    if traffic_directions > 1:
                        client.add_streams(streams[last_stream_b + 1], ports=[port_1])
                elif isinstance(framesize, str):
                    latency = False
            except STLError:
