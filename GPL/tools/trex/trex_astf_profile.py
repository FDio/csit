...
"""This module gets T-REX advanced statefull (astf) traffic profile together
...
def simple_burst(
    ...
    try:
        ...
        # stats.
        # TODO: Add NICs restart (link down/up)?
        client.reset()
        # Load the profile.
        ...
        if warmup_time > 0:
            ...
            client.start(mult=rate, duration=warmup_time)
            # Block until done.
        ...
        # Choose rate and start traffic.
        client.start(
            mult=rate, duration=duration, nc=True,
            latency_pps=rate if latency else 0, client_mask=2**len(ports)-1
        )
