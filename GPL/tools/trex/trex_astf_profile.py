...
def simple_burst(
    ...
    try:
        ...
        if async_start:
            ...
            # Do not block, existing transactions may take long time to finish.
            # We need an action that is almost reset(),
            # but without clearing stats.
            client.stop(block=False)
            client.stop_latency()
            client.remove_rx_queue(client.get_all_ports())
            time_stop = time.monotonic()
            duration_stop = time_stop - time_stopping_starts
            print(f"Stopping took {duration_stop} seconds.")
            # The previous stop might have been too quick.
            client.stop (block=True)
            real_duration_stop = time.monotonic() - time_stop
            print(f"Real stop took additional {real_duration_stop} seconds.")
            print(f"active ports: {client.get_active_ports()!r}")
