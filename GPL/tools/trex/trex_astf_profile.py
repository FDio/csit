...
def simple_burst(
    ...
    """
    def o_i_diff(stats, prev_o=0, prev_i=0):
        """Parse total opackets and ipackets, return differences.

        A closure to simplify computation.
        Accessed parent fields are traffic_directions, port_0 and port_1.

        :param stats: Statistics objects as returned by TRex python client code.
        :param prev_o: The previous value of total opackets.
        :param prev_i: The previous value of total ipackets.
        :type stats: dict
        :type prev_o: int
        :type prev_i: int
        :returns: Opackets difference and ipackets difference.
        :rtype: Tuple[int, int]
        """
        total_o = stats[port_0][u"opackets"]
        total_i = stats[port_1][u"ipackets"]
        if traffic_directions > 1:
            total_o += stats[port_1][u"opackets"]
            total_i += stats[port_0][u"ipackets"]
        return total_o - prev_o, total_i - prev_i
    ...
    try:
        ...
        while new_stat[port_0][u"opackets"] == 0:
            ...
            trex_start_time = time.monotonic()
            time_start += trex_start_time
            o_diff, i_diff = 0.0  # new stat is unidirectional and not reported.
        ...
        if async_start:
            ...
            while client.is_traffic_active(ports=ports):
                ...
                if time_sample >= time_now:
                    ...
                    o_diff, i_diff = o_i_diff(new_stat, o_diff, i_diff)
...
def main():
    ...
    simple_burst(
        ...
        multiplier=args.multiplier, warmup_time=args.warmup_time,
        port_0=args.port_0, port_1=args.port_1, latency=args.latency,
