...
class FilteringRxQueue(PacketVerifier):
    ...
    The filter function is also accessible as an instance field,
    so callers can examine or replace it anytime.
    """
...
def start_4_queues(args):
    ...
    tx_txq, tx_rxq = start_queue_pair(tx_interface, ignore_list=ignore_list_tx)
    rx_txq, rx_rxq = start_queue_pair(rx_interface, ignore_list=ignore_list_rx)
