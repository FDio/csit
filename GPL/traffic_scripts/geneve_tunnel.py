...
def main():
    ...
    args = TrafficScriptArg(
        [u"tx_src_mac", u"tx_dst_mac", u"rx_src_mac", u"rx_dst_mac", u"tunnels"]
    )
    ...
    tunnels = json.loads(args.get_arg(u"tunnels"))

    count = 1
    tx_sent_packets = list()
    for i in range(count):
        for tunnel in tunnels:
            src_ip = ip_address(tunnel[u"src_ip"]) + i
            dst_ip = ip_address(tunnel[u"dst_ip"]) + i
    ...
    while True:
        ...
        if rx_pkt_recv.haslayer(ICMPv6ND_NS):
            ...
            rx_received_packets.append(rx_pkt_recv)
        if len(rx_received_packets) == len(tunnels) * count:
            break
    ...
    rx_sent_packets = list()
    for packet in rx_received_packets:
        rx_inner_ip_pkt = IP(
            src=packet[Ether:2][IP].dst,
            dst=packet[Ether:2][IP].src,
            proto=61
        ) if isinstance(packet[Ether:2][1], IP) else IPv6(
            src=packet[Ether:2][IPv6].dst,
            dst=packet[Ether:2][IPv6].src
        )
        rx_inner_ip_pkt /= Raw()
        rx_inner_pkt = (
                Ether(src=packet[4].dst, dst=packet[4].src) /
                rx_inner_ip_pkt
        )
        size_limit = 78 if isinstance(packet[Ether:2][1], IPv6) else 64
        if len(rx_inner_pkt) < size_limit:
            rx_inner_pkt[Raw].load += b"\0" * (size_limit - len(rx_inner_pkt))

        rx_outer_ip_pkt = IP(
            src=packet[Ether:1][IP].dst,
            dst=packet[Ether:1][IP].src
        ) if isinstance(packet[Ether:1][1], IP) else IPv6(
            src=packet[Ether:1][IPv6].dst,
            dst=packet[Ether:1][IPv6].src
        )
        rx_pkt_send = (
                Ether(src=packet[Ether:1].dst, dst=packet[Ether:1].src) /
                rx_outer_ip_pkt /
                UDP(sport=6081, dport=6081) /
                GENEVE(vni=packet[GENEVE].vni) /
                rx_inner_pkt
        )
        rx_sent_packets.append(rx_pkt_send)
        rx_txq.send(rx_pkt_send)

    tx_received_packets = list()
    while True:
        ...
        if tx_pkt_recv.haslayer(ICMPv6ND_NS):
            ...
            # otherwise process the current packet
            tx_received_packets.append(tx_pkt_recv)
        if len(tx_received_packets) == len(rx_sent_packets):
            break
