...
def main():
    ...
    while True:
        ...
        if rx_pkt_recv.haslayer(ICMPv6ND_NS):
            ...
            received_packets.append(rx_pkt_recv)
        if len(received_packets) == len(tunnels) * count:
            break
    ...
    #     inner_ip_layer = IP if isinstance(packet[5], IP) else IPv6
    #     rx_inner_ip_pkt = inner_ip_layer(src=packet[IP].dst_ip, dst=packet[IP].src_ip, proto=61) \
    #         if inner_ip_layer == IP \
    #         else inner_ip_layer(src=packet[IPv6].dst_ip, dst=packet[IPv6].src_ip)
    #     rx_inner_ip_pkt /= Raw()
    #     rx_inner_pkt = Ether(src=packet[4].dst, dst=packet[4].src) / rx_inner_ip_pkt
    #     size_limit = 78 if inner_ip_layer == IPv6 else 64
    #     if len(rx_inner_pkt) < size_limit:
    #         rx_inner_pkt[Raw].load += b"\0" * (size_limit - len(rx_inner_pkt))
    #
    #     rx_pkt_send = (Ether(src=packet[0].dst, dst=packet[0].src) /
    #                    e_pkt)
