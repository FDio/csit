...
def check_geneve(
    ...
    if pkt_recv[ip_layer:2].src != inner_src_ip:
        raise RuntimeError(
            f"Received packet has invalid outinnerer source address: "
...
def main():
    ...
    args = TrafficScriptArg(
        [u"tx_src_mac", u"tx_dst_mac", u"rx_src_mac", u"rx_dst_mac", u"tunnel"]
    )
    ...
    tunnel = json.loads(args.get_arg(u"tunnel"))

    gen_tunnel_mac = u"d0:0b:ee:d0:00:00"
    gen_udp_dport = 6081

    tx_sent_packets = list()
    src_ip = ip_address(tunnel[u"src_ip"])
    dst_ip = ip_address(tunnel[u"dst_ip"])
    ...
    check_geneve(
        rx_pkt_recv, ip_layer, rx_src_mac, rx_dst_mac, gen_tunnel_mac,
        rx_src_mac, tunnel[u"local"], tunnel[u"remote"], src_ip, dst_ip,
        gen_udp_dport, tunnel[u"vni"]
    )
