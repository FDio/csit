...
def main():
    ...
    tunnels = args.get_arg(u"tunnels")
    print(tunnels)
    tunnels = json.loads(tunnels.replace(u'\"', u'"'))
    # tunnels = json.loads(args.get_arg(u"tunnels").replace(u'\"', u'"'))
    print(tunnels)
    ...
    for i in range(count):
        for tunnel in tunnels:
            print(tunnel)
            ip_layer = IP if ip_address(tunnel[u"src_ip"]).version == 4 \
                else IPv6
            ip_pkt = ip_layer(
                src=tunnel[u"src_ip"], dst=tunnel[u"dst_ip"], proto=61
            ) if ip_layer == IP \
                else ip_layer(src=tunnel[u"src_ip"], dst=tunnel[u"dst_ip"])
