...
def main():
    ...
    for packet in rx_received_packets:
        rx_inner_ip_pkt = IP(
            src=packet[Ether:2][IP].dst_ip,
            dst=packet[Ether:2][IP].src_ip,
            proto=61
        ) if isinstance(packet[Ether:2][1], IP) else IPv6(
            src=packet[Ether:2][IPv6].dst_ip,
            dst=packet[Ether:2][IPv6].src_ip
        )
        rx_inner_ip_pkt /= Raw()
        rx_inner_pkt = (
        ...
        rx_outer_ip_pkt = IP(
            src=packet[Ether:1][IP].dst_ip,
            dst=packet[Ether:1][IP].src_ip
        ) if isinstance(packet[Ether:1][1], IP) else IPv6(
            src=packet[Ether:1][IPv6].dst_ip,
            dst=packet[Ether:1][IPv6].src_ip
        )
        rx_pkt_send = (
                Ether(src=packet[Ether:1].dst, dst=packet[Ether:1].src) /
                rx_outer_ip_pkt /
                UDP(sport=packet[UDP:1].dport, dport=packet[UDP:1].sport) /
                GENEVE(proto=0x6558, vni=packet[GENEVE].vni) /
                rx_inner_pkt
        )
