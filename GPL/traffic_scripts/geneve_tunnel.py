...
def main():
    ...
    for i in range(count):
        for tunnel in tunnels:
            ip_layer = IP if ip_address(tunnel.src_ip).version == 4 else IPv6
            ip_pkt = ip_layer(
                src=tunnel.src_ip, dst=tunnel.dst_ip, proto=61
            ) if ip_layer == IP \
                else ip_layer(src=tunnel.src_ip, dst=tunnel.dst_ip)
