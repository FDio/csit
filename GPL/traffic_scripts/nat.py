...
def main():
    ...
    assert ether[Ether].dst == rx_dst_mac and ether[Ether].src == rx_src_mac, \
        f"Matching MAC addresses unsuccessful: {ether!r}"
    # if rx_dst_mac == ether[Ether].dst and rx_src_mac == ether[Ether].src:
    #     print(u"MAC matched")
    # else:
    #     raise RuntimeError(f"Matching packet unsuccessful: {ether!r}")

    # if ether.haslayer(ip_format):
    #     ip_packet = ether[ip_format]
    # else:
    #     raise RuntimeError(f"No {ip_format} in received packet: {ether!r}")
    ip_pkt = ether.payload
    assert isinstance(ip_pkt, ip_layer), \
        f"Not an {ip_layer!s} packet received: {ip_pkt!r}"
    assert ip_pkt.src == src_ip_out, \
        f"Matching Src IP address unsuccessful: {src_ip_out} != {ip_pkt.src}"
    assert ip_pkt.dst == dst_ip, \
        f"Matching Dst IP address unsuccessful: {dst_ip} != {ip_pkt.dst}"

    proto_pkt = ip_pkt.payload
    assert isinstance(proto_pkt, proto_layer), \
        f"Not a {proto_layer!s} packet received: {proto_pkt!r}"
    if sport_out is not None:
        assert proto_pkt.sport == sport_out, \
            f"Matching Src {proto_layer!s} port unsuccessful: " \
            f"{sport_out} != {proto_pkt.sport}"
    else:
        sport_out = proto_pkt.sport
    assert proto_pkt.dport == dst_port, \
        f"Matching Dst {proto_layer!s} port unsuccessful: " \
        f"{dst_port} != {proto_pkt.dport}"
    if proto_layer == TCP:
        assert proto_pkt.flags == 0x2, \
    ...
    assert ether[Ether].dst == tx_src_mac and ether[Ether].src == tx_dst_mac, \
        f"Matching MAC addresses unsuccessful: {ether!r}"

    ip_pkt = ether.payload
    assert isinstance(ip_pkt, ip_layer), \
        f"Not an {ip_layer!s} packet received: {ip_pkt!r}"
    assert ip_pkt.src == dst_ip, \
        f"Matching Src IP address unsuccessful: {dst_ip} != {ip_pkt.src}"
    assert ip_pkt.dst == src_ip_in, \
        f"Matching Dst IP address unsuccessful: {src_ip_in} != {ip_pkt.dst}"

    proto_pkt = ip_pkt.payload
    assert isinstance(proto_pkt, proto_layer), \
        f"Not a {proto_layer!s} packet received: {proto_pkt!r}"
    assert proto_pkt.sport == dst_port, \
        f"Matching Src {proto_layer!s} port unsuccessful: " \
        f"{dst_port} != {proto_pkt.sport}"
    assert proto_pkt.dport == sport_in, \
        f"Matching Dst {proto_layer!s} port unsuccessful: " \
        f"{sport_in} != {proto_pkt.dport}"
    if proto_layer == TCP:
        assert proto_pkt.flags == 0x12, \
