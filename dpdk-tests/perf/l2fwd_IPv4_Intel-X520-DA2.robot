...
| ... | *[Cfg] DUT configuration:* DUT1 and DUT2 run the DPDK testpmd application
| ... | and use the io forwarding mode. DUT1 and DUT2 tested with
...
| | [Documentation]
| | ... | [Cfg] DUT runs L2 frame forwarding config with 2 threads, 2 phy cores, \
| | ... | 1 receive queue per NIC port. [Ver] Find NDR for 64 Byte frames
| | ... | using binary search start at 10GE linerate, step 100kpps.
...
| | [Documentation]
| | ... | [Cfg] DUT runs L2 frame forwarding config with 4 threads, 4 phy cores, \
