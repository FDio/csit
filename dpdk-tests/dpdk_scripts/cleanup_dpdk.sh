...
sudo ./usertools/dpdk-devbind.py -b ${port1_driver} ${port1_pci} || echo "Unbind ${port1_pci} failed" || exit 1
sudo ./usertools/dpdk-devbind.py -b ${port2_driver} ${port2_pci} || echo "Unbind ${port1_pci} failed" || exit 1
...
rmmod igb_uio || echo "Removing igb_uio failed" || exit 1
