...
def compile_DPDK(node):
    """Compile the DPDK on the DUT node."""
    logger.console('Compile the DPDK on {0}'.format(node['host']))

    ssh = SSH()
    ssh.connect(node)

    (ret_code, stdout, stderr) = ssh.exec_command(
        'cd {0} && tar xJf dpdk-16.07.tar.xz && cd ./dpdk-16.07 && '
        'sed -i \'s/^CONFIG_RTE_LIBRTE_PMD_PCAP=n/CONFIG_RTE_LIBRTE_PMD_PCAP=y/g\' ./config/common_base'
        .format(con.REMOTE_FW_DIR), timeout=100)

    if 0 != ret_code:
        logger.error('Config the DPDK compile environment error: {0}'.format(stdout + stderr))
        raise Exception('Config the DPDK failed')
    else:
        logger.console('Config the DPDK on {0}'.format(node['host']))

    (ret_code, _, stderr) = ssh.exec_command(
        'cd {0}/dpdk-16.07/ && make install T=x86_64-native-linuxapp-gcc'
        .format(con.REMOTE_FW_DIR), timeout=600)
    if 0 != ret_code:
        logger.error('Compile the DPDK error: {0}'.format(stderr))
        raise Exception('Compile the DPDK failed')
    else:
        logger.console('Compile the DPDK on {0}'.format(node['host']))

def install_pcap_dev(node):
    """Install the libpcap-dev for the TLDK test """
    logger.console('Install the libpcap-dev on {0}'.format(node['host']))

    ssh = SSH()
    ssh.connect(node)

    (ret_code, stdout, stderr) = ssh.exec_command(
        'cd {0} && sudo dpkg -i libpcap*.deb'.format(con.REMOTE_FW_DIR), timeout=100)
    if 0 != ret_code:
        logger.error('Install the libpcap-dev error: {0}'.format(stdout + stderr))
        raise Exception('Install the libpcap-dev failed')
    else:
        logger.console('Install the libpcap-dev on {0}'.format(node['host']))

def compile_TLDK(node):
    """Compile the TLDK for the TLDK test """
    logger.console('Compile the TLDK on {0}'.format(node['host']))

    ssh = SSH()
    ssh.connect(node)

    (ret_code, _, stderr) = ssh.exec_command(
        'export RTE_SDK={0}/dpdk-16.07/ && export RTE_TARGET=x86_64-native-linuxapp-gcc && '
        'cd {0}/tldk && make all'.format(con.REMOTE_FW_DIR), timeout=600)
    if 0 != ret_code:
        logger.error('Compile the TLDK error: {0}'.format(stderr))
        raise Exception('Compile the TLDK failed')
    else:
        logger.console('Compile the TLDK on {0}'.format(node['host']))

def setup_hugepages(node):
    """Config the hugepages for the TLDK test """
    logger.console('config the hugepages on {0}'.format(node['host']))

    ssh = SSH()
    ssh.connect(node)

    (ret_code, stdout, stderr) = ssh.exec_command(
        'sudo umount hugetlbfs && echo 1024>/sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages && '
        'sudo mkdir -p /mnt/huge/ && sudo mount -t hugetlbfs nodev /mnt/huge/', timeout=100)

    if 0 != ret_code:
        logger.error('Config the hugepages error: {0}'.format(stdout + stderr))
        raise Exception('Config the hugepages failed')
    else:
        logger.console('Config the hugepages on {0}'.format(node['host']))
...
def install_tldk_test(node):
    ...
    (ret_code, _, stderr) = ssh.exec_command(
        'cd {0} && ./install_tldk.sh'.format(con.REMOTE_FW_DIR), timeout=600)

    if 0 != ret_code:
        logger.error('Install the TLDK error: {0}'.format(stderr))
        raise Exception('Install the TLDK failed')
    else:
...
class SetupTLDKTest(object): # pylint: disable=too-few-public-methods
