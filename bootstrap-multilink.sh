...
for addr in `cat /etc/nodepool/sub_nodes`; do
    ...
    ssh_do jenkins@${addr} --  "env"
    ssh_do jenkins@${addr} --  "id"
    ssh_do jenkins@${addr} --  "cat ~/.ssh/authorized_keys"
    ssh_do jenkins@${addr} --  "echo \"ssh-rsa  virl@tb4-virl\" >> ~/.ssh/authorized_keys"
    ssh_do jenkins@${addr} --  "cat ~/.ssh/authorized_keys"
done
...
ssh_do jenkins@${subnodes[0]} "sudo arping -c 1 -I eth0 -b ${subnodes[1]}"
