...
resource "aws_vpc" "CSITVPC" {
  ...
  tags = {
    "Name" = "CSIT_2n_aws_c5n_${var.testbed_name}-VPC"
    "Environment" = "CSIT-AWS"
  }
}

resource "aws_security_group" "CSITSG" {
  name        = "CSIT_2n_aws_c5n_${var.testbed_name}-sg"
...
resource "aws_instance" "tg" {
  ...
  tags = {
    "Name" = "CSIT.2n_${var.testbed_name}.tg"
    "Environment" = "CSIT-AWS"
  }
}
...
resource "aws_instance" "dut1" {
  ...
  tags = {
    "Name" = "CSIT.2n_${var.testbed_name}.dut1"
    "Environment" = "CSIT-AWS"
  }
}
...
resource "null_resource" "deploy_tg" {
  ...
  provisioner "ansible" {
    plays {
      playbook {
        file_path = "../../resources/tools/testbed-setup/ansible/site.yaml"
        force_handlers = true
      }
      hosts = ["tg_aws"]
      extra_vars = {
  ...
  provisioner "ansible" {
    plays {
      ...
      extra_vars = {
        ...
        # public_ip_list = join(", ", [aws_instance.tg.public_ip, aws_instance.dut1.public_ip])
