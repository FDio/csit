    ...
    task "prod-task1-prometheus" {
      ...
      config {
        ...
        volumes       = [
          ...
          "local/node_export.json:/etc/prometheus/node_export.json",
          "local/docker_export.json:/etc/prometheus/docker_export.json",
...
EOH
      ...
      template {
        change_mode   = "noop"
        destination   = "local/docker_export.json"
        data          = <<EOH
[
  {
    "targets": [
...
EOH
      ...
      template {
        change_mode   = "noop"
        destination   = "local/node_export.json"
        data          = <<EOH
[
  {
    "targets": [
    ...
    task "prod-task1-grafana" {
      ...
      service {
        ...
        check {
          ...
          protocol        = "http"
          header {
            Authorization = ["Basic YWRtaW4sYWRtaW4="]
          }
