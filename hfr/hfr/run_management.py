...
class RunManagement:
    ...
    def _started(self, run: Run):
        ...
        Possible transitions:
        1. started -> started
           - If the run is not running on GHA executor.
        2. started -> waiting
           - The run is running on GHA executor but it is waiting for a testbed.
        3. started -> finished
           - If the run finished.
        4. started -> canceled
    ...
    def _waiting(self, run: Run):
        ...
        The run is waiting for a testbed. If it is waiting too long
        (> app_conf.max_waiting_time), it is cancelled by HFR.
    ...
    def _finished(self, run: Run):
        ...
        if run.tb.status == "pre-reserved" \
            ...
            run.tb.status = "available"
        run.data.nr_of_runs += 1
        run.data.hfr_status = "done"

    def _done(self, run: Run):
        """Definetly done.
