...
class RunManagement:
    ...
    def _queued(self, run: Run):
        ...
        # ======================================================================
        # Only for the demo.
        # Do this in the state "waiting" when the run gets its testbed
        run.tb = free_tb
        # ======================================================================
    ...
    def _started(self, run: Run):
        ...
        2. started -> waiting
           - The run is running in GHA but it is waiting for a
             testbed. 
        3. started -> finished
           - If the run finished.
        4. started -> canceled
    ...
    def _cancelled(self, run: Run):
        ...
        If "re-run" enabled and the max count not reached, change the status to
        "queued" and incerase the counter of runs. Otherwise change the status
        to "finished".
