...
class Testbeds:
    ...
    def _get_testbeds(self) -> list:
        ...
        for file_name in tb_files:
            ...
            try:
                ...
                raise RuntimeError(
                    f"The topology file '{file_name}' does not include "
                    f"information about TG.\n{err}"
                )
            try:
                # HFR does not need this information, so remove it.
                del tb.tg["interfaces"]
            except KeyError:
                pass
    ...
    def check_testbeds(self, node=None):
        ...
        for tb in self._tbs:
            if node and node != tb.node:
                continue
            # Get info from a testbed.
            status, tb.job, tb.run_id = self._check_tb(tb.tg)
            # Do not mark the "pre-reserved" testbed as "available".
            if not (tb.status == "pre-reserved" and status == "available"):
                tb.status = status
            if status == "reserved":
                logging.debug(
                    f"Checked {tb.name} - {tb.status} by {tb.job}-{tb.run_id}"
                )
            else:
                logging.debug(f"Checked {tb.name} - {tb.status}")

    def check_testbed(self, tb) -> tuple:
    ...
    def get_free_tb(self, node) -> Testbed:
    ...
    def unreserve_tb(self, node) -> bool:
    ...
    def to_tb_csv_file(self):
        ...
        # Write to CSV file.
        try:
            directory = self._app_conf.path_to_tb_csv.rsplit("/", maxsplit=1)[0]
            if not os.path.exists(directory):
                os.makedirs(directory)
            with open(self._app_conf.path_to_tb_csv, "w", newline="") as fw:
            ...
            raise RuntimeError(
                ...
                f"'{self._app_conf.path_to_tb_csv}'\n {err}"
            )

    @property
    def testbeds(self) -> list:
