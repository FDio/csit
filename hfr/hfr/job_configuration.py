...
The jobs and their runs are defined in a YAML configuraton file, e.g.:
    ...
    runs:
      ...
      - inputs:
        ...
        params:
          ...
          repeat: 1
    parameters:
      needed_params:  # Only for validation, but neccessary for report jobs
        - "job_type"
        - "dut"
        - "node"
        - "branch"
...
Job configuration after processing
----------------------------------
...
The runs are groupped by node and sorted by priority.
...
class JobConfiguration:
    ...
    def _validate_conf(self):
        ...
        if not self._raw_conf:
            raise RuntimeError(
                f"The job configuration file '{self._conf_file}' is empty."
            )
        wfs = self._raw_conf.get("workflows", None)
        if not wfs:
            raise RuntimeError(
                f"The job configuration file '{self._conf_file}' does not "
                f"include any workflows."
            )
        if not isinstance(wfs, dict):
            raise RuntimeError(
                f"The workflows in the job configuration file "
        ...
        if not isinstance(params, dict):
            raise RuntimeError(
                f"The parameters in the job configuration file "
                f"'{self._conf_file}' must be defined as a dictionary."
            )
        ...
        if not valid:
            raise RuntimeError("The job configuration is not valid.")
    ...
    def _pre_queuing(in_q: dict) -> dict:
        ...
        for itm in in_q:
            ...
            run = Run(**JobConfiguration.RUN_DEFAULTS)
    ...
    def _queuing_sequential(in_q: dict) -> dict:
        ...
        by 10 ndrpdr runs.
        """
        tmp_q = JobConfiguration._pre_queuing(in_q)
    ...
    def _process_conf(self) -> dict:
        ...
        if self._params["queuing"] == "sequential":
            self._runs = JobConfiguration._queuing_sequential(ppconf)
        elif self._params["queuing"] == "mixed":
            self._runs = JobConfiguration._queuing_mixed(ppconf)
        elif self._params["queuing"] == "random":
            self._runs = JobConfiguration._queuing_random(ppconf)

    def read_conf(self):
        """Read the job configuration from the configuration file.
        :raises RuntimeError: If the configuraton is invalid.
        """
        logging.info("Reading the job configuration...")
