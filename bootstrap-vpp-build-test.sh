...
if [ "${#}" -ne "0" ]; then
    ...
    # Get xml file with VPP build versions available on the server
    w3m https://nexus.fd.io/service/local/repositories/fd.io.dev/content/io/fd/vpp/vpp/ > vpp_builds.xml

    # Create paths to the latest vpp build files
    python /tools/vpp_build_test/get_vpp_build_urls.py

    # Download the latest VPP build deb install packages
    wget -q -i vpp_build_urls.txt || exit

    # Delete unnecessary files
    rm -rf vpp_builds.xml
    rm -rf vpp_build_urls.txt
fi
...
rebot ./log_test_set1.xml ./log_test_set2.xml ./log_test_set3.xml

rm -rf ./log_test_set1.xml ./log_test_set2.xml ./log_test_set3.xml
